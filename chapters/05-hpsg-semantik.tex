...
Innerhalb\is{Semantik|(} der HPSG"=Forschung gibt es verschiedene Ansätze
...
\citealt{RS2004a-u,IR2015a-u,Richter2016a-u,SailerRichter2021a-u}, \citealt[Abschnitt~6.2]{KR2024a}), die \emph{Minimal Recursion Semantics}\indexmrsstart
(MRS; \citealt*{CFPS2005a}) und Varianten der \emph{Discourse Representation Theory} (DRT; \citealt{KR93a}) mit Underspecified
Discourse Representation Structures (UDRS;
\citealt{FR95a-u}). Zur MRS gab es 1995 die ersten Veröffentlichungen \citep*{CFMRS95a-u}, und
der 2005er Aufsatz kursierte in verschiedenen Versionen seit dieser Zeit im Netz. Veröffentlichungen wie das Buch von
...
Verarbeitung der Grammatiken durch Computer ermöglicht. Zu weiteren Vorteilen siehe \citew*[Abschnitt~2]{CFPS2005a}.
Das folgende Kapitel führt in MRS ein. Dazu diskutiere ich viele der Anwendungsfälle, die für das
Englische von \citet*{CFPS2005a} ausgearbeitet wurden. Für Details, die in diesem klassischen Artikel
offen geblieben sind, habe ich die implementierte Grammatik von Dan Flickinger konsultiert
\citep*{Flickinger2000a,FCS2000a}. In den ersten drei Auf"|lagen dieses Buches habe ich noch
Situationssemantik verwendet. Leser*innen, die die Ansätze vergleichen möchten, können also frühere
Auf"|lagen konsultieren.%\todostefan{zitieren: \citep{Ginzburg2012a-u,
%\citet{Ginzburg2012a-u,Luecking:Ginzburg:Cooper:2024a} benutzen Type Theory with Records (TTR).}
...
mehrdeutig\is{Mehrdeutigkeit|(}. Leider gibt es Mehrdeutigkeiten auf den verschiedensten sprachlichen Ebenen. Schon bei der Gruppierung der
...
\z
Wenn man diesen Satz äußert, nachdem man über \emph{speech recognition} (Erkennung gesprochener Sprache)
...
(\ref{ex-Jede-Tochter-eines-Mitarbeiters-schläft}) hat zwei Lesarten: In der einen muss es einen bestimmten Mitarbeiter geben
...
syntaktischen Strukturen (auf der sogenannten Logischen Form\is{Logische Form}) entstehen. Frühere HPSG"=Ansätze
...
Mehrdeutigkeiten wie denen, die in (\mex{0}a) oder wegen Ambiguitäten im Sprachsignal vorliegen. Auch ist es so, dass es bei gleichzeitigem
...
können. 
% Jakob Mache: Intonation macht Sachen evtl. eindeutig.
% Stefan: Ja, schon, aber im folgenden Beispiel hilft das nicht viel. Es gibt die Fälle mit extremer Interaktion.
\citet[\page 196]{ADKMNT2003a} geben das folgende Beispiel, das ungefähr zweihundert Lesarten haben soll:
\ea
...
auflösen, wenn man genug Evidenz dafür hat, welche Lesart von der Sprecher*in beabsichtigt war.
...
enthalten sind.\is{Mehrdeutigkeit|)}
...
funktioniert und Abschnitt~\ref{Abschnitt-Skopus} schließlich beschäftigt sich schließlich mit Quantorenskopus und
...
aus? Dass sie gelb ist und sauer? Ist eine grüne Zitrone keine Zitrone \citep{Braisby90}?
% Ganzes Papier.
All das sind spannende
...
% Volk
\relation{lieben} aus, dann sind nur der, die oder das Liebende und der/die/das Geliebte Argumente der
...
\item fünfstellig: \relation{kaufen}   (\emph{Aicke kauft den gebrauchten Mantel von Conny für fünf Euro.})
\end{itemize}
...
\ex lieben(e, Aicke, Conny)
\zl
Das kleine $e$ steht dabei für das Ereignis. Das $e$ wird auch Ereignisvariable\is{Ereignisvariable}
genannt. \relation{Aicke} und \relation{Conny} sind sogenannte Individuenkonstanten\is{Individuenkonstante}. Aus
...
die der Kinder. Man kann hier schön sehen, dass die Schnittmenge dieser beiden Mengen genau der
...
\ex Für jede Entität x in unserem Diskursuniversum, für die gilt, dass sie Kind ist, gilt auch, dass sie schläft.
\ex \blau{$\forall$}x kind(x) \blau{$\to$} schlafen(x)
\zl
\eal
\ex Ein Kind schläft.
\ex Es gibt mindestens eine Entität x in unserem Diskursuniversum, für die gilt, dass sie ein Kind ist und dass sie schläft.
\ex \blau{$\exists$}x kind(x) \blau{$\wedge$} schlafen(x)
\zl
\eal
\label{ex-Das-Kind-schläft}
\ex Das Kind schläft.
\ex Es gibt eine einzige Entität x in unserem Diskursuniversum, die Kind ist, und für diese gilt, dass sie schläft.
...
muss von den Determinatoren kommen. Das ist in (\mex{-2})--(\mex{0}) blau hervorgehoben. Zum
$\iota$-Operator siehe auch \citew[85]{HK98a-u}. 
...
\ea
Mögliche Repräsentation mit semantischen Rollen:\\[2pt]
\ms[schlafen]{
 event & e\\ 
 agens & x \\
}
...
Die Zuordnung von Argumenten zu einzelnen Rollen kann jedoch von Autor zu Autorin stark
...
vorgeschlagen. Statt Proto"=Agens und Proto"=Patiens verwenden sie die Rollen Actor\is{Actor} und Undergoer\is{Undergoer}.
...
\ea
      \ms[sign]{ phon   & list~of~phoneme strings\,\\
           head   & head \\
           spr   & list~of~signs\\
           comps & list~of~signs\\
...
\ea
\label{geom-cat-cont}
      \ms[sign]{ phon & list~of~phoneme strings\,\\[2pt]
           cat  & \ms{ head  & head\\
                       spr   & list~of~signs\\
                       comps & list~of~signs\\
                       arg-st & list~of~signs } \\
...
Strukturteilung nur syntaktische Information an verschiedenen Stellen in AVMs zu identifizieren. Zum Beispiel möchte
man Fälle\is{Koordination|(} symmetrischer\label{page-symmetrische-koordination} Koordination wie in (\mex{1}) so behandeln, dass
man einfach die \textsc{cat}"=Werte der Konjunkte teilt \citep[\page 202]{ps2}:
\eal
...
% Information über Quantoren ein \parencites[Abschnitt~8.2]{ps2}[\page 311]{CFPS2005a}. In MRS werden
% Relationen, Quantoren und Beschränkungen bzgl.\ Skopus allerdings in Listen behandelt, die einfach
% verkettet werden. In der Analyse der Verbstellung, die im Abschnitt~\ref{Abschnitt-V1} erklärt wird,
...
(\mex{1}) zeigt den Lexikoneintrag für \emph{Buch}:\footnote{
  In den folgenden Kapiteln bis zum Kapitel~\ref{Kapitel-Konstituentenreihenfolge} werden Relationen (\rels)\isfeat{rels}
  und Skopus"=Beschränkungen (\hcons)\isfeat{hcons} innerhalb von \cont\isfeat{cont} aufgeführt. Das wird im Abschnitt~\ref{Abschnitt-V1} dann
  revidiert, damit die Relationen nicht mit der Verbspur geteilt werden und in der \relsl der
...
Der Index einer Nominalphrase entspricht dem Index des Kopfnomens der Nominalphrase. Das heißt,
\emph{das Buch} hat denselben Index wie \emph{Buch} in (\ref{le-buch}). Wie das sischergestellt
wird, wird im Abschnitt~\ref{Abschnitt-Semantikprinzip} erklärt.
...
\zs
Die Liste der Relationen ist die leere Liste, da wir, wenn wir einen Satz wie (\mex{1}) äußern, nur
...
NP\sub{[\type{3},\type{sg},\type{fem}]}     & \onems{ cat \ms{ head  & noun \\
                                                               spr   & \eliste\\
                                                               comps & \eliste } \\
                                cont$|$ind \ms{ per & 3 \\
                                                num & sg \\
...
NP$_{\ibox{1}}$ & \onems{ cat  \ms{ head & noun \\
                                   spr  & \eliste\\
		                  comps & \eliste } \\
                       cont$|$ind \ibox{1}\\
                     }\\\\
...
\section{Der semantische Beitrag von Adjektiven}

Nach den Nomina soll jetzt erklärt werden, wie der semantische Beitrag von Adjektiven repräsentiert
wird. (\mex{1}) zeigt einen vorläufigen Eintrag:

\eas
\label{le-kleinen-sem1}%
Lexikoneintrag für \emph{kleine}: (vorläufig):\\
\ms{
cat & \ms{ head & \ms[adj]{
                  mod & \nbar\ind{1} }\\
            arg-st & \eliste
         }\\
cont & \ms{ ind  & \ibox{2} property\\[1mm]
            rels & \liste{ \ms[klein]{
                           arg0 & \ibox{2}\\
                           arg1 & \ibox{1} } } } }
\zs
Das Adjektiv hat eine leere \argstl, da es weder einen Spezifikator noch ein anderes Argument
verlangt. Der \modw sagt aus, dass das Adjektiv eine \nbar modifizieren kann. Der Index des
modifizierten Nomens \iboxb{1} ist identisch mit dem \argone der Relation
\relation{klein}. Adjektive führen einen eigenen Index ein, der dem \argzero der Relation
\relation{klein} entspricht \iboxb{2}. Dieses \argzero ist parallel zu dem von Nomina und Verben, denen wir
uns im folgenden Abschnitt zuwenden werden.

Die Analyse der Adjektiv-Nomen-Kombination in (\mex{1}) zeigt Abbildung~\ref{Abb-kleine-Kind}.
\ea
das kleine Kind
\z
\begin{figure}
\begin{forest}
sm edges
[\nbar\ind{1}\feattab{\rels \nliste{ klein(\ibox{2}, \ibox{1}), kind(\ibox{1}) }}
  [Adj\feattab{\textsc{mod} \ibox{3} \nbar\ind{1}\\
               \rels \nliste{ klein(\ibox{2}, \ibox{1}) }}  [kleine]]
  [\ibox{3} \nbar\ind{1}\feattab{\rels \nliste{ kind(\ibox{1}) }} [Kind]]]
\end{forest}
\caption{Analyse der Kombination von \emph{kleine} und \emph{Kind}}\label{Abb-kleine-Kind}
\end{figure}
Bei der Einsetzung von Adjektiv und Nomen in das Kopf"=Adjunkt"=Schema wird der \modw des Adjektivs
mit dem modifizierten Nomen identifiziert \iboxb{3}. Dadurch wird der referentielle Index des Nomens
\iboxb{1} im Adjektiv verfügbar. Durch die Identifikation des Index-Wertes des modifizierten Nomens 
mit dem Argument des Adjektivs im Lexikoneintrag des Adjektivs wird sichergestellt, dass sich
Adjektiv und Nomen auf dasselbe Element beziehen (\,\ibox{1} in Abbildung~\ref{Abb-kleine-Kind} und
\ibox{1} in (\ref{le-kleinen-sem1})).
Bei der Kombination des Adjektivs mit dem Nomen werden beide \relslen verknüpft und
das Ergebnis wird am Mutterknoten repräsentiert. Der Index der gesamten \nbar ist ebenfalls
\ibox{1}, weil der semantische Index in Strukturen mit Kopf zwischen Mutter und Kopf geteilt
wird. Siehe Abschnitt~\ref{Abschnitt-Semantikprinzip}.

Diese erste Skizze der Modifikation im nominalen Bereich wird in den 
Abschnitten~\ref{Abschnitt-Semantik-Adjektivmodifikation-mutmaßlich} und~\ref{Abschnitt-scheinbar-einfache-Beispiele}
...
Zu Linking"=Theorien im Rahmen der HPSG siehe \citew{Davis96a-u} und \citew{Wechsler95a-u} bzw.\ für
einen Überblick \citew*{HPSGHandbookLinking}.
...
\zs
Da für die Argumentrollen allgemeine Bezeichnungen wie \argone, \argtwo und \argthree
...
Linking für ditransitive Verben wie \emph{geben}:\\
\onems
{ cat$|$arg-st \liste{ []\ind{1}, []\ind{2}, []\ind{3}  } \\[2mm]
...
Strukturen mit den Merkmalen \argone, \argtwo und \argthree haben (siehe auch Abschnitt~\ref{Abschnitt-Typhierarchien}). 
...
können. Die Analyse von (\mex{1}) zeigt Abbildung~\ref{fig-Alle-kleinen-Kinder-schlafen}.
\ea
Alle kleinen Kinder schlafen.
\z
\begin{figure}
\begin{forest}
sm edges
[{V$_e$[\nliste{ every\_q(x, \ldots), klein(p,x), kind(x), schlafen(e,x) }]}
  [{NP$_x$[\nliste{ every\_q(x, \ldots), klein(p,x), kind(x) }]}
    [{Det$_x$[\nliste{ every\_q(x, \ldots) }]} [alle]]
    [{\nbar{}$_x$[\nliste{ klein(p,x), kind(x) }]}
      [{Adj$_p$[\nliste{ klein(p,x) }]} [kleinen]]
      [{\nbar{}$_x$[\nliste{ kind(x) }]} [Kinder]]]]
  [{V$_e$[\nliste{ schlafen(e,x) }]} [schlafen]]]
\end{forest}
\caption{Semantik von \emph{Alle kleinen Kinder schlafen.}}\label{fig-Alle-kleinen-Kinder-schlafen}
\end{figure}
Der Beitrag des Determinators \emph{alle} wird im nächsten Abschnitt diskutiert. Er ist in der
Abbildung einfach als \type{every\_q}(x, \ldots) angegeben. Ansonsten wurde der semantische Beitrag von
...
Abbildung~\ref{fig-Alle-kleinen-Kinder-schlafen}, auftaucht.
...
Index der NP $x$ ist. Das ist aber notwendig, denn der Index dieser NP füllt ja auch eine semantische
...
% }
Die Implikation in (\mex{0}) bewirkt, dass in Strukturen mit Kopftochter der Index der Kopftochter mit
dem der Mutter identisch ist. So wird die Ereignisvariable von Verben ($e$ in Abbildung~\ref{fig-Alle-kleinen-Kinder-schlafen}) nach oben gegeben und der
referentielle Index von Nomen ($x$ in Abbildung~\ref{fig-Alle-kleinen-Kinder-schlafen}) auch.

Die Relationen werden von der folgenden Beschränkung für Zeichen vom Typ \type{phrase} eingesammelt:
\ea
...
\z
%\largerpage
\textsc{dtrs} ist hierbei die Liste, die sich beim Aneinanderhängen der Liste mit der Kopftochter
und der Liste mit den Nicht"=Kopftöchtern ergibt, wenn es eine Kopftochter gibt. Andernfalls
...
\z
\texttt{collect-rels}\isrel{collect-rels} geht dabei durch die Liste der Töchter und hängt den jeweiligen \relsw an eine
...
% Nö. \todostefan{Boxen statt Variablennamen?}
\ea
\label{constraint-collect-rels}
\texttt{collect-rels}([],[]).\\
\texttt{collect-rels}([[\textsc{cont|rels} Rels1]|Dtrs],Rels) :=\\
\mbox{}\hfill \texttt{collect-rels}(Dtrs,Rels2), \texttt{append}(Rels1,Rels2,Rels).
\z
%\medskip
...
Töchter Dtrs.\footnote{%
Etwas unschön ist hier, dass der Operator, der Listen zerteilt, aussieht wie der
...
\emph{rekursiv}\is{Rekursion} genannt.  
...
Wissen für das Verständnis der kommenden Kapitel. Der nun folgende Abschnitt wird etwas technisch,
...
dass diese mehrdeutig sein können. In diesem Abschnitt werde ich noch weitere Merkmale einführen,
...
gesagt. Das Beispiel (\ref{ex-Alle-Kinder-schlafen}) sei hier der Übersichtlichkeit halber
wiederholt:
\eal
...
Jede Tochter eines Mitarbeiters schläft.
\z
\todostefan{Jakob Mache: Jede = \ = all.}
%\largerpage[2]
\noindent
Dieses Beispiel hat die beiden Lesarten in (\mex{1}) und (\mex{2}):
\eal
...
mithilfe dieser Labels auf die Relationen beziehen. Die Verschachtelung ist dann gewissermaßen
...
ersetzen:
\ea
Explizite Konjunktion von Relationen mit \relation{und}:\\
h1:every\_q(x, h2, h3), h2:und(h4:klein(x), h5:kind(x)), h3:schlafen(x)
\z
\citet[\page 284]{CFPS2005a} haben diese Möglichkeit erwogen, sie dann aber verworfen, weil es bei
...
bei intersektiver Modifikation\footnote{%
Intersektive Modifikation kann zum Beispiel vorliegen, wenn ein Adjektiv sich auf eine bestimmte Menge
von Individuen bezieht und ein modifiziertes Nomen auf eine andere Menge. Durch die Modifikation
wird eine Schnittmenge gebildet. Abbildung~\ref{fig-Diskursuniversum-blond-Kind} hat das für die
blonden Kinder verdeutlicht. Im Gegensatz zu interesektiven Modifkatoren gibt es Modifikatoren, die den semantischen Beitrag des
Elements, das sie modifizieren, einbetten:
\eal
\ex ein kleiner Mörder
\ex ein mutmaßlicher Mörder
\zl
Ein kleiner Mörder ist sowohl klein als auch ein Mörder. Ein mutmaßlicher Mörder ist jemand, von dem
gemutmaßt wird, dass er ein Mörder ist. Er kann unschuldig sein. Man kann zwar klein sein, aber
nicht mutmaßlich. Adjektive wie \emph{mutmaßlich} werden in
Abschnitt~\ref{Abschnitt-Semantik-Adjektivmodifikation-mutmaßlich} behandelt.%
} das Label von Kopf und Adjunkt identifiziert werden.
...
sich die Reihenfolge der Quantoren unterscheiden. Einmal ist \relation{every} innerhalb der Klammer von \relation{exist}
und einmal \relation{exist} innerhalb der Klammer von \relation{every}. Wie gleich gezeigt werden wird, kann man die
...
vom Allquantor und x dominiert sein muss, muss h5 von h3 dominiert werden. Es ergibt sich also
...
insgesamt auch nur zwei durch h2 und h3 dominierte Elemente gibt, kann es auch nur eine Auf"|lösung
...
\textmrs{ h0, \{ \begin{tabular}[t]{@{}l@{}}
                 h1:every\_q(x, h2, h3), h4:tochter(x,y), h5:exist\_q(y, h6, h7), h8:mitarbeiter(y),\\
                 h9:schlafen(x) \},
                 \end{tabular}\\
...
%\todostefan{S: Gepunktet und gestrichelt?
%  Rot ist bei schlechtem Licht nicht gut sichtbar.}
...
identifiziert und h3 mit h9 oder h6 wird direkt mit h8 identifiziert und h7 mit h1. Die Abbildungen~\ref{fig-Jede-Tochter-eines-Mitarbeiters-schläft-Allquantor} und~\ref{fig-Jede-Tochter-eines-Mitarbeiters-schläft-Existenzquantor} zeigen die geskopten MRSen.
Die skopusaufgelösten MRSen entsprechen genau den beiden Lesarten aus
(\ref{ex-Quantoren-Mitarbeiter-Tochter}).
\begin{figure}
\begin{tikzpicture}[scoped mrs]
...
Damit ist schon der wesentliche Teil der Analyse erklärt. Es fehlt lediglich die Umsetzung
in Merkmalbeschreibungen für die entsprechenden Lexikoneinträge. 
(\mex{1}) zeigt den Lexikoneintrag für \emph{Tochter}:

\eas
\label{le-Tochter}%
Lexikoneintrag für \emph{Tochter}:\\
\ms{
cat & \ms{ head & \ms[noun]{
                  case & \type{nom}\\
                  }\\
...
Zeichen gehört nicht nur ein Index, sondern auch ein \ltopm. \ltop\isfeat{ltop} steht dabei für \emph{local top}
...
Lexikoneintrag eingeführt wird (dem sogenannten \textsc{key}\isfeat{key}). Bei einfachen Beispielen wie dem
...
ergeben sich die gemeinsamen Label"=Werte h2 in (\ref{ex-every-klein-kind-schlafen}). In der
\textsc{hcons}-Liste eines Lexikoneintrags werden die Skopusbeschränkungen repräsentiert. Für
\emph{Tochter} gibt es keine, weshalb die Liste leer ist.
...
\zs
\emph{jede} ist ein Determinator, der selbst keine Argumente braucht, also eine leere \argstl und
somit auch eine leere \spr- und \compsl hat. Der Allquantor wird in der \relsl eingeführt. Der
\ltopw von \emph{jede} ist identisch mit dem Label des der Relation. \argzero ist das Merkmal für die
Variable. Der Wert von \argzero, also die Variable, ist \ibox{2}. \ibox{2} ist sowohl der Index-Wert
des Quantors als auch der Index-Wert des Nomens, zu dem der Quantor gehört.\footnote{
  Weder der \ltopw noch der \indw von Determinatoren wird in dem hier vorgestellten
  Grammatikfragment für irgend etwas benötigt. Der Vollständigkeit halber sind sie in
  (\ref{le-jede}) dennoch angegeben. In den folgenden Einträgen für Eigennamen (\ref{le-Aicke-1}) bzw.\ (\ref{le-Aicke-2})
  und Possessivpronomina (\ref{le-Possessivpronomen}) und in der Diskussion von Alternativen lasse ich den \ltopw und das \lbl der Quantor-Relation weg.
} Außerdem gibt es je ein Merkmal
...
\ex \ibox{3}:every\_q(\ibox{2}, \ibox{4}, \etag)
\zl
%\largerpage[-2]
Für die Restriktion wurde oben angenommen, dass sie in einer \qeq\vspace{-2pt}\hspace{-.5ex}\hyp
Relation zum Handle der \nbar steht. Das Nomen kann zwar über die \sprl auf den Determinator zugreifen, aber andersrum ist das nicht
...
ist (\,\ibox{3} \qeq~\ibox{1}\,). Die entsprechende \qeq"=Beschränkung steht in der \hconsl\isfeat{hcons}, der Liste
...
dominiert. Der \textsc{body}"=Wert des Quantors wird einfach nicht angegeben. In (\mex{0}) wird das
durch die leere Box \etag\is{\etag} dargestellt. Man kann das Merkmal auch wie in (\ref{le-jede}) einfach ganz weglassen. Die
fehlende Angabe des \body"=Wertes ist Teil der
Unterspezifikation: Irgendein Handle einer anderen Relation oder eines anderen Quantors kann dort
eingefügt werden, Hauptsache, zum Schluss sind alle Variablen im Bereich eines Quantors und keiner der
...
\end{figure}
%\itdopt{Lexikoneintrag für schlafen fehlt noch.}
Wie im Abschnitt~\ref{Abschnitt-Komposition} erklärt, werden die Relationen einfach durch Listenverkettung von den
Töchtern zum Mutterknoten hochgereicht. Genauso funktioniert das für die Handle"=Constraints:\isrel{collect-hcons}
\ea
\type{phrase} \impl\\[1mm]
\onems{
...
zugreifen zu können, obwohl die Nicht"=Kopftochter vom Kopf selegiert wird (\zb über das
Valenzmerkmal \spr). \spec ermöglicht also zusammen mit der Selektion über ein Valenzmerkmal wie \spr eine
...
selegieren. Es ist aber für Spezifikatoren in PPen nicht notwendig, die Projektion der Präposition
sehen zu können. Sie brauchen also kein \specm. Somit muss man Kopf"=Spezifikator"=Strukturen mit Zugriff
...
\mbox{}\hfill\onems{ head-dtr \ibox{1}\\
              non-head-dtrs \nliste{ [\textsc{cat|head|spec} \ibox{1}] } }
\z
%\largerpage


\subsection{Eigennamen und Possessivpronomen}
...
vorliegenden Grammatikfragment wird das anders gemacht. Es wird als Eigenschaft einer bestimmten Entität
angesehen, dass sie \emph{Aicke} heißt. Die Repräsentation mittels Relationen erfolgt
...
Quantoren. Die Repräsentation in (\mex{0}) legt aber eine solche Interpretation nahe. Durch
...
% entsprechende Relation. Der Eintrag für das Pronomen \emph{er} von S.\,\pageref{le-er} kann nun wie folgt um die \rels- und \hconswe
% erweitert werden:
% \eas
...
ihr Buch
\z
Den Lexikoneintrag für \emph{ihr} zeigt (\ref{le-Possessivpronomen}).
% Ich schiebe das per Hand nach hinten, dann stimmen die Beispiel-Nummern.
% \begin{figure}[htb]
% \eas
...
\z
Das Possessivpronomen verhält sich wie ein definiter Artikel und die Restriktion des entsprechenden
Quantors def\_q, nämlich h2, ist \emph{quantifier equal} (\qeq) mit dem Handle der besitzen"=Relation h4, das mit dem Handle von
...
% von vorn wegen Layout nach hier verschoben.
\eas
\label{Lexikoneintrag Possessivpronomen}%
\label{le-Possessivpronomen}%
Lexikoneintrag für das Possessivpronomen \emph{ihr}:\\
...
\zs
Der Index wird in Kopf-Adjunkt-Strukturen immer vom Kopf beigesteuert. Darauf komme ich im Abschnitt~\ref{Abschnitt-Semantikprinzip} zurück.
...
\ex \textmrs{ h1, \{ h1:potentiell(x), h1:mörder(x) \}, \{  \}  }
\ex\label{mrs-potentielle-Mörder}
\textmrs{ h1, \{ h1:potentiell(h2), h2:mörder(x) \}, \{  \}  }
\zl

\noindent
Für das Adjektiv \emph{potentielle} ergibt sich:\LATER{Was ist denn der Indexwert von \emph{potentiell}?}
\eas
...
\zs
Der \ltopw des modifizierten Nomens \iboxb{1} wird unter \relation{potentiell} eingebettet. 
...
Prädikats. Bei intersektiven Adjektiven\is{Adjektiv!intersektives} wie \emph{klein} sind beide Handles identifiziert, weshalb
...
modifizierten \nbar zugreifen können müssen und weshalb man das \specm braucht. Man könnte ja
vermuten, dass man stattdessen vom Nomen auf den Determinator zugreifen kann und
...
reicht aber nicht aus, da es Adjektive wie \emph{potentiell} gibt. Im Lexikoneintrag des Nomens
...
\ex $\forall$x \relationunmarked{affe}(x) $\to$ \relationunmarked{glauben}(x,$\exists$y \relationunmarked{einhorn}(y) $\wedge$ schlafen(y))
\ex $\exists$y \relationunmarked{einhorn}(y) $\wedge$ ($\forall$x \relationunmarked{affe}(x) $\to$ \relationunmarked{glauben}(x,schlafen(y)))
\ex $\forall$x \relationunmarked{affe}(x) $\to$ ($\exists$y \relationunmarked{einhorn}(y) $\wedge$ \relationunmarked{glauben}(x,schlafen(y)))
\zl
...
\ex every\_q(x, affe(x), glauben(x,exist\_q(y, einhorn(y), schlafen(y))))
\ex exist\_q(y, einhorn(y), every\_q(x,affe(x), glauben(x,schlafen(y)))
\ex every\_q(x, affe(x), exist\_q(y, einhorn(y), glauben(x,schlafen(y)))
\zl
...
\hphantom{\textlangle~}\{ h2 \qeq h4, h6 \qeq h11, h8 \qeq h10 \}  }
\z
...
semantischen Beitrag von \emph{einfaches Beispiel} einbettet.\footnote{%
 ...
 findet man in \citew[Abschnitt~4.3]{Mueller99a}. \citet[Abschnitt~3.2]{KR2024a} diskutieren sie im Semantik"=Kapitel des HPSG"=Handbuches.
...
selbst, eine unzulässige Konfiguration. Die Gleichsetzung der \ltopwe von \relation{scheinbar},
\relation{einfach} und \relation{beispiel} kommt in der bisherigen Analyse dadurch zustande, dass
...
\ltopw von \emph{scheinbar einfaches} ist aber identisch mit dem von \emph{scheinbar}, weil ja
\relation{scheinbar} die Relation \relation{einfaches} unter sich einbettet, so dass dann
...
im Lexikoneintrag mit dem des modifizierten Nomens identifiziert wird \citep[Abschnitt~6.3]{CFPS2005a}. 
\eas
...
Es stellt sich die Frage, ob es gut ist, die Tatsache, dass es um skopale oder intersektive
...
auf das Vorliegen einer skopalen oder intersektiven Relation zu testen. Die Repräsentation als
...
das Englische \citep{CF2000a-u} erzeugt werden, überwiegend sogenannte Netze\is{Netz}. Bei ihren
...
Es folgt eine Definition für \emph{Netz}:
\footnote{
...
\end{quote}
In Abbildung~\ref{fig-alle-Affen-glauben-dass-ein-Einhorn-schläft} gibt es das atomare Fragment
h5:glauben(x,h6). h5:glauben(x,h6) hat eine ausgehende Dominanzkante, nämlich h6, aber da h6 atomar ist, gibt es
keinen Knoten, aus dem keine Dominanzkante austritt. Ich habe die Definition also so
abgeändert, dass statt genau einem Knoten ohne Dominanzkante höchstens ein Knoten ohne Dominanzkante
verlangt wird.
% Doch! Dann wäre das Fragment eben größer und würde alles, was schon feststeht einschließen.
% Aber dann wäre in dem Fragment viele Knoten ohne ausgehende Dominanzkanten!
% Aber selbst wenn man 1.\ auf Fragmente mit
...
% über Quantoren haben, die rechts stehen, dann kann man auch Restriktionen für den Body von Quantoren
% formulieren.
}
\begin{definition}
Eine MRS ist ein Netz\is{Netz}, gdw.\ jedes Fragment\is{Fragment} aus dem Dominanzgraph der MRS die folgenden beiden
Bedingungen erfüllt:
\begin{enumerate}
\item Es gibt höchstens einen Knoten ohne austretende Dominanzkante.
\item Wenn ein Knoten X zwei (oder mehr) ausgehende Dominanzkanten hat, \zb zu Y und Z, dann sind Y
  und Z über einen hypernormalen Pfad verbunden, der nicht über X verläuft.
\end{enumerate}
\end{definition}
Die Dominanzkanten\is{Dominanzkante} sind dabei Kanten, die Handles über explizite oder implizite
Dominanzbeschränkungen verbinden. Also die gestrichelten Linien in den oben diskutierten
MRSen. Fragmente sind die Graphen, die durch durchgehende Linien dargestellt werden. Bei Fragmenten
sind die Skopusverhältnisse schon klar.
...
\end{figure}
\LATER{Punkte für Knoten hinzufügen, check, Y2 ist in der mittleren Abbildung zu weit vom Pfeil weg.}
...
Verbindung zwischen Y und Z über Y2 nicht, da sie auch zwei aus Y2 ausgehende Dominanzkanten hat. Setzt man
Y2 irgendwo ein, bleibt das Problem bestehen, dass man Knoten mit zwei ausgehenden Dominanzkanten
hat. Wenn es dagegen Verbindungen über hypernormale Pfade gibt, sind die möglichen Auflösungen der
Dominanzbeschränkungen einfacher zu ermitteln.
...
zwischen l2, l5 und l8 jeweils hypernormale Pfade gibt, die nicht über h1 verlaufen: l2, l12, h6, l5
...
\caption{Dominanzgraph für \emph{Each section is also suitable as s single day tour.} nach
  \citet[132]{FKT2005a}. Die roten Kanten sind Kanten, die sich implizit aus Variablenbindungen oder
...
\zl
Das Possessivpronomen \emph{sein} führt einen Quantor für die Variable von \emph{Kind} ein und eine
...
mit dem Bezugsnomen des Relativsatzes (x) identifiziert wird. Die MRS für (\mex{-1}b) ist:
\ea
\label{MRS EIn Affe dessen Kind schläft lacht}
\textmrs{ h0, \{ \begin{tabular}[t]{@{}l@{}}
                 h1:exist\_q(x, h2, h3), h4:affe(x), h5:def\_q(y, h6, h7), ??:besitzen(e1, x, y),\\
...
Handle von \relation{besitzen} nicht wie bei \emph{ihr} bzw.\ \emph{sein} mit dem Handle des Nomens \emph{Kind}
...
für y in der obigen MRS gut, aber für x funktioniert es nicht, denn x kommt in h8, h4 und h9 vor und
...
\end{figure}
Im Abschnitt~\ref{Abschnitt-rs-sem-Analyse} wird die Analyse vorgestellt, die die korrekte MRS liefert. 
...
Die Typhierarchie für die Untertypen von \type{headed-phrase} mit dem Typ
\type{head-non-adjunct-phrase} zeigt Abbildung~\ref{abb-headed-phrase-head-non-adjunct-structure}.
...
type hierarchy,
 ...
 % } 
    [headed-phrase, l sep*=2, for children={l sep*=4}
      ...
      [head-non-complement-phrase, name=non-arg,calign with current
...
\end{forest}}
\caption{\label{abb-headed-phrase-head-non-adjunct-structure}Typhierarchie für \type{headed-phrase}}
\end{figure}


Damit ist geklärt, wie sich der \ltop"=Wert der Mutter ergibt. Es muss aber auch die Bestimmung des
...
\z
(\mex{0}) besagt: Nur wenn das Adjunkt nicht skopal ist, werden die \ltopwe identifiziert. In allen anderen Fällen
...
\ea
\label{constraint-collect-rels-collect-hcons}
...
Konstruktion ist zum Beispiel die NPN"=Konstruktion \parencites{Matsuyama2004a,Jackendoff2008a,Bargmann2015a}[Abschnitt~6.2]{Bargmann2023a-u}.
\ea
...
Das entsprechend ausbuchstabierte Schema für artikellose Nominalphrasen im Plural zeigt (\mex{1}):
\eas
...
In den folgenden Abschnitten diskutiere ich die Skizze von \citet*{CFPS2005a} für die Behandlung der
intersektiven und skopalen Modifikation (Abschnitt~\ref{Abschnitt Intersektive und skopale Modifikatoren}), die Analyse bestimmter Lesarten bei Modifikation durch das
Adverb \emph{wieder} und warum man dafür keine leeren Elemente in der Syntax braucht (Abschnitt~\ref{Abschnitt-Leere Elemente und sublexikalischer Skopus}), die DP-Analyse
und Possessivpronomina (Abschnitt~\ref{sec-dp-analyse}) und zu guter Letzt die Analyse von
Nominalgruppen ohne Determinierer (Abschnitt~\ref{Abschnitt-Lexikalische Abbindung von Determinatoren}).  

\subsection{Intersektive und skopale Modifikatoren}
\label{Abschnitt Intersektive und skopale Modifikatoren}
...
die beiden Untertypen \type{intersective-phrase} und \type{scopal-phrase}. Für Strukturen vom Typ
\type{intersective-phrase} werden die \ltopwe der beiden Töchter identifiziert. Für Strukturen
...
Grammatik, die bei intersektiver Modifikation auch nur die Struktur für intersektive Modifikation
...
\ex \textmrs{ h0, \{ h1:scheinbar(h2), h3:einfach(x), h1:beispiel(x)  \}, \{ h2 \qeq h3  \}  }
\zl
...
% sie selbst ausgearbeitet hat. 
Man könnte diesen Teil des Problems lösen, indem man einen Zeiger wie
...
\relation{scheinbar}"=Relation. Der Key von \emph{scheinbar einfache} wäre \relation{einfach}. Wenn diese Relation vom Typ \type{scopal-reln} ist, dann handelt es
...
habe ich ein Kopfmerkmal verwendet, das für einen Modifikator sagt, ob er skopal oder intersektiv
...
intersektive und skopale Modifikation zu haben. Es wird ein einziges Schema benutzt. Mit Hilfe
...
\label{Abschnitt-Leere Elemente und sublexikalischer Skopus}
...
Sätze wie (\mex{1}) sind interessant, da sie verschiedene Lesarten haben \citep[siehe][Abschnitt~5.6]{Dowty79a} und es nicht offensichtlich ist, wie man diese ableiten kann.
\ea
...
\relation{offen} beziehen kann. \Citet{Stechow96a} hat deshalb die Analyse in
Abbildung~\vref{Abbildung-wieder-oeffnen-Stechow} entwickelt (siehe S.\,93 und Abschnitt~3).
\begin{figure}
\centering
\begin{forest}
no word baseline
[AgrSP
	...
	[AgrS$'$
		...
		[AgrS]]]
\end{forest}
\caption{\label{Abbildung-wieder-oeffnen-Stechow}Dekomposition in der syntaktischen Struktur nach \citet{Stechow96a}}
\end{figure}%
...
übernommen wurden \parencites[siehe][Abschnitt~4.6.1.1]{MuellerGT-Eng5}[Abschnitt~4.10.2]{MuellerGermanic}. Nominalphrasen 
...
Adverb an die XP\footnote{%
  XP wird für gewöhnlich als Symbol benutzt, wenn die Kategorie X offen gelassen wird. Zum Beispiel
  wenn man über allgemeine Schemata spricht, in die verschiedenen Kategorien wie N, A oder P
  eingesetzt werden können. Siehe Abschnitt~\ref{sec-xbar}. In der Analyse eines konkreten Satzes
  dürfen allerdings nur Knoten mit Kategorien vorkommen, die in der Grammatik der betreffenden
  Sprache verwendet werden. Dass in Abbildung~\ref{Abbildung-wieder-oeffnen-Stechow} XP verwendet
  wird, bedeutet also entweder, dass \citeauthor{Stechow96a} sich auf keine Kategorie festlegen
  wollte oder dass er wirklich eine Kategorie X annimmt. Beides deutet auf Schwächen in der Analyse hin.
} und hat dann Skopus über offen(x) und in der Lesart mit weitem Skopus geht
...
\citet{Egg99a} entwickelt eine Analyse für die \emph{wieder}"=Fälle im Rahmen der \emph{Constraint
  Language for Lambda"=Structures} (CLLS)\is{Constraint Language for
...
\zl
Diese beiden Lesarten kann man unterspezifiziert in einem Dominanzgraphen repräsentieren, wie ihn
Abbildung~\ref{Abbildung-Max-alle-Fenster-oeffnete} zeigt.
\begin{figure}
\begin{tikzpicture}[mrs]
% \draw (-5,10) to[grid with coordinates] (5,-.5);
...
einzigen Lexikoneintrags. (\mex{1}) zeigt den Lexikoneintrag für \stem{öffn}.
%\todostefan{check ob
%  der gleich danach kommt, sonst Punkt als Satzzeichen}
\begin{figure}
\eas
\label{le-öffnen}%
...
\end{figure}
% Abbildungen wegen Layout nach hier verschoben.
\begin{figure}
\begin{tikzpicture}[scoped mrs]
% \draw (-5,10) to[grid with coordinates] (5,-.5);
\node(offen)   at (0,0) {h7:offen(x)};
...
%hier HPSG gewählt. 
Wie Abbildung~\vref{Abbildung-Fenster-oeffnete-MRS} zeigt, handelt es sich bei der
...
macht, die in Abschnitt~\ref{sec-realized} vorgestellt wird. Im Zusammenhang mit Kasusvergabe hat
\citet{Meurers99b} sogenannte Spirits eingeführt. Damit wird es möglich, den Kasus für \emph{diesen
...
die NP für das Objekt \emph{alle Fenster} den \textsc{realized}"=Wert $+$ hat und die Subjekt-NP
...
[ cat & [ head & [\type{adv}\\
  ...
  cont & [ ltop & \ibox{4}\\
           ...
           hcons & < [\type{qeq}\\
                      harg & \ibox{5}\\
                      larg & \ibox{2} ] > \+ \texttt{outscope-realized}\upshape!(!\ibox{4},
                      \ibox{5}, \ibox{1}!)! ] ]
}
\zs
\emph{wieder} modifiziert ein Verb mit einer \compsl \ibox{1}. \emph{wieder} ist ein Adverb, hat
also eine leere \argstl. Der \ltopw von \emph{wieder} ist identisch mit dem
...
es eine \qeq-Beschränkung, die auf der linken Seite das Argument von \emph{wieder} \iboxb{5} hat
und auf der rechten Seite das Handle des Quantors des jeweiligen Arguments des modifizierten Verbs (\,\ibox{5} \qeq l1). Für
...
\ibox{4}, dominiert (l2 \qeq \ibox{4}\,).\todostefan{Constraint angeben?}
% scopal_modifier :=
%  (@modifier,
%   @arg0_le,
%   synsem:loc:(cat:head:(mod:loc:(cat:comps:Subcat,
%                                  cont:ltop:LTop),
%                         scopal:plus),
%               cont:ltop:Label),
%   rels:hd:(lbl:Label,
...
%   h_cons:[(qeq,
%            harg:Handle2,
%            larg:LTop)|outscope_realized(Label,Handle2,Subcat)]).
% outscope_realized(Label,Handle,Subcat,ScopeConstraints) if 
%    when( (Subcat=(e_list;ne_list);
%           ScopeConstraints=(e_list;ne_list)) 
%        , undelayed_outscope_realized(Label,Handle,Subcat,ScopeConstraints)
%        ).
%
% % The other scope constraints cause the MRSes to be graphs that are not normal.
% % This is a problem for utool.
% undelayed_outscope_realized(_Label,_Handle,_,[]) if true,!. % no scope constraints for debugging
%
% undelayed_outscope_realized(_Label,_Handle,[],[]) if true.
%
% % Arguments that are realized are outscoped by a scopal adjunct to their left.
% % The argument Handle is above the quantifier handle.
% undelayed_outscope_realized(Label,Handle,[(realized:plus,
%                                      arg:loc:cont:quant:lbl:QH
%                                     )|Subcat],[(qeq,
%                                                 harg:Handle,
%                                                 larg:QH)|ScopeConstraints]) if
%     outscope_realized(Label,Handle,Subcat,ScopeConstraints).
%
% % Arguments that are not realized yet outscope the scopal adjunct to their reight.
% % The handle for the quantifier body is above the lable of the adjunct.
% undelayed_outscope_realized(Label,Handle,[(realized:minus,
%                                            arg:loc:cont:quant:lbl:QH % body:QH
%                                            % This is a hack that was suggested to me by Stefan Thater 26.02.2009
%                                            % If we refer to the body, the resulting dominance graph is not a net.
%                                            % St. Mü. 01.03.2009
%                                           )|Subcat],[(qeq,
%                                                       harg:QH,
%                                                       larg:Label)|ScopeConstraints]) if
%     outscope_realized(Label,Handle,Subcat,ScopeConstraints).
%
%
...
\LATER{\subsection{Inkrementelle Verarbeitung}}
...
das Spezifikatorprinzip muss formalisiert werden, und außerdem braucht man das Valenzmerkmal \spr
und eine entsprechende Grammatikregel für Determinator"=Nomen"=Strukturen (siehe
Abschnitt~\ref{Abschnitt-SPR-Merkmal}). Eine Analyse, die davon ausgeht, dass der Determinator der
Kopf\is{Kopf} ist, scheint wesentlich einfacher zu sein. Für den Quantor \emph{jede} könnte man
statt (\ref{le-jede}) die Struktur in (\mex{1}) annehmen:

\eas
\emph{jedes} in der DP-Analyse:\\
\ms{
cat & \ms{ head & \ms[det]{
                  case & \type{nom} }\\
           arg-st & \liste{ NP[\ltop \ibox{1}, \textsc{ind} \ibox{2}] }\\
         }\\
cont & \ms{
       rels & \liste{ \ms[every\_q]{
                      arg0 & \ibox{2}\\ 
...
\noindent
Das \argst-Element würde dann auf \comps gemappt und der Determinator würde rechts von sich mit
einer NP (einer vollständigen Nominalprojektion, die im DP-System keinen Determinator brauchen
würde) verlangen. Eine mit diesem Determinator gebildete DP hätte (vom \headw abgesehen) dasselbe Aussehen
...
\end{forest}
\hfill\mbox{}
\caption{DP-Analyse mit Determinator als Kopf}
\end{figure}

Problematisch an dieser DP"=Analyse sind aber die Possessivpronomina. Man könnte sich
hier statt (\ref{le-Possessivpronomen}) folgenden Lexikoneintrag vorstellen:
\eas
\textit{ihren} (falsch, Index des Possessivums statt des Nomens):\\
\ms{ 
  cat & \ms{ head  & \ms[det]{
                     case & nom}\\
             arg-st & \nliste{ NP[\ltop \ibox{1}, \textsc{ind} \ibox{2}] }\\
           } \\
  cont &  \ms{
%       ltop & \ibox{3}\\
       ind  & \ibox{3} \ms{
...
\zs
Das Problem besteht nun darin, dass die Repräsentation für \emph{ihren Kater},
die mit (\mex{0}) gebildet werden kann, die falsche Index"=Information hat, denn das Semantikprinzip
sorgt dafür, dass der semantische Beitrag in einer Kopf"=Komplement"=Struktur vom Kopf kommt. In der
...
\begin{figure}
\eas
\textit{ihren Kater} (falsch, Index des Possessivums statt des Nomens):\\
\ms{ 
  cat & \ms{ head  & \ms[det]{
                     case & nom}\\
             comps & \eliste\\
...
Das Nomen ist jedoch das Element, das eine semantische Rolle eines Verbs füllen kann.
Würde man mit dem Eintrag in (\mex{-1}) den Satz in (\mex{1}) analysieren,
bekäme man eine falsche Repräsentation, in der \emph{friert} über \emph{ihren}
prädiziert. Die semantische Repräsentation wäre dann (\mex{1}c) statt (\mex{1}b).
\eal
\ex Ihren Kater friert.
\ex \textmrs{ h0, \{ h1:def\_q(x, h2, h3), h4:besitzen(e1, i1, x), h4:kater(x), h3:frieren(x) \},\\
              \hphantom{\textlangle~}\{ h2 \qeq h4 \}  }   
\ex  \textmrs{ h0, \{ h1:def\_q(i1, h2, h3), h4:besitzen(e1, i1, x), h4:kater(x), h3:frieren(i1) \},\\
              \hphantom{\textlangle~}\{ h2 \qeq h4 \}  }   
\zl

\noindent
Man könnte versuchen, das Problem zu beheben, indem man den Index
des Nomens zum Index von \emph{ihr} macht. Somit würde dieser Index
...
repräsentiert:
\eas
\textit{ihren} (falsch, Index von Nomen übernommen):\\
%\oneline{%
\ms{ 
  cat & \ms{ head  & \ms[det]{
                     case & nom}\\
...
\citew[Kapitel~6]{ps2}, \citew{Branco2021a} und \citew{MuellerBinding}. Man könnte vorschlagen, die Bindungstheorie auf semantischen Repräsentationen
...
in \textsc{rels} verfügbar sein.\is{Bindungstheorie|)} 
Eine Analyse mit Possessivum als Kopf scheidet also aus. Damit bleiben
nur zwei Möglichkeiten: die NP"=Analyse (Abbildung~\ref{Abbildung-DP-Possessivum} links) und eine Analyse, die einen leeren Determinator\is{leere Kategorie} als Kopf
nimmt, der dann mit Possessivum und NP verbunden wird \citep[\page 50, \page 53]{Abney87a}. Diese
Analyse ist in Abbildung~\ref{Abbildung-DP-Possessivum} rechts dargestellt. Die Kategorie von \emph{ihren}
wäre in solch einer Analyse unklar. Die entsprechend flektierten Possessivpronomina können nur an
der Stelle vor dem Nomen und vor den Adjektiven auftreten, sind aber selbst nicht äquivalent zu DPen
wie \emph{ihren Kater}. 
\eal
\ex[]{Ihr Kater schnurrt.
}
\ex[*]{
Ihr schnurrt.
}
\zl
Die Flexion entspricht jedoch klar der Flexion des Artikels \emph{ein}. Würde man \emph{ihren} die Kategorie Possessivum geben oder irgendeine andere Kategorie,
wäre die Parallelität zum Artikel nicht erfasst.
% seine eine
% ihr ein
% ihre eine
% dein ein
...
\end{forest}
\hfill\mbox{}
\caption{NP-Analyse und DP-Analyse mit leerem Kopf und Possessivpronomen}\label{Abbildung-DP-Possessivum}
\end{figure}

Die Analyse mit einem leeren Kopf für den Determinator und mit einer Spezialkategorie für das Possessivum wäre auf jeden Fall komplizierter
...
\eal
\ex Connys Bruder
\ex Connys Zerstörung der Stadt
\zl
Für beide Beispiele gibt es eine Lesart, in der \emph{Conny} ein semantisches Argument des Nomens ist. Wäre
\emph{Connys} kein syntaktisches Argument der jeweiligen Nomina, wäre unklar, wie das Linking
...
\label{Abschnitt-Lexikalische Abbindung von Determinatoren}
...
\eal
\ex Conny trinkt Milch.
\ex Conny mag Kinder.
\zl
Für Stoffnomina und Nomina im  Plural nimmt sie an, dass diese eine leere \sprl haben, also keinen
Determinator selegieren (zum \sprm siehe Abschnitt~\ref{Abschnitt-SPR-Merkmal}). Der semantische Beitrag des
Determinators wird bereits im Lexikon in die semantische Repräsentation des Nomens (in die
\textsc{frames}"=Liste) aufgenommen. Folgende Repräsentation zeigt, wie Stoffnomina ihrer Meinung
nach behandelt werden sollten:\is{Skopus|(}
\ea
...
Adjektive, das anhand der Beispiele in (\mex{1}) erläutert werden soll, auf die bereits \citet[\page
319]{Netter94} hingewiesen hat:\footnote{
  Siehe auch \citew[Problem 2, 265--266]{SWB2003a}.
}
\eal
\ex[]{
der gute Wein
...
(\mex{0}c) automatisch aus der Analyse, die im Abschnitt~\ref{sec-Adjunkte-psg} vorgestellt
...
\caption{\label{Abb-NP-Analysen}Analysemöglichkeiten für determinatorlose Nominalphrasen: leerer
  Determinator, unäre Projektion, Lexikonregel. Der Lexikonregelansatz macht falsche Vorhersagen.}
\end{figure}
...
bereits in der Grammatik gibt (siehe Kapitel~\ref{Kapitel-Lexikon} zur Organisation des Lexikons in der
...
%% \is{Skopus|)}
\is{Konstruktionsgrammatik (CxG)|)}\is{Semantik|)}
...
\ex mit großem Eifer
\zl
Überlegen Sie, wie Sie Information bzgl.\ morphologischer Eigenschaften des Adjektivs repräsentieren
können. Denken Sie dazu auch über die Eigenschaften des Nomens nach, das modifiziert wird.
...
\citet{KR2024a} geben einen Überblick über Semantik im Rahmen der HPSG. \citet*{HPSGHandbookLinking}
...
damals gemeinsam mit Walter Kasper die deutsche \Verbmobil-Grammatik entwickelt \citep{MK2000a}. Das Buch von
