...
\begin{forest}
sm edges
[{V$_e$[\nliste{ every\_q(x, \ldots), klein(x), kind(x), schlafen(e,x) }]}
  [{NP$_x$[\nliste{ every\_q(x, \ldots), klein(x), kind(x) }]}
    [{Det$_x$[\nliste{ every\_q(x, \ldots) }]} [alle]]
    [{N$_x$[\nliste{ klein(x), kind(x) }]}
      [{Adj$_x$[\nliste{ klein(x) }]} [kleinen]]
      [{N$_x$[\nliste{ kind(x) }]} [Kinder]]]]
...
Adjektiven, Nomen und Verben bereits besprochen.\todostefan{Adjektive fehlen} Bei der Diskussion des Adjektivs \emph{kleine}
...
\zs\todostefan{nach qeq ist mehr Platz als davor}
