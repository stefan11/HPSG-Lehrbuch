...
markiert sein Beispiel (60) mit einem Fragezeichen, ich finde ihn aber sehr markiert.
...
Man beachte, dass alle Argumente der \argstl auf \comps gemappt werden und es zusätzlich noch das
\subjm gibt. Ich werde auf diese unkanonische Verteilung der Argumente im Abschnitt~\ref{sec-ARP-und-Partizipien} zurückkommen.
...
arg-st    & \nliste{ \ibox{1} NP[\str]\ind{2} } $\oplus$ \ibox{3} $\oplus$ \nliste{ V[\type{inf}, \textsc{subj} \sliste{ NP[\str]\ind{2}}, \comps \ibox{3}]}\\[2mm]
}
\zs
...
die Verbalkomplexbildung von \emph{zu helfen} und \emph{versucht} wird das Dativobjekt von \emph{zu
  helfen} zum Dativobjekt von \emph{zu helfen versuche}. Analog zum Fernpassiv würde man nun
erwarten, dass ein Ferndativpassiv mit bekommen wie in (\mex{1}b) möglich ist.
\eal
\ex[]{
...
\subsection{Das Argumentrealisierungsprinzip und nicht-finite Verben}
