...
Subjekt durch die Passivierung unterdrückt, weshalb die Wortfolge nicht wohlgeformt ist.
...
% handelt.
% %SigurÃÂ°sson sagt, dass es da zumindest keine Kongruenz mit dem Nominativ gibt.
...
Bewegungsverben, haben aber eine andere Bedeutung. Sie bilden wie die meisten Bewegungsverben
das Perfekt mit \sein und sind passivierbar:
%\todostefan{Ich habe mich verfahren.}
\eal
\ex\iw{verfahren}
...
\ex denn unter beiden Kirchendächern kann jemand, der anfängt zu denken und auf"|hört
...
Die Aussage, dass expletive Subjekte nicht durch Passivierung unterdrückt werden können, muss also
...
besteht keine Einigkeit darüber, ob \emph{dass}"=Sätze Subjekte oder Komplemente sind \citep*{DL2000a-u,Berman2003b-u,Berman2007a-u,AMM2005a-u,Forst2006a-u}. Die Stellung
...
mit einem Verb kombiniert, das im Aktiv den Dativ regiert. Dieser Dativ wird dann
...
Dativ eine Rezipientenrolle zuweisen muss
\parencites[369]{Eroms78}[21]{Haider86}[144]{Eisenberg94a}[228]{HM94a}[221--222]{Kathol2000a}[\page102]{Gunkel2003b},
%Eroms zitiert Brinkmann75a:217 aber die zweite Auflage ist eigentlich von 71.
ist nicht adäquat.
...
\citet{Wurmbrand2003a} gibt jedoch die folgenden Beispiele mit \word{beginnen}, \word{vergessen} und \word{wagen},
die sie im World Wide Web gefunden hat:
%% \footnote{
...
%% }
\label{bsp-wurde-zu-bauen-begonnen}%
...
kohärenten Konstruktionen möglich. Bei inkohärenten Konstruktionen ist der Nominativ ausgeschlossen, wie die Beispiele in (\mex{1}) zeigen:
\eal
\ex[]{
...
%% Nicht Diebe sind es, die Autofahrer in erster Linie zum Kauf neuer Â«NummerntafelnÂ» zwingen. Es sind meist die Fahrzeugbesitzer selbst, die sich Umtrieb und Kosten auf"|halsen. 1999 wurden 154 gestohlene St. Galler Schilder ausgeschrieben, verloren wurden 723.
...
\zl
        ...
        englischen\il{Englisch} Passivkonstruktionen.%
}
...
\ex Aicke ließ es regnen.\label{bsp-Aicke-liess-es-regnen}
\zl
...
als auch die permissive Lesart möglich (zur Diskussion von (\ref{bsp-Aicke-liess-es-regnen})
siehe auch S.\,\pageref{bsp-Aicke-laesst-es-regnen}).
%
...
%\NOTE{\citet[\page150]{Primus99a-u},} hm ist nicht so eindeutig
...
wie (\mex{1}a) keine \emph{von}"=PP vorkommt.\footnote{
  ...
  \zl
  Man brauchte also zwei verschiedene \emph{haben}: Eins würde Partizipien mit PP-Argument
  selegieren und das PP-Argument zur NP machen und das andere würde Partizipien wie \emph{gegraut}
  verlangen, ohne eine PP zu einer NP umzuwandeln.
}
\eal
...
Konstituente mit \emph{angeführt} als Kopf bildet, \emph{von Grammatikern} aber von \emph{werden} abhängt.\footnote{
  Man könnte sich auf eine scheinbar mehrfache Vorfeldbesetzung\is{Vorfeldbesetzung!scheinbar mehrfache} nach dem Muster von (\ref{bsp-mehr-vf})
...
\label{ex-sanitaeter}
\ex[\#]{
Der Verletzte wurde von Karl zwischen zwei Sanitätern zum Krankenwagen gebracht.
%\NOTE{FB: Das Beispiel finde ich komisch. Für b muss man fast genauso lange überlegen, um einen passenden Kontext
%  zu finden, wie für a.}
}
\ex[]{
...
die zwei Lexikonelemente für die Partizipformen annehmen: einen für das Partizip Perfekt und einen
...
Der Vorteil solcher Anhebungsanalysen\is{Anhebung} ist, dass ein einziges Lexikonelement für das Partizip II zur Analyse
...
verhalten. Aus verschiedenen Gründen, die ich gleich darlegen werde, möchte man das Element,
...
a.&ankommen (unakkusativisch):    & \eliste                         & \sliste{ NP[\type{str}] }\\[2mm]
b.&tanzen   (unergativisch):      & \sliste{ \ibox{1} } & \sliste{ \ibox{1} NP[\type{str}] }\\[2mm]
c.&auf"|fallen (unakkusativisch): & \eliste           & \sliste{ NP[\type{str}], NP[\type{ldat}] }\\[2mm]
d.&lesen      (transitiv):       & \sliste{ \ibox{1} } & \sliste{ \ibox{1} NP[\type{str}], NP[\type{str}] }\\[2mm]
...
muss ein Resultativum bei Wetterverben über ein Objekt und nicht über das \emph{es} prädizieren (siehe auch die Beispiele in
(\ref{bsp-regnen+resultativ-aktiv}) auf S.\,\pageref{bsp-regnen+resultativ-aktiv}):
\eal
\ex Es regnete die Stühle naß.\footnote{
  \citew[\page118]{Wunderlich97c}.
}
\ex {}[\ldots] und am Ende hagelt es die Ernte kaputt.\footnote{
\citew[256]{Felfe2020a}.
}
\zl
...
Argumentblockierungslexikonregel für Partizipien:\\[1mm]
%\begin{tabular}[t]{@{}l@{}}
\onems[stem]{
...
\ea
\begin{tabular}[t]{@{}l@{ }l@{\hspace{5ex}}l@{\hspace{5ex}}l@{}}
  &                               & \textsc{subj}\hspace{.4pt}/\hspace{.4pt}\textsc{da} & \textsc{comps}\hspace{.4pt}/\hspace{-.55pt}\argst\\[2mm]
a.&angekommen  (unakkusativisch): & \eliste                 & \sliste{NP[\type{str}]}\\[2mm]
b.&getanzt     (unergativisch):   & \sliste{NP[\type{str}]}   & \eliste\\[2mm]
c.&aufgefallen (unakkusativisch): & \eliste                 & \sliste{NP[\type{str}], NP[\type{ldat}]}\\[2mm]
d.&gelesen     (transitiv):       & \sliste{NP[\type{str}]}   & \sliste{NP[\type{str}]}\\[2mm]
...
g.&geregnet    (unergativisch):   & \sliste{NP[\type{str}]}   & \eliste\\
\end{tabular}
\z
%\vspace{-\baselineskip}\end{figure}

\noindent
In den Lexikonelementen der Partizipien sind alle \argst"=Elemente auf der \compsl
...
Der Lexikoneintrag in (\mex{1}) zeigt das Passivhilfsverb:\footnote{
  ...
  lediglich, dass die \textsc{da}-Liste des Partizips ein \type{synsem}"=Objekt enthält. Beschränkungen bzgl.\
...
\ms{
head$|$da & \eliste\\
arg-st    & \ibox{1} \oplus\ \sliste{ V[\type{ppp}, \textsc{lex}+, \textsc{da} \nliste{ \npref }, \textsc{comps} \ibox{1}] }\\
}
\zs
...
Wenn wir \emph{gelesen} oder \emph{geschenkt} mit \emph{wird} kombinieren, bekommen
...
% aber sie sind unter CAT und für Französisch wird auch von ARG-ST angezogen.
% und wenn, dann müsste diese Bemerkung in Kapitel 14.
% Die Argumente müssen von der \compsl und nicht von der \argstl des Partizips angezogen werden, weil
% im Vorfeld auch Phrasen stehen können und \argstl nur für Wörter definiert sind.
...
\ex[]{
weil der Roman gelesen wurde
}
\ex[*]{
weil gelesen worden wurde
}
\zl
...
kann auch der unakkusativische Verbalkomplex \emph{gelesen worden} nicht unter 
...
eingebettet wird, dann wird zur \compsl des eingebetteten Verbs nichts hinzugefügt (\,\iboxt{1} = \eliste).
...
\ms{
 head$|$da & \eliste \\[2mm]
 arg-st    & \nliste{ NP[\str]\ind{1}} $\oplus$ \ibox{2} $\oplus$ \liste{ \begin{tabular}{@{}l@{}l@{}}
                  V[&\type{ppp}, \textsc{lex}+, \textsc{da}~\nliste{ NP\sub{\type{ref}} },\\[1mm]
                          &\comps~\nliste{ NP[\type{ldat}]\ind{1}} $\oplus$ \ibox{2}] \\
                  \end{tabular}}\\
...
\z
\citet{Kathol91a}
%\addpages gibt es nicht zum Download Beispiel ist (60)
markiert sein Beispiel (60) mit einem Fragezeichen, ich finde es aber sehr markiert.
...
\zl
Ich finde die Beispiele grenzwertig, aber es sind Korpusbelege.\todostefan{Vorn bei Valenz
  diskutieren? CxG?}
...
\exewidth{(100)}
\ea
\label{ex-Infinitive-mit-zu}%
\begin{tabular}[t]{@{}l@{~}l@{~~}l@{~~~}l@{~~~}l@{}}
  &                               & \textsc{da}         & \textsc{subj}                         & \textsc{comps\hspace{.4pt}/\hspace{-.55pt}arg-st}                       \\[2mm]
a.&anzukommen  (unakkusativisch): & \eliste             & \sliste{ NP[\type{str}] }             & \eliste                                   \\[2mm]
b.&zu tanzen   (unergativisch):   & \sliste{ \ibox{1} } & \sliste{ \ibox{1} NP[\type{str}] }    & \eliste                                   \\[2mm]
c.&aufzufallen (unakkusativisch): & \eliste             & \sliste{ NP[\type{str}] }             & \sliste{ NP[\type{ldat}] }                  \\[2mm]
d.&zu lesen    (transitiv):       & \sliste{ \ibox{1} } & \sliste{ \ibox{1} NP[\type{str}] }    & \sliste{ NP[\type{str}] }                    \\[2mm]
...
g.&zu regnen   (unergativisch):   & \sliste{ \ibox{1} } & \sliste{ \ibox{1} NP[\type{str}] }    & \eliste                                   \\
\end{tabular}
\z

\noindent
Das blockierte Subjekt ist nicht mehr auf der \argst enthalten, ist aber als Wert von \subj im
Lexikonelement für den Infinitiv mit \emph{zu} enthalten und kann also vom modalen \emph{haben}
reaktiviert werden. Das modale \emph{sein} lässt das Subjekt blockiert. Die \compsl ist mit der \argstl identisch. Da \subj- und \compsl mehr Elemente
enthalten können als die \argstl, widerspricht das dem Argumentrealisierungsprinzip. Darauf werde
ich im Abschnitt~\ref{sec-ARP-und-Partizipien} zurückkommen.
%% 
...
der Lexikonregeln, die Wörter in der \type{inf}-Form
lizenzieren. Im Gegensatz zur Partizipregel wird der \daw des Stammes
ignoriert. Stattdessen wird das erste kasusrelevante Argument (Subjektsatz oder NP mit strukturellem
Kasus\is{Kasus!struktureller}, siehe S.\,\pageref{page-kasusrelevante-XP}) im \subjw der Regelausgabe repräsentiert.
\texttt{first-case-xp-str}\isrel{first-case-xp-str} ist eine relationale Beschränkung, die die Liste
\iboxt{1} in die beiden Teile \iboxt{2} und \iboxt{3} teilt, wobei \iboxt{2}
die erste kasusrelevante XP aus \iboxt{1} enthält, wenn es eine gibt,
und \iboxt{3} die anderen Elemente von \iboxt{1} enthält. Wenn \iboxt{1} 
keine kasusrelevante XP enthält, ist \iboxt{2} die leere Liste und \iboxt{1}
und \ibox{3} sind identisch.\isfeat{subj}
%
%\begin{figure}[htbp]
\ea
\label{lr-subj-reduction}%
Argumentblockierungslexikonregel für Infinitive mit \emph{zu}:\\[1mm]
%\begin{tabular}[t]{@{}l@{}}
\ms[stem]{
synsem$|$loc$|$cat & \ms{
head   & verb\\
...
\mbox{}\hfill$\wedge$ \texttt{first-case-xp-str}(\,\ibox{1}, \ibox{2}, \ibox{3}\,)%
%\end{tabular}
\is{Lexikonregel!Argumentblockierung}\isrel{first-case-xp-str}%
\z
%\vspace{-\baselineskip}\end{figure}
%
%\noindent
...
Die Lexikonelemente für das Partizip II mit (NP-)Subjekt unterscheiden sich also dadurch von den
Lexikonelementen der Infinitive mit \emph{zu}, dass sie ein wirkliches Subjekt, in \subj haben, \dash eine NP, die in
...
während bei den Infinitiven mit \emph{zu} alle Subjekte, \dash auch die mit
Objekteigenschaften, in \subj repräsentiert werden.

Wie schon erwähnt, werden alle Argumente der \argstl auf \comps gemappt und es gibt zusätzlich noch das
\subjm. Ich werde auf diese unkanonische Verteilung der Argumente im Abschnitt~\ref{sec-ARP-und-Partizipien} zurückkommen.
...
\zl
\largerpage
Im Eintrag für das \sein, das in modalen Konstruktionen benutzt wird, wird genauso wie beim
Vorgangspassiv (siehe (\ref{le-werden-passive-da}) auf S.\,\pageref{le-werden-passive-da})
...
arg-st    & \nliste{ \ibox{1} NP[\str]\ind{2} } $\oplus$ \ibox{3} $\oplus$ \nliste{ V[\type{inf},
  \textsc{subj} \sliste{ NP[\str]\ind{2}}, \comps \ibox{3}]} }
\zs
...
\ms{
head$|$da & \sliste{ NP[\str]\ind{2} }\\[2mm]
arg-st    & \ibox{3} $\oplus$ \nliste{ V[\type{inf}, \textsc{subj} \sliste{ NP[\str]\ind{2}}, \comps
  \ibox{3}] } }
\zs
...
(\mex{1}) wird durch (\mex{2}) beschrieben.\footnote{%
Die \argstwe habe ich der Übersichtlichkeit halber weggelassen. In (\mex{1}) wäre der \argstw mit
dem \compsw identisch, in (\mex{2}) entspricht der \argstw von \emph{versucht} der \compsl in
(\mex{2}) zuzüglich der Beschreibung des selegierten Infinitivs mit \emph{zu}.
}
\eas
\emph{zu reparieren}:\\
\ms{
head$|$subj & \sliste{ NP[\str] }\\
comps       & \sliste{ NP[\str] } }
\zs
...
\ms{
head$|$da & \sliste{ NP[\str] }\\
comps     & \sliste{ NP[\str] }}
\zs

\noindent
Das Objekt von \emph{zu reparieren} ist in der \compsl von \emph{zu reparieren versucht} enthalten,
und das Subjekt von \emph{versucht}, das mit dem Subjekt von \emph{zu reparieren} koindiziert ist,
ist blockiert (in \da).
...
\eas
Lexikoneintrag für \emph{erlaub-}:\\
\oneline{\ms{
head$|$da & \sliste{ \ibox{1} }\\[2mm]
...
\eas
Partizipform \emph{erlaubt}:\\
\ms{
head$|$da & \sliste{ NP[\str] }\\
arg-st    & \sliste{ NP[\ldat]\ind{2} } $\oplus$ \ibox{3} $\oplus$ \sliste{ V[\type{inf}, \textsc{subj} \sliste{ NP[\str]\ind{2}}, \comps \ibox{3}] }}
\zs

\noindent
Kombiniert man (\mex{0}) mit dem Eintrag für \emph{auszukosten}, bekommt man (\mex{1}).
\eas
\emph{erlaubt} in \emph{auszukosten erlaubt}:\\
\ms{
head$|$da & \sliste{ NP[\str] }\\
arg-st    & \sliste{ NP[\ldat], NP[\str], V }
}
\zs
\largerpage
...
Da das Objekt von \emph{auszukosten} das erste Element mit strukturellem Kasus in der \argstl des
Passivhilfsverbs in \emph{auszukosten erlaubt wurde} ist, muss es im Nominativ stehen.
%
...
Das Komplement von \emph{auszukosten} \iboxb{7} wird von \emph{erlaubt} angehoben. Die \compsl
...
Die einzige NP mit strukturellem Kasus auf dieser Liste und der entsprechenden \argstl ist das Objekt von \emph{auszukosten}.
Diese NP muss also im Nominativ realisiert werden.

Moana Schulze (p.\,M.\,2020) hat mich auf ein Problem hingewiesen, dass sich ergibt, wenn \emph{bekommen}/\emph{erhalten}/\emph{kriegen} beim
Dativpassiv Hilfsverben ohne weitere Beschränkungen bezüglich des eingebetteten Verbs sind. Durch
die Verbalkomplexbildung von \emph{zu helfen} und \emph{versuche} wird das Dativobjekt von \emph{zu
  helfen} zum Dativobjekt von \emph{zu helfen versuche}. Analog zum Fernpassiv würde man nun
erwarten, dass ein Ferndativpassiv mit \emph{bekommen} wie in (\mex{1}b) möglich ist.
\eal
\ex[]{
weil ich dem Tier zu helfen versuche.
}
\ex[*]{
Das Tier bekommt zu helfen versucht.
}
\zl
\largerpage
Mir ist unklar, worauf die Unakzeptabilität von (\mex{0}b) zurückzuführen ist. Vielleicht liegt es
ja daran, dass zwei markierte Konstruktionen -- das Fernpassiv und das Dativpassiv mit zweistelligen
Verben -- interagieren.
\is{Passiv!Fern-|)}\is{Anhebung|)}
%
...
%, das Zustandspassiv 
und das modale Passiv mit \emph{sein}. Sieht man von der Form des eingebetteten Verbs ab, ist der
...
\ms{
 head$|$da     & \sliste{ \ibox{1} }\\
 arg-st & \sliste{ \ibox{1} NP[\type{str}] } $\oplus$ \ibox{2} $\oplus$ \liste{ V[\type{bse}, \textsc{lex}+, \textsc{da}~\sliste{ NP\sub{\type{ref}}}, \textsc{comps}~\ibox{2}]}
% \begin{tabular}{@{}l@{}l@{}}
...
Adjektivableitungsregel für Partizipien:\\[1mm]
%
%
\onems[word]{
...
Argumentrealisierungsprinzip (S.\,\pageref{Prinzip-Argumentrealisierung-einfach} bzw. S.\,\pageref{Prinzip-Argumentrealisierung-einfach-SUBJ-lexical-sign}) teilt die \argstl eines Wortes in Subjekt, Spezifikator und Komplemente
auf. Dadurch, dass das Subjekt in der Ausgabe der Regel spezifiziert ist, muss es auf der \argstl
des Eingabeverbs eine NP mit strukturellem Kasus geben. Die jeweils erste kasusrelevante XP
...
Der semantische Beitrag des Adjektivstamms wird per Konvention zum Lexikoneintrag des flektierten Adjektivs
...
gespalten (zum Wirkungsbereich des Argumentrealisierungsprinzip wird im
Abschnitt~\ref{sec-ARP-und-Partizipien} noch mehr gesagt). Im Fall von \emph{klein} sind \spr und
\comps die leere Liste. Bei Adjektiven wie \emph{treu} und \emph{stolz} kann die \compsl auch eine NP oder PP
...
\ldots & \ms{ cat & \ms{ head    & \ms[verb]{
                     vform & ppp\\
                     da   & \ibox{1} \sliste{ NP[\str]\ind{2} }\\
                     subj & \ibox{1} \\
                     }\\
           spr    & \eliste\\
           comps  & \ibox{3}\\
           arg-st & \ibox{3} \sliste{ NP[\str]\ind{4} }\\
         }\\
cont & \ms{ ltop & \ibox{5}\\
            ind  & \ibox{6} event}}\\
...
unakkusativischen Verbs. Dieses (NP[\str]\ind{4}) wird dann zum Subjekt des
Adjektivs. NP[\str]\ind{2} ist im Adjektivstamm nicht mehr enthalten.
...
\eas
\emph{angekommene} (attributives adjektivisches Partizip):\\
\ms{
\ldots & \ms{ cat & \ms{ head    & \ms[adj]{
                     mod & \nbar\ind{3}\\
                     subj & \ibox{2}\\
...
Kasus des Subjekts des adjektivischen Partizips ist. (\mex{1}) zeigt das am Beispiel von
\emph{Kinder}. Wenn \emph{Kinder} im Dativ steht, muss \emph{einem nach dem anderen} verwendet
werden, wie man in (\mex{1}a) sehen kann. Dennoch beeinflusst der Kasus des Kopfnomens in (\mex{1}c)
nicht den Kasus von \emph{eines nach dem anderen} in der Adjektivphrase.
\eal
\ex[]{
weil ich den Kindern einem nach dem anderen helfe
}
\ex[*]{
weil ich den Kindern eines nach dem anderen helfe
}
\ex[]{
den [eines         nach  dem           anderen]$_i$ eingeschlafenen Kindern$_i$
}
\ex[*]{
den [einem         nach  dem           anderen]$_i$ eingeschlafenen Kindern$_i$
}
\zl
\largerpage[-2]
Die Lexikonregel in (\ref{lr-prenom-adj})
...
Im Vergleich zu früheren Auf"|lagen dieses Buches sind die Analysen komplexer geworden, weil die
Argumentstrukturliste als zugrundeliegende Struktur angenommen wurde. Daraus ergibt sich aber zum
ersten Mal auch die Möglichkeit, die Daten in (\mex{1}) vernünftig zu erfassen:
\eal
\ex[]{\label{ex-der-dieses-Problem-lösende-Linguist}
...
\ex[]{\label{ex-das-von-ihm-zu-lösende-Problem}
das (von ihm) zu lösende Problem
}
\zl
In früheren Auf"|lagen war ich davon ausgegangen, dass Subjekt von Infinitiven mit und ohne
\emph{zu} unter \subj repräsentiert wurden und alle anderen Argumente unter \subcat (hier
\comps). Möchte man annehmen, dass das Suffix \suffix{d} in
(\ref{ex-der-dieses-Problem-lösende-Linguist}) und in (\ref{ex-das-von-ihm-zu-lösende-Problem}) das
gleiche ist, dann müssen sich die Infinitivformen unterscheiden. Der Unterschied kann nun darin
bestehen, dass das Partizip II und der Infinitiv mit \emph{zu} ein Argument blockieren, während das
beim Infinitiv ohne \emph{zu} nicht der Fall ist. Bei Annahme einer Argumentstrukturliste kann man
den Unterschied erfassen, wenn man annimmt, dass bei den Lexikonelementen für Partizip II und
Infinitiv mit \emph{zu} ein Element von der \argstl entfernt wird (das \da"=Element bei der
Partizipbildung und das \subj"=Element bei den \emph{zu}"=Infinitiven). Bei den Infinitiven ohne
\emph{zu} wird hingegen nichts entfernt. Das Argumentrealisierungsprinzip verteilt die Argumente
ganz normal auf die Valenzlisten: Das Subjekt kommt auf die \subjl und alle anderen Argumente auf
die \compsl. Es ergeben sich also folgende Valenzmerkmale für unsere Beispielverben:
\largerpage[-2]
\ea
\label{ex-Infinitive-ohne-zu}%
\begin{tabular}[t]{@{}l@{~}l@{~~}l@{~~~}l@{~~~}l@{}}
  &                             & \textsc{subj}                & \comps                     & \argst                        \\[2mm]
a.&ankommen  (unakk):   & \sliste{ NP[\type{s}] }    & \eliste                    & \sliste{ NP[\type{s}] }     \\[2mm]
b.&tanzen    (unerg):   & \sliste{ NP[\type{s}] }    & \eliste                    & \sliste{ NP[\type{s}] }     \\[2mm]
c.&auffallen (unakk):   & \sliste{ NP[\type{s}] }    & \sliste{ NP[\type{ldat}] } & \sliste{ NP[\type{s}], NP[\type{ldat}] }                 \\[2mm]
d.&lesen     (trans):   & \sliste{ NP[\type{s}] }    & \sliste{ NP[\type{s}] }  & \sliste{ NP[\type{s}], NP[\type{s}] }                  \\[2mm]
e.&schenken  (ditrans): & \sliste{ NP[\type{s}] }    & \sliste{ NP[\type{ldat}], NP[\type{s}] } & \sliste{ NP[\type{s}], NP[\type{ldat}], NP[\type{s}] } \\[2mm]
f.&helfen    (unerg):   & \sliste{ NP[\type{s}] }    & \sliste{ NP[\type{ldat}] } & \sliste{ NP[\type{s}], NP[\type{ldat}] }                  \\[2mm]
g.&regnen    (unerg):   & \sliste{ NP[\type{s}] }    & \eliste                    & \sliste{ NP[\type{s}] }               \\
\end{tabular}
\z
Aus Platzgründen haben ich \type{str} als \type{s} abgekürzt. Die Lexikonregel für die Bildung des
Partizips I hat die folgende Form:\footnote{%
  Diese Regel ist parallel zur Regel für die Adjektivableitung aus den Partizip-II-Formen
  in (\ref{lr-adjective-formation-da-approach}). Neben der Verbform in der Eingabe der Lexikonregel
  unterscheiden die Lexikonregeln sich lediglich darin, dass in (\mex{1}) das Suffix \suffix{d} angehängt wird. Die Parallelität
  lässt sich erfassen, indem man einen gemeinsamen Obertyp für beide Regeln annimmt. 
}
\eas
\label{lr-adjective-formation-da-approach_partizip-I}%
Adjektivableitungsregel für Infinitive mit und ohne \emph{zu}:\\[1mm]
%
%
\onems[word]{
  phon \ibox{1}\\
  synsem$|$loc$|$cat|head \onems[verb]{ vform \type{bse-or-inf} \\ 
                                               } 
%                          arg-st & \sliste{ \ibox{1}  } $\oplus$ \ibox{2} \\ 
} $\mapsto$\\[10mm]
\hfill\onems[stem]{
 phon \ibox{1} $\oplus$ \phonliste{ d }\\
 synsem$|$loc$|$cat|head \ms[adj]{ subj  & \sliste{ NP[\type{str}]\sub{\type{ref}} } \\
                                               }
%                         arg-st & \ibox{2}\\
}\is{Lexikonregel!Adjektivderivation}
\zs
Die Grundlage der Partizipbildung ist die \argstl des Eingabeverbs. Da diese bei den Infinitiven
ohne \emph{zu} das Subjekt enthält, sind über die Lexikonregel in (\ref{lr-prenom-adj}) fast alle
attributiven Formen bildbar:
\eal
\ex der ankommende Zug
\ex das tanzende Kind
\ex der dem Betrachter auf"|fallende Elefant
\ex das dieses Buch lesende Kind
\ex das dem Delphin einen Ball schenkende Kind
\ex das dem Delphin helfende Kind
\zl
Lediglich die Form \emph{regnende} ist nicht bildbar, wenn man davon ausgeht, dass \emph{regnen} ein
Expletivum zum Subjekt verlangt. Mit referentiellem Subjekt kann man aber auch eine \emph{regnende
  Wolke} (\raisebox{-2pt}{\includegraphics[height=.9em, keepaspectratio]{Figures/cloud-with-rain}})
bilden. Ansonsten wird jeweils die erste NP mit strukturellem Kasus zum Subjekt, und dieses Subjekt
ist dann koindiziert mit dem Nomen, das modifiziert wird.

Vergleicht man die \argstlen in (\ref{ex-Infinitive-ohne-zu}) mit denen in (\ref{ex-Infinitive-mit-zu}), ist unmittelbar
klar, wieso manche Phrasen mit den \emph{zu}"=Formen nicht bildbar sind:
\eal
\ex[*]{
der anzukommende Zug
}
\ex[*]{
das zu tanzende Kind
}
\ex[*]{
der dem Betrachter auf"|zufallende Elefant
}
\ex[*]{
der auf"|zufallende Betrachter
}
\ex[*]{
das dieses Buch zu lesende Kind
}
\ex[]{\label{ex das zu lesende Buch}
das zu lesende Buch
}
\ex[*]{
das dem Delphin einen Ball zu schenkende Kind
}
\ex[]{
\label{ex-der dem Delphin zu schenkende Ball}
der dem Delphin zu schenkende Ball
}
\ex[*]{
das dem Delphin zu helfende Kind
}
\ex[*]{
der zu helfende Delphin
}
\zl
Die \emph{zu}"=Infinitive \emph{anzukommen}, \emph{zu tanzen}, \emph{auf"|zufallen} und \emph{zu regnen} haben keine
NP mit strukturellem Kasus in der \argstl, weshalb es auch kein Subjekt geben kann, das mit dem
modifizierten Nomen referenzidentisch sein könnte. Es bleiben \emph{zu lesen} und \emph{zu
  schenken}. Da das logische Subjekt der Verben blockiert ist \citep[29--30]{Haider84b}, sind bei diesen Verben die einzigen
NPen mit strukturellem Kasus in der \argstl die zugrundeliegende Akkusativobjekte. Das entspricht der Modifikation, die in
(\ref{ex das zu lesende Buch}) und (\ref{ex-der dem Delphin zu schenkende Ball}) zu sehen ist.
\is{Derivation!Adjektiv|)}\is{Passiv!adjektivisches Partizip|)}%
\is{Argumentblockierung|)}

\todostefan{Offen:\\
Den Sänger jodeln ließ der König.\\
* Der Sänger zu jodeln fing gegen 18:00 an.\\
Zu jodeln fing der Sänger gegen 18:00 an.\\
Vielleicht kann man ja das Subjekt für infinitive ohne zu in der COMPS-Liste lassen. Allerdings: der
jodelnde Sänger.
}
...
die die Identifikation des semantischen Indexes des unterdrückten Subjekts mit dem referentiellen Index der NP in
...
\eas
Präposition \emph{von} für unterdrückte Subjekte im Passiv:\\
...
\noindent
Man beachte, dass es in dieser Analyse egal ist, welche Rolle das unterdrückte Subjekt hat. Es kann
ein Agens sein oder aber auch wie bei \emph{sehen} ein experiencer. Die Rolle wird vom Verb
festgelegt. In diesem Buch habe ich ohnehin \argone, \argtwo usw.\ verwendet, aber wenn man
elaborierte Linkingtheorien entwickeln will, ist diese Eigenschaft der hier vorgestellten
Passivanalyse wichtig.

Der Lexikoneintrag für die Präposition in (\mex{0}) interagiert auch mit der Analyse der adjektivischen Partizipien.
...
werden, könnte das Agensargument auch syntaktisch in Kopf"=Komplement"=Strukturen realisiert werden.
...
Man könnte argumentieren, dass der Satz in (\mex{0}) durch die Bindungstheorie\is{Bindungstheorie}\footnote{
  Die Bindungstheorie\is{Bindungstheorie} sagt etwas darüber aus, wann referentielle Nominalphrasen in einem Satz
...
die Sätze wären genauso ungrammatisch. In \citew[294]{Mueller2003e} habe ich eine Analyse entwickelt,
...
\subsection{Das Argumentrealisierungsprinzip und Patizipien und \emph{zu}-Infinitive}
\label{sec-ARP-und-Partizipien}

In\is{Prinzip!Argumentrealisierungs-|(} den ersten drei Auf"|lagen dieses Buches kam die Argumentstrukturliste noch nicht
...
des Dänischen\il{Dänisch} \citep{MOeDanish}. Das Dänische ist -- anders als das Deutsche -- eine SVO-Sprache. Wie bereits im
...
entwickelte ich eine Analyse des Passivs, die gleichermaßen für das Dänische\il{Dänisch}, das Deutsche und das
Englische\il{Englisch} funktioniert. Sie nimmt Bezug auf die \argstl und verwendet sprachspezifische Mappings auf
die Valenzmerkmale. Diese Analyse wurde in \citew{MuellerGermanic} dann auch auf das Isländische\il{Isländisch}
übertragen. Das Subjekt-Merkmal kommt in der Analyse nicht vor. Modale Infinitive werden
...
macht es möglich, Lexikoneinträge für die verschiedenen Varianten von \emph{haben} zu haben, die parallele syntaktische
...
\eal
\ex Perfekt in den germanischen SVO-Sprachen:\\
    \ms{ spr    & \sliste{ NP[\str]$_i$ }\\
         comps  & \sliste{ NP[\str]$_j$ }\\
         arg-st & \sliste{ NP[\str]$_i$, NP[\str]$_j$ } }
\ex Passiv in den germanischen SVO-Sprachen:\\
...
wird. In der hier vorgestellten Grammatik hat \spr nur bei Nomina ein Element (siehe
Abschnitt~\ref{Abschnitt-SPR-Merkmal}). Subjekte werden in der \subjl repräsentiert, wenn sie nicht
direkt mit dem Kopf verbunden werden können. Das ist \zb bei prädikativen und attributiven
Adjektiven so und, so der Vorschlag, auch beim Partizip II und bei Infinitiven mit \emph{zu}. (\mex{1}) zeigt prototypische
...
\noindent
(\mex{1}) zeigt eine vereinfachte Form des Argumentrealisierungsprinzips. (\mex{1}) entspricht dem 
Prinzip in (\ref{Prinzip-Argumentrealisierung-einfach}) auf
...
braucht also eine Relation, %wie \texttt{first-np-str}\isrel{first-np-str}, 
die eine Liste mit der ersten NP mit strukturellem Kasus bzw.\ mit dem Subjektsatz als \subjw
liefert bzw.\ die leere Liste, wenn es kein entsprechendes kasusrelevantes Argument gibt. Die restlichen Elemente werden dann auf \spr und \comps verteilt.
}
\ea
\label{Prinzip-Argumentrealisierung-einfach-SUBJ-zwei}%
Argumentrealisierungsprinzip\is{Prinzip!Argumentrealisierungs-} (vereinfachte Version):\\[2pt]
\type{word} \impl
\avm{
[head|subj & \1\\
 spr    & \2\\
...
\z
%\todostefan{Ausführliche Version}
...
\z
\largerpage
\type{lexical-sign} ist der Obertyp von \emph{root} und \emph{word}, so dass das Prinzip sowohl auf
Stämme als auch auf Wörter angewendet wird (siehe Abbildung~\ref{abb-hierarchie-lexikon} auf S.\,\pageref{abb-hierarchie-lexikon}).
% steht schon oben
% Die Formulierung in (\mex{0}) ist eine Vereinfachung. Man muss sicherstellen, dass die erste
% kasusrelevante XP (eine NP mit strukturellem Kasus oder ein Satzargument) in die \subjl kommt, wenn
% es eine solche XP gibt. Wenn es keine gibt, ist die \subjl leer.\todostefan{gilt bei Infinitiven
%   ohne zu}

Zurück zu den Einträgen für Infinitive und Partizipien in (\ref{ex-inf-pppARG-ST}): 
Die beiden Beschreibungen von infiniten Verben sind mit dem Argumentrealisierungsprinzip
inkompatibel, denn die NP[\str]$_i$ in der \subjl ist nicht Element 
...
Pazifist und Kriegstreiber, was auch ein Widerspruch war, der irgendwie gelöst werden musste.

Statt Defaults verwende ich aber eine Technik, die ich im Zusammenhang mit der Einsetzung von
Expletivpronomen\is{Pronomen!Expletiv-} in dänischen unpersönlichen Passiven entwickelt habe
\citep{MuellerDanishImpersonalPassive}: Im Dänischen\il{Dänisch} sind subjektlose Konstruktionen nicht zulässig.
Anders als im Deutschen muss es immer ein Subjekt geben. Das Dänische\il{Dänisch} erlaubt unpersönliche
...
\subj"= und \comps"=Listen. Da es im Dänischen\il{Dänisch} aber keine subjektlosen Konstruktionen gibt, würde
...
darin, diese Lexikonelemente von der Anwendung der Subjektbedingung auszunehmen. Genauso kann man
davon ausgehen, dass die Partizipien im Deutschen keine kanonischen Lexikonelemente sind und das
Argumentrealisierungsprinzip für diese Elemente nicht gelten muss. Mit entsprechenden
Ausnahmereglungen kann man also die Lexikonelemente wie in (\ref{ex-inf-pppARG-ST}) formulieren, ohne eine
inkonsistente Theorie zu bekommen.%
\is{Prinzip!Argumentrealisierungs-|)}
%\itdopt{Alles noch mal durchlesen}
...
\zl
Nimmt man an, dass der Bedeutungsbeitrag von \emph{gelesen wurde} mit dem von \emph{gelesen} identisch ist,
...
\ms{
arg-st \ibox{1} $\oplus$ \liste{ \ms{ loc \ms{ cat  & \ms{ head & \ms[verb]{ vform & ppp\\
                                                                                 }\\
...
\eas
\emph{gewählt wird} bei reflexivem Agensausdruck:\\[1mm]
\ms{ cat  & \ms{ head & verb\\
...
könnten dann auf dieses Hilfsmerkmal Bezug nehmen \parencites[299]{MRS}[239, 241]{MK2000a}. Die
Alternative ist, ein Element der \argstl besonders zu kennzeichnen, und von dieser Möglichkeit wurde
im vorangegangenen Abschnitt Gebrauch gemacht.
...
Der folgende Abschnitt beschäftigt sich mit einem alternativen Objekt"=zu"=Subjekt"=Anhebungsansatz
und die Abschnitte~\ref{sec-kathol-passive}, \ref{sec-kathol-passive-raising} und~\ref{sec-ryu-passive} diskutieren 
Analysen, die auf Koindizierung basieren.




\subsection{\citew{Kathol91a} und \citew{Pollard94a}: Auszeichnung des Akkusativobjekts}
\label{kathol-pollard}\label{Abschnitt-Kathol-Pollard-ERG-Merkmal}
...
Unergativische Verben haben als \ergw die leere Liste.
...
a.&ankommen (unakkusativisch):    & \sliste{ \ibox{1} NP[\type{str}] } & \sliste{ \ibox{1} } & \eliste\\[2mm]
b.&schlafen   (unergativisch):      & \sliste{ NP[\type{str}] }          & \eliste           & \eliste\\[2mm]
...
e.&helfen      (unergativisch):   & \sliste{ NP[\type{str}] }          & \eliste           & \sliste{ NP[\type{ldat}] }\\
\end{tabular}%
\z
%\vspace{-\baselineskip}\end{figure}
...
\ms{
 ...
 comps & \ibox{2} $\oplus$ \nliste{ 
                  V[\type{ppp}, %\textsc{flip}$-$, 
                                \textsc{lex}+, \textsc{subj}~\nliste{ NP[\type{str}]\sub{\type{ref}} }, \textsc{erg}~\ibox{1},
...
\end{forest}
\caption{\label{abb-unpers-pass}Pollards Analyse des Verbalkomplexes \emph{geholfen werden} in:\ \emph{dass dem Kind geholfen werden kann}}%
\end{figure}
...
[\ms{ head & \ibox{1} \\
  ...
  [\iboxt{4}~\onems{ loc$|$cat \onems{ head  \ms[verb]{ vform & ppp \\
                                ...
                                comps~\ibox{3} \sliste{ \ibox{5} NP[\type{str}] } $\oplus$ \ibox{2} \eliste\\
                              }} [geliebt]]
  [\onems{ head~\ibox{1} \ms[verb]{ vform & bse \\
                              subj & \ibox{3} \\
                              erg  & \ibox{3} \\
...
durch Pollards Kasusprinzip Nominativ zugewiesen \citep[294]{Pollard94a}.
...
\subsection{\citew{Kathol94a}: Zwei Merkmale zur Blockierung}
...
\ea
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{}}
  &                               & \textsc{ext}                           & \textsc{subj}                  & \textsc{comps}\\[1mm]
a.&angekommen  (unakkusativisch): & \sliste{ \ibox{1} NP[\type{nom}] } & \sliste{ \ibox{1} }         & \eliste    \\[2mm]
b.&geschlafen  (unergativisch):   & \sliste{          NP[\type{nom}] } & \eliste                   & \eliste    \\[2mm]
c.&geliebt     (transitiv):       & \sliste{          NP[\type{nom}] } & \sliste{ NP[\type{acc}] }  & \eliste    \\[2mm]
\end{tabular}
\z
\eal
...
\ms{
subj   & \ibox{3}\\
comps & \ibox{2} $\oplus$ \ibox{1} $\oplus$ \nliste{ V[\textsc{subj} \ibox{2}, \textsc{ext} \ibox{3}, \textsc{comps} \ibox{1}] }\\
} $\wedge$ \ibox{2} $\neq$ \ibox{3}
\ex \sein (Perfekthilfsverb):\\
\ms{
subj   & \ibox{2}\\
comps & \ibox{1} $\oplus$ \nliste{ V[\textsc{subj} \ibox{2}, \textsc{ext} \ibox{2}, \textsc{comps} \ibox{1}] }\\
}
\ex\label{le-werden-kathol} \emph{werden} (Passivhilfsverb):\\
\ms{
subj   & \sliste{ NP[\type{nom}]\ind{2} }\\
comps & \ibox{1} $\oplus$ \nliste{ V[\textsc{subj} \nliste{ NP[\type{acc}]\ind{2} }, \textsc{comps} \ibox{1}] }\\
}
\zl
...
der in diesem Kapitel besprochenen Ansätze, Mehrfacheinträge für Verben in den jeweiligen
Formen zu vermeiden.
...
\subsection{\citew{Kathol94a}: Objekt-Subjekt-Koindizierung}
...
constraint: (\,\ibox{2} = \nliste{ NP[\type{acc}]\ind{4}} $\wedge$ \ibox{3} = \nliste{ NP[\type{nom}]\ind{4}})\\[1mm]
otherwise: (\,\ibox{2} = \ibox{3} = \eliste)
\end{tabular}
\z
...
\subsection{\citet{Ryu97a}: Merkmal für externes und internes Argument}
...
\eas
\emph{werden} (Hilfsverb für das persönliche Passiv, \citealt[\page377]{Ryu97a}):\\
\onems{
head$|$subj \nliste{ NP[\emph{nom}]\ind{2} }\\
comps \nliste{ PP[\type{von}]\ind{1} } $\oplus$ \ibox{4}  $\oplus$ \liste{ \ms{ head   & \ms[verb]{ vform & psp\\}\\
                                                                          comps & \nliste{ NP[\emph{acc}]\ind{2} } $\oplus$ \ibox{4}\\[2mm]
                                                                          argstr  & \ms{
extarg & \sliste{ \ibox{1} }\\
intarg & \sliste{ \ibox{2} }\\
args   & \sliste{ \ibox{1} } $\oplus$ \nliste{ \ibox{2} } $\oplus$ \ibox{3}\\
}\\
}}
}
\zs
...
\eas
\emph{werden} (Hilfsverb für das unpersönliche Passiv, \citealt[\page379]{Ryu97a}):\\
%\resizebox{\linewidth}{!}{%
\onems{
head$|$subj \eliste\\
comps \nliste{ PP[\type{von}]\ind{1} } $\oplus$ \ibox{4}  $\oplus$ \liste{ \ms{ head   & \ms[verb]{ vform & psp\\}\\
...
Innerhalb der Mainstream Generative Grammar wird in Kern und Peripherie unterschieden. Von der
Kerngrammatik nimmt man an, dass sie mithilfe angeborenen sprachspezifischen Wissens erworben wird
\citep[7--8]{Chomsky81a}. Dass nun einfach ein Phänomen, das früher zur Peripherie gezählt wurde
\parencites[70]{Chomsky81a}[13, 48]{Hyams88a-u}, in die Kerngrammatik wandert, zeigt, wie
willkürlich die Zuordnung von Phänomenen zu Kern\is{Kerngrammatik} und Peripherie\is{Peripherie} ist
\citep{MuellerKernigkeit}. Inzwischen hat sich herausgestellt, dass die Annahme einer reichhaltigen
genetisch determinierten Universalgrammatik nicht sinnvoll ist \citep[Kapitel~13]{MuellerGT-Eng5}. Statt von
Kern und Peripherie zu sprechen, scheint es sinnvoller, Sprachen zu vergleichen und zu erforschen,
was sie gemeinsam haben und was in vielen Sprachen oder gar in bestimmten Sprachgruppen
vorkommt. Statt von Kern spreche ich von \emph{Kernigkeit}. In je mehr Sprachen ein Phänomen vorkommt,
desto kerniger ist es \citep{MuellerKernigkeit}. Dieses Konzept ist unabhängig von der Annahme einer
Universalgrammatik nutzbar.}
...
\ex weil Conny die Kinder [lächeln sieht]
\zl
...
Zur Erinnerung sei hier die Prosaform des Kasusprinzips von S.\,\pageref{case-p} wiederholt:
\begin{prinzip-break}[Kasusprinzip]\is{Prinzip!Kasus-}
\label{case-p-zwei}
\begin{itemize}
\item Das erste kasusrelevante Element in der Argumentstrukturliste eines Kopfes bekommt
  Nominativ\is{Kasus!Nominativ}, wenn es eine NP mit strukturellem Kasus ist, es sei denn es wird von einem übergeordneten Kopf angehoben. 
...
\noindent
\textcites{Meurers99b}[Abschnitt~10.4.1.4]{Meurers2000b} gibt eine sehr komplexe Formalisierung eines ähnlichen Prinzips, die vollständige Äußerungen
durchwandert und Valenzlisten darauf"|hin überprüft, ob Argumente angehoben wurden oder nicht (siehe
...
\ex {}[Den Aufsatz gelesen] hat Conny am Wochenende.
\zl
...
sondern in der \compsl nur als realisiert markiert werden (siehe auch \citew[\page
560]{Higginbotham85a} und \citew[239]{Winkler97a}
%und \citew{Speas90a} 
...
\begin{schema}[Kopf-Komplement-Schema, binär verzweigend mit "`Geistern"', vorläufig]
...
\onems{
      ...
      non-head-dtrs \liste{ \onems{ synsem \ibox{2} \onems{ loc$|$cat \ms{ spr   & \type{list of spirits}\\
...
\noindent
Die Merkmale \textsc{argument}\isfeat{argument} und \textsc{realized}\isfeat{realized} wurden von \citet[236]{Prze99} vorgeschlagen
...
zeigt die Valenz- bzw.\ \argst-Listen für die Sätze (\ref{bsp-der-aufsatz-gelesen-zwei}).
\ea
...
c. & \emph{der Aufsatz gelesen wurde}:  & \argst \sliste{ \st{NP[\str]}$_k$, V }\\
d. & \emph{den Aufsatz gelesen hat er}: & \argst \sliste{ \st{NP[\str]}$_j$, \st{NP[\str]}$_k$, V }\\
\end{tabular}
\z
\todostefan{Kann man nicht \argst projizieren und \comps abarbeiten?}
\noindent
...
\begin{samepage}
\begin{schema}[Kopf-Komplement-Schema, binär verzweigend mit "`Geistern"', endgültig]
...
\ibox{1} = \type{list\_of\_not\_case\_xp} $\oplus$ \sliste{ \ibox{2} NP[\str, \textsc{raised}$-$] } $\oplus$ \etag
\impl\\\\
...
\oneline{\ibox{1} = \type{list\_of\_not\_case\_xp} $\oplus$ \sliste{ NP[\str, \textsc{raised}$-$] } $\oplus$ \etag{} $\oplus$ \sliste{ \ibox{2} NP[\str, \textsc{raised}$-$] } $\oplus$ \etag \impl}\\\\
\mbox{}\hspace{3cm} \ibox{2} = \textrm{NP[\sacc]}
\end{tabular}
\end{itemize}\todostefan{Statt NP[str] bei der ersten NP Satz oder NP[str]}

\noindent
Der Ausdruck in a bedeutet folgendes: Wenn sich die \argstl so unterteilen lässt, dass es einen
Listenanfang gibt, der keine kasusrelevante XP (NP mit strukturellem Kasus oder Satzargument) enthält, und einen Listenrest, der mit einer Nominalphrase mit strukturellem
...
Die Unterteilung der Liste \ibox{1} in den Listenanfang vom Typ \type{list\_of\_not\_case\_xp}
ist wichtig, da sonst das Passiv von ditransitiven Verben nicht analysiert werden könnte, denn die
\argstl von Passivhilfsverben mit passivierten ditransitiven Verben enthält an erster Stelle das
Dativobjekt und erst danach folgt die NP mit strukturellem Kasus des zugrundeliegenden
Akkusativobjekts. 
\eal
\label{ex-dass dem Kind der Roman geschenkt wurde-Passiv}
\ex dass dem Kind der Roman geschenkt wurde
\ex \argst von \emph{wurde} in (\mex{1}a): \nliste{ NP[\type{ldat}], NP[\type{str}], V }
\zl
Auch die bereits in Kapitel~\ref{kasus-adamp} angesprochenen Fälle des Fernpassivs
mit Objektkontrollverben wären nicht analysierbar (siehe auch S.\,\pageref{page-remote-passive-erlauben}). Bei solchen
Fernpassiven kann an der ersten Stelle der \argstl ebenfalls ein Dativ stehen und erst an zweiter Stelle eine
...
Nominalphrase mit strukturellem Kasus (diejenige, die Nominativ bekommt) oder ein Satzargument geben. Alle weiteren
nicht angehobenen Nominalphrasen mit strukturellem Kasus \iboxb{2} müssen dann im Akkusativ stehen.
Durch die beiden \etag in (\mex{0}b) kann die Liste nach der ersten NP[\str]/dem Satz beliebig zerlegt werden,
...
\citet{MOe2013a} haben gezeigt, wie man das Dänische\il{Dänisch}, Englische\il{Englisch} und Deutsche Passiv mittels
...
\LATER{\citet{RV80a-u} auch für Englisch nur ein Partizip}
...
% unklar, warum ich mir das aufgeschrieben habe
