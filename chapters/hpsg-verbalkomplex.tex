...
In verschiedenen anderen Arbeiten (\zb \citew[Kapitel~3]{Uszkoreit87a}\ia{Uszkoreit}) 
...
daß das Buch lesen niemand wird
}
\ex[*]{\label{bsp-das-lesen-niemand-wird}
das Buch, das lesen niemand wird
}
\zl
Daß Stellungen wie die in (\mex{0}) prinzipiell möglich sind, zeigt (\mex{1}):
\eal
...
\citet*{HN94a}\ia{Hinrichs}\ia{Nakazawa} 
...
der beteiligten Verben, also mit \emph{das Buch} und \emph{niemand} kombiniert.\footnote{
        ...
        \citet*{Johnson86a}\iaf{Johnson, Mark}
...
dann einfach die \catwe der Konjunkte identifiziert. Würde man -- wie \zb \citet*{BvN96,BvN98a}\ia{Bouma}\ia{van Noord} --
...
so wäre die Koordination in (\mex{0}) nicht als symmetrische Koordination zu erklären.\footnote{
        ...
        nicht völlig auszuschließen, daß (\mex{0}) auch mit einer flachen Struktur zu erklären ist.
...
%% Die separate
%% Repräsentation des Subjekts infiniter Verben sagt vorher, daß Subjekte nicht innerhalb
...
%%
%% Für die Analyse der scheinbar mehrfachen Vorfeldbesetzung würde ebenfalls vorausgesagt, daß
...
\ms[cat]{
 head   & \textit{verb}\\*
 subcat & \ibox{1} $\oplus$ \sliste{ \textsc{V[\textit{bse\/}, \textsc{lex}+, subcat~\ibox{1}]}} \\
}
\zs
%% Hierbei ist V[\textit{bse\/}, \textsc{lex}+, subcat~\ibox{1}] eine Abkürzung für (\mex{2}):
%% \ea
%% \ms{
...
%% }
%% \z
\emph{werden} selegiert ein Verb in der \textit{bse\/}"=Form\istype{bse}\footnote{%
...
\emph{Karl} und \emph{mir} von {\em helfen}. 
\ea
daß Karl mir helfen wird
\z
...
\textsc{subcat}"=Liste für {\em hilft}. Man sagt auch, daß das Hilfsverb die Argumente des
...
\iboxt{4}~\onems{ loc \onems{ head  \ms[verb]{ vform & bse \\
                                ...
                                subcat~\ibox{2} \liste{ NP[\textit{nom\/}], NP[\textit{dat\/}] }\\[2mm]
                              }\\
                }} &
\rnode{3}{
...
\z
Will man unechte Mehrdeutigkeiten vermeiden, so muß man sicherstellen, daß Sätze wie (\mex{0}) nur auf die
...
für (\ref{le-wird}) muß gelten, daß \iboxt{1} nur vollständig gesättigte
% nicht prädikative 
...
\iboxt{5}~\onems{ loc \onems{ head  \ms[verb]{ vform & ppp  \\
                                ...
                                subcat~\ibox{2} \sliste{ NP[\textit{nom\/}], NP[\textit{dat\/}] } \\
                              }\\
                }
} & \rnode{5}{
\onems[cat]{ head~\ibox{3} \ms[verb]{ vform & bse \\
                            } \\
            subcat ~ \ibox{2} $\oplus$ \sliste{ \ibox{5} } \\
          }
}\\
\\[3ex]
\rnode{6}{geholfen} & \rnode{7}{haben} & \rnode{8}{wird}\\
\end{tabular}
\ncline{1}{2}\ncline{1}{3}%
\ncline{2}{4}\ncline{2}{5}%
\ncline{4}{6}\ncline{5}{7}\ncline{3}{8}%
...
\end{figure}
Das Perfekthilfsverb \emph{haben} bettet das Partizip \emph{geholfen} (ein Verb mit \textsc{vform} \textit{ppp\/}\istype{ppp})
...
ausgeschlossen ist:
%\NOTE{WS: Vielleicht hab ich schon zu lange draufgesehen, aber \emph{daß das Buch lesen niemand wird}
...
Daß\is{Vorfeldbesetzung|(}\is{Extraktion|(}\is{Partial Verb Phrase Fronting@\emph{Partial Verb Phrase Fronting} (PVP)|(} man Phrasen wie \emph{ein Märchen erzählen} für Sätze braucht, in denen sich diese Wortgruppe im Vorfeld
...
kann ein Verb von einer eingebetteten Spur verlangen, daß diese den \textsc{lex}-Wert\isfeat{lex} + hat. Der \textsc{lex}-Wert
...
zulässig.\footnote{
        Das heißt, daß es für HPSG"=Grammatiken nicht sinnvoll ist, ein Strukturerhaltungsprinzip\is{Prinzip!Strukturerhaltungs-}
        zu formulieren,
...
%\nodemargin5pt
\begin{tabular}[t]{@{}c@{\hspace{8mm}}c@{\hspace{8mm}}c@{\hspace{10mm}}c@{\hspace{8mm}}c@{\hspace{10mm}}c@{\hspace{8mm}}c@{}}%llll}
\multicolumn{3}{@{}c@{}}{\rnode{12}{V[\begin{tabular}[t]{@{}l@{}}
  \textit{fin},\\\textsc{subcat} \sliste{ },\\
  \textsc{slash} \sliste{ }] \\
  \end{tabular}}
}\\
\\*[4ex]
\multicolumn{2}{@{}l@{}}{\rnode{13}{\hspace{7mm}V[\begin{tabular}[t]{@{}l@{}}
                                              \rnode{13top}{\textsc{lex}} $-$,\\
                                              \textsc{loc} \ibox{1} [\begin{tabular}[t]{@{}l@{}}
                                                                 \textit{bse},\\
                                                                 \textsc{s\rnode{13bot}{u}bcat} \ibox{2} \sliste{ \ibox{3}, \ibox{4} }\,]]\\
                                                                \end{tabular}\\
                                              \end{tabular}}} & \multicolumn{3}{@{}l@{}}{\rnode{1}{V[\begin{tabular}[t]{@{}l@{}}
                                              \rnode{1top}{\textit{fin}},\\\textsc{subcat} \sliste{ },\\
                                              \textsc{slash} \sliste{ \rnode{1bot}{\ibox{1}} }] \\
                                              \end{tabular}}
                                  }\\
\\*[4ex]
\rnode{14}{\ibox{5} NP[\textit{dat\/}]}               &   \rnode{15}{V[\begin{tabular}[t]{@{}l@{}}
                                                                 \rnode{15top}{\textit{bse}},\\ 
                                                                 \textsc{subcat} \sliste{ \ibox{3}, \ibox{4}, \ibox{5} }\,]\\
                                                                \end{tabular}}                       & \rnode{wirdv}{V[\begin{tabular}[t]{@{}l@{}}
                                              \rnode{wirdvtop}{\textit{fin}},\\\textsc{subcat} \sliste{ \ibox{7} }] \\
                                              \end{tabular}} & \multicolumn{3}{@{}l@{}}{\rnode{emptyv}{\ibox{7} V[\begin{tabular}[t]{@{}l@{}}
                                              \rnode{emptyvtop}{\textit{fin}},\\\textsc{subcat} \sliste{ },\\
                                              \textsc{slash} \sliste{ \rnode{emptyvbot}{\ibox{1}} }] \\
                                              \end{tabular}}}\\
\\*[4ex]
&&\rnode{wirdvlast}{V[\begin{tabular}[t]{@{}l@{}}
    \textsc{subcat} \ibox{2} $\oplus$ \sliste{ \ibox{6} }]\\
    \end{tabular}} &\rnode{2}{\ibox{3} NP[\textit{nom\/}]} & \multicolumn{2}{@{}l@{}}{\rnode{3}{V[\begin{tabular}[t]{@{}l@{}}
                                                                    \rnode{3top}{\textit{fin}},\\\textsc{subcat} \sliste{ \ibox{3} },\\
                                                                    \textsc{slash} \sliste{ \ibox{1} }]
                                                                    \end{tabular}}
                                                       }\\
\\*[4ex]
&               &                                   &               & \rnode{4}{\ibox{4} NP[\textit{acc\/}]}& \multicolumn{2}{@{}l@{}}{\hspace{5mm}\rnode{5}{V[\begin{tabular}[t]{@{}l@{}}
                                                                                                       \rnode{5top}{\textit{fin}},\\\textsc{subcat} \ibox{2} \sliste{ \ibox{3}, \ibox{4} }\\
                                                                                                       \textsc{slash} \sliste{ \ibox{1} }]\\
                                                                                                       \end{tabular}}
                                                                                           }\\
\\*[4ex]
&               &                                   &               &                                   & \rnode{6}{\ibox{6} V[\begin{tabular}[t]{@{}l@{}}
                                                                                                        \textsc{lex} +,\\
                                                                                                        \textsc{loc}~\ibox{1}, \\
                                                                                                        \textsc{slash} \sliste{ \ibox{1} }]\\
                                                                                                        \end{tabular}} & \rnode{7}{V[\begin{tabular}[t]{@{}l@{}}
                                                                                                                                    \rnode{7top}{\textit{fin}},\\
                                                                                                                                    \textsc{subcat} \ibox{2} \sliste{ \ibox{3}, \ibox{4} } $\oplus$ \sliste{  \ibox{6} }\,] \\
                                                                                                                                    \end{tabular}}\\
\\*[5ex]
...
\ncline{13bot}{14}\ncline{13bot}{15top}%
\ncline{14}{16}\ncline{15}{17}%
\ncline{wirdv}{wirdvlast}%
\ncline{wirdvlast}{wirdl}%
\end{sideways}}
...
\z
%% Der Kontrast in (\mex{1}) ist dadurch erklärt, daß in (\mex{1}a) eine prädikative PP
...
\citet*{Chung93a}\iaf{Chung} hat für das Koreanische\il{Koreanisch}
und \citet*{Rentier94}\iaf{Rentier} für das Niederländische\il{Niederländisch} vorgeschlagen,
...
%% als (\mex{1}) wiederholt -- nimmt er an, daß \emph{es zu lesen} eine Phrase
...
%% Haider, \citeyear[\page110]{Haider86c}; \citeyear[\page128]{Haider90b}\iadata{Haider}.%
%% }
%% \z
...
%%
%% Für Verben wie \word{scheinen} nimmt Reape an, daß das Verb einen 
...
In \citew[\page93--94]{Mueller2002b} habe ich gezeigt, daß die Tatsache,
daß \emph{den Wagen} in (\mex{1}) Akkusativ hat, nicht zu erklären wäre, wenn man annehmen würde,
daß sich in (\mex{1}) zwei unabhängige Konstituenten im Vorfeld befinden.\footnote{
...
Betrachtet man (\mex{1}b), stellt man fest, daß die Nominativ-NP allein vorangestellt werden kann.
\eal
\ex[]{
...
worden. Hinrichs und Nakazawas Arbeiten gehen auf Arbeiten im Rahmen der Kategorialgrammatik\is{Kategorialgrammatik (CG)} zurück \citep{Geach70a}.
}