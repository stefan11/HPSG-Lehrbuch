...
von \textsc{cat} ist eine Merkmalstruktur mit den Merkmalen \textsc{head} und \textsc{subcat}. (\mex{1})
...
man Fälle\is{Koordination|(} symmetrischer\label{page-symmetrische-koordination} Koordination wie in (\mex{1}) so behandeln, daß
...
%% sein oder aber auch eine Liste mit einem oder mehreren Elementen. Wichtig ist nur, daß
%% die \subcatl mit drei Elementen beginnt, deren Index mit Werten von Argumentrollen innerhalb von \cont
%% identisch ist.
...
\\
&                                   &                      & & \hspace{6mm} \relation{geben}($e,b,m$)\\*[3ex]
...
\end{figure}
$e$, $b$ und $m$ stehen dabei verkürzend für \emph{er}, \emph{das Buch} und \emph{dem Mann}.
...
in die in Abbildung~\ref{fig-type-sign} entwickelte Hierarchie unter \textit{sign} integriert werden muß.
\begin{figure}[htbp]
\centerline{\begin{pspicture}(0.6,-2)(13.2,6.4)
%\psgrid
...
%
\rput[B](10,0){\rnode{hnas}{\textit{head-non-adjunct-phrase}}}
\rput[B](10,-2){\rnode{hars}{\textit{head-argument-phrase}}}
%
...
Nominalphrasen muß am Mutterknoten repräsentiert sein. Dazu wird das Merkmal \textsc{indices}\isfeat{indices}
