...
Argumentanziehung und Verbalkomplexbildung (Kapitel~\ref{Kapitel-Verbalkomplex} und~\ref{Kapitel-anhebung}) und Passiv (Kapitel~\ref{Kapitel-passiv}).
\largerpage
...
\eal
\label{ex-Partikeln-Adjektiv-Adverb-Nomen-Präposition}
\ex\iw{trockenlegen}
...
Conny fuhr Rad.
\ex\iw{umfärben}
Conny färbte den Mantel um.
\ex\iw{sitzenbleiben}
Conny blieb sitzen.
\zl
%
Verben wie \emph{trockenlegen} und \emph{sitzenbleiben} werden zu den Partikelverben gezählt, weil sie sich nicht transparent
aus ihren Bestandteilen aufbauen lassen. So unterscheidet sich zum Beispiel \emph{trockenlegen} von
\emph{klein schneiden}. Während man sagen kann, dass Kim schneidet und im Ergebnis sind dann
Zwiebeln klein, kann man nicht sagen, dass Kim legt oder Sümpfe legt und dadurch das Gebiet, auf dem
sich vorher Sümpfe befanden, trocken ist. Siehe auch \citew[224]{Zifonun99a} zu \emph{kaltstellen}
und \emph{richtigstellen}. Auch die Partikeln, die von der Form her Präpositionen entsprechen,
tragen recht unterschiedlich zur Gesamtbedeutung bei (siehe
Abschnitt~\ref{sec-transparente-und-nicht-transparente-Partikelverben}). Bei \emph{anfangen} und
\emph{auf"|hören} kann man weder \emph{an} noch \emph{auf} mit den homophonen Präpositionen
gleichsetzen. Ich gehe also davon aus, dass es eine eigene Kategorie Verbpartikel gibt und dass
Elemente wie die in (\mex{0}) aufgezählten mehrere syntaktische Kategorien haben können. Siehe auch
\citew{Olsen99c} zu Überlegungen zur Kategorie Verbpartikel im Zusammenhang mit Adverbien und Präpositionen.
\largerpage

Neben den in (\ref{ex-Partikeln-Adjektiv-Adverb-Nomen-Präposition}) aufgeführten Partikelarten gibt es aber auch Partikeln wie \emph{dar} (\word{darlegen}), 
\emph{inne} (\word{innehalten}) und \emph{acht} 
(\word{achtgeben}), die nicht unter diese Kategorien fallen \citep[89]{Uszkoreit87a}.
...
\word{anstrengen} in \emph{sich anstrengen} \citep[4]{DeKuthy1994a}. Partikelverben können
...
\label{sec-transparente-und-nicht-transparente-Partikelverben}
...
ist, ist auch sofort erklärt, warum das Verb ohne die Partikel in die linke Satzklammer gestellt werden kann. Die Voranstellung
...
möglich sind. Es wird zwar oft behauptet, dass die Partikel nicht ins Vorfeld gestellt werden
kann,\footnote{
% Olsen95b-u:21
...
% 01.2026: Sebastian Nordhoff und Bruno finden, dass der geht.
}
\ex[*]{
...
Die Struktur in (\mex{1}) zeigt den Lexikoneintrag für das
...
\ldots & \ms{ cat & \ms{
                    head   & verb\\
                    arg-st & \nliste{ NP[\type{str}]\ind{1}, NP[\type{str}]\ind{2}, Part[\type{vor}, \textsc{lex}+] } } \\
              cont & \ms{ ltop & \ibox{3}\\
                          ind  & \ibox{4} event } }\\
...
\argstl genau angegeben. Das entspricht der Analyse von Idiomen, die \citet{KE94a} vorgeschlagen
haben: Ein Kopf hat die idiomatische Bedeutung und selegiert weitere Bestandteile des
Idioms.\todostefan{Boas: Mini-Idioms}
...
vorgestellt wurde: Die Verberststellungsregel wird auf ein Lexikonelement angewendet,
das die Verbpartikel selegiert. Die Information über die Valenz der Tochter der V1"=Regel wird über DSL zur Verbspur transportiert, und da sie dort mit der
Valenz\-information der Spur identifiziert wird, verhält sich die Spur genau so,
als stünde an ihrer Stelle die Tochter der V1"=Regel. Die Spur bildet mit der Partikel einen
...
jedoch keine Partikel. Man braucht also ein weiteres Lexikonelement für das Partikelverb.
Ein entsprechendes Lexikonelement lizenziert die Lexikonregel in (\ref{lr-pv-prel}):
%
\begin{figure}
\eas
\label{lr-pv-prel}%
...
und lizenziert dann ein Lexikonelement für den Stamm \stem{lach}, das mit der Partikel \emph{los}
kombiniert werden kann. Die vorliegende Lexikonregel ist eine vereinfachte Version. Ich werde später
zeigen, wie sie erweitert werden muss, damit auch \emph{anlachen} analysiert werden kann.
...
\footnote{%
  ...
  "`Adjuncts-as-Complements"' bekannt (siehe auch Fußnote~\ref{fn-adjuncts-as-complements} auf S.\,\pageref{fn-adjuncts-as-complements}). Zur Kritik an dieser Analyse siehe
...
[{V[\comps \ibox{1}]}
  ...
  [{V[\comps \ibox{1} $\oplus$ \nliste{ \ibox{2} Part }]}, l sep+=\baselineskip
    [{\ibox{3} V[\argst \ibox{1} \sliste{ NP[\str] }]},edge label={node[midway,right]{PV-LR}}
      [lach-]]]]
\end{forest}
...
der \argstl des Basisverbs formulieren. Man kann also sicherstellen, dass
...
Wert wird durch die Merkmalbeschreibung des Lexikoneintrags des Verbs,
...
\ldots & \ms{ cat & \ms{ head & verb\\
                         arg-st & \upshape (\,\ibox{1} \nliste{ NP[\str]\ind{2} }\,)  $\oplus$\\
                                & \liste{ \onems{ loc \onems{ cat|head \onems[particle]{ mod$|$loc \ms{
...
Das Ergebnis der Flexion ist ein Lexikonelement, das (\mex{0}) sehr ähnlich ist: Bei finiten
Verben ändert sich die phonologische Form, \dash, Flexionsendungen werden mit der morphologischen
...
können, zeigt das Beispiel in (\mex{1}):
\ea
\label{bsp-den-atem-an-haelt}
...
\ldots & \ms{ cat & \ms{ head & verb\\
                         arg-st & \upshape (\,\ibox{1} \nliste{ NP[\str]\ind{2} }\,)  $\oplus$ \ibox{3} $\oplus$ \ibox{4}\\
                                & \liste{ \onems{ loc \onems{ cat \onems{ head \onems[particle]{ mod$|$loc \ms{
                                                                  cat & \onems{ head  \type{verb} \\
                                                                                arg-st \ibox{1}  \\
                                                                          }\\
                                                                  cont & \ms{ ltop & \ibox{5}\\
...
%}
\zs
Dieser Lexikoneintrag ähnelt den Lexikoneinträgen für AcI-Verben (vergleiche den Eintrag für
\emph{sehen} in (\ref{le-sehen}) auf S.\,\pageref{le-sehen}): Der \subjw und \compsw
...
[{V[\comps \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3}]}
  ...
  [{V[\comps \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3} $\oplus$ \nliste{ \ibox{4} Part[\subj
     ...
     [{\ibox{5} V[\argst \ibox{1} \sliste{ NP[\str] }]}, edge label={node[midway,right]{PV-LR}}
       [lach-]]]] 
\end{forest}
...
noch h1 bzw.\ e1 geschrieben wird, aber natürlich müsste statt h2 und e2 an allen Stellen in der Abbildung  h1 und e1 stehen. 
...
\eas
\emph{anlacht} (Kombination von \emph{lacht} mit \emph{an$_5$}):\\
%\resizebox{\linewidth}{!}{%
\ms{%
...
\z
Für die Analyse von \noword{anloslacht} müsste die Partikelverblexikonregel zweimal auf \stem{lach}
angewendet werden. Bei der zweiten Regelanwendung würde aber die \argstl bereits zwei Elemente
...
\citeauthor{Wells47a} (\citeyear[\page106]{Wells47a}, siehe auch \citeauthor{McCawley82a} \citeyear[\page91]{McCawley82a}).
...
Er schlägt den folgenden Lexikoneintrag für das nicht transparente Partikelverb \word{auf"|wachen} 
vor:

\eas
\label{le-aufwachen-kathol}%
\mbox{\emph{auf"|wachen} (nach \citet[\page246]{Kathol95a}):}\\
\onems{
...
und nicht"=kompositionalen Partikelverben. Die kompositionalen Partikelverben werden über das Verbalkomplexschema
...
Damit kann man den semantischen Beitrag verschiedener Partikeln vergleichen.
...
%\NOTE{SN: Rein glaube ich, dass er fährt, nicht raus. SM: Das ist aber einfach ein Adverb.}
\ea[*]{
...
eine Verbpartikel im Vorfeld. Zusätzlich befindet sich \zb in  (\ref{bsp-den-atem-an-haelt}) auf
S.\,\pageref{bsp-den-atem-an-haelt} -- hier als (\mex{1}a) wiederholt --
...
\dash, in (\ref{bsp-den-atem-an-haelt-Topo}) befindet sich \emph{den Atem} innerhalb des Vorfelds
des gesamten Satzes im Mittelfeld und \emph{an} in der rechten Satzklammer,
in (\ref{bsp-wieder-an-tritt-Topo}) befindet sich \emph{wieder} im Mittelfeld und \emph{an} in der rechten Satzklammer,
und in (\ref{bsp-los-damit-Topo}) befindet sich \emph{los} in der rechten Satzklammer und \emph{damit}
ist extraponiert\is{Extraposition}, \dash, es befindet sich im Nachfeld\is{Feld!Nach-}.
\eal
\label{bsp-partikel+anderes+material-topo-felder}
\ex\label{bsp-den-atem-an-haelt-Topo}
{}[\sub{VF} [\sub{MF} Den Atem] [\sub{RS} an]] hielt die ganze Judenheit.
\ex\label{bsp-wieder-an-tritt-Topo}
{}[\sub{VF} [\sub{MF} Wieder] [\sub{RS} an]] treten auch die beiden Sozialdemokraten.
\ex\label{bsp-los-damit-Topo}
...
aus, dass ein Verbalkomplexelement vor dem Verb in Initialstellung steht \parencites[128]{Kathol95a}[50]{Kathol2001a}[\page143]{Crysmann2002a}.%
\is{Vorfeldbesetzung|)}

Die Theorie, die ich hier vorgeschlagen habe, ist der von \citeauthor{Kathol95a} und \citeauthor{Crysmann2002a} vorzuziehen,
...
The \vcompv of \emph{auf"|wachen} in (\ref{le-aufwachen-kathol}) is the empty list.
...
Partikelverben im Niederländischen und in deutschen Dialekten auf"|weisen und
...
\citet[Kapitel~8]{Goldberg95a}. Probleme, die sich für Goldbergs Analyse ergeben,
...
die im Rahmen der LFG\indexlfgstart arbeitet,
...
\eal
\ex Maar dan laat de met een klassiek filmsterren uiterlijk gezegende Litvinova het helemaal in de
soep lopen en komen we terecht op het terrein van filosofische verhandelingen over de betekenis van
liefde en \emph{rondgeren} door een niet nader geïdentificeerd naaldbos.\footnote{
\url{http://www.subjectivisten.org/cinema/archives/001530.php}. 06.01.2007.
}\\
`Doch dann bringt die mit dem Aussehen eines klassischen Filmstars gesegnete Litvinova alles
völlig durcheinander, und wir landen auf dem Gebiet philosophischer Abhandlungen über die Bedeutung
der Liebe und beim Herumgerenne in einem nicht näher bezeichneten Nadelwald.'
\ex In haar paniek ging alles echter niet meer zo best, en ook haar gewiebel en \emph{rondgeren} maakte de zaak er niet beter op.\footnote{
...
\ex
% En wij gaan eens kijken wat leuk is voor onze 2e verdieping bove Karel de teju en 
ik ben nog opzoek naar een klimplant die 
...
%`Und wir schauen mal, was sich gut für unseren zweiten Stock, über Karel, eignet, und 
...
% zitiert Booij 2002a: 206
  ...
  erklären, warum Partikeln ihrer Meinung nach nicht iteriert werden können: Dadurch dass
  ...
  immer ein V verlangen. Die Diskussion von Beispiel (\ref{ex-vorschenken}) mit dem Verb \emph{vorschenken} hat jedoch gezeigt, dass es prinzipiell beliebig
  viele Verbpartikeln pro Verb geben kann.%
}
\ea 
{}[ an   [ [ streng ]\sub{V-Stamm} ]\sub{V} ]\sub{V$'$}
\z
Man muss also die Tatsache, dass Partikelverben flektiert sind, extra in der Grammatik
aufschreiben. Es folgt nicht automatisch aus der Tatsache, dass Partikelverben Verben sind. Davon
abgesehen gibt es in der LFG normale Phrasenstrukturregeln, die eine oder mehrere Töchter zu einem
Mutterknoten in Beziehung setzen. Komplexe Gebilde wie in (\mex{0}) können nicht direkt beschrieben
werden \citep[75]{Belyaev2023a-u}.
...
\z
In Frameworks wie GB\indexgb wird für gewöhnlich angenommen, dass Flexionsaffixe mit Phrasen
kombiniert werden \parencites[\page 258--259]{FF87a}[57]{Chomsky91a-u}. Wenn Booij diesen Annahmen folgt, kann das Flexionsaffix mit einer VP kombiniert
werden. Das V würde in (i) dann für einen Verbstamm stehen.%
} 
\largerpage

Ein weiteres problematisches Beispiel ist das adjektivische Partizip \emph{angestrengte}. Wie in
Abschnitt~\ref{sec-adj-formation} dargelegt, wird dieses aus dem Partizip II \emph{angestrengt} abgeleitet. Handelte es
sich bei \emph{angestrengt} um ein V$'$, müsste ein syntaktisches Objekt die Grundlage für eine
Derivation in der Morphologie sein.
...
\end{tikzpicture}
\zs
Das Zeichen `<' drückt aus, dass X0 unmittelbar vor V0 stehen muss (S.\,126). Goldberg argumentiert
für solche Konstruktionen, da die Kombination aus X0 und V0 nicht der Bedeutung der
Einzelteile entspricht.

Bei der Bildung des Futurs steht das Hilfsverb\is{Verb!Hilfs-} \emph{x{\^a}stan} immer direkt vor dem
...
Abbildung~\ref{goldberg-persisch-vererbung-korrekt}) ist diese Strategie der Default"=Auf"|lösung
nicht anwendbar, denn keine der beiden Beschränkungen \emph{x{\^a}stan-agr}  <  V0\sub{past}
und X0 < V0 ist spezifischer als die andere. (Zur Erinnerung: `<' steht für unmittelbares
Vorausgehen. Entweder das Futurhilfsverb oder das Präverb steht unmittelbar vor V0. Die Stellungsinformation
ist also inkompatibel.) Das heißt, man kann die Berechnung der verschiedenen
...
Diese Probleme sind leicht zu lösen, indem man die persischen komplexen Prädikate als lexikalische
Konstruktionen repräsentiert \citep{MuellerPersian}: Das V0 verlangt das X0. Das V0 kann mit dem Futur"=Hilfsverb
kombiniert werden und der sich ergebende Komplex wird dann mit dem X0 kombiniert. Die
verschiedenen V0 sind komplett unabhängig von Futur"=Hilfsverben, weshalb die oben diskutierten
Probleme nicht auf"|treten.
...
\z
Genauso muss im Thüringischen\il{Thüringisch} und im Fränkischen\il{Fränkisch} die Partikel vor den
