...
\zs\iw{los|uu}
\itdopt{Should the event be shared or ltop?}
\noindent
...
\relation{beginnen}"=Relation verweist, die wiederum die Relation des Eingabeverbs als \argone hat.
...
%\resizebox{\linewidth}{!}{%
\onems{%
synsem|l \ms{ cat|arg-st & \upshape (\,\ibox{1} \nliste{NP[\str]\ind{2}}\,)  $\oplus$\\
...
wurde. Auf diese Weise lässt sich \emph{anlacht} parallel zu \emph{lachen sieht}
...
gelinkt \iboxb{2}. Die Labels von \emph{gerichtet-auf} und dem modifizierten Verb sind identifiziert
...
in der \subjl \iboxb{3} und \compsl \iboxb{4} der Partikel  angezogen werden.\footnote{%
In früheren Auflagen des Buches habe ich auch die Elemente der \compsl der Partikel übernommen. Das
macht die Argumentanziehung von der Partikel parallel zu anderen Argumentanziehungen im
Prädikatskomplex (vergleiche (\ref{le-sehen}) den Eintrag für das AcI-Verb \emph{sehen} auf Seite~\pageref{le-sehen}). Da aber die \compsl von Partikeln immer die leere Liste ist, ist das redundant.}
Die Regel lizenziert bei der Eingabe des Stammes \stem{lach} 
...
\argstl des Matrixverbs eingefügt. In (\mex{0}) ist das \iboxt{3} $\oplus$ \iboxt{4}. Auf das Einsetzen des \compswes
habe ich verzichtet, da \comps bei Partikeln ohnehin immer die leere Liste ist.
...
[{V[\comps \ibox{1} $\oplus$ \ibox{2}]}
  [\ibox{3} Part\feattab{\textsc{mod} \ibox{4}\\
                         \subj \ibox{2} \sliste{ NP[\str] },\\
                         \comps \eliste } [an]]
  [{V[\comps \ibox{1} $\oplus$ \ibox{2} $\oplus$ \nliste{ \ibox{3} Part[\subj
      \ibox{2}] }]}, l sep+=\baselineskip
     [{\ibox{4} V[\comps \ibox{1} \sliste{ NP[\str] }]}, edge label={node[midway,right]{PV-LR}}
       [lach-]]]] 
\end{forest}

\caption{Kombination von \emph{an} und \emph{lachen} (Valenzinformation)}\label{fig-anlachen-val}
\end{figure}
\emph{an} enthält ein Element in \subj \iboxb{2}. Das Basisverb steuert
...
für \emph{loslachen} in Abbildung~\vref{fig-loslachen-sem} und wird hier deshalb nicht separat
diskutiert.
\itdopt{Stimmt nicht, weil es intersektiv ist. Noch nachtragen.}
Das Ergebnis der Kombination von \emph{lachen} mit den Tags in (\ref{le-lach-+particle}) und \emph{an$_5$} zeigt (\mex{1}).
