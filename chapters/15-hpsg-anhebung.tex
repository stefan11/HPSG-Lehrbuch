...
Verbale Köpfe können eine Verbalprojektion als Komplement verlangen. Der Kopf bestimmt
Eigenschaften seines Komplements, und bei verbalen Argumenten gehört die Verbform zu 
...
\zl
Eine Kette von Verben, die in Kopf"=Komplement"=Beziehung stehen, nennt Bech eine subordinative
...
\is{Feld!Verbal-}%
Zu jedem Verb gehört ein Verbalfeld (F), das das Verb selbst, alle nichtverbalen
...
wenn sie zu zwei verschiedenen Kohärenzfeldern gehören.\footnote{
        ...
        einführen.
        In (i) gibt es die Felder $F^1$ = \emph{sie wollte}, $F^2$ = \emph{sein} und
        $F^3$ = \emph{der Königin immer treu} bzw.\ $F^1$ = \emph{sie immer wollte}, $F^2$ = \emph{sein} und
        $F^3$ = \emph{der Königin treu}.
        \ea
        dass sie der Königin immer treu\iwf{treu} sein wollte
        \z
...
\ea
weil ${\overbrace{\underbrace{\rule[-0.5ex]{0cm}{2.5ex}{\textrm{getanzt~wurde}}}_{S_1}}^{K_1}}$
\z
...
\subsection{Tests zur Unterscheidung kohärent und inkohärent konstruierender Verben}
\label{sec-coh-incoh}

Ob\is{Kohärenz|(}\is{Inkohärenz|(} Verben eine inkohärente Konstruktion eingehen können oder ob sie immer kohärent konstruieren,
...
\z
Der Satz in (\mex{0}) ist dreideutig, da alle drei Verben zum selben Kohärenzfeld
...
weil $\overbrace{ \underbrace{\rule[-0.5ex]{0cm}{2.5ex}\mbox{es ihr jemand} }_{R}~
                    \underbrace{\rule[-0.5ex]{0cm}{2.5ex}\mbox{zu lesen versprochen\iw{versprechen} hat}}_{\mbox{\footnotesize S}}
                  }^{K}$\footnote{
% Haider: weil es ihr jemand zu lesen versprochen hat
        Siehe \textcites[\page 110]{Haider86c}[\page 128]{Haider90b}.
}
\z
Das Objekt von \emph{zu lesen} (\emph{es})
und das Subjekt und Objekt von \emph{versprochen} (\emph{jemand} und \emph{ihr})
...
\ea 
weil    ihr den Aufsatz jemand   zu lesen versprochen hat
\z
...
Ein\is{Intraposition|(} weiteres Kriterium zur Unterscheidung zwischen kohärent und inkohärent
konstruierenden Verben ist die Möglichkeit, in Relativsätzen das eingebettete Verb mit
...
worden (\mex{1}b).\footnote{
  ...
  (\citealp*{Riemsdijk85,Haider85c,Grewendorf86,Trissler88,Riemsdijk94}). Die Autor*innen
  ...
  als auch die Voranstellung des einzelnen Relativpronomens zulassen \citep[Abschnitt~10.7]{Mueller99a}.
}
\eal
...
(\mex{1}c) dagegen ist möglich, da \emph{versuchen} inkohärent konstruieren kann.\NOTE{JB: Findet
  (\mex{1}a+b) bei entsprechend langer Pause okay, FB auch. FB: Nichts ist so wie es scheint zu
  sein}
\largerpage
\eal
\ex[*]{
...
In Abschnitt~\ref{sec-pvp} wurde bereits gezeigt, dass die Unmöglichkeit der
...
der Anhebungs- bzw.\ Kontrollverben. Im Folgenden sollen Eigenschaften dieser beiden Verbklassen 
vorgestellt werden.
...
Beispiel für expletive Prädikate sind die Witterungsverben:\footnote{
        ...
        \z
        Siehe hierzu auch Abschnitt~\ref{sec-expletivum-in-obj-position}.%
}
\eal
\ex[*]{
...
%
Für Sätze wie (\mex{0}a) kann man annehmen, dass das Subjekt des eingebetteten Verbs
...
Das kann man erfassen, indem man festlegt, dass bei solchen Objektanhebungsverben ein eventuell beim
eingebetteten Verb vorhandenes Subjekt mit einem Objekt des Matrixverbs identisch ist.
Ob das eingebettete Prädikat ein Subjekt hat oder nicht, ist dabei für die Wohlgeformtheit irrelevant.
...
\eal
\ex\iw{sehen}
Der Wächter sah den Einbrecher   und seinen Helfer stehen bleiben.
\ex\iw{erlauben}
Der Wächter zwang den Einbrecher und seinen Helfer stehen zu bleiben.
\zl
...
bereits im Abschnitt~\ref{sec-kasus-nicht-realisierter-subj} diskutiert (siehe S.\,\pageref{bsp-tueren-hoehle}).
...
wie man aus dem Kasus von \emph{einer nach dem anderen} schließen kann.\footnote{
        ...
        und dass sich auf kasuslose NPen beziehende Adverbialphrasen im Deutschen im Nominativ stehen müssen
        und im Polnischen im Dativ bzw.\ Instrumental.

        \citet{Hennis89} diskutiert Daten der Sprache Malayalam\il{Malayalam}. In
...
Subjekt nicht adäquat ist, da in (\mex{1}) eine PP eine NP kontrolliert.\footnote{%
        \citet*[\page 139]{ps2} geben das folgende englische\il{Englisch}
        Beispiel:
        \ea
        Kim appealed to Sandy to cooperate.
        \z
}\todostefan{Abstand nach Beispiel zu klein}
\ea\iw{erwarten}
...
Interessanterweise ist das bei Anhebungsprädikaten anders:\footnote{
       ...
       wenn man statt \emph{die Männer} das Pronomen \emph{sie} verwendet.
        ...
        \z
        Das Pronomen ist hinsichtlich seines Kasus morphologisch unterspezifiziert. Für manche Sprecher*innen
...
Die Phasenverben (Verben wie \emph{beginnen}, \emph{anfangen} und \emph{auf"|hören})
...
\eal
dass Conny Kirby nicht zu kennen scheint
\zl
In der einen Lesart scheint es der Fall zu sein, dass Conny Kirby nicht kennt (enger Skopus) und in der
anderen Lesart scheint es nicht der Fall zu sein, dass Conny Kirby kennt.
...
immer obligatorisch kohärent konstruieren \citep[siehe z.\,B.][\page 128]{Haider90b}.
\eal
\label{bsp-intra-extra-scheinen}
\ex[]{
dass Conny Kirby zu kennen scheint
}
\ex[*]{
dass Conny Kirby zu kennen zumindest scheint
}
\ex[*]{
dass Conny scheint, Kirby zu kennen
%\NOTE{FB: findet den okay}
}
\zl

% Haider93a:244 Idioms
\noindent
Sogenannte Phasenverben\is{Verb!Phasen-|(} wie \word{anfangen}, \word{auf"|hören} und 
...
"`dritte Konstruktion"'\is{dritte Konstruktion|(}\footnote{
	Die Bezeichnung \emph{dritte Konstruktion} stammt von \citet*[42]{dBR89}.
	Sie haben sie für eine ähnliche Konstruktion im 
	Niederländischen\il{Niederländisch} verwendet. 
...
%
	...
	\citet*[\page 151]{Uszkoreit87a} verwendet den Begriff \emph{Focus Raising}.
        ...
        Siehe jedoch \citew[Abschnitt~17.5]{Mueller99a}.%
...
\ex Für manche einfache Schützen wurde aus dem Krieg ein großes Abenteuer, [von dem zu erzählen] 
    sie kaum mehr auf"|hören konnten.\footnote{
...
\ex
dass Aicke eine erfolgreiche Sportlerin zu werden versprach
\ex\label{ex-versprach-ein-x-zu-werden} 
dass Aicke versprach, eine erfolgreiche Sportlerin zu werden
\zl
In einer Lesart versprach Aicke etwas, in der anderen
wird ausgesagt, dass es wahrscheinlich war, dass Aicke eine erfolgreiche
Sportlerin werden würde. In der ersten Lesart handelt es sich um das Kontrollverb, in der
...
\citet[\page 16]{Cook2001a} diskutieren die Beispiele in (\mex{1}b) bzw.\ (\mex{1}c):\footnote{
  Es gibt eine Verwendung von \emph{weil} mit V2-Satz. Ein solcher \emph{weil}"=Satz könnte in
...
dritte Konstruktion nicht von allen Sprecher*innen gleichermaßen akzeptiert wird, wären bei
...
\z
%
Der Satz (\mex{0}) kann bedeuten, dass Conny jemandem verspricht, nicht einzuschlafen. Er kann aber
...
\eal\iw{versprechen}
\ex dass Conny jemandem das Buch zu lesen verspricht
\ex dass Conny das Buch jemandem zu lesen verspricht
\zl
...
\ea
\label{ex-weil-es-ihr-jemand-zu-lesen-versprochen-hat-zwei}
...
%%         Die Unakzeptabilität der Beispiele, die die beiden Autor*innen geben, ist allerdings
...
\ea
\label{bsp-Aicke-laesst-es-regnen}
Aicke lässt es regnen.
\z
(\mex{0}) hat die Lesart, wo Aicke den Regen Regen sein lässt (permissiv\is{Verb!permissives}), aber auch die
Lesart, in der Aicke das Regnen verursacht (kausativ\is{Verb!kausatives}\is{Kausativ!-konstruktion}).\footnote{
...
Auf S.~209 führen sie Beispiele mit unterschiedlichen Wahrnehmungsverben\is{Verb!Wahrnehmungs-} an, die ich
ins Deutsche übertragen habe:
\eal
\label{ex-AcI-Verben-und-Rollen}
...
Einige Autor*innen haben behauptet, dass kohärente Konstruktionen mit Objektkontrollverben
...
bei einfachen Lexikoneinträgen gibt.\footnote{
  ...
  Ausnahmen zu einer solchen Beschränkung gibt.%
        ...
        das ja auch die Autor*innen tun, die eine bisententiale Struktur, \dash eine Struktur, in der zwei
        ...
        sind für valenz- bzw.\ argumentstrukturbezogene Bindungstheorien wie die von \textcites{PS92a}[Kapitel~6]{ps2}
        kein Problem.
}
...
zwei Akkusative\is{Kasus!Akkusativ}, was es im Deutschen bei einfachen Verben nicht gibt.\footnote{
        ...
        siehe Abschnitt~\ref{sec-struc-lex-kas}.

        In der kohärenten Konstruktion in (\ref{bsp-trauen-kohaerent}) auf S.\,\pageref{bsp-trauen-kohaerent}
...
\z
Für das Englische wurde ein Dominanzschema für eine VP"=Projektion
angenommen, die das Verb mit allen vom Subjekt verschiedenen Argumenten enthält.
Ein weiteres Dominanzschema kombiniert dann die VP mit dem Subjekt \citep[38--39]{ps2}. In späteren
Arbeiten wurden die Subjekte im Englischen unter \subj \parencites[334--336]{Borsley89b}[Kapitel~9]{ps2} oder \spr \citep*[Abschnitt~4.3]{SWB2003a} separat von anderen Argumenten repräsentiert.
Für das Deutsche
...
\zl
\citet[295]{Pollard90a} und \textcites[268]{Kiss92}[Abschnitt~3.1.1, Abschnitt~3.2.4]{Kiss95a} haben deshalb vorgeschlagen, das Subjekt infiniter
...
vor, sondern ist nur mit dem modifizierten Nomen koindiziert. Siehe S.\,\pageref{bsp-nominativ-adj}.
\ea
...
automatisch durch das Kopfmerkmalsprinzip entlang des Kopf"|pfades einer Projektion
...
spr    & \ibox{2} \eliste\\
comps  & \ibox{3} \liste{ NP[\str], NP[\ldat] }\\
arg-st & \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3} }
\zs
\ea
...
(zu Lexikonregeln siehe Abschnitt~\ref{sec-lr}). Die \sprl ist bei Verben immer leer. Die \compsl des finiten Verbs in (\mex{-1}) enthält alle Elemente der \argstl; die \subjl
ist leer. Beim infiniten Verb ist das Subjekt nicht in der \compsl enthalten, sondern in der
\subjl. Die \argstl und das Mapping sind hier nicht angegeben. Zu den Details siehe Abschnitt~\ref{sec-analysis-da-modal-inf}.
\is{Merkmal!\textsc{subj}|)}
...
und in den Arbeiten von \citet[Kapitel~7]{ps2}, \citet[160]{Kiss95a}
...
(\mex{1}) zeigt den \locw von \emph{scheinen}:\footnote{%
Die Punkte stehen für den Pfad \textsc{synsem|loc}.
}
%% \footnote{
...
\ldots & \ms{ cat & \ms{% head   & verb \\ 
                    arg-st & \ibox{1} $\oplus$ \ibox{2} $\oplus$ \nliste{ V[\type{inf},
...
auf S.\,\pageref{le-wird}. Er unterscheidet sich in der \vform des selegierten Verbs und in der Erwähnung des \subjms.
...
die Beschränkung (\ref{constr-non-complex-forming}) auf S.\,\pageref{constr-non-complex-forming} gelten,
...
die keine Nominalphrase mit strukturellem Kasus verlangen. Siehe auch S.\,\pageref{page-kongruenz-scheinen}.
...
genauso wie der einfache Kopf \emph{kennt}. Die Argumentanordnungen können genauso
analysiert werden wie die Umordnung der Argumente von \emph{kennt}. Siehe dazu
Abschnitt~\ref{sec-mf}.
...
[\ms{ head   & \ibox{1}\\
   ...
   [\ms{ head & \ibox{1} \ms[verb]{ subj  & \eliste\\
                                    vform & fin \\
                                  }\\
...
\z
Es gibt nur Lesarten mit weitem Quantorenskopus.
...
\zl
Bei der Kombination von \emph{niemand beginnt} mit \emph{den Aufsatz zu lesen} mittels Kopf"=Komplement"=Schema
ist das Komplement eine vollständig gesättigte Phrase. Die bisherige Version des
Kopf"=Komplement"=Schemas (siehe S.\,\pageref{schema-bin}) sagt nichts über den phrasalen Status des Komplements aus: Sowohl gesättigte
als auch ungesättigte Komplemente können mit Köpfen kombiniert werden. Normalerweise verlangen Köpfe,
...
Kopf"=Komplement"=Strukturen wird deshalb ins Kopf"=Komplement"=Schema integriert:

\begin{schema}[Kopf-Komplement-Schema (binär verzweigend, vorläufige Version)]
\label{schema-bin-sat}
\begin{tabular}[t]{@{}l@{}}\is{Schema!Kopf"=Komplement"=}
...
kombiniert wird. Eine Kombination von \emph{zu lesen} und \emph{beginnt} ist nur mit dem
...
%\resizebox{\linewidth}{!}{%
\ms{
\ldots & \ms{ cat & \ms{ arg-st & \begin{tabular}[t]{@{}l@{}}
                                  \nliste{ NP[\type{str}]\ind{1}  } $\oplus$ \ibox{2} $\oplus$ \\
                                  \nliste{ V[\type{inf}, \textsc{subj}
                                    \sliste{NP[\type{str}]\ind{1}}, \textsc{comps}~\ibox{2}, \ltop \ibox{3}] }
                                  \end{tabular}}\\
              cont & \ms{ ltop & \ibox{4}\\
                          ind  & \ibox{5} event } }\\ 
...
Der Lexikoneintrag in (\mex{1}) zeigt den \localw für das \aciv \emph{sehen}:\footnote{
        ...
        annehmen. Zu Reapes Ansatz siehe auch Abschnitt~\ref{sec-Reape-Linearisierung}.%
%Einige Probleme, die sich für Reapes Ansatz ergeben,
%        wurden bereits im Abschnitt~\ref{sec-domain-union-fuer-verbalkomplex} diskutiert.
}
%% $^,$\footnote{
...
\ms{ \ldots & \ms{ cat & \ms{ arg-st & \begin{tabular}[t]{@{}l@{}}
                                     \nliste{ NP[\type{str}]\ind{1} } $\oplus$ \ibox{2} $\oplus$ \ibox{3} $\oplus$\\
                                     \nliste{ V[\type{bse}, \textsc{lex}+, \textsc{subj}~\ibox{2},
                                         \textsc{comps}~\ibox{3}, \ltop \ibox{4}] }
                                     \end{tabular}}\\[2mm]
                   cont & \ms{ ltop & \ibox{5}\\
                               ind  & \ibox{6} event } }\\[10mm]
rels & \liste{  \ms[sehen]{
              lbl  & \ibox{5}\\
              arg0 & \ibox{6}\\
              arg1 & \ibox{1}\\
...
\ldots & \ms{ cat & \ms{% head   & verb \\
           arg-st & \begin{tabular}[t]{@{}l@{}}
                    \nliste{ NP[\str]\ind{1}, NP[\type{ldat}]\ind{2} } $\oplus$ \ibox{3} $\oplus$\\
                    \nliste{ V[\type{inf}, \textsc{subj}~\sliste{NP[\type{str}]\ind{2}},
                      \textsc{comps}~\ibox{3}, \ltop \ibox{4}]}
                    \end{tabular}
% \begin{tabular}{@{}l@{}l}
...
der bereits auf S.\,\pageref{erfolg-auszukosten-erlaubt-kasus} diskutiert wurde,
...
\z
Zu den Details der Passivanalyse siehe Abschnitt~\ref{sec-remote-passive-hpsg}.
\is{Verb!Objektkontroll-|)}%
\is{Kohärenz|)}%
...
\ms{ head   & verb \\
           comps & \begin{tabular}{@{}l@{}}
                    \nliste{ NP[\type{nom}], NP[\type{acc}]\ind{1} } $\oplus$ \nliste{ VP[\type{bse}, \textsc{subj}~\sliste{ NP[\type{nom}]\ind{1}}]}\\
                    \end{tabular}
% \begin{tabular}{@{}l@{}l}
%                             \textrm{V[}&\textrm{\type{inf}, \textsc{lex}+, \textsc{subj}~\sliste{NP[\type{str}]\ind{2}},}\\
%                                     & \textrm{\textsc{comps}~\ibox{3} ]:\ibox{4}}\\
...
für das Dativpassivhilfsverb im Abschnitt~\ref{sec-anal-dativpassiv}.
...
Konstituenten zulässt (siehe auch Abschnitt~\ref{sec-Reape-Linearisierung}). 
...
Für (\ref{ex-weil-es-ihr-jemand-zu-lesen-versprochen-hat-zwei}) -- hier als (\mex{1})
wiederholt -- nimmt er an, dass \emph{es zu lesen} eine Phrase ist, die unter
\emph{ihr versprochen} eingebettet ist, und die so entstandene Phrase ist unter \emph{jemand hat}
eingebettet.
\ea
\label{ex-weil-es-ihr-jemand-zu-lesen-versprochen-hat-drei}
weil    es       ihr       jemand   zu lesen versprochen hat\footnote{
\textcites[\page 110]{Haider86c}[\page 128]{Haider90b}.%
}
\z
...
Abschnitt~\ref{sec-Reape-Linearisierung}). Die Elemente in solchen Listen 
...
Abschnitt durch Argumentanziehung und freie Abbindung der Argumente im Kopf"=Komplement"=Schema
erreicht.

Für Anhebungsverben wie \word{scheinen} nimmt Reape an, dass das Anhebungsverb einen nicht"=finiten
Satz einbettet, der auch das Subjekt enthält. Für (\mex{1}) wäre
...
müssen \parencites[Abschnitt~5.1]{Kathol98b}[Abschnitt~21.1]{Mueller99a}. 
...
wird und dann mit diesem kongruieren kann, wenn das Matrixverb finit ist, ist also
vorzuziehen. Diese Daten stellen übrigens auch ein Problem für phrasale
Konstruktionsgrammatikansätze\is{Konstruktionsgrammatik (CxG)}, wie sie in
Abschnitt~\ref{Abschnitt-phrasale-Valenz} besprochen wurden, dar.\todostefan{TAG macht das mit Anhebung semantischer
  Rollen in der frame-Semantik.}

Außer den Kongruenzdaten stellt auch das sogenannte Fernpassiv, das im Abschnitt~\ref{sec-remote-passive-phen}
...
Kasus. Zu den Details siehe Abschnitt~\ref{sec-remote-passive-hpsg}.
Auch für das Fernpassiv habe ich noch nie eine Analyse im Rahmen phrasaler
Konstruktionsgrammatikansätze gesehen.%
...
\citet{Bech55a} beschreibt neben den hier diskutierten Anordnungen von Verben in kohärenten Konstruktionen noch die sogenannte
Oberfeldumstellung\is{Oberfeldumstellung}\is{Feld!Ober-}. (\mex{1}) zeigt ein Beispiel für diese Konstruktion:
\ea
...
Eine weitere Stellungsvariante, die von \citet{Bech55a} jedoch nicht beschrieben wird, ist die sogenannte
...
Rahmen der HPSG findet man in \citew[Abschnitt~17.5]{Mueller99a}.
...
Lexikonregel vor, die Modifikatoren in die Valenzliste einfügt. Damit sind die Modifikatoren Argumente des
...
Japanische\il{Japanisch} Kausativkonstruktionen.\is{Kausativ!-konstruktion} Solche lexikalischen
...
in Korpora kann man unter Bezugname auf das Prinzip der Kooperativität erklären. Sprecher*innen wollen normalerweise
...
Umstellungen verbieten. Ich gehe stattdessen davon aus, dass viele Umstellungen durch Regeln ausgeschlossen
...
Selektionsrestriktionen verschieden sind, denn das sind die Merkmale, die eine Hörer*in/""Leser*in verwenden
...
die kooperative Sprecher*in/""Autor*in eine weniger komplexe Konstruktion verwenden, was die 
