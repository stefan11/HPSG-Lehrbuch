...
\ea
\catw für \emph{lachst}:\\
\ms{ head & \ms[verb]{ vform & fin }\\
%     spr & \eliste\\
     arg-st & \nliste{ NP[\nom]\sub{[2,\type{sg}]} } }
\z
In diesem Lexikoneintrag sind die Person- und Numerus"=Werte des Subjektindexes spezifiziert. Wird
\emph{lachst} mit einer Subjektsnominalphrase kombiniert, muss diese in der zweiten Person Singular
stehen.
...
\eal
\ex Ich gab dem Kind das Buch.
\ex Robin gab dem Kind das Buch.
\zl
...
\emph{gab} wäre dann \type{1\_or\_3} mit den Untertypen \type{1} und \type{3}. Man kann das auch
noch weiter treiben und die Werte für \textsc{per} und \textsc{num} in eine Typhierarchie
integrieren \citet[19]{Flickinger2000a}.
}
...
\ms{
 head & \ms[verb]
        { vform & fin } \\
 spr    & \eliste\\
 arg-st & \nliste{ NP[\type{nom}]\sub{[1 $\vee$\! 3, \type{sg}]}, NP[\type{dat}], NP[\type{acc}]   } }
\zs
...
Verlangt ein Verb keine Nominalphrase im Nominativ\is{Verb!subjektloses}, muss es in der dritten Person Singular stehen.
...
Für die Beispiele in (\mex{-1}) und (\mex{0}) ergeben sich die folgenden \argstlen für die finiten Verben:
\ea
...
\end{tabular}
\z
Diese Argumentstrukturlisten enthalten jeweils nur eine NP, und bei Nominativ-NPen muss es
Kongruenz geben, bei Nicht"=Nominativ"=NPen muss das finite Verb in der dritten Person stehen.

Bei Verben mit mehr Argumenten sind die Verhältnisse genauso. (\mex{2}) zeigt
die \argstlen für die Verbalkomplexe in (\mex{1}):
\eal
...
Bisher stand das Nominativ"=Element immer an der ersten Stelle in der \argstl. Das ist aber nicht
...
In einem Satz wie (\mex{1}) wird das Objekt von \emph{lesen} (\emph{die Aufsätze}) zum Subjekt des gesamten Satzes.
\ea
...
\z
Die \argstlen für das finite Verb im Aktivsatz in (\mex{1}) und im Passivsatz in (\mex{0}) zeigt (\mex{2}):
\ea
...
gibt, muss das Verb in der dritten Person Singular stehen.\footnote{
  ...
  Er ist ein Lügner.
  \z
  Hier kann man davon ausgehen, dass das Verb mit der ersten NP im Nominativ (dem
...
Hindi hat wesentlich komplexere Kongruenzphänomene \citep{Arsenault2002a}, und man kann auch \emph{Subjekt} nicht ohne weiteres
mit \emph{Nominativ} gleichsetzen. Durch ein Kongruenzprinzip, das sicherstellt, dass das Verb
mit dem 
%am wenigsten obliquen 
ersten Nominativ"=Element kongruiert, werden die Hindi"=Daten und auch die
...
Einteilung ergibt sich Tabelle~\ref{Tabelle-Wunderlich-Adjektivflexion}, wobei \type{strong}\istype{strong} heißt, dass der Determinator
...
\lspbottomrule
\end{tabular}
\caption{Adjektivflexion und Werte für Flexionsklasse, Numerus, Genus und Kasus nach Wunderlich}\label{Tabelle-Wunderlich-Adjektivflexion}
\end{table}
...
des Adjektivs \emph{interessantes}, das bereits im Kapitel über Modifikation auf S.\,\pageref{le-kleinen-sem}
besprochen wurde:

\eas
\label{le-interessantes-agr}%
Kongruenzinformation im \catw für \emph{interessantes}:\\
\ms{ head & \onems[adj]
                      { %prd & $-$ \\
            mod$|$loc|cat \onems{spr \tabularliste{~ \upshape Det[\begin{tabular}[t]{@{}l@{}}
...
\noindent
In (\mex{0}) wird anders als im auf S.\,\pageref{le-kleinen-sem}
angegebenen Eintrag nur auf die morpho"=syntaktischen Eigenschaften des Determinators Bezug genommen.
Über den referentiellen Index des Adjektivs wird im Lexikoneintrag für das Adjektiv gesagt, dass er mit dem des Nomens
identisch sein muss, sonst nichts. Behandelt man die morpho"=syntaktischen Merkmale des Determinators als
...
\ex Nun sitz ich hier, wie ein altes \emph{Weib}, \emph{das} \emph{ihr} Holz von Zäunen stoppelt
      und \emph{ihr}\iw{ihr!Possessivum} Brot an den Türen, um \emph{ihr} hinsterbendes, freudloses Dasein noch einen
...
und Adjektiv dürfen aber nie \type{fem} sein.\footnote{
  ...
  siehe \citew[\page417--418]{Mueller99a}. \citet[199--200]{WZ2003a} diskutieren
  \emph{Mädchen}-Beispiele im Serbokroatischen (BKS)\il{Serbokroatisch (BKS)}.%
%ein altes Weib, das ihr Holz von den Zäunen stoppelt das = neutrum ihr = femininum
%
}
...
seine Freundin
\z
Den \localw für das Possessivum\is{Pronomen!Possessiv-} zeigt (\mex{1}):
\eas
\label{le-seine-agr}%
...
%                      gen & fem \\
%                      num & sg\\
% % Tabelle -> dtype dtype ist aber für Adjektive
% % Wenn Numerus unspezifiziert wäre, dann wäre der Kasus aber zu strikt.
% % Kasus ist im Plural immer gleich.
%                      dtype & strong \\
%                      spec & \ms{ loc|cont & \ms{ ltop & \ibox{1}\\
%                                              ind  & \ibox{2}\\
%                                            }}\\
%                      }\\
%              arg-st & \eliste\\
...
Wir haben im Abschnitt~\ref{sec-kopfeigenschaften} auf S.\,\pageref{page-kopf-merkmal}
