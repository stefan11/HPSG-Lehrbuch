...
\ex Der Mann, [{\em der\/}] Maria küßt, liebt sie. \label{r1}
\ex Der Mann, [{\em den\/}] Maria küßt, liebt sie. \label{r1b}
\ex Der Mann, [{\em dem\/}] Maria zuhört, liebt sie.\label{r1c}
\ex Der Mann, [von {\em dem\/}] Maria geküßt wird, liebt sie. \label{r2}
\ex Die Stadt, [in {\em der\/}] Karl arbeitet, ist attraktiv. \label{r3}
\ex Ich machte Änderungen, [{\em deren\/}\iw{deren!Relativpronomen} Tragweite] mir nicht bewußt war.\label{r4}
\ex es hätte die FDP zerrissen und Kandidat Scharping das Signal gebracht, [{\em dessen\/} entbehrend] er schließlich scheiterte.\footnote{
  taz, 20.10.1998, S.\,1.}
\ex Das ist ein Umstand, [{\em den\/} zu berücksichtigen] meist vergessen wird.\label{r5}\footnote{
...
In (\mex{0}) handelt es sich bei den Relativpronomina\is{Pronomen!Relativ-|(} um sogenannte {\em d\/}"=Pronomina, aber
auch {\em w\/}"=Pronomina kommen als Relativum vor, wie die Beispiele in (\mex{1}) zeigen.
\eal
\label{bsp-rs-w-pron}
% weiterführender RS
...
\ex Macht kaputt, [\emph{was}] euch kaputtmacht!\footnote{
        Ton, Steine, Scherben, {\em Warum geht es mir so dreckig?\/}, erschienen bei Indigo, David Volksmund Prod.\ als LP und CD, 1971.
      }
\zl
\eal
...
\eal
\ex Der Mann$_i$, [von {\em dem\/}$_i$] Maria geküßt wird, liebt sie.
\ex Die Stadt$_i$, [in {\em der\/}$_i$] Karl arbeitet, ist attraktiv.
\ex Änderungen$_i$, [{\em deren\/}$_i$ Tragweite] mir nicht bewußt war
\ex das Signal$_i$, [{\em dessen\/}$_i$ entbehrend] er schließlich scheiterte
\ex ein Umstand$_i$, [{\em den\/}$_i$ zu berücksichtigen] meist vergessen wird
\zl
...
\Tree[.{PP[\textsc{rel} \sliste{ \ibox{1} }]}
          [.P von ]
...
\ea
Mann, [von dessen Schwester]$_i$ [Maria ein Bild \_$_i$ gemalt hat]
\z
...
\Tree[.{RS[\textsc{rel} \eliste, \textsc{slash} \eliste] }
          [.{PP[\textsc{loc} \ibox{2}, \textsc{rel} \sliste{ \ibox{1} }]}
            [.P von ]
            [.{NP[\textsc{rel} \sliste{ \ibox{1} }]}
                 [.{DET[\textsc{rel} \sliste{ \ibox{1} }] } dessen ]
                 [.N Schwester ] ] ]
          [.{S[\textit{fin\/}, \textsc{slash} \sliste{ \ibox{2} }] } \edge[roof]; 
              {Maria ein Bild \_ gemalt hat} ] ]
\end{tikzpicture}
...
\eal
\ex[*]{
Der Mann, den Maria, schläft.
}
\ex[]{
Der Mann, den Maria kennt, schläft.
}
\zl
...
Projektionen von Verben wie \emph{Maria den Mann kennt} können nicht modifizieren.
...
\ea[*]{
\label{bsp-lp-rs}
Mann, [\sub{S} Maria \_$_i$ liebt] den$_i$
}
\z
...
\eal
\ex[*]{
Der Mann, [ein Bild von dessen Schwester] Maria malt, schläft.
}
\ex[*]{
...
\eal
\ex der Stuhl, [auf dem] Maria sitzt
...
zeigt Abbildung~\vref{abb-mann-von-dessen-schwester}.
\ea
Mann, von dessen Schwester Maria ein Bild gemalt hat
\z
\begin{figure}[htbp]
\centerline{%
\begin{tabular}[t]{@{}cc@{}cccccc@{}}%llll}
\multicolumn{3}{c}{\rnode{1}{\nbar\ind{1} }}\\
\\[4ex]
\rnode{2}{\ibox{2}\nbar\ind{1}} & \multicolumn{5}{c}{\rnode{3}{RS[\textsc{mod} \ibox{2}, \textsc{rel} \eliste, \textsc{slash} \eliste] }}\\
\\[4ex]
& \multicolumn{3}{c}{\rnode{4}{PP[\textsc{loc} \ibox{3}, \textsc{rel} \sliste{ \ibox{1} }]}}  & \multicolumn{2}{c}{\rnode{9}{S[\textit{fin\/}, \textsc{slash} \sliste{ \ibox{3} }] }}\\
\\[4ex]
                                    & \rnode{P}{P}   & \multicolumn{2}{c}{\rnode{8}{NP[\textsc{rel} \sliste{ \ibox{1} }]}} \\
\\[4ex]
                  &                           & \rnode{10}{DET[\textsc{rel} \sliste{ \ibox{1} }] } & \rnode{11}{N}\\
\\*[3ex]
\rnode{12}{Mann} & \rnode{13}{von}            & \rnode{14}{dessen}         & \rnode{15}{Schwester}                 & M\rnode{16}{aria ein Bild \_ gemalt h}at \\
\end{tabular}
\ncline{1}{2}\ncline{1}{3}%
\ncline{3}{4}\ncline{3}{9}%
\ncline{4}{8}%
...
\ncline{11}{15}%
\ncline{9}{16}}
\caption{\label{abb-mann-von-dessen-schwester}Perkolation und Abbindung der \textsc{rel}- und \slashwe}
\end{figure}
...
\scalebox{0.8}{%
\begin{sideways}%
\begin{pspicture}(-0.2,-4)(22.1,6.4)
%\psgrid
\rput[B](4,6){\rnode{sign}{\textit{sign}}\istype{sign}}
%
\rput[B](1,4){\rnode{word}{\textit{word}}\istype{word}}
\rput[B](7,4){\rnode{phrase}{\textit{phrase}}\istype{phrase}}
%
\rput[B](4,2){\rnode{nhs}{\textit{non-headed-phrase}}\istype{non"=headed"=phrase}}
\rput[B](10,2){\rnode{hs}{\textit{headed-phrase}}\istype{headed"=phrase}}
%
\rput[B](1,0){\rnode{rc}{\type{relative-clause}}}\istype{relative"=clause}
\rput[B](5,0){\rnode{hnas}{\type{head"=non-adjunct-phrase}}}\istype{head"=non"=adjunct"=phrase}
\rput[B](10,0){\rnode{hnars}{\type{head"=non"=argument"=phrase}}}\istype{head"=non"=argument"=phrase}
\rput[B](15,0){\rnode{hnfs}{\type{head"=non"=filler"=phrase}}}\istype{head"=non"=filler"=phrase}
\rput[B](20,0){\rnode{hnss}{\type{head"=non"=specifier"=phrase}}}\istype{head"=non"=specifier"=phrase}
\rput[B](5,-4){\rnode{has}{\textit{head"=adjunct-phrase}}\istype{head"=adjunct"=phrase}}
\rput[B](10,-4){\rnode{hars}{\textit{head"=argument-phrase}}\istype{head"=argument"=phrase}}
\rput[B](15,-4){\rnode{hfs}{\textit{head-filler-phrase}}\istype{head"=filler"=phrase}}
\rput[B](20,-4){\rnode{hss}{\textit{head"=specifier-phrase}}\istype{head"=specifier"=phrase}}
%
...
\end{pspicture}%
\end{sideways}}
\caption{\label{abb-sign-relcl}Typhierarchie für \textit{sign}}
\end{figure}
