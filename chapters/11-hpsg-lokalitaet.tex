...
\ea
% hier phrase hinzuschreiben wäre falsch, denn \argst gibt es nicht bei Phrasen.
% eigentlich kann es die abgebildete Struktur auch nicht geben.
\onems{ phon \type{list~of~phoneme strings}\\
     ...
     rels  \type{list of relations}\\
     hcons  \type{list of qeqs}\\
     head-dtr  \type{sign}\\
     non-head-dtrs  \type{list~of~signs}\\ 
     dtrs           \type{list of signs}\\
}
\z
Man könnte \zb einen Kopf spezifizieren, der ein Argument mit dem \textsc{phon}"=Wert $\langle$~\emph{dem}, \emph{Mann} $\rangle$ selegiert. Genauso ist es möglich,
...
Information als Wert des Merkmals \textsc{syntax-semantics} (\textsc{synsem}) zusammengefasst wird. (\mex{1})
...
\noindent
Die Schemata müssen entsprechend angepasst werden. Das neue Kopf"=Komplement"=Schema\is{Schema!Kopf"=Komplement"=}
hat folgende Form:
...
\end{schema}
Statt wie in der Version auf S.\,\pageref{schema-Kopf-Komplementschema-prel2} ein Element
...
non-head"=dtrs & \liste{ \onems{ cat  \ms{ head$|$mod & \ibox{2} \\
                                           comps   & \eliste } \\
                             cont|ltop \ibox{1} \\
                           }}\\
}
...
\emph{machten} in Verberststellung steht und dass \emph{den Garaus} vor das Verb gestellt wurde. Dass diese
...
Es gibt (mindestens) zwei Möglichkeiten, Idiome ohne den Zugriff auf \phon zu beschreiben. Eine
...
\mbox{\type{i\_strings\_rel}}. \textsc{lid} ist Teil des \catwes und kann somit selegiert werden. Idiome
lassen sich damit über Selektionsketten beschreiben: Ein Kopf selegiert ein bestimmtes idiomatisches
Element und dieses kann weitere selegieren. Obwohl es also in Sags Ansatz nicht nötig wäre, \phon zu
selegieren, argumentiert er für eine Architektur, die das ermöglicht. Sein Vorschlag wird im folgenden
...
Sign"=Based\is{Construction Grammar (CxG)!Sign-Based (SBCG)|(}
...
verwenden sie ein \textsc{mother}"=Merkmal\isfeat{mother}. Das Kopf"=Komplement"=Schema mit der hier
...
\type{head-complement-construction}\istype{head"=complement"=construction} \impl\\
\ms{
mother|syn|val|comps \ibox{1}\\
head-dtr \ibox{2} [syn|val|comps  \ibox{1} $\oplus$ \nliste{ \ibox{3} } ]\\
dtrs \liste{ \ibox{2}, \ibox{3} } \\
}
\z
...
über die Töchter.\footnote{
   Wie \citeauthor*{SWB2003a} habe ich die Strukturteilung des Kopfwertes in (\ref{feat-geom-swb})
   und (\ref{feat-geom-Müller})  weggelassen. Auch Teilung
   von am Mutter-Knoten repräsentierter Valenzinformation wurde nicht berücksichtigt, damit die
   Merkmalbeschreibungen lesbar bleiben.%
}
\ea
\label{feat-geom-swb}
\ms[head-complement-construction]{
mother & \ms[phrase]{
         phon  & \phonliste{ eine, Pizza, aß }\\
         syn   & \ms{ head  & verb\\
                      val|comps & \sliste{ NP }\\
                    }\\
         sem   & \ldots\\
...
head-dtr & \ibox{1} \ms[word]{
           syn & \ms{ head   & verb\\
                      val|comps & \sliste{ NP, \ibox{2}\,NP }\\
                    }\\
           sem   & \ldots\\
           }\\
dtrs & \sliste{ \ibox{2}, \ibox{1} }\\
}
\z
...
head-dtr  \ibox{1} \ms[word]{
           synsem$|$loc & \ms{ cat & \ms{ head   & verb\\
                                          comps & \sliste{ NP, \ibox{2}\,NP }\\
                                        }\\
                               cont & \ldots \\
                             }
           }\\
dtrs  \sliste{ [\textsc{synsem} \ibox{2}], \ibox{1} }\\
}
\z
In (\mex{0}) gehören die Merkmale \textsc{head-dtr} und \textsc{dtrs} zu den Merkmalen, die
...
Beschränkungen für die Töchter des Objekts in der \textsc{dtrs}"=Liste zu formulieren, was bei
...
Es gibt mehrere Argumente gegen die in SBCG angenommene Merkmalsgeometrie: Der erste Grund dafür,
sie abzulehnen, ist ein empirischer: Die Lokalitätsbeschränkung macht es unmöglich, komplexe
phrasale Einheiten zu beschreiben. Solche Beschreibungen braucht man, wenn es Idiome gibt, die über
Satzgrenzen hinaus gehen und deren Teile nur in bestimmten Konfigurationen vorkommen. \citew{RS2009a} diskutieren die folgenden Idiome als Beispiel:
\eal
...
\ea
Aicke weiß, dass Conny kommt.
\z
Da diese Beschreibung die Argumentstruktur von \emph{dass} enthält, kann man auf das Argument von
\emph{dass} zugreifen. \emph{wissen} hat also Zugriff auf \emph{Conny kommt}. Damit hat aber
\emph{wissen} auch Zugriff auf die Argumentstruktur von \emph{kommt}, weshalb \emph{Conny} für
\emph{wissen} zugänglich ist. Das sollte aber gerade durch die restriktivere Merkmalsgeometrie
ausgeschlossen werden.\footnote{%
  Siehe \citet*[15--16]{AGS1998a} für eine Analyse, in der die \argstl von Elementen selegiert wird,
  die durchaus auch interne Struktur haben können. \citet[328,332]{AG2000a} gehen davon aus, dass
  Adverb--Verb-Kombinationen und Koordinationen von Verben unter Verben eingebettet werden können
  und dass die \argstl der Verben also an phrasalen Projektionen verfügbar sein müssen. Auch
  Meurers' Geister \citeyearpar{Meurers99b} und Benders Ansatz \citeyearpar{Bender2008a} für
  diskontinuierliche Konstituenten im Wambaya\il{Wambaya} erlauben eine nicht-lokale Selektion.%
} \citeauthor{RS2009a} weisen darauf hin, dass neben den Argumenten auch Adjunkte
...
dtrs \onems{ filler-dtr  \ms[word]{ synsem$|$loc & \ibox{1} \\
          ...
          h-dtr  \onems{ lf$|$exc  `\textup{\normalfont a horse kicks x\ind{2}}'\\
                             (dtrs$|$h-dtr)$^+$ \onems[word]{
                                                     synsem$|$loc$|$cat \ms{ head    & \ms{ tense &
                                                         pres\\ }\\
                                                                             listeme & treten\\
                                                                           }\\
                                                     arg-st \tabularliste{ NP[\textsc{listeme} \type{pferd}, \textsc{def} $-$, \type{sg}],\\
...
NP im Singular mit dem Kopf \emph{Pferd} vorkommt. Das zweite Element in der
...
kann. Das ist mit der klassischen HPSG"=Merkmalsgeometrie, der sogenannten Construction"=Based
HPSG von \citet{Sag97a}, möglich, mit der Merkmalsgeometrie der SBCG jedoch nicht.
%\citet[\page 841]{VanEynde2018a-u} spezifiziert den Typ von Töchtern

Die Beschränkungen bezüglich der \emph{Pferd}"=Sätze in (\mex{0}) sind zu strikt, denn es
Idiomvarianten, bei denen das Akkusativpronomen nicht im Vorfeld steht:
\eal
\ex 
%Und von wegen, die dort lebende Menschen hätte keine Bedürfnisse wie Strassen, schulen,
%Krankenhäuser, 
ich glaub es tritt mich ein Pferd wenn ich einen derartigen Unsinn lese.\footnote{%
...
\ex omg dieser xBluuR der nn ist wieder da ey nein ich glaub es tritt mich ein Pferd!!\footnote{%
  \url{http://forum.gta-life.de/index.php?user/3501-malcolm/},
  10.12.2015.
}
\ex ich glaub jetzt tritt mich ein pferd\footnote{%
  \url{http://www.castingshow-news.de/menowin-frhlich-soll-er-zum-islam-konvertieren-7228/},
  2018-02-20.
}
\zl
Auf alle Fälle muss man aber sicherstellen, dass das Pronomen im Satz mit dem Subjekt von
\emph{glauben} koreferent ist. 

\citet{Sag2007a} und \citet*{KMSF2025a-u} haben eine Idiom"=Theorie entwickelt, die spezielle
Lexikoneinträge annimmt, die eine idiomatische Bedeutung haben und wieder andere idiomatische
Lexikoneinträge selegieren können. So kann man zum Beispiel das Idiom \emph{kick the bucket} mit
Hilfe eines speziellen Lexikoneintrags für \emph{kick} analysieren, der \relation{sterben}
bedeutet. \emph{kick} verlangt dann eine NP mit dem Nomen \emph{bucket}, das nichts bedeutet und
seinerseits wieder einen Artikel \emph{the} verlangt, der auch nichts bedeutet
(\citealt[Abschnitt~11.4]{KE94a}, \citealt{Everaert2010a-u}, G.\ \citealt[\page 21]{GMueller2011a}).
Man könnte analog \emph{glauben} eine spezielle Variante von
\emph{treten} selegieren lassen, die \emph{Pferd} und ein Pronomen selegiert. SBCG kann über das \textsc{xarg}"=Merkmal auf das Subjekt eines
eingebetteten Verbs zugreifen. Für die Pferde-Sätze müsste man den Zugriff auf das Objekt
erlauben, damit die Koreferenz des Akkusativpronomens mit dem Subjekt von \emph{glauben} erzwungen
werden kann. Lexeme können sogar auch festlegen, welches ihrer abhängigen Elemente extrahiert ist
und somit im Vorfeld steht, weil sie die \textsc{nonlocal}"=Werte der selegierten Elemente festlegen
können. Ob man auf diese Weise alle Idiome erfassen kann, ist eine offene empirische Frage. 

Sollte es möglich sein, mit speziellen Lexikonelementen über die Selektion entlang von Kopf"|pfaden
alle Idiome zu analysieren, verbleibt dennoch ein Problem: Wie kommt man in der Sprachentwicklung
dazu, dass die entsprechenden Selektionsketten ausgebildet werden? Mit dieser Frage zusammen hängt
auch die Frage nach abgespeicherten Chunks, die zwar zu einem großen Teil kompositionell aufgebaut
sind, jedoch von Sprecher*innen als Einheiten gespeichert
werden
\parencites{Bybee95a-u}{Langacker87a-u}[228--229]{PJ2005a}[64]{Goldberg2006a}[Kapitel~3]{Bybee2010a-u}. Abbildung~\ref{fig-locality-sbcg-hpsg}
zeigt den Unterschied zwischen SBCG und Constructional HPSG für das Beispiel in (\mex{1}):
\ea
Sandy ate it.
\z
...
\begin{forest}
sm edges
[S, tikz={\node[draw,red,inner ysep=-2pt,fit=()(!1)(!l)] {};}
  [NP-Sandy,tier=vp]
  [VP, tier=vp, fit=band, tikz={\node[draw,red,inner ysep=-2pt,fit=()(!1)(!l)] {};}
    [V-ate]
    [NP-it]]]
\end{forest}
\hfill
\begin{forest}
sm edges
[S, s sep+=1.9em, tikz={\node[draw,red,inner xsep=6pt,inner ysep=0pt,fit to=tree]{};}%tikz={\node[draw,red,fit=()(!11)(!lll)] {};}
  [NP-Sandy, tier=vp]
  [VP, tier=vp, tikz={\node[draw,red,inner xsep=4pt,inner ysep=-2pt,fit to=tree]{};}%tikz={\node[draw,red,fit=()(!11)(!ll)] {};}
    [V-ate]
    [NP-it]]]
\end{forest}
\hfill\mbox{}
\caption{Links SBCG, rechts Constructional HPSG: In SBCG gibt es immer nur Konstruktionen für lokale
  Bäume. In HPSG können Töchter komplex sein und das ganze Gebilde kann als solches gespeichert sein.}\label{fig-locality-sbcg-hpsg}
\end{figure}

Ich habe den phonologischen Beitrag mit an die Kategorien geschrieben, den \phon ist ja Bestandteil
der jeweiligen Knoten. Das wird normalerweise in Bäumen nur anders dargestellt. 

In SBCG werden jeweils linguistische Objekte mittels Konstruktionen in Beziehung zueinander
gesetzt. Zum Beispiel dominiert S die Knoten NP und VP in einer Subjekt-Kopf-Konstruktion und VP
dominiert V und NP in einer Kopf-Komplement-Konstruktion. In der HPSG-Version auf der rechten Seite
ist die Kopf-Komplement-Phrase hingegen als VP Teil der Subjekt-Kopf-Phrase. Es gibt ein linguistisches Objekt, das die
komplette Struktur enthält. Bei der Analyse auf der linken Seite braucht man jeweils Konstruktionen,
die von außen alles zusammenhalten, während rechts die Konstruktionen Bestandteil des Objekts
sind. Man kann das mit Exoskeletten\is{Skelett} und Endoskeletten in der Tierwelt vergleichen. Interessant ist
nun, dass ja verschiedentlich festgestellt wurde, dass auch nicht-idiomatische Wendungen bei
entsprechender Frequenz als Chunks gespeichert werden. In SBCG gibt es aber keine Chunks, die
irgendwie die Ableitungsstruktur enthalten würden. Man müsste quasi eine neue Datenstruktur
schaffen, die zu jedem Knoten die Konstruktion und die Töchter speichert. Das bedeutet aber, dass
man die Nicht-Lokalität nur ausgelagert hat. Das Skelett ist außen drum rum, aber man braucht eine
Verbindung zwischen den Skelettteilen, sonst würde alles zerfallen. Im Gegensatz dazu beschreibt
Constructional HPSG komplexe Objekte. Es liegt nahe, dass auch völlig transparente Objekte als
Ganzes gespeichert werden können, wenn sie entsprechend frequent sind. Bei Speicherung kann es dann
auch eine Lexikalisierung mit Bedeutungswandel geben.

Man könnte behaupten, dass man nur die Kategorie im obersten Knoten braucht, denn diese
repräsentiert ja den phonologischen Beitrag, die Wortart der Projektion und den gesamten
semantischen Beitrag des linguistischen Objekts. Das wäre jedoch nicht ausreichend, wenn es rein
syntaktisches Priming gibt. In der Psycholinguistik\is{Psycholinguistik} spricht man von \emph{Priming}\is{Priming}, wenn die
Aktivierung bestimmter Hirnareale durch entsprechende Benutzung dazu führt, dass diese Areale wieder
benutzt werden, weil sie eben schon aktiv sind. So wurde zum Beispiel gezeigt, dass Versuchspersonen
bevorzugt mit Passivsätzen antworten, wenn im Dialog davor bereits Passivsätze oder ähnlich
strukturierte Sätze mit Ortsangaben verwendet wurden. Priming an sich vermag noch nicht zwischen SBCG und Constructional HPSG zu unterscheiden,
denn auch die SBCG-Konstruktionen werden ja aktiviert und somit geprimet. Wenn allerdings Chunks,
die nicht mehr verarbeitet werden, weil sie als Ganzes gespeichert sind, ähnliche Strukturen primen,
dann wäre das ein Hinweis darauf, dass in Chunks Strukturen enthalten sind. Damit dieses Argument
stichhaltig ist, muss man natürlich syntaktische Strukturen haben von denen man sicher ist, dass
diese Strukturen der Auslöser für Primingeffekte sind und nicht etwa bestimmtes lexikalisches
Material oder gar nur phonologisches Material. Für Passiv konnten \citet*{ZBGS2019a} zeigen, dass hier das
Hilfsverb \emph{be} und die Präposition \emph{by} die Auslöser des Primings sind. Die Autor*innen
gehen davon aus, dass Phonologie, Informationsstruktur, Semantik und lexikalisches Material wichtig
sind. Sie schließen nicht aus, dass es syntaktisches Priming geben kann (S.\,2), haben aber für alle Fälle
von denen behauptet wurde, dass es sich um syntaktisches Priming handelt, nachgewiesen, dass andere
Faktoren maßgeblich sind. Also: Wenn es syntaktische Priming gibt, hat die Architektur der
Constructional HPSG den Vorteil, dass linguistische Objekte mit interner Struktur direkt modelliert
werden. Eine externe Struktur für die Repräsentation von Konstruktionen und den durch sie
lizenzierten Objekten ist nicht nötig.
...
auszuarbeiten, kann man erahnen, wenn man sich \citew{Richter2004a-u} bzw.\ die Zusammenfassung in
\citew{Richter2024a} anschaut.
}
...
\z
Durch die Spezifikation des Zyklus\is{Zyklus!in Merkmalbeschreibung} in (\mex{0}) ist die gesamte Beschreibung selbst innerhalb des
...
\citet[\page 37]{ps} schließen Zyklen\is{Zyklus!in Merkmalbeschreibung|(} per Definition aus. Würde man annehmen, dass
...
dtrs & \sliste{ [synsem \ibox{2} loc$|$cat$|$head$|$spec \ibox{1} ], [synsem \ibox{1}
  loc$|$cat$|$comps \liste{ \ibox{2} }]  } \\
}
\z
Die Nicht"=Kopf"|tochter (an erster Stelle in \textsc{dtrs}) ist in der Valenzliste des Nomens
enthalten, und die Kopf"|tochter (an zweiter Stelle in \textsc{dtrs}) wird
über \textsc{spec} vom Possessivum selegiert. Folgt man dem Pfad
\textsc{dtrs$|$""hd$|$""synsem$|$""loc$|$""cat$|$""head$|$""spec$|$""loc$|$""cat$|$""comps$|$""hd}\footnote{
...
Beginn des genannten Pfades nämlich als Wert von \textsc{dtrs$|$""hd$|$""synsem} auf"|tritt.
...
nur relevante Merkmale teilt und nicht die gesamte Struktur.
% Hier wäre eine Quelle schön. Das habe ich aber wohl nie dokumentiert. In der Diss steht was zu
% Zyklen und verwirrten Prolog-Interpretern.
So würden keine Zyklen entstehen, aber
die Analyse wäre komplexer. 

Ein weiteres Beispiel für die Verwendung zyklischer Strukturen ist die Verbspur in
(\ref{le-verbspur}) auf S.\,\pageref{le-verbspur}. Auch sie ließe sich umformulieren, doch wäre
...
\is{Zyklus!in Merkmalbeschreibung|)}
...
Töchter von Töchtern Bezug nehmen dürfen \citep[143--145]{ps}.
...
somit bei der von \citet{Sag97a} eingeführten Merkmalsgeometrie. Diese wird auch im 2021 erstmals erschienen
HPSG"=Handbuch verwendet, das die Forschungsergebnisse seit 1985 zusammenfasst \citep{HPSGHandbook}.
\is{Lokalität|)}\is{Idiom|)}\is{Construction Grammar (CxG)!Sign-Based (SBCG)|)}
...
\furtherreading{
Ein wichtiger Beitrag zur Lokalität von Selektion ist \citew{Sag2007a}.
Eine allgemeinere Diskussion von Lokalität in verschiedenen Grammatiktheorien findet sich in
\citew[Abschnitt~11.7]{MuellerGTBuch1} bzw.\ aktualisiert in der englischen Version \citew[Abschnitt~18.2]{MuellerGT-Eng5}.
}