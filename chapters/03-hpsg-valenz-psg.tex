...
die mit einem Atom eines Elements verbunden werden können. Sauerstoff hat die Valenz 2: Es kann sich
mit Wasserstoff zu H$_2$O verbinden. Man kann nun die Elemente in Valenzklassen einteilen.
...
\citew{AEEHHL2003a-ed,AEEHHL2006a-ed}. Das Buch von Tesnière wurde 1980 in Teilen ins Deutsche
...
Die in Abschnitt~\ref{sec-psg} diskutierten Phrasenstrukturgrammatiken\is{Phrasenstrukturgrammatik}
...
man in der HPSG -- wie in der Kategorialgrammatik\is{Kategorialgrammatik (CG)} \citep{Ajdukiewicz35a-u,Steedman2000a-u} -- Beschreibungen der Argumente eines
...
bildbar, \noword{helfbar} dagegen nicht. Siehe auch Abschnitt~\ref{sec-bar-Derivation-GPSG} und Abschnitt~\ref{sec-bard-pv}. 
...
In den Büchern von \citeauthor{ps} bis 1994 und in den ersten drei Auf"|lagen dieses Buches hieß das
Valenzmerkmal auch \subcat für Subkategorisierung \parencites{ps}[Kapitel~1--8]{ps2}{Mueller99a}.
...
für Kopf"=Komplement"=Kombination benutzen:
\ea
...
c.& N[\comps \ibox{1}] &  N[\comps \ibox{1} $\oplus$ \sliste{ \ibox{2} } ], \ibox{2}\\
...
\compsl.
Dabei ist `$\oplus$'\is{$\oplus$} (\emph{append}\isrel{append}) eine Relation zur Verknüpfung zweier Listen:
\ea
\begin{tabular}[t]{@{}l@{~}l@{}}
...
V[\comps \ibox{1}] $\to$ \ibox{2} NP, V[\comps \ibox{1} \eliste $\oplus$ \sliste{ \ibox{2} NP } ]
\z
...
Die leere Liste entspricht der Liste der noch zu sättigenden Argumente \iboxb{1}.
...
wir im Abschnitt~\ref{sec-psg} behandelt haben, ist sie anderer Natur:
...
\z
Wie bereits in Abschnitt~\ref{sec-grundlegendes} angemerkt, müssten als Wert von \phon phonologische Umschriften der Wörter angegeben werden,
was oft der Einfachheit halber nicht gemacht wird.
Stattdessen verwendet man die orthographische Form. Zur Behandlung der Phonologie in HPSG siehe
\citew{BK94b} und \citew{Hoehle99a-u}.

NP[\type{nom}], NP[\type{dat}] und NP[\type{acc}] stehen für komplexe Merkmalbeschreibungen, die intern
genauso aufgebaut sind wie der Eintrag für \emph{gibt} in (\mex{0}). Wie die Kasusinformation
repräsentiert wird, wird auf S.\,\pageref{page-ref-case-feat} erklärt. Die Reihenfolge der
...
\emph{aussetzen} ist die Reihenfolge \type{acc} vor \type{dat} die unmarkierte \parencites[\page
249]{Wegener85b}[\page 60]{Hoberg81a}. 
%
...
Das Deutsche zählt typologisch zu den Subjekt-Objekt-Verb-Sprachen (SOV), so wie \zb auch Afrikaans\il{Afrikaans},
Niederländisch\il{Niederländisch}, Japanisch\il{Japanisch}, Koreanisch\il{Koreanisch} und Persisch\il{Persisch} \citep{Haider2020a}. Das Englische\il{Englisch}, die skandinavischen Sprachen und die
...
ein Subjekt im Satz geben. Das wird in den Abschnitten~\ref{sec-argst-Liste}
und~\ref{sec-Subjekt-und-Valenz} genauer besprochen. Bei den SVO-Sprachen
...
in anderen das \sprm. Da ich das \subjm anders verwende, benutze ich wie \citet*[Abschnitt~4.3]{SWB2003a} das \sprm
für die Repräsentation des Subjekts in SVO-Sprachen \citep[Abschnitt~4.3]{MuellerGermanic}.

Die Struktur in Abbildung~\ref{fig-Aicke-expects-Conny} ist das englische Gegenstück zu
...
[{V[\spr \eliste, \comps \eliste]}
   [{NP[\type{nom}]} [Aicke] ]
   [V\feattab{
      \spr \sliste{ NP[\type{nom}] }, \comps \eliste}
     [V\feattab{
...
\emph{sleeps}  & \sliste{ NP[\type{nom}] } & \eliste                                  & \sliste{ NP[\type{nom}] }\\
\emph{expects} & \sliste{ NP[\type{nom}] } & \sliste{ NP[\type{acc}] }                  & \sliste{ NP[\type{nom}], NP[\type{acc}] }\\
\emph{speaks}  & \sliste{ NP[\type{nom}] } & \sliste{ PP[\type{about}] }                & \sliste{ NP[\type{nom}], PP[\type{about}] }\\
\emph{gives}   & \sliste{ NP[\type{nom}] } & \sliste{ NP[\type{acc}], NP[\type{acc}] }  & \sliste{ NP[\type{nom}], NP[\type{acc}], NP[\type{acc}] }\\
\emph{serves}  & \sliste{ NP[\type{nom}] } & \sliste{ NP[\type{acc}], PP[\type{with}] } & \sliste{ NP[\type{nom}], NP[\type{acc}], PP[\type{with}] }\\  
\end{tabular}
\z
...
\begin{tabular}[t]{@{}ll@{~~}l@{~~}l@{}}
Verb            & \spr                      & \comps                                      & \argst\\
\emph{schläft}  & \eliste & \sliste{ NP[\type{nom}] }                                  & \sliste{ NP[\type{nom}] }\\
\emph{erwartet} & \eliste & \sliste{ NP[\type{nom}], NP[\type{acc}] }                  & \sliste{ NP[\type{nom}], NP[\type{acc}] }\\
\emph{spricht} & \eliste & \sliste{ NP[\type{nom}], PP[\type{über}] }                 & \sliste{ NP[\type{nom}], PP[\type{about}] }\\
\emph{gibt}    & \eliste & \sliste{ NP[\type{nom}], NP[\type{dat}], NP[\type{acc}] }  & \sliste{ NP[\type{nom}], NP[\type{dat}], NP[\type{acc}] }\\
\emph{dient}   & \eliste & \sliste{ NP[\type{nom}], NP[\type{dat}], PP[\type{with}] } & \sliste{ NP[\type{nom}], NP[\type{dat}], PP[\type{with}] }\\  
\end{tabular}
\z
...
ausgehende Verteilung der Valenzinformation im Kapitel~\ref{Kapitel-Lexikon} über das Lexikon und im
Abschnitt~\ref{sec-ARP-und-Partizipien} über nicht-kanonische Verteilung bei nicht-finiten Verben noch einmal zurück.
...
sogenannte Linking\is{Linking}) auf \argst gemacht werden \citep*{DKW2024a}. Wir werden uns das im Kapitel~\ref{Kapitel-Semantik}
über Semantik genauer ansehen. Die Argumentstrukturliste ist für die Kasusvergabe (siehe Kapitel~\ref{Kapitel-Kasus})
und für die Bindungstheorie\is{Bindungstheorie} \citep{PS92a,Branco2021a,MuellerBinding} relevant.
\is{Valenz|)}
...
Abschnitt~\ref{sec-intro-arg-adj}\is{Adjunkt|(}\is{Modifikator|(} wurde festgestellt, dass die Form
...
man Adjunkte ihren Kopf über ein Merkmal (\textsc{modified})\isfeat{mod} selegieren
lassen.\footnote{\label{fn-adjuncts-as-complements}%
  In früheren Varianten der HPSG ging man davon aus, dass der Kopf das Adjunkt selegiert. So nahmen
  \citet[161]{ps} an, dass zu jedem Kopf eine Menge gehört, in der Beschreibungen möglicher Adjunkte
  enthalten sind. Zum Beispiel wären dann bei Nomina eine Beschreibung einer Adjektivphrase und einer
  Präpositionalphrase in dieser Menge. Diese Analyse wurde dann aber aus semantischen Gründen
  aufgegeben. \citet[Abschnitt~1.9]{ps2} gehen stattdessen davon aus, dass
  Adjunkte den Kopf, den sie modifizieren,
  selegieren. \textcites[64]{Miller92d-u}[83]{Warner93a-u}[21]{AG97a-u}[60]{MSI99a}[103]{Kim2000a-u} und \citet[131]{Sag2020a}
  schlagen für verschiedene Sprachen Lexikonregeln für die Einführung bestimmter Adjunkte (\zb Negation von Verben) in Valenzlisten vor.
  \Citet{NB94} haben für das Niederländische eine Analyse entwickelt, in der Adjunkte generell per
  Lexikonregeln in Valenzmerkmale aufgenommen werden. Auf diese Analyse komme ich im
  Abschnitt~\ref{sec-lr-for-transp-pvs} zurück. Ich übernehme sie für bestimmte
  Partikelverbkonstruktionen. \citet*[42]{BMS2001a} schlagen ebenfalls vor, einige Adjunkte als
  valenzabhängige Elemente zu behandeln, verwenden jedoch dafür nicht Lexikonregeln, sondern
  spezielle Merkmale. Zu solchen merkmalbasierten Ansätzen siehe auch Abschnitt~\ref{sec-vererbung-koenig}.% 
% Man braucht dazu das SCOPAL-Merkmal, das ich vorschlage. Dann geht es. Aber so wie es im Artikel
% ist, funktioniert es nicht. 
%Dieser Ansatz funktioniert jedoch so wie er nicht, weil Adjunkte die
%  andere Adjunkte semantisch einbetten anders als im Ansatz von \citeauthor{NB94} nicht korrekt
...
NP zu bilden. Die Abkürzung dafür ist \nbar. Ausschnitte des Lexikoneintrags für
\emph{interessantes} zeigt (\mex{1}):\footnote{
Der Übersichtlichkeit halber sind \spr- und \compsw hier direkt angegeben. Eigentlich sind im
Lexikon die \argst-Listen spezifiziert und das Argumentrealisierungsprinzip verteilt die Argumente
auf die Valenzlisten. Für Adjektive nehme ich in diesem Buch an, dass der \sprw immer die leere
Liste ist. Deshalb ist der \compsw immer identisch mit dem von \argst.
}
\eas\is{Adjektiv}%
...
und das deshalb leere \spr- und \comps-Listen hat. Adjektive wie \emph{treu} in (\mex{1}) würden entsprechend
...
dem wir uns mit Kopfmerkmalen und dem Kopfmerkmalsprinzip\is{Prinzip!Kopfmerkmals-} (siehe S.\,\pageref{prinzip-hfp})
...
%         (MRS)\indexmrs
%         zugrunde, die ab der vierten Auf"|lage auch in diesem Buch verwendet wird. Mit dieser Semantik treten die Probleme bei der Beschreibung der Semantik
...
\end{tabular}
\z
Phrasen wie (\mex{1}b) werden somit korrekt ausgeschlossen, da die Präposition eine NP in der
\compsl hat und somit mit der Spezifikation der Adjunkttochter im obigen Schema inkompatibel ist:
\eal
...
\zl
In (\ref{regelschema-psg-adjunct}) wählt die Adjunkttochter über \ibox{3} den Kopf, den sie modifizieren will. Der \sprw und der \compsw der Kopftochter werden mit dem \sprw und dem \compsw der Mutter
identifiziert (\,\ibox{1} und \ibox{2}\,). Das erfasst, dass in Kopf-Adjunkt-Strukturen keine
Argumente des Kopfes abgebunden werden. So ergibt sich \zb bei der Adjunktion an eine \nbar wieder
eine \nbar.
...
comps & \eliste }
\z
Diese Repräsentation entspricht der des Adjektivs \emph{interessantes} in (\ref{le-interessantes}) und kann 
...
Die erste Auf"|lage dieses Buches ist 2006 erschienen. Ich war zu der Zeit, in der ich an diesem Buch
...
auftaucht. 2013 im Rahmen der Lexical\indexlfg Functional Grammar \citep{Toivonen2013a,ADT2013a} und zuletzt in Tree Adjoining
Grammar \citep{KO2013a-u,LK2017a,Seyffarth2023a}. Ich habe also seit 2003 viele, viele Aufsätze und ja, sogar ein Buch veröffentlicht, die
zeigen, warum bestimmte Ansätze nicht funktionieren und wie man die Phänomene stattdessen
beschreiben kann. Alle der Publikationen wurden von Gutachter*innen begutachtet (peer
...
sondern auch in der Generalisierten Phrasenstrukturgrammatik (Generalized Phrase Structure Grammar,
GPSG), LFG, Simpler Syntax und in TAG. HPSG zählt selbst zur Klasse der konstruktionsgrammatischen Theorien. Seit 1997 gibt
es Constructional HPSG \citep{Sag97a} und Sign"=Based Construction Grammar\is{Construction Grammar
  (CxG)!Sign-Based (SBCG)} ist auch eine 
...
Die Generalisierte Phrasenstrukturgrammatik \citep*{GKPS85a} ist eine Vorgängertheorie der
HPSG. Ivan Sag ist einer der Köpfe hinter HPSG \citep{ps,ps2} und war auch Koautor des Hauptbuches
über GPSG. Pauline \citet{Jacobson87b} hat eine Rezension des GPSG-Buches verfasst, in der sie auf
einige Probleme der GPSG hingewiesen hat, die sich leicht im Rahmen der Kategorialgrammatik lösen lassen.
Carl Pollard hat bei Ivan Sag in Stanford mit einer kategorialgrammatischen Arbeit
\citep{Pollard84a-u} promoviert. Er wurde dann Koautor der beiden wichtigsten HPSG-Bücher aus der Zeit
nach der Veröffentlichung des GPSG-Buches. Im Folgenden möchte ich einige der Probleme der GPSG
besprechen, die so ähnlich auch für andere phrasale Ansätze gelten. GPSG geht davon aus, dass Valenz
mithilfe von Zahlen kodiert wird. Jeder Kopf hat ein \subcatm, dessen Wert eine Zahl ist. Diese Zahl
sagt etwas darüber aus, in welche Regeln ein Kopf eingesetzt werden kann. Die Zahl des Kopfes muss
zu der Zahl einer Regel passen. Dabei kann es recht verschiedene Regeln mit derselben Zahl
geben. Zum Beispiel eine Regel für Verbphrasen im Aktiv und eine mit derselben Zahl für Verbphrasen
im Passiv. Für ihre Grammatik des Englischen haben \citet[57, 60]{GKPS85a} zum Beispiel die beiden folgenden
Regeln:
\eal
\ex VP $\to$ H[3], NP, PP[\emph{to}]
\ex VP[PAS] $\to$ H[3], PP[\emph{to}], (PP[\emph{by}])
\zl
Die erste Regel kann mit dem Verb \emph{handed} verwendet werden, wie es in \emph{handed the sword
  to Tracy} vorkommt. Die zweite Regel kann für die Analyse von \emph{The sword was handed to
  Tracy.} genutzt werden.

Im folgenden Abschnitt bespreche ich ein morphologisches Problem für GPSG-Ansätze, auf das ich schon in
\citew[129]{MuellerGTBuch1} hingewiesen habe. 

\subsubsubsection{Morphologie}
\label{sec-bar-Derivation-GPSG}

Es gibt im Deutschen Derivationen, die Bezug auf Valenzeigenschaften ihres Kopfes nehmen. So muss
zum Beispiel der Verbstamm, der mit dem Suffix \suffix{bar} kombiniert wird, zu einem Verb mit
Akkusativobjekt gehören. Sowohl \emph{lösen} als auch \emph{vergleichen} verlangt einen
Akkusativ. Die Verben \emph{schlafen} und \emph{helfen} dagegen verlangen entweder gar kein Objekt
oder eins im Dativ.
\eal
\ex[]{
lösbar \jambox{(Nominativ, Akkusativ)}
}
\ex[]{
vergleichbar \jambox{(Nominativ, Akkusativ, PP[mit])}
}
\ex[*]{
schlafbar \jambox{(Nominativ)}
}
\ex[*]{
helfbar  \jambox{(Nominativ, Dativ)}
}
\zl
Man beachte, dass \emph{lösen} und \emph{vergleichen} beide ein Akkusativobjekt verlangen, aber
ihre Valenz ansonsten verschieden ist. Sie müssten also in einer GPSG-Grammatik verschiedene Zahlen
als \subcatw haben.

Die \bard entspricht der Passivierung mit einer zusätzlichen Modalbedeutung \citep[233]{Gelhaus77}:
\eal
\ex Alex liest den Roman.
\ex Der Roman kann gelesen werden.
\ex Der Roman ist lesbar.
\zl
Da bei Passivierung von \emph{helfen} ein unpersönliches Passiv entsteht, ist die \bard nicht
möglich, denn das Argument von \emph{helfen} steht im Dativ und das Adjektiv müsste einen Nominativ verlangen:
\eal
\ex[]{
Alex hilft dem Kind.
}
\ex[]{
Dem Kind kann geholfen werden.
}
\ex[*]{
Das Kind ist helfbar.
}
\ex[*]{
Dem Kind ist helfbar.
}
\zl
Wie (\mex{0}b) zeigt, gibt es keine semantischen Gründe, die gegen die Bildung von \noword{helfbar}
sprechen. Vielmehr ist es so, dass ein so genanntes persönliches Passiv möglich sein muss, \dash, es
muss ein Akkusativobjekt geben. Da man aber an der Zahl, die zu einem Verb gehört, nicht erkennen
kann, ob es einen Akkusativ verlangt, kann GPSG die \bard nicht adäquat erfassen. Mit der in diesem
Kapitel vorgestellten Repräsentation ist das hingegen kein Problem. Siehe \zb \citet{Riehemann97a}
für eine Analyse der \bard im Rahmen der HPSG.

\subsubsubsection{Voranstellung von Phrasenteilen}

Auf ein anderes Problem haben \citet{Nerbonne86a} und \citet{Johnson86a} in ihren Arbeiten im Rahmen
der GPSG selbst hingewiesen. Sie untersuchen die Voranstellung von
Verbalphrasenteilen\is{Voranstellung von Phrasenteilen}, wie sie in (\mex{1}) zu sehen sind:
\eal
\ex {}[Erzählen] wird er seiner Tochter ein Märchen können.
\ex {}[Ein Märchen erzählen] wird er seiner Tochter können.
\ex {}[Seiner Tochter ein Märchen erzählen] wird er können.
\zl
Das Problem an diesen Sätzen ist, dass sich im Vorfeld eine Konstituente befindet, die aus genau
einem Verb besteht (\mex{0}a), einem Verb und seinem Akkusativobjekt (\mex{0}b) und einem Verb mit
Akkusativ- und Dativobjekt (\mex{0}c). Alle Argumente des Verbs, die nicht gemeinsam mit dem Verb im
Vorfeld stehen spuken im Mittelfeld rum, weshalb Nerbonne die Konstruktion wohl auch
Poltergeistkonstruktion genannt hat. Wollte man diese Sätze mit der Valenzrepräsentation der GPSG
beschrieben, so müsste es für \emph{erzählen} die Möglichkeit geben mit jeder Teilmenge seiner
Objekte eine Phrase zu bilden. Damit wäre man aber noch nicht fertig, denn man müsste auch erklären,
wie die Nominalphrasen im Mittelfeld lizenziert sind.

Auch dürfen obligatorische Argumente nicht fehlen. Wenn sie im Vorfeld nicht enthalten sind, müssen
sie im Mittelfeld realisiert werden (\mex{1}b,c):
\eal
\ex[]{
Kim hat die Nudeln verschlungen.
}
\ex[]{
Verschlungen hat Kim die Nudeln.
}
\ex[*]{
Verschlungen hat Kim.
}
\ex[*]{
Verschlungen hat Kim den Nudeln.
}
\zl
Und die Nominalgruppen im Mittelfeld müssen zu den Anforderungen des Verbs im Vorfeld
passen: Liegt statt wie in (\mex{0}b) wie in (\mex{0}d) eine Dativ-Nominalphrase vor, wird der Satz
ungrammatisch. Nerbonne und Johnson haben das Problem gelöst, aber die Lösung bestand darin, dass
sie die lexikalische Repräsentation von Valenz aus der Kategorialgrammatik in GPSG nachgebaut haben.

In Abschnitt~\ref{sec-pvp} wird erklärt, wie die Voranstellung von Verbphrasenteilen in der HPSG
analysiert werden kann.

%\subsubsection{Vererbung}

\subsubsection{Potentielle und wirkliche Struktur}

Man kann verschiedene Theorien bzw.\ Theorieschulen dahingehend einordnen, ob sie die Argumente
einem Kopf zuordnen oder ob sie annehmen, dass die Argumente in bestimmten Strukturen realisiert
werden, ohne dass es beim Kopf Valenzinformation gäbe. Der hier vorgestellte Ansatz geht davon aus, dass im Lexikon Repräsentationen von Argumenten
enthalten sind und diese Argumente dann später in dafür vorgesehenen Konfigurationen realisiert
werden können. Zum Beispiel habe ich in (\ref{le-gibt-1}) den Lexikoneintrag für \emph{gibt} angegeben. Mit
diesem Lexikoneintrag kann man den Satz in (\mex{1}a) analysieren. In der
Analyse wird jeweils ein Komplement mit einem Verb bzw.\ einer Verbprojektion kombiniert. Man kann den Lexikoneintrag aber auch
für verschiedene andere Sätze verwenden. Zum Beispiel für Sätze, die ein Adjunkt enthalten wie
(\mex{1}b). Auch in Verberstsätzen wie (\mex{1}c) und (\mex{1}d) spielt der Lexikoneintrag eine
Rolle, wie in den folgenden Kapiteln erklärt werden wird. Bei (\ref{ex-Vorfeldellipse}) handelt es sich um einen Satz mit Vorfeldellipse \parencites[Abschnitt~1.5]{Huang84}{Fries88b}[574--576, Abschnitt~4.1.1.1]{Hoffmann97a}. Dieser Satz könnte die
Antwort auf die Frage \emph{Was macht Aicke?} sein.
\eal
\ex {}[dass] Aicke dem Eichhörnchen die Nuss gibt
\ex {}[dass] Aicke dem Eichhörnchen jetzt die Nuss gibt
\ex {}Gibt Aicke dem Eichhörnchen eine Nuss?
\ex
\label{ex-Vorfeldellipse}
Gibt dem Eichhörnchen eine Nuss.
\zl
Im Lexikoneintrag ist also potentielle Struktur erfasst. Ein Lexikoneintrag steht für eine Klasse
von Bäumen, die von einer bestimmten Grammatik mit diesem Lexikoneintrag lizenziert werden
\parencites{MuellerPotentialStructure}[Kapitel~22]{MuellerGT-Eng5}.

Andere Ansätze gehen davon aus, dass bestimmte Konfigurationen existieren, in die etwas eingesetzt
wird. Bei \citet[192]{Goldberg95a} war das eine Satzkonfiguration ([SUBJ [V OBJ OBL]]) und bei \citet[534]{GJ2004a} eine
VP-Konfiguration. Ich habe in diversen Veröffentlichungen darauf hingewiesen, dass solche Analysen
problematisch sind, weil Teile von Konstruktionen weit entfernt realisiert werden können, so dass
die Konfiguration, der ja Bedeutung zugeschrieben wird, nicht unbedingt vorliegen muss. \citet[453]{Goldberg2013a}
schreibt dazu:
\begin{quote}
[A]rgument structure constructions do not specify phrase structure trees or word order
directly [\ldots]. Other constructions that they combine with do. [\ldots{} It] is sometimes assumed that
`phrasal' means a particular tree configuration must be specified and that linear order is
necessarily fixed in advance (e.g. Müller, 2006). But it is not necessary, nor advantageous to make this
assumption. \citep[453]{Goldberg2013a}
\end{quote}
Das Beispiel in (\ref{ex-Vorfeldellipse}) zeigt, dass auch die Behauptung, dass die phrasalen
Argumentstrukturkonstruktionen nur Argumente auflisten würden, die dann irgendwo
in anderen Konstruktionen realisiert werden müssen, nicht funktioniert. Sobald man die Stufe der
Valenzbeschreibungen verlässt und behauptet, dass bestimmte Slots vorhanden sein müssen, kann man
solche Konstruktionen nicht mehr erklären, es sei denn man würde zulassen, dass das Subjekt durch
ein leeres Element realisiert wird. Da leere Elemente innerhalb der Konstruktionsgrammatik per Dogma
verboten sind \citep[\page 219]{Goldberg2003b}, ist dieser Ausweg aber versperrt.

Es bleibt dann nur, dass man sagt: Wenn \emph{geben} mit zwei Argumenten, \dash ohne Subjekt
realisiert wird, gibt es dafür eine spezielle Argumentstrukturkonstruktion. Leider ist die
Vorfeldellipse jedoch nicht auf Subjekte beschränkt. Auch Akkusativobjekte können weggelassen werden:
\eal
\ex {}[Ich] Finde den Abdruck der Anzeige der chemischen Industrie bezüglich Gen- und Biotechnologie 
      in der taz einen ziemlichen Fehlgriff. [Das] Halte ich auf Dauer für politisch nicht tragbar 
      und [ich] werde das Probeabo mit ziemlich bitterem Beigeschmack antreten und bei Wiederholung 
      auf gar keinen Fall verlängern.\footnote{
        Leserbrief in der taz, 24.02.98, S.\,14
      }
\ex\label{rock-muss-man-zitieren}
Rock ist ergiebig und geduldig. [Den] Muß man nur zitieren. Und so ist die Musik der
      große Spaß, die Sause, die eingängige Melodie, das verbotene Haareschütteln, und die
      Gitarre, die lächerlich ist oder von Gott kommt, [das] kann man sich aussuchen.\footnote{
        "`BHs statt Teddys"', taz, 15.11.1997
      }
\zl
\citet[30, 42]{Fries88b} hat gezeigt, dass bei Parallelität von Frage und Antwort sogar auch Dativ- und
Präpositionalobjekte weglassbar sind. Das bedeutet aber, dass nicht das Vorliegen von allen
Argumenten, die prinzipiell zu einer Argumentstrukturkonstruktion gehören, sondern die \emph{potentielle}
Realisierung ausschlaggebend ist. 

Einen ähnlichen Fall stellen Kontrollkonstruktionen wie die in (\mex{1}) dar. In solchen
Konstruktionen werden Subjekte nie ausgedrückt \citep[Abschnitt~4]{Mueller2007d}:
\ea
Aicke hat dem Kind empfohlen, [diesen Roman zu lesen].
\z
Die lesende Person kommt zwar im Satz vor, aber nicht in der Verbphrase \emph{diesen Roman zu
  lesen}. Wie im Abschnitt~\ref{sec-rais-contr-identity-coindexing} klar werden wird, muss das Subjekt von \emph{zu lesen} im Nominativ
stehen, \emph{dem Kind} steht aber im Dativ.

Die beiden Fälle haben gezeigt, dass man den Ansatz mit phrasalen Argumentstrukturkonstruktionen
nicht dadurch retten kann, dass man sagt, dass die Bestandteile einer Argumentstrukturkonstruktion
in beliebiger Reihenfolge im Satz stehen müssen, denn es gibt immer Möglichkeiten, Teile der
Konstruktion wegzulassen. Insgesamt ist es schwierig, gegen phrasale Ansätze
zu argumentieren, denn diese sind meistens nicht formal ausgearbeitet.




\subsubsection{Sprachübergreifenden Generalisierungen}
\label{sec-CxG-sprachübergreifende-Generalisierungen}

\citet[540, 541]{GJ2004a} nehmen für Sätze wie (\mex{1}) die Konfiguration in (\mex{2}) an:
\eal
\ex The trolley rumbled through the tunnel.
\ex The wagon creaked down the road.
\zl
\ea
NP V PP
\z
Sie nehmen an, dass mit dem Verb eine Geräuschemission verbunden sein muss und dass die
Gesamtkonfiguration dann eine gewisse Semantik beisteuert.

Nun gibt es aber eine völlig parallele Konstruktion im Deutschen \citep[922]{MuellerUnifying}:
\ea
dass die Straßenbahnen um die Ecke quietschen
\z
Es scheint also so zu sein, dass die Anordnung überhaupt nichts mit dem Phänomen zu tun hat. Das
Charakteristische der Konstruktion scheint vielmehr zu sein, dass diese Elemente überhaupt gemeinsam miteinander vorkommen. Das würde in einem Ansatz
richtig erfasst, der sagt, dass es für die \emph{rumpel}- und \emph{quietsch}-Verben neben dem
Eintrag als monovalentes Verb noch einen weiteren Eintrag mit einer Pfad-PP als Argument gibt, der
dann eine Fortbewegungsbedeutung hat. In welcher Reihenfolge die Argumente und der Kopf angeordnet
werden müssen, hängt von unabhängigen Eigenschaften ab. Im konkreten Fall ist das Englische eine
SVO-Sprache und das Deutsche eine SOV- und V2-Sprache (siehe Abschnitt~\ref{Abschnitt-V1} und
Kapitel~\ref{Kapitel-nla}). Daraus ergibt sich für sehr viele Kopf-Argument-Muster die jeweilige
Anordnung im Englischen oder im Deutschen. Diese Eigenschaften haben aber ansonsten
nichts mit Valenz oder phrasalen Konstruktionen zu tun.

Im Abschnitt~\ref{Abschnitt-Lexikonregeln} stelle ich Lexikonregeln vor, mit denen man aus Lexikonelementen
mit bestimmten Eigenschaften andere Lexikonelemente ableiten kann. Im
Abschnitt~\ref{Abschnitt-Konstruktionsgrammatik-phrasal} geht es dann nochmal um phrasale Konstruktionsgrammatikansätze.

\subsection{Subjekte und Valenz}
\label{sec-Subjekt-und-Valenz}
...
in Abbildung~\vref{fig-Aicke-Conny-erwartet-ip} angenommen \parencites[siehe z.\,B.][\page 49]{Grewendorf88a}[171]{Lohnstein2014a}.
%
\begin{figure}
\centerline{%
\begin{forest}
...
\parencites[vergleiche z.\,B.][]{Haider82,Haider93a,Haider97a,Haider2010a,Grewendorf83a,Kratzer84a,Webelhuth85a,%
Sternefeld85b}%
[Kapitel~4]{Scherpenisse86a}%S. 31, Kapitel~4
{Fanselow87a,Grewendorf88a,Duerscheid89a,Webelhuth90,Oppenrieder91a,Wilder92a,Grewendorf93}%
[30]{Frey93a}%
%Duerscheid89a:60
{Lenerz94a}%
[30]{Meinunger2000a}[Abschnitt~IV.3]{Sternefeld2006a-u}[\page
172]{BG2014a-u}. \citet[Abschnitt~IV.3]{Sternefeld2006a-u} und \citet[Kapitel~2]{Haider2010a} kommen
zu dem Schluss, dass es im Deutschen keine Evidenz für eine IP gibt.
...
\parencites[vergleiche][Abschnitt~6.1]{ps}[Abschnitt~1.4]{ps2}[\page 34]{GSag2000a-u}.\footnote{
	Hierbei ist unwesentlich, ob man flache oder binär verzweigende Strukturen
	annimmt (siehe Abschnitt~\ref{sec-konstituentenreihenfolge-alternativen}). Wesentlich ist, ob das Subjekt durch
...
davon aus, dass Subjekte nicht subkategorisiert sind \parencites[siehe z.\,B.][\page26--28]{Chomsky93a}[\page33]{Hoekstra87a}.
...
(siehe \citew{KE94a,Sailer2000a,SS2003a} und \citew{Soehn2006a} zu Idiomanalysen im Rahmen der HPSG). 
...
(Zur Unterscheidung zwischen unakkusativischen und unergativischen Verben siehe Abschnitt~\ref{sec-unakkusativitaet}).

Die Behauptung, dass Subjekte nie Bestandteile von idiomatischen Wendungen sind, die ein frei belegbares
Argument enthalten, ist weit verbreitet \parencites[siehe z.\,B.][\page92]{denDikken95a}[\page112--116]{Kratzer96a} und wird auch explizit für das Deutsche formuliert, \zb bei \citet[\page50]{Grewendorf2002a}.
...
Verben eben kein zugrundeliegendes Subjekt haben (siehe Abschnitt~\ref{sec-unakkusativitaet}).
...
unbelebt, was Einfluss auf die Passivierbarkeit haben dürfte.\footnote{
        Siehe jedoch \citew[Abschnitt~3.1.2]{Mueller2002b} zu Passiven mit
        unbelebtem Subjekt.%
}
...
Mit 2:6 gab sie den ersten Satz übernervös ab, drehte darauf"|hin von Wut gepackt teuflisch auf und holte sich den zweiten Satz unter Zuhilfenahme göttlicher Passierschläge und perfekter Lobs ebenfalls mit 6:2.\footnote{
  taz, 22.10.1991, S.\,13.
}
\zl
...
%\citep[175]{Carnie2002a}.\footnote{
...
(zu semantischen Indices siehe Abschnitt~\ref{sec-index}).

Ich nehme im Folgenden an, dass sowohl expletive Objekte als auch expletive Subjekte
vom entsprechenden Kopf syntaktisch selegiert werden, jedoch keine semantische Rolle füllen. 
...
(nicht referierenden \emph{it} bzw.\ \emph{es}) in Subjektsposition vorherzusagen.
...
\emph{Extended Projection Principle} siehe auch \citew[Abschnitt~6.2.1]{Haider93a}, 
...
%% Stattdessen gibt es ein Kopf"|merkmal \textsc{subj}, das als Wert eine maximal 
...
\item Was bewirkt das Argumentrealisierungsprinzip?
\item Haben Verben im Deutschen Spezifikatoren?
\end{enumerate}}

\exercises{
\begin{enumerate}
\item Geben Sie die Argumentstruktur- und Valenzlisten für folgende Wörter an:
      \eal
      \ex er
