...
cat  & \ms{ head & verb \\
            subcat & \liste{ NP[\type{nom}]\ind{1}, NP[\type{dat}]\ind{2}  } \\
          } \\
...
\ea
\onems{ 
cat$|$subcat \liste{ NP[\type{nom}], NP[\type{dat}] } \\
}
\z
...
\onems[stem]{
  cat~ \ms{ head & verb  \\ 
            subcat & \liste{ NP[\type{nom}], NP[\type{acc}]$_{\ibox{1}}$ } $\oplus$ \ibox{2} \\
          } \\
} $\mapsto$ \\
\onems[word]{
  cat \ms{ head & \ms{ vform & passiv-part } \\
           subcat & \liste{ NP[\type{nom}]$_{\ibox{1}}$ } $\oplus$ \ibox{2} \\
         } \\
}
\z
...
\ex Eingabe:\\
\ms{
cat & \onems{ subcat      \liste{ NP[\type{nom}]\ind{1}, NP[\type{acc}]\ind{2} } \\
       }\\
cont & \ms[schlagen]{
      agens       & \ibox{1}\\
      patiens & \ibox{2}\\
...
\ex Ausgabe:\\
\ms{
cat  & \onems{ subcat      \liste{ NP[\type{nom}]\ind{2} } \\
       }\\
cont & \ms[schlagen]{
      agens       & \ibox{1}\\
      patiens & \ibox{2}\\
...
den Vertretern beider Theorien, dass immer Form"=Bedeutungs"=Paare betrachtet werden. Die beschriebenen
...
\z
Das Problem ist, dass wir in (\mex{0}) die \subcatl spezifiziert haben. Dieser
