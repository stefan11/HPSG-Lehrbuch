...
mich also nur auf einige Grundprobleme. Ich werde die bereits im Abschnitt~\ref{sec-lr}
...
%                                                        subj  & \eliste \\
                                                  arg-st  \type{list\_of\_not\_np\_str} $\oplus$ \nliste{ NP[\str]$_{2,sg}$ } $\oplus$ \etag\\
                                                } \\
                     cont \ms{ ltop & \ibox{3}\\
                               ind  & \ibox{4} } }\\
...
In der Lexikonregel in (\mex{0}) wird verlangt, dass das erste Element in der \argstl des Verbs mit
...
Anfang, der Elemente enthalten kann, die keine Nominalphrasen mit strukturellem Kasus sind (\type{list\_of\_not\_np\_str}), ist
notwendig, um die im Abschnitt~\ref{sec-remote-passive-hpsg} diskutierten Beispiele des Fernpassivs\is{Passiv!Fern-} mit
Objektkontrollverben\is{Verb!Objektkontroll-} erfassen zu können (siehe auch
S.\,\pageref{Kasusprinzip} zum Kasusprinzip).%
\is{Kongruenz|)}
...
Der Bedeutungsbeitrag des Verbs (\ltop \ibox{7}\,) wird unter einen Modaloperator eingebettet
...
\eas
\label{le-lösbar}%
\stem{lösbar} (inklusive Valenzmerkmalen und \subj):\\
\ms{
...
[Adj\feattab{\subj \nliste{ NP[\str]$_y$ }, \comps \eliste,\\
  ...
  [\ibox{3} Part\feattab{\textsc{mod} \ibox{4} [\ltop \ibox{5} h2, \textsc{ind} \ibox{6} e1],\\
                         \comps \nliste{ NP[\str]$_y$ },\\
                         \cont \ibox{6} [\ltop h3=h2, \textsc{ind} e2=e1],\\ 
