...
mich also nur auf einige Grundprobleme. Ich werde zeigen, wie man die bereits im Abschnitt~\ref{sec-lr}
erwähnten Lexikonregeln für die Modellierung morphologischer Zusammenhänge nutzen kann, und den alternativen Ansatz, der Affixe als eigenständige
Morphe annimmt, diskutieren.
\largerpage

\section{Die Phänomene}

In der Morphologie beschäftigt man sich mit Morphen\is{Morph} bzw.\ Morphemen. Ein \emph{Morph} ist
...
Morphems\is{Morphem} werden, wie oben schon gesagt, \emph{Allomorphe} genannt. \suffix{e}, \suffix{en}, \suffix{n}, \suffix{s} und
...
%% wie \emph{Hund} vorkommen kann -- gibt es auch noch sogenannte freie Morpheme. Das sind Morpheme,
...
des \emph{e} zurückführen lässt: Das \emph{e} wird eingefügt, wenn an der Morphemgrenze
(der Stelle an der zwei Morpheme\is{Morphem} bzw.\ Morphe\is{Morph} zusammenstoßen) ein \emph{d} oder ein \emph{t} auf ein
...
+ $\to$ e / \{d, t\} \_ \{s, t\}
\z
Dies ähnelt den Phrasenstrukturregeln aus Abschnitt~\ref{sec-psg}, wobei die Information hinter dem `/'
den Anwendungskontext für die Regel angibt. Verwendet man diese morphophonologischen Regeln,
reicht es anzunehmen, dass \suffix{st} die zweite Person Singular markiert. Die Regel erklärt auch,
...
eingefügte \emph{e} wird durch die Regel in (\mex{-1}) erklärt, da das Präteritumsmorph
auf \emph{t} endet und die Person"=Numerus"=Markierung mit \emph{s} beginnt.\todostefan{liebt+t = liebte erklären}
...
bei der Derivation dagegen ändert sich die Wortart (\mex{1}a,b) bzw.\ (\mex{1}c,d) -- und damit
gewöhnlich auch die Bedeutung -- oder die Wortart bleibt gleich (\mex{1}b,c) und die Bedeutung wird geändert.
\eal
\ex \stem{schlag} (Verb)
...
logischen Repräsentation des Verbs widerspiegeln. Das heißt, es muss eine eigene Relation für die
Tempus-Information geben, nicht nur ein Merkmal bei einem Event oder Ähnliches. 
\ea
Sie lacht nicht, aber sie wird lachen.
\z

\noindent
Die Linguist*innen sind sich nicht einig, wie sie Derivation und Flexion genau definieren.
...
so liegt \zb für die zweite Person Singular die Spezifikation in (\mex{1})
nahe.\is{Kongruenz|(}\todostefan{FB: list-of... muss man erklären}
\begin{figure}
\eas
\label{lr-verbal-inflection}%
Lexikonregel für die zweite Person Singular Präsens (vorläufig):\\[1mm]
\onems[fin-verb-infl-lr]{
phon   $f(\,\ibox{1}, \phonliste{ st })$\\
synsem$|$loc \ms{ cat &  \ibox{2} \onems{ head$|$vform  \type{fin} \\
%                                                        subj  & \eliste \\
                                                  arg-st  \etag $\oplus$ \nliste{ NP[\snom]$_{2,sg}$ } $\oplus$ \etag }\\[4mm]
                  cont & \ms{ ltop & \ibox{3}\\
                               ind  & \ibox{4} } }\\[10mm]
rels \liste{ \ms[present]{
               lbl & \ibox{3}\\
               arg0 & \ibox{4}\\
               arg1 & \ibox{5} } } $\oplus$ \ibox{6}\\[8mm]
%                                       arg2 & \ms[t-overlap]{
%                                              arg1 & \ibox{4}\\
%                                              arg2 & now\\
%                                              }\\
...
endet (siehe Regel (\ref{Regel-e-einfuegung}) auf S.\,\pageref{Regel-e-einfuegung}), die
Verknüpfung des Eingabestamms mit einem -\emph{e}- und dem \suffix{st} (\emph{redest}).  

Eine finite Form kongruiert mit dem Subjekt, wenn es eins gibt. Das Subjekt kann ein Satz oder eine
NP mit strukturellem Kasus sein. Wenn es kein Subjekt gibt, oder wenn das Subjekt nicht nominal ist,
muss das Verb in der dritten Person Singular stehen. Für die zweite Person Singular muss das Subjekt
aber eine NP mit strukturellem Nominativ sein.
Im Kapitel~\ref{Kapitel-kongruenz}
wurde eine Analyse der Kongruenz vorgestellt, die auf die Index"=Merkmale des Subjekts Bezug nimmt.
In der Lexikonregel in (\mex{0}) wird verlangt, dass ein Element in der \argstl des Verbs mit
strukturellem Nominativ einen Index mit der Person \emph{2} und dem Numerus \emph{sg} hat.
...
denn \emph{grauen} hat kein Argument mit strukturellem Nominativ. Die Unterteilung der \argstl in einen
Anfang, der andere Elemente enthalten kann, ist notwendig, um Passive von ditransitiven Verben
(siehe S.\,\pageref{ex-dass dem Kind der Roman geschenkt wurde-Passiv}) und
die im Abschnitt~\ref{sec-remote-passive-hpsg} diskutierten Beispiele des
Fernpassivs\is{Passiv!Fern-} mit Objektkontrollverben\is{Verb!Objektkontroll-} erfassen zu
können. Es gibt pro \argstl aber maximal eine NP mit strukturellem Nominativ, so dass die Kongruenz
korrekt geregelt ist (siehe auch S.\,\pageref{Kasusprinzip} zum Kasusprinzip).%
\is{Kongruenz|)}%
\largerpage
...
eingebettet.\footnote{ 
  Die Tempus"=Repräsentation ist eine Vereinfachung. Siehe \citew{Reichenbach47a-u} und darauf
  aufbauende Arbeiten.%
}
...
%% die Valenzliste von \emph{wurde} an erster Stelle eine NP mit lexikalischem Dativ enthält (vergleiche S.\,\pageref{abb-remote-pass-da-hm-erlauben}).
%% \eal
...
\ldots & \ms{ cat & \ms{ head   & verb \\
                         arg-st & \liste{ NP[\str]\ind{1}$_{2,sg}$ } }\\
              cont & \ms{ ltop & \ibox{2}\\
                          ind  & \ibox{3} event } }\\ 
rels & \liste{ \ms[present]{
               lbl & \ibox{2}\\
               arg0 & \ibox{3}\\
               arg1 & \ibox{4} }, \ms[lachen]{ 
               lbl  & \ibox{4} \\
               arg0 & \ibox{3} \\
               arg1 & \ibox{1} } }
}\iw{lachen}
...
\suffix{bar}"=Adjektiven wie \zb \emph{brennbar} gibt es produktiv gebildete \suffix{bar}"=Adjektive.
Bei diesen Adjektiven wurde ein Verbstamm eines transitiven Verbs mit dem Suffix \suffix{bar} kombiniert.
\eal
\ex Der Delphin löst das Problem.
...
Die produktive Lexikonregel zur Bildung von \suffix{bar}"=Adjektiven zeigt (\mex{1}). Siehe auch \citew[68]{Riehemann98a}
...
synsem & \onems{ loc  \ms{ cat  & \ms{ head   & adj\\ 
                                       arg-st & \ibox{2} \\
                                      } \\
                            cont & \ms{ ltop & \ibox{3}\\
                                        ind  & \ibox{4} } } }\\
rels & \liste{ \ms[modal-op]{
               lbl  & \ibox{3}\\
%               arg0 & \ibox{4}\\
               arg1 & \ibox{5} } } $\oplus$ \ibox{6}\\[6mm]
hcons & \eliste\\
%hcons & \liste{ \ms[qeq]{
%                harg & \ibox{5}\\
%                larg & \ibox{7} } }\\[6mm]
lex-dtr & \onems[stem]{
           ...
           synsem$|$loc \ms{ cat  & \ms{ head   & \ms[verb]{
                                         ...
                                         arg-st & \sliste{ \ibox{8} NP[\str] } $\oplus$ \ibox{2} \upshape(\etag $\oplus$ \nliste{ NP[\str] } $\oplus$ \etag)\\
                                       } \\
                             cont & \ms{ ltop & \ibox{5}\\
                                         ind  & \ibox{4} } }\\
          rels \ibox{6} }\\
...
Die erste NP -- die auch das designierte Argument sein muss \iboxb{8} -- wird wie bei der Passivierung unterdrückt, 
und die restliche \argstl \iboxb{2} wird zur \argstl des Adjektivs. Das Argumentrealisierungsprinzip\is{Prinzip!Argumentrealisierungs-}
...
Der Bedeutungsbeitrag des Verbs (\ltop \ibox{5}\,) wird unter einen Modaloperator eingebettet.
% (\,\ibox{5} \qeq \ibox{7}\,). Wie beim Einhorn-Beispiel mit \emph{glauben}, das im Abschnitt~\ref{Abschnitt glauben und Einhörner}
% besprochen wurde, gibt es zwei Lesarten für (\mex{1}):
% \ea
% mit einem Eichhorn vergleichbar
% \z
% Der Existenzquantor kann Skopus über den Modaloperator oder unter dem Modaloperator haben. Im ersten
% Fall muss es in der entsprechenden Welt ein Einhorn geben und im zweiten kann etwas mit einem
% Einhorn verglichen werden, ohne dass es eins gibt.\todostefan{Geht das nicht auch bei: Ich
%   vergleiche ihn mit einem Einhorn.?}
\newpage
...
\ldots & \ms{ cat & \ms{ head   & \ms[verb]{
                                    da & \sliste{ \ibox{1} } }\\
                           arg-st & \nliste{ \ibox{1} NP[\str]\ind{2}, NP[\str]\ind{3}} }\\
                cont & \ms{ ltop & \ibox{4}\\
                            ind  & \ibox{5} } }\\
rels & \liste{ \ms[lösen]{ 
             lbl  & \ibox{4}\\ 
...
\eas
\label{le-lösbar}%
\stem{lösbar} (inklusive Valenzmerkmale und \subj):\\
\ms{
\ldots & \ms{ cat & \ms{ head & \ms[adj]{ subj & \sliste{ NP[\type{str}]\ind{3} } \\
                           } \\
           spr    & \eliste\\
           comps  & \eliste \\
           arg-st & \sliste{ NP[\type{str}]\ind{3} } \\
         }\\
cont & \ms{ ltop & \ibox{6}\\
            ind  & \ibox{5} } }\\
rels & \liste{ \ms[modal-op]{
...
beim Partizip um ein flektiertes Verb. Die Adjektivbildungsregel, die bereits im Abschnitt~\ref{sec-adj-formation}
auf S.\,\pageref{lr-adjective-formation-da-approach} diskutiert wurde, lizenziert einen Adjektivstamm, der dann wieder
...
\parencites[z.\,B.][\page165]{Bierwisch87a}[\page934]{SW94a}[\page46]{Stiebels96a}[104--105]{Luedeling2001a}. Es handelt sich
...
\citet[\page163]{Bierwisch87a} diskutiert die beiden Analysen für \emph{auf"|hören}
...
}\todostefan{baseline funktioniert bei forest nicht}
%
\begin{figure}
a. 
\begin{forest}
...
wie \emph{auf"|gehört} machen eine solche Analyse plausibel. Allerdings ist
...
In \citew[277]{Mueller2003a} habe ich dargelegt, dass diese Beispiele nicht problematisch
...
das \prefix{ge} zwischen Partikel und Verb steht, da ja bei Annahme dieser
Struktur \stem{auf"|hör} mit \prefix{ge} und \suffix{t} kombiniert würde.
...
Sie besteht aus dem Präfix \prefix{Ge} und dem Suffix \suffix{e}. 
Deverbale \prefix{Ge} "~\emph{e}"=Nomina haben die Bedeutung `andauerndes/wiederholtes V-en'
und sind mit einer negativen Konnotation verbunden \citep[104]{Luedeling2001a}:
\ea
Sein Gepfeife ging Conny auf die Nerven.
\z
...
im vorigen Kapitel vorgestellten Analyse der Partikelverben die Flexions-
...
Das Lexikonelement\is{Semantik|(} in (\ref{le-lachst}) auf S.\,\pageref{le-lachst} ist das Ergebnis der Anwendung der Lexikonregel in
(\ref{lr-verbal-inflection}) auf den Verbstamm \stem{lach}. Abbildung~\vref{fig-losfaehrst-sem} zeigt, was passiert, wenn man die
...
Sie enthält zusätzlich jedoch noch Flexionsinformation.\footnote{Die oberste \relsl hätte eigentlich
  eine andere Reihenfolge: \relation{beginnen} muss am Anfang der Liste stehen, da die Liste mittels
  ...
  Originalaufsatz zu MRS wird auch eine Menge bzw.\ Multimenge verwendet und Mengenelemente sind nicht geordnet \citep*[293]{CFPS2005a}.
}
\begin{figure}
\begin{forest}
sm edges
[V\feattab{\cont [\ltop \ibox{1}, \textsc{ind} \ibox{2} ],\\
           \rels \nliste{ h1:present(h2), h2:beginnen(e1,h3), h3:lachen(e2,x) }}
  [\ibox{3} Part\feattab{\textsc{mod} \ibox{4} [\cont|\ltop \ibox{5} h3 ],\\
                         \cont \ibox{6} [\ltop h2, \textsc{ind} e1],\\ 
                         \rels \nliste{ h2:beginnen(e1,h3) } } [los]]
  [V\feattab{ \comps \sliste{ \ldots{} \ibox{3} Part[\textsc{mod} \ibox{4}, \cont \ibox{6}] },\\
              \cont  [\ltop \ibox{1} h1, \textsc{ind} \ibox{2} e1],\\
              \rels \nliste{ h1:present(h2), h3:lachen(e2,x) } }, l sep+=\baselineskip
% Flexions LR
  [V\feattab{ \comps \sliste{ \ldots{} \ibox{3} Part[\textsc{mod} \ibox{4}, \cont \ibox{6}] },\\
              \cont  \ibox{6} [\ltop h2, \textsc{ind} \ibox{2} e1],\\
              \rels \nliste{ h3:lachen(e2,x) } }, l sep+=\baselineskip,edge label={node[midway,right]{Flexions-LR}}
% PV LR
    [\ibox{4} V\feattab{\cont|\ltop \ibox{5} h3,\\
                        \rels \nliste{ h3:lachen(e2,x) }},edge label={node[midway,right]{PV-LR}}
      [lach-]]]]]
\end{forest}
...
\iboxb{6} geteilt. Damit ist der \ltopw von \stem{lach} mit selegierter Partikel identisch mit dem
\ltopw der Partikel, der Teil der Bedeutung von Partikel + Verb ist (\ltop h2). Dieser \ltopw wird in der Ausgabe der Lexikonregel für die Verbflexion
...
flektierten Form des Stammes \stem{lach} kombiniert wird, wird die von der Partikel beigesteuerte Relation, die zu
...
einer Verbpartikel kombiniert wird, wird klar, was die Verbpartikel an semantischer Information beisteuert.\is{Semantik|)}  
...
in (\ref{le-lach-+particle}) auf S.\,\pageref{le-lach-+particle} ist in (\mex{1}) zu sehen:
...
\ms{%
\ldots & \ms{ cat & \ms{ head   & verb\\
                         arg-st & \upshape (\,\ibox{1} \nliste{NP[\str]\ind{2} $_{2,sg}$}\,)  $\oplus$ \ibox{3} $\oplus$ \ibox{4}\\
...
\noindent
Obwohl der Bedeutungsbeitrag der Partikelverbkombination (\ltop \ibox{7} und \textsc{ind} \ibox{8}\,) noch unterspezifiziert
...
bildet. Der Beitrag, der von der Partikel kommen wird (\ltop \ibox{7}\,), wird so unter
...
\ldots &  \ms{ cat  & \ms{ head   & noun\\
                           arg-st & \upshape \nliste{ Det } $\oplus$ \texttt{realize-nomilization-args}(\ibox{2}) }\\
%                          comps & \eliste\\
               cont & \ms{ ltop & \ibox{3}\\
                           ind  & \ibox{4} \ms{ per & 3\\
                                                num & sg\\
...
rels & \liste{%
       ...
       ltop & \ibox{3}\\
       arg0 & \ibox{4}\\
       arg1 & \ibox{5} } } $\oplus$ \ibox{6}\\   
lex-dtr & \onems[stem]{
           phon   \ibox{1} \\
           synsem$|$loc \onems{ cat \ms{ head   & verb\\
                                         arg-st & \ibox{2} }\\
                                cont|ltop  \ibox{5} \\
                        }\\
           rels \ibox{6}
...
mit einer unbetonten\is{Betonung} Silbe in \suff{er}, \suff{el}, \suff{en} endet.
...
erfasst wird, dass der Numerus"=Wert\is{Numerus} in der Ausgabe der Lexikonregel ebenfalls
...
Der referentielle Index \iboxb{4} ist identisch mit dem Wert des \argzero"=Merkmals
der \relation{wiederholtes-ereignis}"=Relation. Diese Relation bettet den Bedeutungsbeitrag des
eingebetteten Verbstamms (\ltop \ibox{5}\,) ein.

In Nominalisierungen können Argumente von nominalisierten Verben nicht, im Genitiv oder als PP realisiert werden, wenn
...
\zl
In (\ref{lr-gee-nom}) wird deshalb die Beschränkung \texttt{realize-nomilization-args} verwendet,
die die Argumente des Verbs auf Argumente des Nomens abbildet. Dabei ist es auch möglich, dass kein
Argument realisiert wird. Die Realisierung von selegierten Verbpartikeln ist obligatorisch, weil man
sonst für \emph{Gesinge} zwei Analysen bekäme: die, bei der \emph{Gesinge} von \stem{sing}
abgeleitet wurde, und die, wo die \geen auf \stem{sing} + Partikel angewendet und die Partikel nicht
realisiert wurde.

% das Grauen vor dem Henker -> auch subjektlose Verben koennen nominalisiert werden

Als\is{Semantik|(} Beispiel für die Anwendung der Regel soll zuerst die Ableitung von \emph{Gerenne} diskutiert
werden. Als Eingabe zur Lexikonregel dient der Stamm des Simplexverbs \stem{renn}.
Der Eintrag für \stem{renn}, der in (\mex{1}) angegeben ist, ist parallel
zu dem bereits auf S.\,\pageref{le-lachen} diskutierten für \stem{lach}.
...
\noindent
Die Agens"=Rolle von \emph{rennen} (\argone) ist an die NP mit strukturellem Kasus gelinkt
\iboxb{1}. Alternativ werden auch Formen von \emph{Gerenne} ohne NP-Argument bzw.\ mit einem PP-Argument lizenziert.
...
(\mex{1}) zeigt den Lexikoneintrag für \emph{herum}.
Der Eintrag ähnelt dem von \emph{los}, der auf S.\,\pageref{le-los-asp}
angegeben wurde. Allerdings ist \emph{herum} intersektiv: Wenn jemand herumrennt, rennt er und das
Rennen ist ziellos. \ltop- und \indw des modifizierten Verbs sind also mit dem \ltop- und \indw der
Partikel identisch.
%
\eas
\label{le-herum-part}%
...
\ms{
\ldots & \ms{ cat & \ms{ head & \ms[particle]{ mod  & \upshape V[\textsc{arg-st} \sliste{ NP[\str]
      }, \ltop \ibox{1}, \textsc{ind} \ibox{2}] \\
%                                                   subj & \eliste \\
                                                  } \\
%                             comps  & \eliste\\
                             arg-st & \eliste \\
...
\begin{forest}
sm edges
[N\feattab{\cont [\ltop \ibox{1}, \textsc{ind} \ibox{2} ],\\
           \rels \nliste{ h1:wiederholt(i1,h2), h2:ziellos(e1), h2:rennen(e1,x) }}
  [\ibox{3} Part\feattab{\textsc{mod} \ibox{4} [\ltop \ibox{5} h2, \textsc{ind} \ibox{6} e1],\\
                         \cont \ibox{7} [\ltop h3=h2, \textsc{ind} e2=e1],\\ 
                         \rels \nliste{ h3:ziellos(e2) } } [herum]]
  [N\feattab{ \comps \sliste{ \ldots{} \ibox{3} Part[\textsc{mod} \ibox{4}, \cont \ibox{7}] },\\
              \cont  [\ltop \ibox{1} h1, \textsc{ind} \ibox{2} i1],\\
              \rels \nliste{ h1:wiederholt(i1,h3), h2:rennen(e1,x) } }, l sep+=\baselineskip
% Flexions LR
  [V\feattab{ \comps \sliste{ \ldots{} \ibox{3} Part[\textsc{mod} \ibox{4}, \cont \ibox{7}] },\\
              \cont  \ibox{7} [\ltop h3, \textsc{ind} e2],\\
              \rels \nliste{ h2:rennen(e1,x) } }, l sep+=\baselineskip,edge label={node[midway,right]{\gee-Nominalisierungs-LR}}
% PV LR
    [\ibox{4} V\feattab{\ltop \ibox{5} h2, \textsc{ind} \ibox{6} e1,\\
                        \rels \nliste{ h2:rennen(e1,x) }},edge label={node[midway,right]{PV-LR}}
      [renn-]]]]]
\end{forest}
}
...
((\ref{lr-pv}) auf S.\,\pageref{lr-pv}) auf den im Lexikon gelisteten Eintrag \stem{renn} angewendet
werden. Das Ergebnis der Regelanwendung ist ein lexikalisches Zeichen,
das eine Partikel selegiert \iboxb{3}.
Der Bedeutungsbeitrag dieser Partikel \iboxb{7} wird mit der Bedeutung
...
und bettet dessen semantischen Beitrag (h3) unter die Relation \relation{wiederholtes-ereignis} ein.
Im nächsten Schritt wird das Nomen flektiert (in Abbildung~\ref{fig-herumgerenne} nicht dargestellt)
und danach mit der Partikel kombiniert. In der Partikel wird der \ltopw und der Index des
...
der Kopf in einer head"=cluster"=Struktur ist, ist sein Bedeutungsbeitrag (\ltop \ibox{1},
\textsc{ind} \ibox{2}\,) 
...
Beitrag des Verbs zugreifen (\ltop \ibox{5}, \textsc{ind} \ibox{6}\,)  und das Label
und die Ereignisvariable der \relation{ziellos}"=Relation mit den Werten von \relation{rennen} identifizieren.
Da der semantische Beitrag (\ltop h3) durch die Nominalisierungsregel unter \relation{wiederholtes-ereignis}
eingebettet wird und da h3 mit h2 identifiziert wurde, ist die semantische Repräsentation für die gesamte Nominalisierung (\mex{1}), was genau dem erwünschten Resultat entspricht.
\ea
\textmrs{ h1, \{ h1:wiederholtes-ereignis(i1,h2), h2:ziellos(e1), h2:rennen(e1,x) \}, \{ \}}
\z
\is{Semantik|)}
\if0
\noindent
...
in \pref{lr-pv} auf S.\,\pageref{lr-pv} lizenziert wird. Dieser Eintrag ähnelt dem von \stem{lach}, 
...
\stem{renn} (mit Selektion einer Partikel):\\
\ms{%
\ldots & \ms{ cat & \ms{ head & verb\\
...
Das Ergebnis ohne angezogene nominale Argumente zeigt (\mex{1}).
\eas
...
%phon & f\textrm{(\,\phonliste{ ge }, \ibox{1}, \phonliste{ e }\,)}\\
\ldots &  \ms{ cat  & \ms{ head   & noun\\
                           arg-st & \liste{ Det, \onems{ l \onems{ c \onems{ h \onems[particle]{ mod$|$l \ms{
                                                                  cat & \onems{ head  \type{verb} \\
                                                                                %arg-st \ibox{1}  \\
                                                                          }\\
                                                                  cont & \ms{ ltop & \ibox{5}\\
...
Die Valenzmerkmale der Partikel (\subj \ibox{3}, \comps \ibox{4}\,) und die \argst des durch die
Partikel modifizierten Verbs (\argst \ibox{1}\,), die noch in (\ref{le-renn-+particle}) enthalten sind, habe ich
in (\mex{0}) weggelassen, da ich für das Beispiel davon ausgehe, dass von der Partikel keine
Argumente angezogen werden.

Die Bedeutung von \emph{rennen} + Partikel ist in (\mex{0}) ein Argument der
Relation \relation{wiederholtes-ereignis} \iboxb{8}. Dieser Wert kommt vom Eingabezeichen der
\gee-Nominalisierungs-LR. Da die Eingabe im konkreten Fall \stem{renn} + Partikel ist, entspricht der
\contw der Tochter dem \contw der Partikel. Unter \relation{wiederholtes-ereignis} wird also der
noch unbekannte Beitrag der Partikel eingebettet. Das Ergebnis der Kombination der Partikel \emph{herum} in \pref{le-herum-part}
...
In (\ref{le-gerenne+particle}) war das \argone von \relation{wiederholtes-ereignis} noch unbekannt \iboxb{8},
aber im Lexikoneintrag für \emph{herum} in (\ref{le-herum-part}) wird der \ltopw und der Index mit dem \ltopw und dem Index
des modifizierten Verbs in dem Label und Argument von \relation{ziellos} identifiziert, da
\emph{herum} intersektiv ist (\,\ibox{1} und \ibox{2}\,). Da die Tochter der
...
ein Argument von \relation{wiederholtes-ereignis} ist, wird der \ltopw von \emph{herum} zum Argument
von \relation{wiederholtes-ereignis}. Im Ergebnis sind also in
(\mex{0}) das Label und der Index von \relation{ziellos} mit dem Label bzw.\ Index von \relation{rennen} identisch.
...
Auf S.\,\pageref{lr-bar-adj} habe ich bereits die
Lexikonregel \pref{lr-bar-adj} für \bard vorgestellt.

Im Folgenden sollen komplexe Derivationen wie \emph{anfahrbar}
in (\ref{ex-anfahrbar-bleiben}) -- hier als (\mex{1}) wiederholt -- diskutiert werden:
\ea
 "`Die Kneipen,        Theater  und Geschäfte müssen \emph{anfahrbar} bleiben."'\footnote{
...
Partikelverbregel wurde bereits im Abschnitt~\ref{sec-lr-for-transp-pvs} besprochen.
...
\begin{forest}
sm edges
[{Adj[\argst \ibox{1}]}, l sep+=\baselineskip
%\bard LR
  [{V[\begin{tabular}[t]{@{}l@{}}
...
%
%PV LR
     [{V[\argst \ibox{3} \sliste{ NP[\str] }]}, edge label={node[midway,right]{PV-LR}}
        [fahr]]]]
\end{forest}
...
enthält (\,\ibox{3} $\oplus$ \ibox{1} (\,\etag $\oplus$ \nliste{ NP[\str] } $\oplus$ \etag{}\,) wobei
\ibox{3} eine Liste mit einer NP mit strukturellem Kasus ist).
...
Abbildung~\vref{fig-anfahrbar-val} zeigt die Kombination von \emph{an}$_5$ und \emph{fahrbar}.\footnote{
  Zu den verschiedenen Formen der Partikel \emph{an} siehe S.\,\pageref{verschiedene-ans}.%
} Der Übersichtlichkeit halber habe ich die Nummerierung von Abbildung~\ref{fig-p-fahrbar-val} beibehalten.
\begin{figure}
\begin{forest}
sm edges
[{Adj[\subj \sliste{ \ibox{7} }, \comps \eliste]}
   [\ibox{6} Part\feattab{
                 \subj \ibox{4} \eliste,\\
                 \comps \ibox{5} \nliste{ \ibox{7} NP[\str] } } [an]]
   [Adj\feattab{
        \subj \sliste{ \ibox{7} }, \comps \nliste{ \ibox{6} },\\
        \argst \ibox{1} \sliste{ \ibox{7}, \ibox{6} } }, l sep+=\baselineskip
%\bard LR
  [{V[\begin{tabular}[t]{@{}l@{}}
...
\end{figure}
Die Partikel hat eine NP[\str] in ihrer \compsl \iboxb{5}. Der \subjw der Partikel \iboxb{4} ist die leere Liste.
Die Verknüpfung von \ibox{3} (der \argstl des Basisverbs \stem{fahr}) mit \ibox{4} und \ibox{5} ist
deshalb eine Liste, die zwei NP[\str] enthält. Die \bard-LR trennt nun eine NP mit strukturellem
Kasus (das Subjekt von \stem{fahr} bzw.\ \stem{fahr} + Partikel, wie es in \emph{anfahren} vorkommt) ab und reicht den Rest der \argstl \iboxb{1} als \argstl an
das Adjektiv weiter.\footnote{Eigentlich müsste auf \stem{fahrbar} eine Flexionsregel angewendet
...
(\,\ibox{2} = \ibox{3} $\oplus$ \ibox{1} (\,\etag $\oplus$ \nliste{ NP[\str] } $\oplus$ \etag{}\,)) 
verletzt wäre: \iboxt{2} würde nur zwei Elemente enthalten, nämlich das Subjekt von \stem{fahr} und
die Partikel. Da \ibox{3} aber eine Liste mit einer NP mit strukturellem Kasus ist, muss \ibox{2}
...
\begin{forest}
sm edges
[Adj\feattab{\subj \nliste{ NP[\str]$_y$ }, \comps \eliste,\\
             \cont [\ltop \ibox{1}, \textsc{ind} \ibox{2} ],\\
             \rels \nliste{ h1:modal-op(h2), h2:gerichtet-auf(e1,y), h2:fahren(e1,x) }}
  [\ibox{3} Part\feattab{\textsc{mod} \ibox{4} [\ltop \ibox{5} h2, \textsc{ind} \ibox{6} e1],\\
                         \comps \nliste{ NP[\str]$_y$ },\\
                         \cont \ibox{7} [\ltop h3=h2, \textsc{ind} e2=e1],\\ 
                         \rels \nliste{ h3:gerichtet-auf(e2,y) } } [an]]
  [Adj\feattab{ \subj \nliste{ NP[\str]$_y$ }, \comps \nliste{ \ibox{3} },\\
                \argst \sliste{ NP[\str]$_y$, \ibox{3} Part[\textsc{mod} \ibox{4}, \cont \ibox{7}] },\\
              \cont  [\ltop \ibox{1} h1, \textsc{ind} \ibox{2} e2],\\
              \rels \nliste{ h1:modal-op(h3), h2:fahren(e1,x) } }, l sep+=\baselineskip
% bar-Derivation-LR
  [V\feattab{ \argst \sliste{ NP[\str]$_x$, NP[\str]$_y$, \ibox{3} Part[\textsc{mod} \ibox{4}, \cont \ibox{7}] },\\
              \cont  \ibox{7} [\ltop h3, \textsc{ind} \ibox{2} e2],\\
              \rels \nliste{ h2:fahren(e1,x) } }, l sep+=\baselineskip,edge label={node[midway,right]{\bard-LR}}
% PV LR
    [\ibox{4} V\feattab{\ltop \ibox{5} h2, \textsc{ind} \ibox{6} e1\\
                        \rels \nliste{ h2:fahren(e1,x) }},edge label={node[midway,right]{PV-LR}}
      [lach-]]]]]
\end{forest}
}
...
Lexikonregel wird der \contw \iboxb{7} mit dem \contw der Partikel in der \argstl identifiziert.
Die Lexikonregel für die  \bard bettet den \ltopw aus diesem \contw (h3) unter einen Modaloperator \emph{modal-op} ein.
Zu diesem Zeitpunkt ist keine Partikel vorhanden, so dass man nicht weiß, wie \ltop- und \indw der
...
Eintrags für  \stem{fahr} so spezifiziert ist.
Dadurch kann die Partikel \emph{an} auf den semantischen Beitrag des Basisverbs \stem{fahr} zugreifen
und \ltopw und \textsc{ind} mit ihm teilen, so dass das Label von \relation{gerichtet-auf} mit dem
von \relation{fahren} geteilt wird (h3=h2). Da das Adjektiv der Kopf in der head-cluster-Struktur
...
\ea
\textmrs{ h1, \{ h1:modal-op(h2), h2:gerichtet-auf(e1,y), h2:fahren(e1,x) \}, \{ \} }
\z
\iw{\bars|)}
\largerpage
...
kann. 
\LATER{FB: Aber wie würde das bei Dir gehen? Partikel+Nomen/adj. werden doch schon syntaktisch
...
Untertypen von \type{phrase} sondern auch als Untertypen von \type{word} zulässt. 
...
\parencites{Orgun96a}{Riehemann98a}{AW98a}{Koenig99a}[Abschnitt~6.2.5]{Mueller2002b}{Crysmann2002a} wurden innerhalb des HPSG"=Rahmens
...
von Lexikonregeln nicht gebraucht, da Morphemveränderungen über entsprechende Funktionen in den Lexikonregeln
geregelt werden. 
...
In vielen Fällen sind die Ansätze ineinander überführbar \parencites[\page168--169]{Koenig99a}[Abschnitt~6.2.5.2]{Mueller2002b},
...
Für die \bard könnte man zum Beispiel folgendes Suffix für \suffix{bar} annehmen:\todostefan{INDEX
  hinzufügen. Was macht der?}
\largerpage
\eas
\oneline{\onems{
phon   \phonliste{ bar }\\
synsem$|$loc  \ms{ cat  & \ms{ head   & \ms[adj]{ 
                                        ...
                                        comps & \ibox{3} $\oplus$ \sliste{ V[\comps \liste{
                                            NP[\str], \ibox{2} NP[\str] } $\oplus$ \ibox{3}, \ltop \ibox{4}]} \\
                                      } \\
                            cont & \ms{ ltop & \ibox{5} } }\\
\addlinespace[3pt]
rels \liste{ \ms[modal-op]{
...
}}\is{Morphem!-\suffix{bar}-Derivation}%
\zs
...
Zum Abschluss dieses Abschnitts soll noch darauf hingewiesen werden,
...
verbindet \citep[Abschnitt~2.1.1.4 und S.\,169--171]{Crysmann2002a}. 
...
flektierten Word kombiniert wird.  Derivationen wie (\mex{1}b) sind somit ausgeschlossen:
...
%% dagegen Derivationsaffixe explizit als Präfixe und Suffixe zu markieren. Stattdessen schlägt er
