...
\ldots & \ms{ cat & \ms{ %head   & verb \\
                             arg-st & \liste{ NP[\str]\ind{1}$_{2,sg}$ } \\[2mm]
                           }\\
                  cont & \ms{ ltop & \ibox{2}\\
                              ind  & \ibox{3} event } }\\ 
rels & \liste{ \ms[present]{
...
lex-dtr & \onems[stem]{
           ...
           synsem$|$loc \ms{ cat  & \ms{ head   & \ms[verb]{
                                         ...
                                         arg-st & \sliste{ \ibox{7} NP[\str] } $\oplus$ \ibox{2} \upshape(\etag $\oplus$ \nliste{ NP[\str] } $\oplus$ \etag)\\
                                       } \\
                             cont & \ms{ ltop & \ibox{5} } }\\
          rels \ibox{6} }\\
...
\itdopt{Was ist der Index des Adjektivs? Das Event des Verbs? Das schnell lesbare Buch. Igitt. Da
  muss bar auf schnell les- angewendet werden.}
Eingabe der Regel ist ein transitives Verb, \dash ein Verb mit zwei Nominalphrasen mit strukturellem Kasus.
Die erste NP -- die auch das designierte Argument sein muss \iboxb{7} -- wird wie bei der Passivierung unterdrückt, 
...
[V\feattab{\cont [\ltop \ibox{1}, \textsc{ind} \ibox{2} ],\\
           \rels \nliste{ h1:present(h2), h2:beginnen(h3), h3:lachen(x) }}
...
\noindent
Obwohl der Bedeutungsbeitrag der Partikelverbkombination (\ltop \iboxb{7} und \textsc{ind} \ibox{8}) noch unterspezifiziert
...
\ldots &  \ms{ cat  & \ms{ head   & noun\\
                           arg-st & \upshape \nliste{ Det } $\oplus$ \texttt{sublist}(\ibox{2}) }\\
%                          comps & \eliste\\
               cont & \ms{ ltop & \ibox{3}\\
                           ind  & \ibox{4} \ms{ per & 3\\
                                                num & sg\\
...
lex-dtr & \onems[stem]{
           phon   \ibox{1} \\
           synsem$|$loc \onems{ cat \ms{ head & \type{verb}\\
                                         arg-st & \ibox{2} }\\
                                cont|ltop  \ibox{5} \\
                        }\\
           rels \ibox{6}
...
\zs
\itdopt{überall hinschreiben, dass es head|verb ist?}
...
\noindent
Die Agens"=Rolle von \emph{rennen} (\argone) ist nicht an ein Element in der Valenzrepräsentation
...
Da der semantische Beitrag (\ltop h2) durch die Nominalisierungsregel unter \relation{wiederholtes-ereignis}
eingebettet wird, ist die semantische Repräsentation für die gesamte Nominalisierung (\mex{1}), was genau dem erwünschten Resultat entspricht.
\ea
