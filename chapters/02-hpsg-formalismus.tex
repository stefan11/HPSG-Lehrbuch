...
alle linguistischen Objekte modelliert werden.
}{% GT-Buch
...
des Buches so klein wie möglich zu halten. Die interessierte Leser*in sei auf
...
wie die äußere Struktur hat: \textsc{vorname}, \textsc{nachname}, \textsc{geburtstag},
\textsc{vater} und \textsc{mutter}.
...
töchter    & \liste{ \ms{
                     ...
                     mutter     & \ldots\\
                     töchter    & \eliste } } }
\z\todostefan{Unterste Klammer soll eingezogen sein, nicht abschließend
  mit der letzten Klammer.}
%\largerpage[2]
...
nachname   & meier\\
geburtstag & 10.10.1985\\
\textbf{\textsc{geschlecht}} & \textbf{\itshape männlich}\\
vater      & \ldots\\
mutter     & \ldots\\
\textbf{\textsc{kinder}}     & \liste{ \ms{
                     vorname    & clara\\
                     nachname   & meier\\
                     geburtstag & 10.10.2004\\
                     \textbf{\textsc{geschlecht}} & {\textbf{\itshape weiblich}}\\
                     vater      & \ldots\\
                     mutter     & \ldots\\
                     kinder     & \eliste\\ } } }
\z
...
Mengen werden mit geschweiften Klammern geschrieben.\footnote{
 ...
 Ich würde in diesem Buch Mengen nur zum Aufsammeln von semantischer Information verwenden.
 Dies kann genauso gut mit Hilfe von Listen gemacht werden, weshalb ich hier auf die
 Einführung der Mengen verzichte und statt der Mengen Listen verwende.%
}
...
\z
Der Typ wird \textit{kursiv} gesetzt.\pagebreak 
...
In Modellen müssen Typen immer maximal spezifisch sein (\emph{sort resolved}; \cites[18]{ps2}[102]{Richter2024a}). In unserem Beispiel
...
Die Beschreibung in (\mex{1}) erfasst die Zusammenhänge korrekt.
\largerpage[1.2]
%\begin{figure}[htbp]
\ea
...
formuliert. % Man beschreibt Sätze und Wörter, nicht die Grammatikregeln.
...
man auch von \emph{Unifizierbarkeit}.\footnote{
  ...
  Beschränkungen beider Beschreibungen zugleich erfüllen müssen (\emph{constraint satisfaction}; \cite[94]{Richter2024a}). Da
...
Wichtig ist, dass die Eigenschaften, die der Detektiv kennt, zu den Eigenschaften, nach denen die Auftraggeber*in sucht,
passen. Außerdem ist wichtig, dass der Detektiv nur gesicherte Information verwendet, er darf sich nicht irgendwelche
Eigenschaften der gesuchten Person ausdenken. Die Unifikation der Anfrage aus (\mex{-1}a) mit der Information
...
dass die gesuchte Person die Eigenschaften aus der Anfrage und die neu herausgefundenen Eigenschaften hat.
\is{Unifikation|)}
...
auch \emph{Signatur}\is{Signatur} genannt \citep[102]{Richter2024a}. % FR: Relationen gehören auch zur Signatur.
Als Grammatiker*in verwendet man die Typen in
...
\dash, ein Modell eines der Sätze in (\mex{0}) enthält eine der vier Formen für \emph{Frau} mit dem jeweiligen Kasus.
...
%genus & fem }
{}[\textsc{genus} \type{fem}]
%\todostefan{now the distance between Frau and the AVM is too big}
%kasus & Kasus\\
\ex\label{avm-mann}%
Mann:\\
\ms{
...
maximal spezifischen Typen als Kasus"=Wert. Sie müssen sortenaufgelöst (\emph{sort-resolved}) sein. Die zu (\mex{0}) gehörenden Merkmalstrukturen zeigen die
...
\end{figure}
In diesen Darstellungen gehört zu jedem Knoten ein Typ (\type{Nomen}, \type{fem}, \type{Nominativ},
\ldots), wobei die Typen in den Merkmalstrukturen immer maximal spezifisch sind, \dash keine
Untertypen mehr haben. Es gibt immer einen Eingangsknoten (im Beispiel \type{Nomen}), und die anderen
...
\item (Zusatzaufgabe) Im folgenden Kapitel wird die Relation \emph{append}\isrel{append} eine Rolle
...
\furtherreading{
Dieses Kapitel sollte die Leser*in auf leicht verständliche Weise an getypte Merkmalbeschreibungen und -strukturen heranführen.
...
wichtig. Die interessierte Leser*in sei deshalb auf die folgenden Publikationen verwiesen:
%
