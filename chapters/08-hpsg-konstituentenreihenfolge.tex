...
so dass es erst im Anschluß an das Kapitel~\ref{Kapitel-nla} möglich ist, alternative
...
% wohl wegen ARG-ST
  ...
  \citet[\page185]{FR92} und \citet[\page218--223]{Kiss95b} nehmen ebenfalls eine \compsl und ein Valenzprinzip an, das es erlaubt,
...
\sliste{ NP[\type{nom}] }, weshalb der \compsw der Verknüpfung aus \emph{das Buch} und \emph{kennt}
...
Obliqueness"=Hierarchie\is{Obliqueness}, \dash auf die Reihenfolge der Elemente in der \compsl und nicht auf die
...
(\mex{1}b) beschrieben werden:\footnote{
  Sätzen wie in (i) wenden wir uns im Kapitel~\ref{Kapitel-nla} zu.
  \ea
...
\ex \onems{
phon \ibox{1} $\oplus$ \ibox{2}\\ 
cat$|$comps \ibox{3} $\oplus$ \ibox{4}\\
head-dtr \ms{ phon & \ibox{1}\\
              cat  & \ms{ head$|$initial +\\
                          comps \ibox{3} $\oplus$ \ibox{5} $\oplus$ \ibox{4} \\
                        }\\
            }\\
...
\emph{unechte Mehrdeutigkeit}\is{Mehrdeutigkeit!unechte}. Durch die Spezifikation des \compswes wird
...
loc \onems{ cat  \ms{ head & \ms[verb]{ vform & fin\\
                           ...
                           comps & \sliste{ \onems{ loc \onems{ cat \onems{ head \ms[verb]{
                                                                             dsl & \ibox{1}\\
                                                                             }\\
                                                                      comps \eliste\\
                                                                    }\\
...
[{V[\type{fin}, \comps \eliste{}]}
  ...
  [{\ibox{2} NP[\type{dat}]}
    [dem Affen, roof]]
  [{V[\type{fin}, \comps \sliste{ \ibox{1}, \ibox{2}, \ibox{3} }] }
    [gibt]]]
\end{forest}
...
delay={where tier={word}{inner xsep=0}{}}, % unclear why it has to happen here with delay.
  [V\feattab{\comps \sliste{},\\
    ...
    [,% This empty node holds all the complements. The idea is to put it next
      ...
      for parent={calign=first}, % The sister comps (Buch, Frau) should be
