...
und~5.1]{Meurers2000b}{Mueller2005c,MuellerGS}.
...
Koordinationsprobleme deshalb hier in einem Anhang diskutieren.
...
die Behandlung der Koordination führt nun aber zu einem Problem mit den Zyklen in der
...
\head und muss deshalb in beiden Konjunkten gleich sein, da die \catwe der Konjunkte identifiziert
...
beibehalten will, auf die zyklische Verbspur verzichten muss. Das hat auch den Vorteil, dass man die
unplausiblen Verteilungen von \dslwen aus Abbildung~\ref{Abbildung Kennt und liebt Aicke diese Band}
nicht bekommt.
...
\ex {}[Zum zweiten Mal] [die Weltmeisterschaft] errang Clark 1965 \ldots\footnote{%
        Der deutsche Straßenverkehr, 1968, Heft 6, p.\,210, quoted after \citet*[\page 224]{Neumann69a-u}.
%\citep*[\page 162]{Benes71}
      }\label{bsp-zum-zweiten-mal-die-Weltmeisterschaft}

\ex {}[Zum zweiten Mal] [die Weltmeisterschaft] hat Clark 1965 errungen.
\zl
Die einzige sinnvolle Analyse für solche Daten scheint mir eine mit einem leeren Kopf zu sein: die
...
dass die vorangestellten Konstituenten von einem Verb abhängen müssen. Normalerweise ist das ein
finites Verb wie in (\ref{bsp-zum-zweiten-mal-die-Weltmeisterschaft}), aber es ist auch möglich,
dass das Verb wie in (\mex{0}b) infinit ist. In der
...
Im Folgenden bespreche ich weitere Vorschläge, die in der Literatur gemacht wurden.
...
analysiert. Die ursprüngliche Idee dafür kam von \citet{Gazdar81a}. Unter dem Pfad
...
phon & \phonliste{}\\
loc  & \ibox{1} \\
nonloc & \upshape [\textsc{inher|slash} \nliste{ \ibox{1} }]
}
\zs
...
Bewegung nach rechts ist begrenzt. Man könnte also das Problem mit (\mex{0}) einfach lösen, indem
man \emph{dass} einen Satz mit leerer \dsl-Menge selegieren lässt \citep[232]{Kiss95b}.\footnote{
...
einem Ansatz mit \dsl unter \head machen: In \emph{dass}-Sätzen müssen die Verben overt sein:
\ea[*]{
...
[V
  ...
  [{[\nonloc|\inher|\slasch \nliste{ \ibox{1} }]}]]
\end{forest}
\caption{Analyse mit \slasch-Elementen vom Typ \type{sign}}
\end{figure}

Das würde wohl funktionieren, allerdings ist ein Hauptbestandteil der Analyse der Voranstellung von
partiellen Verbphrasenm (siehe Abschnitt~\ref{sec-pvp}), dass sich das \lexm nicht in \local, sondern unter \synsem befindet. Deshalb
...
Füller teilt und den \lexw ignoriert. Das wäre jedoch eine unschöne Stipulation.
\begin{figure}
\begin{forest}
[V
  [\ms{ loc & \ibox{1}\\
  ...
  [\ms{\nonloc|\inher|\slasch \nliste{ \ms{ loc & \ibox{1}\\
                                            nonloc & \ibox{2}} }}]]
\end{forest}
\caption{Analyse mit \slasch-Elementen vom Typ \type{sign} aber einzeln geteilten Werten in \type{sign}}
\end{figure}
...
Eine weiter Möglichkeit besteht darin, dass man \dsl nicht unter \head spezifiziert, um die Zyklen
...
immer mit dem \dsl-Wert der Tochter identifiziert wird.
