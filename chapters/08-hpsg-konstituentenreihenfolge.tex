...
\zs
\medskip
% bug Und warum ist nach flushright so viel Platz nach unten?
...
Die Lexikonregel in (\mex{0}) lizenziert ein Verb, das einen Satz (\iboxt{1} in
...
loc \ms{ cat & \ms{ head & \ms[verb]{ vform & fin\\
                                      initial & $+$\\
                                      dsl     & none }\\
                    comps & \sliste{ \onems{ loc \onems{ cat \onems{ head|dsl \ibox{1}\\
                                                                     comps \eliste }\\
                                                         cont|hook \ibox{2} }\\
                                              }}\\
                  }\\
...
\noindent
Der \hook enthält jeweils den \ltop und den Index.
...
% \begin{forest}
% sm edges
% [V{[\textsc{cont} \subnode{c1}{\ibox{2}}\,]}, baseline
% 	[V{[\textsc{cont} \subnode{c2}{\ibox{2}}\,]}
% 		[V{[\textsc{cont} \subnode{c3}{\ibox{1}} [\ltop h1, \rels \nliste{ h1:kennen(j, roman) } ]]}, tier=np,edge label={node[midway,right]{V1-LR}}
% 			[kennt$_j$]]]
% 	[V\feattab{\textsc{dsl}|\textsc{cont} \subnode{c4}{\ibox{1}},\\
%                    \textsc{cont} \subnode{c5}{\ibox{2}}\,}
% 		[NP{[\textit{nom}]}, tier=np
% 			[jemand]]
% 		[V\feattab{\textsc{dsl}|\textsc{cont} \subnode{c6}{\ibox{1}},\\
%                            \textsc{cont} \subnode{c7}{\ibox{2}}\,}
% 			[NP{[\textit{acc}]}
% 				[den Roman, roof]]
% 			[V\feattab{\textsc{dsl}|\textsc{cont} \subnode{c8}{\ibox{1}},\\
%                                    \textsc{cont} \subnode{c9}{\ibox{2}} $=$ \ibox{1}\,}
% 				[\trace$_j$]]]]]
% \end{forest}
\oneline{%
\begin{forest}
sm edges
[V\ms{cont \ms{ltop & \subnode{ltop5}{\ibox{1}} h1\\
               rels & \nliste{ \ldots, h1:k(j, r) }}}, baseline
	[V\ms{cont  & \ms{ltop & \subnode{ltop4}{\ibox{1}} h1\\
                          rels & \eliste }\\
              comps & \sliste{ \ibox{2} dsl|cont \subnode{dsl0}{\ibox{3}} }}
		[V\ms{cont \subnode{c1}{\ibox{3}} \ms{ltop & h1\\
                                                      rels & \nliste{ h1:k(j, r) } }}, tier=np,edge label={node[midway,right]{V1-LR}}
			[kennt$_j$]]]
	[\ibox{2} V\ms{dsl|cont \subnode{dsl1}{\ibox{3}}\\
              cont \ms{ltop & \subnode{ltop3}{\ibox{1}} h1\\
                       rels & \nliste{ \ldots, h1:k(j, r) }}}
		[NP{[\textit{nom}]}, tier=np
			[jemand]]
		[V\ms{dsl|cont \subnode{dsl2}{\ibox{3}},\\
                      cont \ms{ltop & \subnode{ltop2}{\ibox{1}} h1\\
                               rels & \nliste{ \ldots, h1:k(j, r) }}}
			[NP{[\textit{acc}]}
				[den Roman, roof]]
			[V\ms{dsl|cont \subnode{dsl3}{\ibox{3}}\\
                              cont \subnode{cont-trace}{\ibox{3}}\ms{ltop & \subnode{ltop1}{\ibox{1}} h1\\
                                                                     rels & \nliste{ h1:k(j, r) } }}
				[\trace$_j$]]]]]
\end{forest}
...
% % Lexical rule
% \draw[brown] plot [smooth] coordinates {(c1.north) (-3.8,-3.6) (-3,-3.6) (-1.6, -3.6) (dsl0.south west)};
% \draw[blue]  plot [smooth] coordinates {(dsl0.north) (-1,-1.5) (1,-1.2) (dsl1.north west)};
% % DSL percolates down from clause to trace.
% \draw[red]   plot [smooth] coordinates {(dsl1.east) (6,-1.0) (6.5,-2.5) (dsl2.east)};
% \draw[red]   plot [smooth] coordinates {(dsl2.east) (8,-3) (8.5,-4.5) (dsl3.east)};
% % LTOP is projected upwards from trace to complete verbal projection
% \draw[green] plot [smooth] coordinates {(ltop2.north east) (8.5,-3) (9,-4.5) (ltop1.north east)};
% \draw[green] plot [smooth] coordinates {(ltop3.north east) (6,-1.5) (6.5,-3) (ltop2.north east)};
% % LTOP von V1 übernommen aus COMPS
% \draw[blue] plot [smooth] coordinates {(ltop4.north east) (0,-1.1) (3,-1.4) (ltop3.north)};
% % LTOP is projected from V1 head to complete V1 clause
% \draw[green] plot [smooth] coordinates {(ltop4.135) (-3,.4) (ltop5.135)};
% \end{tikzpicture}
}
...
[V\ms{cont \ms{ltop  & \subnode{ltop5}{\ibox{1}} h2\\
	...
	[V\ms{cont  & \ms{ltop & \subnode{ltop4}{\ibox{1}} h2\\
		...
		[V\ms{cont \subnode{c1}{\ibox{3}} \onems{ltop \type{h1}\\
                                                         rels  \nliste{ h1:k(j, r) }\\
                                                         hcons  \eliste}}, tier=np,edge label={node[midway,right]{V1-LR}}
			[kennt$_j$]]]
	[\ibox{2} V\ms{dsl|cont \subnode{dsl1}{\ibox{3}}\\
...
Zum Schluss müssen noch Sätze wie (\mex{1}) ausgeschlossen werden:
\ea[*]{
...
wird und in syntaktische Strukturen eintritt, den \textsc{dsl}"=Wert \emph{none} haben muss. (\mex{1}) leistet das
Verlangte:
\ea
...
Di Beschränkung in (\ref{constraint-DSL}) unterscheidet sich von der von \citet[\page207]{Meurers2000b} angegebenen \ua
...
Alternative HPSG"=Ansätze werden ausführlich in \citew{Mueller2004b} und in \citew{Mueller2005c,Mueller2005d,MuellerGS} diskutiert.
