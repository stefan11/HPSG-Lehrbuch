...
Das\is{Konstituentenstellung|(} Deutsche ist eine Sprache mit relativ freier Konstituentenstellung.
...
\ex\label{ex-weil Aicke den Stock dem Affen gibt}
...
an beliebigen Positionen zwischen den Argumenten stehen. Für (\ref{ex-weil Aicke dem Affen den Stock gibt}) ergeben
...
S.\,\pageref{schema-bin-prel} und Schema~\ref{ha-schema-prel} auf S.\,\pageref{ha-schema-prel}).
...
Liste beliebiger Länge \iboxb{3}.\footnote{
  ...
  Vorteil, dass die Elemente geordnet sind und man somit Beschränkungen formulieren kann, die sich auf
  die Prominenz von Argumenten beziehen. Siehe \citew{MuellerBinding} zur Bindungstheorie und
  Kapitel~\ref{Kapitel-Kasus} zur Kasusvergabe. Vorstellbar wäre eine Argumentstrukturliste, deren
  Elemente in mengenwertige Valenzmerkmale gemappt werden. So etwas wurde bisher nicht vorgeschlagen.
%S.\,\pageref{page-obliquen-h} zur Obliqueness"=Hierarchie. % Nee, steht da nicht drin. \citep[\page ]{Mueller2004b}.
% wohl wegen ARG-ST
...
Wie\is{Linearisierung!-sregel|(} auf S.\,\pageref{abstraktion-linearisierung} bei der Diskussion der Regelschemata in
...
(\citealt*[Abschnitt~3.2]{GKPS85a}, \citealt[Kapitel~5]{Uszkoreit87a}). Grammatiken, die eine solche Trennung zwischen \emph{immediate dominance} und
...
die im Abschnitt~\ref{sec-psg} auf S.\,\pageref{ditrans-ps-regeln} diskutiert wurde. Will man
...
Kopf kommt zuerst:                                   & Beispiel: \emph{schläft Aicke}\\
%
\ms{
head-dtr & \ibox{1}\\
non-head-dtrs & \nliste{ \ibox{2} } \\
...
\end{tabular}
\item \begin{tabular}[t]{@{}ll@{}}
Kopf kommt zum Schluss:                            & Beispiel: \emph{Aicke schläft}\\
%
\ms{
head-dtr & \ibox{1}\\
non-head-dtrs & \nliste{ \ibox{2} } \\
...
\z
\texttt{collect-phon}\isrel{collect-phon} geht dabei durch die Liste der Töchter und sammelt analog zu
\texttt{collect-rels} auf S.\,\pageref{constraint-collect-rels} die \phonwe auf.
%
% \begin{itemize}
% \item \begin{tabular}[t]{@{}ll@{}}
...
in denen die Argumente immer nur auf einer Seite des Kopfes stehen. Abbildung~\ref{abb-DP-Zerstörung-der-Stadt-durch-die-Soldaten} zeigt die
DP"=Analyse von (\ref{ex-die-Zerstörung-der-Stadt-dds}).
%\ea
...
\end{forest}
\caption{DP-Analyse von \emph{die Zerstörung der Stadt durch die Soldaten}}\label{abb-DP-Zerstörung-der-Stadt-durch-die-Soldaten}
\end{figure}

\citet{MuellerHeadless} und \citet{MyPM2021a} und auch ich im Abschnitt~\ref{sec-dp-analyse} des
vorliegenden Buches argumentieren jedoch gegen DP-Analysen, weshalb ich
bei der in der HPSG auch ansonsten üblichen NP"=Analyse geblieben bin (siehe Abschnitt~\ref{Abschnitt-SPR-Merkmal} und \citealt{VanEyndeNominalStructures}
für einen Überblick).
...
Ich nehme also -- wie schon im Abschnitt~\ref{Abschnitt-SPR-Merkmal} dargelegt -- an, dass es für Determinatoren (allgemeiner Spezifikatoren) ein eigenes
...
\end{figure}
Die Kombination einer \nbar mit dem Determinator wird dann durch das folgende Schema lizenziert, das analog zum Kopf"=Komplement"=Schema
auf S.\,\pageref{schema-Kopf-Komplementschema-prel2} ist.
\begin{schema}[Spezifikator-Kopf-Schema]\is{Schema!Kopf"=Spezifikator"=}
\label{schema-spr-h}
...
Nominalphrase in (\ref{ex-die-Zerstörung-der-Stadt-dds}) kombiniert werden. Zur Analyse von
Nominalstrukturen siehe auch Abschnitt~\ref{Abschnitt-Valenzprinzip}.
...
Das Deutsche zählt typologisch zu den SOV"=Sprachen\is{SOV}\is{Verbletztsprache|(} \citep{Haider2020a}. Innerhalb der
Transformationsgrammatik\indexgb 
wird deshalb angenommen, dass die Stellung Subjekt Objekt Verb die zugrundeliegende Stellung ist \parencites{Bach62a}[\page34]{Bierwisch63a}{Reis74a}[Kapitel~1]{Thiersch78a}.\footnote{%
  Bierwisch schreibt die Annahme einer zugrundeliegenden Verbletztstellung \citet{Fourquet57a} zu. Eine Übersetzung des von Bierwisch zitierten
...
Ansonsten steht das Finitum in der linken Satzklammer.\footnote{
  ...
  Komplementierer mit Verbzweitsätzen kombiniert \citep[\page 196]{Diesing2004a}. Die Abfolge im
  Jiddischen wäre dann \emph{dass dem Affen hat Aicke gegeben den Stock}.
...
110]{Jacobs86a}, innerhalb der Lexical\indexlfg Functional Grammar \citep[14]{Berman96a-u} und innerhalb der HPSG
\label{Seite-V1-via-Lexikonregel}\parencites{KW91a}{Oliva92b}{Netter92}{Frank94}[Abschnitt~2.2.4.2]{Kiss95a}[Abschnitt~3.1.1]{Feldhaus97}[Abschnitte~4.1
und~5.1]{Meurers2000b}{Mueller2005c,MuellerGS}[Abschnitt~6.2.5]{Holler2005a-u}{Crysmann2022a}.

Die Annahme der Verbletztstellung\label{page-verbletzt} als Grundstellung wird durch folgende Beobachtungen
motiviert:\footnote{
  Zu den Punkten~1 und~2 siehe auch \citew[\page34--36]{Bierwisch63a}. Zum Punkt~3 siehe \citew[Abschnitt~2.3]{Netter92}.%
}
\begin{enumerate}
...
\item Verben, die aus einem Nomen durch \wis{Rückbildung} entstanden sind, können oft nicht
...
Die Beispiele wurden zuerst von \citet[\page 62]{Haider93a} veröffentlicht. Haider bezieht sich aber
auf ein Manuskript von \citet{Hoehle91b}, das jetzt in einem Buch mit allen Aufsätzen von Tilman
Höhle erschienen ist. Die Beispiele sind auf den Seiten 370--371.
}
\eal
\ex[]{
weil sie das Stück heute urauf"|führen
}
\ex[*]{
Sie urauf"|führen heute das Stück.
}
\ex[*]{
...
\item Man könnte argumentieren, dass die Rückbildungen speziell sind, weil es sich um morphologische
  Bildungen handelt, die allein aus diesem Grund zusammen realisiert werden müssen. \textcites[\page 63]{Haider93a}[\page 59--60]{Haider2010a},
\citet[\page 105]{Vikner2001a} und \citet{Fortmann2007a} haben
aber ähnliche Verben gefunden, die ebenfalls nicht trennbar sind: Verben mit zwei Partikeln. Zum
Beispiel besteht \emph{vorankündigen} aus der Kombination von \emph{an} und \emph{kündigen} und
einer weiteren Partikel \emph{vor}.
\ea 
\label{ex-dass-sie-es-vorankündigt}
dass sie es vor-an-kündigt
\z
Die folgenden Beispiele zeigen, dass die Verben nicht vorangestellt werden können. Es ist nicht
möglich, nur das Verb voranzustellen (\mex{1}a). Auch die Voranstellung des Verbs mit nur einer
Partikel (\mex{1}b) oder mit beiden Partikeln (\mex{1}c) ist ausgeschlossen:
\eal
\ex[*]{
...
\zl
Die Beispiele zeigen, dass die Partikelverben mit zwei Partikeln nur in der Verbletztstellung
möglich sind.

\item Genauso ist es unmöglich, Verben in die Initialposition zu stellen, wenn es im Satz die Phrase
  \emph{mehr als} mit Bezug auf das Verb gibt \parencites[Abschnitt~3.1]{Haider97c}[\page 732]{Meinunger2001a}: 
\eal
\ex[]{
...
%
Adjunkt und Verb können gemeinsam in der Letztposition stehen, aber das Verb kann nicht initial
platziert werden.
%\largerpage[2]
\largerpage

\item Verben in infiniten Nebensätzen und in durch eine Konjunktion eingeleiteten
finiten Nebensätzen stehen immer am Ende (von Ausklammerungen ins Nachfeld und Oberfeldumstellungen abgesehen):
\eal
...
und das Verb in Letztstellung.\setlength{\footexindent}{13.5pt}\footnote{%
...
bzw.\ \emph{Trace} oder \emph{Gap} genannt) ist in (\mex{1}) als \_$_i$ gekennzeichnet. 
...
\eas
Verbspur für \emph{kennt} (vorläufig):\\
\ms[word]{
phon & \phonliste{}\\
...
\zs
Dieser Lexikoneintrag unterscheidet sich vom normalen Verb \emph{kennt} nur in seinem
\phonw. \argst- und \sprw sind hier nicht angegeben. Dazu später mehr.


Die syntaktischen Aspekte der Analyse von (\ref{bsp-kennt-jemand-den-Roman}) sind in Abbildung~\vref{verb-movement-syn-simple}
dargestellt. Die Kombination der Spur mit \emph{den Roman} und \emph{jemand} folgt den Regeln und Prinzipien, die
bisher schon besprochen wurden. Es stellt sich aber sofort die Frage, wodurch das Verb \emph{kennt}
  ...
  [{V[\comps \eliste]}
    ...
    [{V[\comps \sliste{ \ibox{1} } ]}
      ...
      [{V[\comps \sliste{ \ibox{1}, \ibox{2} } ]}
        [\trace]]]]]
\end{forest}
\caption{\label{verb-movement-syn-simple}Vorläufige Analyse von \emph{Kennt jemand den Roman?}}
\end{figure}

Will man erfassen, dass sich das finite Verb in Initialstellung wie ein Komplementierer verhält
\citep{Hoehle97a}, so liegt es nahe, \emph{kennt} in
...
unterscheiden sich dann von Komplementierern lediglich dadurch, dass sie eine Projektion einer Verbspur
...
enthalten, die zu \emph{gibt} gehört, so würde man Wortfolgen wie (\mex{1}b) analysieren können: 
\eal
...
durch eine Koindizierung ausgedrückt (siehe (\ref{bsp-kennt-jemand-den-Roman})). In HPSG wird Identität von Information immer durch Strukturteilung
...
Merkmale bis auf das \phonm. Um die benötigte Strukturteilung vornehmen zu können, werden die
Merkmalstrukturen stärker strukturiert: Syntaktische und semantische Information wird unter dem
Pfad \textsc{local}\isfeat{loc} gebündelt. Die Datenstruktur in (\ref{geom-cat-cont}) auf S.\,\pageref{geom-cat-cont}
...
\noindent
Die Verbspur für \emph{kennt} hat dann die folgende Form, wobei \argst und \spr hier wieder
weggelassen wurden:

\eas
\label{le-verbspur-kennt-ohne-dsl}%
Verbspur für \emph{kennt} (vorläufig):\\\samepage
\ms[word]{
phon & \phonliste{}\\
...
im nächsten Kapitel beschäftigen werden.\footnote{% 
  ...
  besprochen werden, um eine lokale Bewegung handelt \citep[Abschnitt~3.1]{Frank94b}.

  Der Vorschlag, Information über die Verbspur als Teil der Kopf"|information nach oben zu reichen,
  stammt von \citet[\page 187]{Oliva92b}. Oliva gibt allerdings nur syntaktische Information weiter.%
}
...
\onems[word]{
phon  \phonliste{}\\[1mm]
loc   \ibox{1} \ms{ cat  & \ms{ head & \onems[verb]{ vform \type{fin}\\
                                                      ini   $-$\\
                                                      dsl   \ibox{1}}\\
%                                 spr   & \ibox{1}\\
                                 comps & \nliste{ \npnom\ind{2}, \npacc\ind{3} }\\
...
\zs
Durch die Teilung des \localwes mit dem \dslw in (\mex{0}) ist die Information
...
Regel\is{Lexikonregel!Verberststellung|(} lizenziert:\footnote{\label{fn-V1-Lexikonregel-oder-Schema}%
Im Abschnitt~\ref{Abschnitt-Lexikonregeln} wurde erklärt, dass man Lexikonregeln auch so formalisieren
...
\noindent
Das durch diese Regel lizenzierte Verb selegiert eine maximale Projektion der Verbspur (\comps \eliste),
...
(\textsc{initial}$-$) sein. Die Ausgabe ist ein Verb in Erststellung (\textsc{initial}+).\footnote{%
In früheren Auf"|lagen dieses Buches habe ich in der Valenzliste des Ausgabeverbs noch angegeben, dass das
Komplement eine Verbalprojektion sein muss (\head \type{verb}). Das ist aber nicht nötig,
denn der \dslw des selegierten Komplements enthält ja die Information darüber, dass die Spur den \dslw \ibox{1}
(finites Verb in Letztstellung) haben muss. Daraus ergibt sich für die Spur, dass sie ein
Verb sein muss, da ja in der Spur der \dslw und der \localw identifiziert werden.
Somit kommen als Komplemente des Verberstverbs nur Verbalprojektionen in Frage. Auch für die
nicht-zyklische Version der Verbspur, die in Abschnitt~\ref{Abschnitt-Verbspuren ohne Zyklen} diskutiert wird, muss die Wortart nicht
angegeben werden, denn der Kopf der selegierten Phrase muss einen \dslw vom Typ \type{local} haben
und somit kommt nur die Spur mit dem \headw \type{verb} in Frage.} 
...
[S
  [{V[\comps \sliste{ \ibox{1} }]} 
    [{V[\comps \ibox{2}]},tier=np,edge label={node[midway,right]{V1-R}} 
       [kennt$_j$] ] ]
    [{\ibox{1} V\feattab{\textsc{dsl|\ldots|comps} \ibox{2},\\
                 \comps \eliste}}
         [{\ibox{3} NP[\type{nom}]},tier=np [jemand] ]
         [{V\feattab{\textsc{dsl|\ldots|comps} \ibox{2},\\
                 \comps \sliste{ \ibox{3} }}}
...
die Regel eingesetzten Tochter (\,\ibox{1} in (\ref{lr-verb-movement})). Teil des \dslwes ist auch die Valenzinformation, die in
Abbildung~\ref{verb-movement-syn} dargestellt ist \iboxb{2}. Da \dsl ein Kopfmerkmal ist, ist der
\dslw der VP mit dem der Verbspur identisch, und der \locw der Verbspur mit dem \dslw identifiziert ist, ist auch die \comps"=Information des Verbs
...
dass der \textsc{index}"=Wert (die Ereignisvariable) und der \ltopw entlang der Kopf"|projektion im Baum nach
oben gereicht werden. Im letzten Projektionsschritt in Abbildung~\ref{verb-movement-syn}
...
\noindent
Bei der Kombination des Verbs in Erststellung mit der Projektion der Verbspur werden der \ltopw und
der Index von der Verbspurprojektion übernommen und wegen des Semantikprinzips dann weiter nach oben
gereicht und so zum Beitrag der gesamten Konstruktion. Abbildung~\ref{verb-movement-sem} zeigt die
...
% Lexical rule
\draw[brown] plot [smooth,mark=none,mark indices={2,3,4}] coordinates {(c1.north) (-3.3,-4) (-2.8,-3.7) (-1.6, -3.7) (dsl0.south west)};
% [3] links zu [3] rechts
...
Kopfmerkmalen entlang des Kopf"|pfades weitergegeben, so dass er auch an der Verbspur vorhanden
...
h1. Nach Semantikprinzip wird der \ltopw den Kopf"|pfad entlang nach oben gegeben und das V1"=Verb
übernimmt den \ltopw von seinem Komplement. Da das V1"=Verb der Kopf des gesamten Satzes ist, wird
...
\relsl: einmal vom Verb in Erststellung und einmal von der Verbspur in Letztstellung.\footnote{
  Das wird im Abschnitt~\ref{Abschnitt MRS und Fernabhängigkeiten} revidiert. Für die Analyse von
...
Satzes (\mex{1}) zeigt Abbildung~\ref{verb-movement-adjunkt-sem}.
\ea
			...
			[NP{[\textit{acc}]}%,fit=band
				[den Roman, roof]]
                        [V\ms{dsl|cont \subnode{dsl3}{\ibox{3}},\\
                              cont \ms{ltop & \subnode{ltop1}{\ibox{1}} h2\\
...
vorkommen würde \iboxb{3}, wird via \dsl mit dem \contw der Spur geteilt. Der \hconsw für die
...
\relation{kennen}. Die \relation{nicht}"=Relation und die \relation{kennen}"=Relation werden in \rels nach
...
Zum Schluss müssen noch Abfolgen wie (\mex{1}) ausgeschlossen werden:
\ea[*]{
...
(\mex{0}) muss durch eine Beschränkung ausgeschlossen werden, die besagt, dass ein Verb, wenn es overt realisiert
wird und in syntaktische Strukturen eintritt, den \textsc{dsl}"=Wert \emph{none} haben muss
\citep[207]{Meurers2000b}.\footnote{%
Bei Meurers ist \dsl Teil des \textsc{marking}-Wertes, also Teil von \textsc{cat}, aber nicht Teil
von \head. Auch gilt Meurers' Implikation nur für finite Verben, weil er davon ausgeht, dass
Kopfbewegung nur finite Verben betrifft. Die scheinbar mehrfache Vorfeldbesetzung ist jedoch auch
bei infiniten Verben möglich (siehe Abschnitt~\ref{Abschnitt-Verbspuren ohne Zyklen}).
} (\mex{1}) leistet das Verlangte:
\ea
\label{constraint-DSL}%
\ms{ head-dtr & \ms[word]{ phon & non-empty-list \\
                         }
} \impl \onems{ loc$|$cat$|$head$|$dsl \type{none} }
\z
...
kombiniert wird. Da die Kopfmerkmale entlang des Kopf"|pfades geteilt werden, hat auch \emph{kennt}
den \dslw \type{none}. Im Lexikon kann es aber einen unspezifizierten \dslw haben, so dass es als
Tochter für die V1-Regel fungieren kann.
\begin{figure}
\begin{forest}
sm edges
[{V[\head \ibox{1} ]}
...
\eas
\label{le-verbspur}%
Allgemeine Kopfspur nach \citet[207]{Meurers2000b}:\is{Spur!Verb-}\\
\ms{
phon & \phonliste{}\\
...
\zs
Dies mag auf den ersten Blick verwundern, doch wenn man sich das Zusammenspiel der V1-Regel
...
\item Diese Form des Verbs steht in Initialstellung und verlangt als Argument eine vollständige Projektion
      einer Verbspur.
...
die Analyse von Verbzweitsätzen erklärt. Vorher sollen aber im nun folgenden
Abschnitt~\ref{sec-Anhang-Verbbewegung} noch Probleme des Ansatzes mit
koordinierten Verben und alternative HPSG"=Ansätze zur Konstituentenstellung
und die Behandlung der Konstituentenstellung in anderen Theorien diskutiert werden. Der
Abschnitt~\ref{Abschnitt-Beschriebungsmittel-HPSG} fasst dann alle bisher eingeführten Bestandteile der Theorie zusammen.
\largerpage


\section{Anhang}
\label{sec-Anhang-Verbbewegung}


Die Analyse mit der zyklischen Verbspur ist sehr elegant und mein Favorit. Es gibt jedoch ein
Problem mit Koordinationsdaten. Koordination ist ein komplexes Phänomen und es könnte sein, dass man
selbst Fälle, die einfach zu sein scheinen, anders lösen muss. Ich möchte mögliche Lösungen für die
Koordinationsprobleme deshalb hier in einem Anhang diskutieren. Dieser Abschnitt setzt die Lektüre
von Kapitel~\ref{Kapitel-nla} und Abschnitt~\ref{sec-pvp} voraus und ist nur für fortgeschrittene Leser*innen geeignet.

In früheren Publikationen zum Thema Satzstellung habe ich darauf hingewiesen, dass in der linken
Satzklammer auch wie in (\mex{1}) koordinierte Verben stehen können.
\ea
Kirby kennt und liebt diese Band.
\z
Das bedeutet aber, dass die Regel für die Ableitung solcher Voranstellungen keine Lexikonregel,
sondern eine unäre syntaktische Regel sein muss (siehe auch
Fn.\,\ref{fn-V1-Lexikonregel-oder-Schema} auf S.\,\pageref{fn-V1-Lexikonregel-oder-Schema}). Gerade
die Behandlung der Koordination führt nun aber zu einem Problem mit dem Zyklus in der
Verbspur. Dieses werde ich in Abschnitt~\ref{sec-Verbbewegung mit zyklischen Strukturen} erklären. Es scheint
mehrere Lösungen für das Problem zu geben, diese sind aber alle mit der Behandlung eines anderen
komplexen Phänomens inkompatibel: der scheinbar mehrfachen Vorfeldbesetzung.


\subsection{Probleme mit Koordination und zyklischen Strukturen}
\label{sec-Verbbewegung mit zyklischen Strukturen}

Die Kopfspur in (\ref{le-verbspur}) ist sehr elegant und erfasst direkt, dass Verbbewegung nur
innerhalb der Kopfdomäne stattfinden kann. Im Gegensatz zu Voranstellungen von Konstituenten ins Vorfeld und Extrapositionen ist
die Verbumstellung nur in einem eingeschränkten Bereich möglich.
\largerpage
...
(\ref{constraint-DSL}) erfasst alle Vorkommen von Verben in Kopf"|positionen. In
Koordinationsstrukturen sind die Verben bzw.\ ihre Projektionen aber nicht in Kopf"|position. Das
...
\ex[*]{
Kennt$_j$ [der Affe [[lacht] und [den Stock \trace$_j$]]].
}
\ex[*]{
Gibt$_j$ [der Affe [[lacht] und [dem Kind den Stock \trace$_j$]]].
}
\zl
Die Struktur in (\mex{0}b) entsteht dadurch, dass eine Verbspur für \emph{gibt} (\trace$_j$) mit
zwei Argumenten (\emph{dem Kind} und \emph{den Stock}) kombiniert
wird, so dass nur noch ein Subjekt gebraucht wird. Dann wird die Projektion \emph{dem Kind den Stock
  \trace$_j$} mit dem intransitiven Verb \emph{lacht} koordiniert. Dabei wird der \dslw des
Verbletztverbs \emph{lacht} mit dem der Verbspur identifiziert. Es entsteht also ein \emph{lacht} mit einem
\dslw, der später mit dem \localw von \emph{gibt} identifiziert wird. Das sind unsinnige und unerwünschte
...
Verbalprojektionen für die Analyse von Sätzen wie (\mex{1}) hilfreich sein \parencites[Abschnitt~2]{Hoehle91}[6]{Frank94}:
\ea
\label{ex-Koordination-V1-als-ATB}
...
\head und muss deshalb in beiden Konjunkten identisch sein, da die \catwe der Konjunkte geteilt
werden), ist auch die Valenzliste des Verbs in beiden Konjunkten identisch. Das bedeutet, dass (\mex{0})
...
semantisch kompatiblem Material bestünden, wäre die Analyse letztendlich falsch, da die
\textsc{index}- und \ltopwe der NPen identifiziert würden und somit die sich ergebende MRS nicht wohlgeformt wäre.\footnote{%
  Ein ähnliches Argument findet sich bei \citet[75]{Kathol2000a}. Kathol nimmt an, dass das
  Hilfsverb wie in (i.a) jeweils Teil des Verbalkomplexes in den Konjunkten ist. Da im ersten
  Konjunkt zwei Argumente und im zweiten drei angezogen werden müssen, entstünde ein Widerspruch.
\eal
\ex Peter hat$_i$ [das Buch gekauft \_$_i$] und [es dann seiner Schwester geliehen \_$_i$].
\ex Peter hat$_i$ [[das Buch gekauft] und [es dann seiner Schwester geliehen] \_$_i$].
\zl
Ich würde dagegen annehmen, dass der \lexw einer Koordination unbestimmt ist, so dass die Struktur
die Form in (i.b) haben kann. Es gibt nur eine Verbspur, die nur das Subjekt der Koordination
anzieht. Da das \lexm nur dazu da ist, unechte Mehrdeutigkeiten auszuschließen und da es wegen der
Koordination keine Gefahr für unechte Mehrdeutigkeiten gibt, sind solche Strukturen problemlos
analysierbar. Das Grundproblem mit viel einfacheren Sätzen wie (\ref{ex-Koordination-V1-als-ATB}) bleibt allerdings bestehen.
} Sätze wie
(\mex{0}) muss man also anders analysieren. Siehe \citew{Crysmann2003c} und \citew{BS2004a} zu Vorschlägen für
...
identifiziert werden \citep[\page 202]{ps2}. Wie Abbildung~\ref{Abbildung Kennt und
  liebt Aicke diese Band} zeigt, gelangt auf diese Weise der \localw von \emph{kennt und liebt}
\iboxb{2} innerhalb der \catwe der beiden Konjunkte (jeweils \ibox{3}\,) zu den Verben \emph{kennt} und \emph{liebt}. 
...
\dslw hat. Mit solch einer Beschränkung hätte man die falsche Koordination in der rechten
Satzklammer in (\ref{ex-Probleme bei zyklischen Verbspuren mit Koordination}) und (\ref{ex-Schläft und schnarcht Aicke schläft und schnarcht}) ausschließen können,
...
beibehalten will, auf die zyklische Verbspur verzichten muss. Ohne die Zyklen bekäme man auch die
unplausible Verteilung von \dslwen aus Abbildung~\ref{Abbildung Kennt und liebt Aicke diese Band}
nicht. Im folgenden Abschnitt diskutiere ich eine Spur ohne Zyklen.

\subsection{Verbspuren ohne Zyklen und scheinbar mehrfache Vorfeldbesetzung}
\label{Abschnitt-Verbspuren ohne Zyklen}

\citet[57]{Crysmann2022a} hat eine Verbspur vorgeschlagen, die im hier benutzten Format wie folgt
aussehen würde:
\eas
\label{le-Verbspur-nicht-zyklisch}%
Nichtzyklische Verbspur nach \citet[57]{Crysmann2022a}:\\
\ms{
phon & \eliste\\
...
\zs
Im Gegensatz zur allgemeinen Kopfspur in (\ref{le-verbspur}) ist die Information über die Wortart (\type{verb})
und Stellung (\textsc{ini}$-$) direkt festgelegt. Valenzinformation und semantische Information ist
direkt geteilt. Diese Spur kann statt der zyklischen Spur verwendet werden und in den
Computerimplementationen zu diesem Buch verwende ich sie auch. Die Analyse mit der zyklusfreien Spur
hat aber einen Makel: Die Wortart der Spur in (\ref{le-Verbspur-nicht-zyklisch}) und
auch der \vformw, den Crysmann nicht angegeben hat, ist im Prinzip egal. Das liegt daran, dass der \dslw der Spur zwar Information über
die Wortart und die dazugehörigen Merkmale enthält, diese jedoch in der Spur nicht geteilt
werden. Das führt nur deshalb nicht zu Problemen, weil die Verbspur immer phonologisch leer ist, und
phonologisch leere Verben mit \vform \type{fin} und \vform \type{bse} und \vform \type{inf} sehen
gleich aus: Es ist nichts zu sehen.


Man könnte annehmen, dass man dieses Problem einfach aus der Welt schaffen kann, indem man den
\vformw auf \type{fin} festlegt. Die Spur würde dann auch für alle bisher besprochenen Fälle von
Verbbewegung funktionieren. Eine wesentliche Motivation für die Verbbewegungsanalyse sind jedoch Daten mit scheinbar
mehrfacher Vorfeldbesetzung\is{Vorfeldbesetzung!scheinbar mehrfache} wie die in (\mex{1}):
\eal
\ex {}[Zum zweiten Mal] [die Weltmeisterschaft] errang Clark 1965 \ldots\footnote{%
        Der deutsche Straßenverkehr, 1968, Heft 6, S.\,210, zitiert nach \citet*[\page 224]{Neumann69a-u}.
%\citep*[\page 162]{Benes71}
      }\label{bsp-zum-zweiten-mal-die-Weltmeisterschaft}

\ex
\label{bsp-zum-zweiten-mal-die-Weltmeisterschaft-hat}
{}[Zum zweiten Mal] [die Weltmeisterschaft] hat Clark 1965 errungen.
\zl
Die einzige sinnvolle Analyse für solche Daten scheint mir eine mit einem leeren Kopf zu sein: eine mit
Verbbewegungsspur. Ich kann hier nicht die komplette Analyse motivieren und erklären. Dafür gibt es
inzwischen ein eigenes (fast fertiges) Buch \citep{MuellerGS}. Wichtig für die Diskussion hier ist,
dass die vorangestellten Konstituenten vom selben Verb abhängen müssen. Normalerweise ist das ein
finites Verb wie in (\ref{bsp-zum-zweiten-mal-die-Weltmeisterschaft}), aber es ist auch möglich,
dass das Verb wie in (\ref{bsp-zum-zweiten-mal-die-Weltmeisterschaft-hat}) infinit ist.\footnote{%
  Das Beispiel ist konstruiert, weil hier ein Minimalpaar benötigt wird. Siehe aber
  (\ref{bsp-alle-traeume-gleichzeitig}) auf S.\,\pageref{bsp-alle-traeume-gleichzeitig} für ein
  Korpusbeispiel mit nicht-finitem Verb.
} In der
nicht-zyklischen Spur werden aber weder Wortart noch Verbform mit den Werten in \dsl geteilt. Wie
ich oben schon festgestellt habe, ist das für die Abdeckung der Grammatik kein Problem. Ob die Spur
nun finit oder infinit ist, ist egal, da Spuren ja keine Form haben. Allerdings muss man wohl
einräumen, dass eine Spur mit \vform \type{bse}, die in Relation zu einem Element mit \vformw
\type{fin} steht, unbefriedigend ist. Folgt man den Grundannahmen, die ich in Abschnitt~\ref{sec-modelle-theorien} erklärt
habe, müssen alle Merkmale einen maximal spezifischen Wert haben. Es ist also keine Option, den
\vformw unspezifiziert zu lassen, denn das würde bedeuten, dass es in den Modellen vier
Möglichkeiten für die Auf"|lösung des \vformwes gäbe: \type{fin}, \type{bse}, \type{ppp} und
\type{inf}. In der Analyse von (\mex{0}a), die in Abbildung~\ref{abb-merhvf1} zu sehen ist, steht
die komplexe Konstituente im Vorfeld \iboxb{5} zu einer Extraktionsspur im Mittelfeld in
Beziehung. Im Vorfeld werden die beiden Konstituenten mit einer Verbspur kombiniert. Diese Verbspur
muss Information mit dem zugehörigen Verb teilen. Im konkreten Beispiel ist das \emph{errang}. Die
\vform des Verbs ist finit. Wenn es eine solche Teilung gibt, ist die Spur im Vorfeld ausreichend instantiiert.
Parallel ist die Analyse von (\ref{bsp-zum-zweiten-mal-die-Weltmeisterschaft-hat}) in
Abbildung~\ref{anal4-vlast}. Hier wäre die \vform der Vorfeldspur \type{ppp}, wenn eine entsprechende Teilung vorläge.
...
[V{[\comps \eliste]}
	[V\ibox{5}\,{[\dsl \ibox{7}, \comps \sliste{ \ibox{1} }]}
		[PP
			[zum zweiten Mal, roof]]
		[V{[\dsl \ibox{7}, \comps \sliste{ \ibox{1} }]}
			[\ibox{2} NP{[\textit{acc}]}
				[die Weltmeisterschaft, roof]]
...
\caption{\label{abb-merhvf1}Analyse von Mehrfachvoranstellung mit leerem Kopf}
\end{figure}
	...
	[V\feattab{\comps \eliste,\\
		...
		[V{[\comps \sliste{ \ibox{6}}]}
			[V{[\comps \ibox{3} $\oplus$ \sliste{ \ibox{8} }]}, tier=np,edge label={node[midway,right]{V1-LR}}
				[hat]]]
		[\ibox{6}\,V\feattab{\comps \eliste,\\ 
                                     \textsc{slash} \sliste{ \ibox{5} }}, l sep+=2ex
			[\ibox{1} NP{[\textit{nom}]}, tier=np
				[Clark]]
			[V\feattab{\comps \ibox{3},\\
                                   \textsc{slash} \sliste{ \ibox{5} }}
...
\caption{\label{anal4-vlast}Analyse von \emph{Zum zweiten Mal die Weltmeisterschaft hat Clark 1965 errungen.}}
\end{figure}

Man kann das Problem entschärfen, indem man auch die Kopfmerkmale einzeln teilt. (\mex{1}) auf
S.\,\pageref{le-Verbspur-nicht-zyklisch-mit-VFORM-Teilung} zeigt die entsprechend angepasste Spur.
\begin{figure}
\ea
\label{le-Verbspur-nicht-zyklisch-mit-VFORM-Teilung}%
Nichtzyklische Verbspur mit expliziter Teilung von Kopfmerkmalen:\\
\ms{
phon & \eliste\\
loc & \ms{ cat & \ms{ head & \onems[verb]{
                             vform \ibox{1}\\
                             ini   \ibox{2}\\
                             dsl   \ms{ cat & \ms{ head & \ms{ vform & \ibox{1}\\
                                                               ini   & \ibox{2} }\\
                                                   spr   & \ibox{3}\\ 
                                                   comps & \ibox{4} }\\
                                        cont & \ibox{5} }}\\
...
\end{figure}
Die Wortart selbst kann man nicht teilen, denn die ist der Typ der Struktur unter \head. Wollte man
sie teilen, müsste man den gesamten \headw teilen und hätte deshalb wieder eine Zyklus. 

Eine Möglichkeit, denselben Effekt zu bekommen, besteht darin, diese einzelnen Teilungen in der V1"=Regel
vorzunehmen. Statt den \localw des Eingabeverbs \iboxb{1} mit dem \dslw des selegierten Arguments des
Verberstverbs zu teilen, werden nur \vform, Valenz und semantischer Beitrag geteilt. Damit entsteht
kein Zyklus im Tochter"=Verb und alle Verbletztverben können \dsl \type{none} sein. Das Schönheitsproblem mit
der Verteilung des \dslwes in die Töchter einer Koordination aus Abbildung~\ref{Abbildung Kennt und
  liebt Aicke diese Band} bleibt allerdings bestehen und die Notwendigkeit von fünf Teilungen statt
einer weist auf ein konzeptuelles Problem hin.

Im Folgenden bespreche ich weitere Vorschläge für Verbspuren, die in der Literatur gemacht wurden.


\subsection{Verbbewegung über den \texorpdfstring{\textsc{nonloc}}{NONLOC}"=Mechanismus}

Kapitel~\ref{Kapitel-nla} beschäftigt sich mit so genannten nichtlokalen Abhängigkeiten. Diese
werden in der HPSG über die Weitergabe von Information über fehlende Elemente in Bäumen
analysiert. Die ursprüngliche Idee wurde von \citet{Gazdar81a} im Rahmen der GPSG\indexgpsg entwickelt. Unter dem Pfad
\textsc{nonlocal|slash} gibt es eine Liste mit Elementen, die in einer Struktur fehlen. Information
über fehlende Elemente wird von den Töchtern zum Mutterknoten nach oben gereicht, bis diese
Information -- \zB durch ein Element im Vorfeld -- abgebunden wird. In den beiden vorigen
Abbildungen war das schon zu sehen. \citet[Abschnitt~4.7]{KW91a} und \textcites[Abschnitt~2.2.4.2]{Kiss95a}[Abschnitt~5.4.2]{Kiss95b} haben nun vorgeschlagen, auch
die Verbbewegung mittels eines unter \nonlocal angesiedelten Merkmals zu analysieren.
% Holler2005a-u:6.2.5.

\eas
\label{le-verbspur-DSL-in-NONLOC}%
Verbspur mit \dsl in \nonloc nach \textcites[72]{Kiss95a}[230]{Kiss95b}:\is{Spur!Verb-}\\
\ms{
phon & \phonliste{}\\
loc  & \ibox{1} \\
nonloc & \upshape [\textsc{inher|dsl} \nliste{ \ibox{1} }]
}
\zs
% Seitenzahl stimmt nur, wenn die Formatierung dem Original entspricht
\citet[125]{Frank94b} hat darauf hingewiesen, dass Verbbewegung anders als \zB die Vorfeldbesetzung
nicht Satzgrenzen überschreiten kann. Umstellungen wie (\mex{1}) müssen ausgeschlossen werden:
\ea
Max$_j$ kennt$_i$ \trace$_j$ glaubt, dass Susanne Fritz \trace$_i$.
\z
(\mex{0}) wäre möglich, wenn die Information über die Verbspur \trace$_i$ über den \emph{dass}-Satz
hinaus nach oben gereicht würde. \emph{glaubt} würde als Verbletztverb mit seinem Subjekt \trace$_j$
und dem \emph{dass}-Satz kombiniert werden. Das Verb \emph{kennt} in Initialstellung würde mit einer
Verbprojektion, der das Verb \emph{kennt} fehlt, kombiniert. 

Verbspuren, die die zu teilende Information als Kopfmerkmal repräsentieren, machen hier die
richtigen Vorhersagen: Die Information über das umgestellte Verb kann genau so weit nach oben
gereicht werden, wie die Projektion der Kopfmerkmale geht. Bei Merkmalen unter \nonloc muss man
extra Barrieren einziehen. Das wäre aber möglich und auch nicht wirklich ungewöhnlich, denn auch die
Bewegung nach rechts ist begrenzt \citep[Abschnitt~5.1.2]{Ross67a}. Man könnte also das Problem mit (\mex{0}) einfach lösen, indem
man \emph{dass} einen Satz mit leerer \dsl-Liste selegieren lässt \citep[232]{Kiss95b}.\footnote{
Kiss verwirft die Alternative, \dsl vom Prinzip der nicht-lokalen Merkmale (siehe S.\,\pageref{Prinzip-NONLOC}) auszunehmen und nur
entlang von Kopf"|pfaden zu projizieren.
} Das muss man übrigens auch bei
einem Ansatz mit \dsl unter \head machen: In \emph{dass}-Sätzen müssen die Verben overt
sein:\footnote{
Hollers Beispiel \citeyearpar[231]{Holler2005a-u} in (i) wäre ohnehin nicht analysierbar, weil
man zwar ohne Beschränkungen im Komplementierer eine Verbspur mit \emph{mit einem Ball} und das
Ergebnis dann mit \emph{dass} kombinieren könnte, aber das Ergebnis wäre dann eine CP und das
Verberstverb \emph{spielt} braucht auch in Hollers Analyse (S.\,228) eine Projektion eines Verbs in Letztstellung.
\ea[*]{
Der Affe spielt dass mit einem Ball.
}
\z
}
\ea[*]{
Ich glaube, dass Kim den Roman \trace
}
\z

\noindent
Ist also alles gut? Ist dieser Ansatz also einer ohne zyklische Strukturen, in dem trotzdem alle
Information zwischen Spur und overtem Verb geteilt wird? Nein! Nichts ist gut, denn für die Analyse
der scheinbar mehrfachen Vorfeldbesetzung\is{Vorfeldbesetzung!scheinbar mehrfache}, die in den beiden vorigen Abbildungen dargestellt wurde,
wird die Information über die Verbspur im Vorfeld im Verbalkomplex in der rechten Satzklammer
benötigt. Das komplexe Vorfeld steht in einer Fernabhängigkeit zur Verbspur in der rechten
Satzklammer. Die Objekte, die in solchen Vorfeldabhängigkeiten geteilt werden, sind
\type{local}-Objekte. Das bedeutet, aber dass alles, was unter \nonlocal steht, nicht in
Fernabhängigkeiten geteilt werden kann. Im Vorfeld in Abbildung~\ref{abb-merhvf1} kann also die
Information der Verbspur über den \nonlocal-Mechanismus nach oben bis zur höchsten Projektion
gelangen, aber dort ist sie nicht Bestandteil von \local \iboxb{5} und also auch nicht Teil der
Extraktionsspur in der rechten Satzklammer. Das ist bei Analysen, die \dsl unter \head haben,
anders \citep[Fn.\,21]{Mueller2002f}.


\subsection{Objekte vom Typ \texorpdfstring{\type{sign} oder \type{synsem} im \textsc{nonloc}}{sign
    oder synsem im NONLOC}"=Mechanismus}

Das im vorigen Abschnitt dargestellt Problem besteht darin, dass die über \nonlocal weitergeleitete Information über die
Konstituente im Vorfeld die \dsl-Information nicht enthält. \citet[10]{HN94b} haben vorgeschlagen, in
\slasch nicht nur \type{local}"=Objekte zu teilen, sondern Objekte vom Typ \type{sign}. Diese würden
dann auch \dsl einschließen, wie Abbildung~\ref{fig-NONLOC-sign} zeigt.
...
[V
  ...
  [{[\nonloc|\inher|\slasch \nliste{ \ibox{1} }]}]]
\end{forest}
\caption{Analyse mit \slasch-Elementen vom Typ \type{sign}}\label{fig-NONLOC-sign}
\end{figure}

Das würde wohl funktionieren, allerdings ist ein Hauptbestandteil der Analyse der Voranstellung von
partiellen Verbphrasen (siehe Abschnitt~\ref{sec-pvp}), dass sich das \lexm nicht in \local, sondern unter \synsem befindet. Deshalb
wird es nicht zwischen Spur und Füller geteilt. Somit wird lokal im Verbalkomplex die Komplexbildung
erzwungen, aber gleichzeitig werden komplexe Phrasen als Füller im Vorfeld zugelassen. Der
Phrasenstatus wird einfach zwischen Vorfeld und Spur nicht geteilt. Das könnte man mit der obigen
Analyse nachbauen, indem man bei der Kopftochter nur die \local- und \nonlocal-Merkmale mit dem
Füller teilt und den \lexw ignoriert (siehe Abbildung~\ref{fig-NONLOC-sign-ohne-LEX}). Das wäre
jedoch eine unschöne Stipulation, da man ja die Merkmalsgeometrie genau so anlegen will, dass genau
die Information geteilt wird, die zusammen gruppiert ist.
\largerpage
\begin{figure}
\begin{forest}
[V
  [\ms{ loc & \ibox{1}\\
        nonloc & \ibox{2}\\
        lex    & \ldots } ]
  [\ms{\nonloc|\inher|\slasch \liste{ \ms{ loc & \ibox{1}\\
                                           nonloc & \ibox{2}} }}]]
\end{forest}
\caption{Analyse mit \slasch-Elementen vom Typ \type{sign} aber einzeln geteilten Werten in \type{sign}}\label{fig-NONLOC-sign-ohne-LEX}
\end{figure}



\subsection{Explizite Weitergaben der Verbspurinformation innerhalb von \texorpdfstring{\local}{LOCAL}}

Eine weitere Möglichkeit besteht darin, dass man \dsl nicht unter \head spezifiziert, um die Zyklen
zu vermeiden und nicht unter \cat, um die Probleme mit der Koordination zu vermeiden. Da man aber
wegen der scheinbar mehrfachen Vorfeldbesetzung\is{Vorfeldbesetzung!scheinbar mehrfache} \dsl innerhalb von \local haben will, müsste \dsl
also unter \local sein.

\eas
\label{le-verbspur-DSL-unter-LOCAL}%
Verbspur mit \dsl in \local:\is{Spur!Verb-}\\
\ms{
phon & \phonliste{}\\
loc  & \ms{ cat  & \ibox{1}\\
            cont & \ibox{2}\\
            dsl  & \ms{ cat & \ibox{1}\\
                        cont & \ibox{2} } } }
\zs
Ohne weiteres Zutun, würde \dsl aber unter \local sitzen und wäre an höheren Projektionen der Spur
nicht verfügbar. Mit \dsl als Kopfmerkmal wird es automatisch entlang des Kopf"|pfades nach oben
gereicht. Bei \dsl als \local-Merkmal müsste man zusätzlich festlegen, dass der \dsl-Wert der Mutter
immer mit dem \dsl-Wert der Kopf"|tochter identifiziert wird. Normalerweise sind aber alle Merkmale,
die entlang von Kopf"|pfaden projiziert werden, innerhalb von \head repräsentiert. Also auch hier
eine unschöne Stipulation.

Zusammenfassend kann man sagen, dass keine der Alternativen perfekt ist. Vielleicht kann man einfach
die Koordination anders behandeln. Dann könnte man bei der zyklischen Spur von
\citet[207]{Meurers2000b} bleiben, die die Verhältnisse meiner Meinung nach am besten erfasst. Von den
anderen Möglichkeiten ist die mit der nichtzyklischen Spur in (\ref{le-Verbspur-nicht-zyklisch}) die mit den geringsten
Auswirkungen auf die Gesamtarchitektur, weshalb ich diese auch in den Computerimplementationen für
die in diesem Buch besprochenen Fragmente verwendet habe.

\section{Zwischenzusammenfassung der bisher eingeführten Beschreibungsmittel}
\label{Abschnitt-Beschriebungsmittel-HPSG}
...
\begin{enumerate}
\item einer Signatur bestehend u.\,a.\ aus einer Typhierarchie, in der alle Typen mit ihren dazugehörigen
  Merkmalen und möglichen Werten angegeben sind. Die Signatur spannt gewissermaßen den Raum auf, in
  dem man eine Theorie formulieren kann.
...
des Deutschen in seine HPSG"=Analyse übernommen. Die Analyse des Satzes (\ref{ex-weil Aicke den
  Stock dem Affen gibt}) von S.\,\ref{ex-weil Aicke den Stock dem Affen gibt} zeigt
...
%% Mitunter wird mit Lernbarkeitsargumenten\is{Lernbarkeit} gegen flache Strukturen argumentiert \citep[Abschnitt~2.5]{Haegeman94a-u}.
...
%% im Abschnitt~\ref{sec-xbar} angegebenen \xbar-Regeln bzw.\ die HPSG"=Schemata ist die Anzahl der benötigten
...
der einzelnen Adjunkte beziehen muss. \citet{Kasper94a} hat gezeigt, wie man die Berechnung der
Semantik in einer solchen Analyse bewerkstelligen kann. Er verwendet komplexe relationale Beschränkungen, die alle
...
sich auf einfache Beschränkungen wie \emph{append}\isrel{append} beschränken, vorzuziehen.
...
mehrfache Vorfeldbesetzung\is{Vorfeldbesetzung!scheinbar mehrfache|(} \citep{Mueller2005d,MuellerGS} auf adäquate Weise zu analysieren. Beispiele für
solche problematischen Voranstellungen sind in (\mex{1}) aufgeführt (siehe auch S.\,\pageref{bsp-mehr-vf}):
\eal
\label{bsp-smvfb}
...
\ex
\label{bsp-alle-traeume-gleichzeitig}
...
Völlig unklar bleibt bei ganz flachen Analysen, wieso Verbpartikeln vor dem finiten Verb
stehen können sollen \citep[662--663]{MuellerGT-Eng5}, denn normalerweise stehen die in der rechten Satzklammer:
\eal
\label{ex-complex-vf}
\ex
{}[\sub{vf} [\sub{mf} Den Atem]  [\sub{vc} an]] hielt die ganze Judenheit.\footnote{%
Lion Feuchtwanger, \emph{Jud Süß}, S.\,276, zitiert aus \citew[\page 56]{Grubacic65a}.
}
\ex\label{bsp-wieder-an-tritt-vorn}
{}[\sub{vf} [\sub{mf} Wieder] [\sub{vc} an]] treten auch die beiden Sozialdemokraten.\footnote{%
  taz, bremen, 24.05.2004, S.\,21.
}
\ex\label{ex-los-damit}
{}[\sub{vf} [\sub{vc} Los]        [\sub{nf} damit]]    geht es schon   am 15. April.\footnote{%
        taz, 01.03.2002, S.\,8.%
    }%
\zl

\noindent
Bei einem Ansatz, der Verbspuren annimmt, kann man davon ausgehen, dass sich in (\mex{-1}) und (\mex{0}) eine Projektion
einer solchen Spur im Vorfeld befindet \citep[316]{Mueller2005d}, \dash, dass das gesamte Material
vor dem Finitum eine komplexe Konstituente bildet. Damit ist das Vorfeld eine eigene verbale Domäne
und es kann wieder ein Mittelfeld, eine rechte Satzklammer und ein Nachfeld geben, was wunderbar zu
den Beispielen in (\mex{0}) passt. Bei Linearisierungsansätzen gibt es dagegen keine einfache Möglichkeit, 
die Konstituenten im Vorfeld zu einer Konstituente zusammenzufassen.\footnote{%
Die Ansätze von \citet{Wetta2011a,Wetta2014a-u,Wetta2018a} werden im Abschnitt~\ref{sec-Wetta} diskutiert.
} Man
könnte natürlich -- wie in \citews[6]{Mueller2002f}[121]{Mueller2002c} vorgeschlagen 
-- einen leeren Kopf im Vorfeld annehmen, nur wäre dieser dann ein spezielles leeres Element,
das nirgends sonst in der Grammatik gebraucht würde
und nur zur Erfassung der scheinbar mehrfachen Vorfeldbesetzung stipuliert würde \citep[322]{Mueller2005d}.

Alternativ könnte man Spezialregeln formulieren, die das Material
im Vorfeld zu einer Konstituente kombinieren \citep[Abschnitt~7.6]{Welke2019a-u}. Dabei stellt sich natürlich die Frage nach
der syntaktischen Kategorie dieser Konstituente. Nähme man an, dass es sich um eine verbale
Projektion handelt, müsste man kopf"|lose Spezialregeln annehmen, da die Konstituenten vor dem finiten
Verb in (\mex{0}) keine Verben sind und demzufolge nicht der Kopf einer verbalen Projektion sein
können. Wetta hat entsprechende Regeln im Rahmen einer HPSG-Variante, der Sign-Based Construction Grammar\indexsbcg,
vorgeschlagen. Ich komme auf seine Ansätze in Abschnitt~\ref{sec-Wetta} zurück.
\is{Verzweigung!flache|)}\is{Vorfeldbesetzung!scheinbar mehrfache|)}
...
von sich selbst enthält. (\mex{1}) zeigt eine Beschreibung, die für alle Köpfe zutrifft:\footnote{%
Eine einfachere Repräsentation benutzt zyklische Strukturen: Das Domänenelement wäre dann identisch
mit der gesamten AVM.
}
\ea
\ms[word]{
phon   & \ibox{1} \\
...
In Kopf"=Komplement- und Kopf"=Adjunkt"=Strukturen werden Adjunkt- und Komplementtöchter in diese Liste eingesetzt 
und relativ zum Kopf angeordnet. Das sieht formal wie folgt aus:\footnote{
  In \citew[90]{Mueller2002b} gilt diese Beschränkung für Verbalkomplexe nicht. Es werden nicht die
  Nicht-Kopftöchter in die Domäne der Kopftochter eingesetzt, sondern die Domänenelemente
  der Nicht-Kopftochter werden mit denen der Kopftochter vereinigt.%
}
\ea
...
(weil) Aicke dem Affen den Stock gibt\ex\label{bsp-Aicke-den-Stock-dem-Affen-gibt}
(weil) Aicke den Stock dem Affen gibt
...
Abbildung~\ref{abb-linearisierung-kont} zeigt die Analyse von (\ref{bsp-Aicke-dem-Affen-den-Stock-gibt}). Die Elemente,
...
Abbildung~\ref{abb-linearisierung-diskont}. Im Gegensatz zu
...
%\noindent
Abbildung~\ref{abb-linearisierung-diskont-verb} zeigt schließlich die Analyse eines Satzes mit Verberststellung. Hier
...
Abbildung~\ref{abb-linear-surface} stellt die Analyse von (\mex{0}c) noch einmal anders dar. Die Konstituenten sind hier entsprechend
...
dass mehrere Konstituenten im Vorfeld eine gemeinsame Konstituente bilden. Siehe hierzu S.\,\pageref{bsp-smvfb}.
\largerpage
...
da das Kopf"=Komplement"=Schema auf S.\,\pageref{schema-Kopf-Komplementschema-prel2} die Kombination von Argumenten
...
\subsubsection{Linearisierungsdomänen und scheinbar mehrfache Vorfeldbesetzung}
\label{sec-Wetta}

\is{Vorfeldbesetzung!scheinbar mehrfache|(}
Im Abschnitt~\ref{sec-flache-strukturen} über ganz flache Strukturen habe ich bereits die scheinbar mehrfachen
Vorfeldbesetzungen erwähnt.\footnote{
Dieser Abschnitt basiert auf Material, das auch in \citew{MuellerGS} enthalten ist. Dieses Buch
hätte eigentlich schon längst veröffentlicht sein sollen. Aber verschiedene Projekte haben sich
immer wieder vorgedrängelt. So auch die Fertigstellung der vierten Auflage des HPSG-Lehrbuches.
} Ich habe darauf hingewiesen, dass solche Ansätze nicht erklären können, wieso
Verbpartikel und anderes Material gemeinsam im Vorfeld stehen, wenn sie davon ausgehen, dass dort
einfach mehrere Konstituenten stehen dürfen. In diesem Abschnitt möchte ich explizite Vorschläge von
\citet{Wetta2011a,Wetta2014a-u} diskutieren. Wetta arbeitet im Rahmen der linearisierungsbasierten HPSG
\citep{Reape94a,Kathol95a,Kathol2001a,Mueller99a,Mueller2002b}. \citet{Wetta2011a} nimmt eine
relationale Beschränkung an, die aus beliebig vielen Konstituenten eine neue Konstituente bildet:\footnote{%
  Ich habe zum ersten Domain"=Element der Mutter ein \textsc{dom}"=Merkmal hinzugefügt, das im
  Original fehlt. 
}
\begin{exe}
\ex Diskursprominenzkonstruktion für das Deutsche nach \citet[\page 264]{Wetta2011a}:
\begin{xlist}
...
\zl
% Wieso werden bei der Compaction noch mal die Elemente von L1 gescrambled? Ist das nicht eine Ebene
% zu tief? 25.10.2023
Die Liste L$_1$ in (\mex{0}) ist eine Liste von Elementen, die als prominent (\textsc{prom}+)
markiert werden. Aus diesen wird ein Element gebildet. Die Bildung neuer Domainelemente, die aus
Teilen der kombinierten Elemente bestehen, nennt man auch \emph{Compaction} (Kompaktion) \citep[175]{KP95a}. Das neu
erzeugte Element wird im Vorfeld angeordnet. Die Töchter, die nicht als prominent markiert werden,
sind in L$_2$ enthalten.%
% Könnte man machen, indem man Linearisierungsbeschränkungen sagen lässt, dass es nur ein
% PROM+-Element pro Domäne geben darf.
\footnote{
  Dass in L$_2$ nur Elemente mit \textsc{prom}"=Wert $-$ enthalten sind, ist in 
  (\mex{0}b) nicht vorgegeben. Das könnte man aber leicht als Beschränkung für L$_2$ in (\mex{0}b) einfügen.
}
Ihre \domwe werden mittels $\bigcirc$ mit dem prominenten Element kombiniert und können also
bezüglich des prominenten Elements beliebig angeordnet werden, solange keine
Linearisierungsbeschränkung verletzt ist. Abbildung~\ref{fig-v3-wetta2011} zeigt das Ergebnis der
Anwendung von (\mex{0}b).
% Das macht einfach keinen Sinn. Ich verstehe nicht, wieso er die DOMs einpackt und dann wieder
% auspackt. Er braucht das in keinem Fall. 03.06.2026
% \footnote{%
%   \citet[\page 259]{Wetta2011a} nimmt an, dass der \domw von \emph{die Weltmeisterschaft}
%   ein einzelnes Objekt ist. Das ist ungewöhnlich. Normalerweise wird angenommen, dass \emph{die
%     Weltmeisterschaft} zwei \domain"=Objekte hat: \emph{die} und \emph{Weltmeisterschaft}
%   \parencites[175]{KP95a}[81]{Kathol2001a}{Babel}[162]{Mueller99a}. Ohne Wettas ungewöhnliche Annahme würde die
%   Beschränkung \texttt{doms}$_\bigcirc$ nicht gebraucht und L$_1$ und L$_2$ könnten direkt für die
%   Berchnung des \domwes der prominenten Konstituente bzw.\ der Mutter benutzt werden.
% }
\begin{figure}
\oneline{%
\begin{forest}
...
\end{forest}}
\caption{Vn mit partieller Kompaktion nach \citet[\page 264]{Wetta2011a}}\label{fig-v3-wetta2011}
\end{figure}
Die Töchter der Konstruktion sind \emph{Clark}, \emph{die Weltmeisterschaft}, \emph{zum zweiten Mal}
und \emph{errang}. \emph{zum zweiten Mal} und \emph{die Weltmeisterschaft} werden als \textsc{prom}+
markiert und ihre \domwe werden mit $\bigcirc$ kombiniert und das Ergebnis ist der \domw des ersten
Elements in der \doml der Mutter. Es stellt sich natürlich die Frage, welche Eigenschaften das neu
erzeugte Objekt im Vorfeld hat. Das ist in Wettas Ansatz nicht spezifiziert. Er geht jedoch davon
aus, dass Domain-Objekte vom Typ \type{sign} sind. Das bedeutet, dass diese Objekte syntaktische und
semantische Eigenschaften haben müssen. Wie im Abschnitt~\ref{sec-modelle-theorien} erklärt, sind
HPSG"=Grammatiken Beschreibungen von Modellen. Beschreibungen sind partiell. Alles, was nicht
maximal spezifisch ist, kann im Rahmen der Typdefinitionen beliebig variieren \citep{King99a-u,Richter2024a}. Im
Abschnitt~\ref{sec-modelle-theorien} wurde das Nomen \emph{Frau} als Beispiel besprochen. Bei \emph{Frau} muss man keinen
Kasuswert angeben, denn \emph{Frau} ist mit allen vier Kasus des Deutschen kompatibel. Da der Typ
\type{case} als maximale Typen genau die vier Kasus des Deutschen hat, muss nichts weiter gesagt
werden. Wenn man das nun auf Wettas Ansatz anwendet, dann bedeutet das, dass seine Theorie unendlich
viele Modelle beschreibt, denn die syntaktischen Eigenschaften des Elements im Vorfeld sind nicht
spezifiziert. Da Valenzlisten Teil der syntaktischen Eigenschaften sind und da diese beliebig lang
sein können, gibt es unendlich viele Modelle. Wetta könnte das Problem leicht beheben, indem er
die Kategorie des vorangestellten Elements spezifiziert, also auch die entsprechenden Valenzlisten.
Normalerweise ist die Kategorie bei partieller Kompaktion die des Kopfes
\parencites[178]{KP95a}[101]{Kathol2001a}{Babel}[244]{Mueller99a}, aber bei scheinbar mehrfachen
Vorfeldbesetzungen ist keine der vorangestellten Konstituenten der Kopf des vorangestellten Materials.
Wetta könnte einfach festlegen, dass das erzeugte Objekt ein Verb von der Art ist, wie man es in der
Analyse mit leerem verbalen Kopf bekommt, aber dann hätte man relationale Beschränkungen, die aus
dem Nichts Struktur erzeugen, statt einen leeren verbalen Kopf anzunehmen, der einem sichtbaren Verb
entspricht. Die relationale Beschränkung wäre ein Ad-hoc-Instrument, etwas, das es nirgends sonst in
der Grammatik gibt.

\citet[Abschnitt~5.4.6]{Wetta2014a-u} verwirft die relationale Beschränkung, die mehrere
vorangestellte Elemente in eine Konstituente kompaktiert und formuliert ein Schema, das besagt, dass
beliebig viele Elemente vor dem Verb stehen können, wenn diese als prominent markiert sind. Alle
nicht prominenten Elemente müssen dem Verb folgen. Das löst das Problem mit den unbekannten
Eigenschaften der neu geschaffenen Vorfeld"=Konstituente, aber es kann nicht erklären, wieso sich
der präverbale Bereich wie eine eigene verbale Domäne mit Mittelfeld, rechter Satzklammer und
Nachfeld verhält (siehe Abschnitt~\ref{sec-flache-strukturen}).

Weder \citet{Wetta2011a} noch \citew{Wetta2014a-u} gehen auf Beispiele wie
(\ref{ex-der-maria-einen-ring}) ein und es wäre auch nicht trivial, solche Daten in seinen Ansatz zu
integrieren, denn für Fernabhängigkeiten nimmt er den \slasch"=Mechanismus an (siehe Abschnitt~\ref{sec-Vorfeldbesetzung}).
\eal
\label{ex-der-maria-einen-ring}
\ex {}[Der Maria] [einen Ring] glaube ich nicht, daß er je schenken wird.\footnote{
\citew[\page 67]{Fanselow93a}.
}
\ex {}[Kindern] [Bonbons] denke ich, daß  man besser nicht gibt.\footnote{
        G.\ \citew[\page 261]{GMueller98a}.
}
\zl

\noindent
\citet{Wetta2018a} hat dann eine Variante des Füller-Kopf-Schemas (siehe Schema~\ref{hf-schemaa} auf
S.\,\pageref{hf-schemaa})
vorgeschlagen, die mehrere Elemente in \slasch zulässt und diese dann in die Stellungsdomäne des
Kopfes einsetzt.\footnote{%
  Ich habe das Schema so angepasst, dass das C innerhalb der Liste steht. Wetta berücksichtigt
  nicht, dass Elemente aus L$_1$ ebenfalls etwas in ihren \textsc{gap}"=Listen haben können. Diese
  Information wird einfach ignoriert und es wird festgelegt, dass der \gapw der Mutter die leere
  Liste ist. Deshalb wird (i.a) mehrdeutig, denn wie (i.b) zeigt, kann ein Attribut zu \emph{Schwester}
  extrahiert werden.
\eal
\ex Der Marie einen Ring glaubt die Schwester nicht, dass er je schenken wird.
\ex Von Conny kenne ich nur die Schwester.
\zl
Die Information über die Extraktion des Attributs von \emph{Schwester} wird einfach weggeworfen.
}
\ea
...
hd-dtr & \upshape H: \ms{ syn \ms{ cat & Y: \ms{ vf & fin\\
                          ...
                          val & L$_1$ $\oplus$ \liste{ C: \ms[clause]{
                                                           syn & \ms{ gap & L$_2$\\
                                                                    } } }\\
                       }\\
}
...
\z
H ist dabei der Kopf des Matrixsatzes, C ist das Satzkomplement von H, L$_1$ ist die Liste der
Argumente von H und L$_2$ ist die Liste der \textsc{gap}"=Elemente, die aus dem eingebetteten Satz
stammen. \gap wird in SBCG statt \slasch verwendet. Die \dtrsl der kompletten Konstruktion besteht aus dem Ergebnis des Shuffelns einer Liste,
die den Kopf enthält mit L$_1$ und L$_2$ und einer Liste, die den Komplementsatz enthält. Das
bedeutet, dass Elemente aus L$_1$ vor oder nach Elementen aus den anderen Listen angeordnet werden
können, so lange die Reihenfolge der Elemente in L$_1$ nicht verändert wird.\footnote{%
  Das macht falsche Vorhersagen in Bezug auf die Anordnung der Argumente in L$_1$: Wie im
  Abschnitt~\ref{sec-mf} dargelegt, sind prinzipiell alle Abfolgen der Argumente eines Kopfes
  möglich. So sind auch beide Abfolgen von Subjekt und Objekt von \emph{gebeten} in (i) möglich.
\eal
\ex {}[Über dieses Thema]$_i$ hat noch niemand den Mann gebeten, [[einen Vortrag \_$_i$ zu halten].
\ex {}[Über dieses Thema]$_i$ hat den Mann noch niemand gebeten, [[einen Vortrag \_$_i$ zu halten].
\zl
  Dass das Schema in (\ref{schema-f-g-wetta}) nur eine dieser Anordnungen ableiten kann, ist es
  empirisch nicht adäquat. Dasselbe Problem gibt es mit mehreren Schemata in
  \citew{Wetta2014a-u}. So zum Beispiel mit dem Schema auf S.\,164: Die Elemente von L können
% Die DTRS werden geshuffelt. In den Domain-Objekten kann geschuffelt werden. Dadurch ändert sich
% aber nicht die Reihenfolge der Elemente in der Valenzliste.
  gescrambelt werden, aber das Schema lässt das nicht zu. Das Problem kann behoben werden, indem man
  eine weitere relationale Beschränkung annimmt, die eine Liste auf alle Listen abbbildet, die
  Permutationen der ursprünglichen Liste enthalten. Statt L$_1$ mit den anderen Listen in Schema 
  (\ref{schema-f-g-wetta}) zu kombinieren, würde man dann \texttt{permutations}(L$_1$) mit den
  Listen kombinieren. Die Annahme einer solch zusätzlichen relationalen Beschränkung, die nirgends
  sonst in der Grammatik gebraucht wird, würde zur Komplexität von Wettas Ansatz beitragen.
}
Für die Elemente von L$_2$ gilt das zur Stellung der Elemente von L$_1$ Gesagte analog.

Mit Wettas Schema kann man Sätze wie (\ref{ex-der-maria-einen-ring}) analysieren, aber es gibt keine
Erklärung dafür, warum die Elemente vor dem finiten Verb vom selben Kopf abhängen müssen. Dass dem
so ist, zeigen die Beispiele von \citet[\page 99]{Fanselow87a} in (\mex{1}):
\eal
\label{ex-mult-front-same-verb}
\ex[]{
Ich glaube  dem Linguisten nicht, einen Nobelpreis  gewonnen zu haben.
}
\ex[*]{
Dem Linguisten einen Nobelpreis  glaube  ich nicht gewonnen zu haben.
}
\zl
Die Dativ-NP \emph{dem Linguisten} hängt von \emph{glaube} ab und die Akkusativ-NP \emph{einen
  Nobelpreis} von \emph{gewonnen}. Die beiden Nominalphrasen können nicht gemeinsam im Vorfeld
stehen. Vielleicht ist es irgendwie möglich, auszuschließen, dass die entsprechenden Elemente in die
\slashl bzw.\ \textsc{gap}"=Liste gelangen. Trivial ist es jedenfalls nicht. 


\citew[\page 171]{Wetta2014a-u} erfasst diese Einschränkung, indem er Vn-Stellungen als lokale
Umstellungen analysiert. Da es in seinem 2014er Ansatz bei Vn-Stellungen keine Fernabhängigkeiten
gibt, folgt, dass die vorangestellten Elemente Satzgenossen sein müssen: Sie müssen allesamt vom
höchsten Verb abhängen. 
\citet{Wetta2018a} erklärt dann Vn nicht mehr als lokale Umstellung. Aber
sobald man Fernabhängigkeiten in Vn-Konstruktionen erlaubt, bekommt man wieder das Problem, dass die
vorangestellten Konstituenten von verschiedenen Köpfen abhängen können. Das Schema
(\ref{schema-f-g-wetta}) scheint so formuliert zu sein, dass die vorangestellten Elemente vom selben
Verb abhängen, weil sie aus demselben Teilsatz kommen müssen. Dem ist aber nicht so, denn sie können
innerhalb des Teilsatzes ja von verschiedenen Köpfen abhängen. Rui Chaves (p.\,M.\,2018) hat
vorgeschlagen, dass man das Problem beheben könnte, indem man informationsstrukturelle
Beschränkungen bezüglich des vorangestellten Materials formuliert. Er hat vorgeschlagen, alle
vorangestellten Elemente als [\textsc{prominent}+] zu markieren und dann festzulegen, dass alle
Elemente mit solch einer Markierung vom selben semantischen Prädikat abhängen müssen. 
% Auch diese
% Beschränkung wäre nicht ohne Weiteres zu formalisieren, denn Eine solche
% Beschränkung wäre jedoch ver
...
% \ea
% Peter denkt, Klaus kommt morgen.
% \z
...
% \ea[*]{
% Peter Klaus denkt, kommt morgen.
% }
% \z
...
%
Analysen mit Bezug auf informationsstrukturelle Aspekte, die davon ausgehen, dass verschiedene
einzelne Konstituenten vorangestellt werden, versagen aber bei Beispielen wie
(\ref{bsp-ihnen-für-heute}):
\ea\label{bsp-ihnen-für-heute}
{}[Ihnen] [für heute] [noch] [einen schönen Tag] wünscht Claudia Perez.\footnote{
  Claudia Perez, Länderreport, Deutschlandradio. Ich danke Arne Zeschel für dieses Beispiel.
}%
\z
Wie \citet*[117]{MBC2012a} festgestellt haben, geht es bei solchen Sätzen gerade nicht im die
Diskursfunktion der vorangestellten Konstituenten. Vielmehr geht es darum, das Mittelfeld zu
entleeren und den präsentativen Effekt dort zu verstärken. Damit scheiden auch Analysen wie die von
\citet[\page 482]{Speyer2008a} aus, die Bezug auf funktionale Projektionen wie
SceneP\is{Kategorie!funktionale!Scene}, FocP\is{Kategorie!funktionale!Foc} und
TopP\is{Kategorie!funktionale!Top} \parencites{Rizzi97a-u}{Grewendorf2002a} nehmen. Zu solchen
Projektionen siehe auch Abschnitt~\ref{sec-TopP-FocP-KontrP}. 

Sätze wie (\ref{ex-los-damit}) auf S.\,\pageref{ex-los-damit}, hier als (\mex{1}) wiederholt, sind
ebenfalls problematisch, weil die vorangestellten Elemente keinen besonderen
informationsstrukturellen Status haben. \emph{los} und \emph{damit} als \textsc{prom}+ zu markieren
und zu behaupten, dass alle \textsc{prom}+-Elemente vor dem finiten Verb angeordnet werden, ist
nicht angemessen. Vielmehr gibt es eine vorangestellte Phrase und deren Eigenschaften müssen
modelliert werden. Nimmt man keine Phrase an, sondern einzeln vorangestellte Elemente, gibt es
keinen einfachen Weg, die Zusammenhänge zu erfassen.
\ea
\label{ex-los-damit-zwei} 
[Los] [damit] geht es schon am 15. April.\footnote{
        taz, 01.03.2002, S.\,8.
    }
\z
Wie bereits in Abschnitt~\ref{sec-flache-strukturen} erwähnt, entspricht die Stellung der Elemente
vor dem finiten Verb der Stellung, die man sonst ohne Voranstellung erwarten würde. \emph{los} ist
die rechte Satzklammer und \emph{damit} ist extraponiert. Setzt man einfach \emph{los} und
\emph{damit} in die gleiche Stellungsdomäne wie das finite Verb ein, bekäme man eine rechte
Satzklammer (\emph{los}) und ein \nf (\emph{damit}) links von der linken Satzklammer (\emph{geht}).
Das würde gängigen Linearisierungsregeln (\zB, \citealt[50]{Kathol2001a}; siehe auch S.\,\pageref{page-Kathol-1-2-3-4}) widersprechen.\footnote{
  Es gibt eine technische Lösung für das Problem: Man könnte Linearisierungsregeln annehmen, die nur
  für Konstituenten mit dem gleichen \textsc{prom}"=Wert gelten. \textsc{prom}+ würde für das
  Vorfeld gelten, \textsc{prom}$-$ für den Rest des Satzes. Da wie eben dargelegt, das Vorfeld
  nichts mit Prominenz zu tun haben muss, könnte man das \textsc{prom}"=Merkmal in \textsc{vorfeld}
  umbenennen. (i) zeigt die entsprechenden Linearisierungsregeln: 
\eal
\ex Mittelfeld [\textsc{vorfeld}  \ibox{1}] $<$ rechte Satzklammer [\textsc{vorfeld}  \ibox{1}]
\ex rechte Satzklammer [\textsc{vorfeld}  \ibox{1}] $<$ Nachfeld [\textsc{vorfeld}  \ibox{1}]
\zl         
Siehe \citew*{KKP95} zu Linearisierungsregeln mit Strukturteilungen.
}

Ein\label{page-MVF-Idiome} anderes Problem, das \slasch-basierte Ansätze wie mein eigener aus dem Jahr 2000 haben
\citep[Abschnitt~2.8.3.2]{Mueller2000d}, besteht ebenfalls bei Wettas revidierter Analyse: Es ist
schwierig die Voranstellung von Idiomteilen zu beschränken \citep[307]{Mueller2005d}. In
(\mex{1}) sind zwei Idiombestandteile vorangestellt und in (\mex{2}) befindet sich der erste Bestandteil des
Idioms im Vorfeld. Wie (\mex{3}) zeigt, ist es jedoch nicht
möglich, den zweiten Idiomteil einzeln voranzustellen:

\eal
\label{ex-Mehrfachvoranstellung-Idiome}
\ex[]{\label{ex-oel-ins-feuer}
[Öl] [ins Feuer] goß gestern das Rote-Khmer-Radio\footnote{
        taz, 18.06.1997, S.\,8.
}
}
\ex[]{
...
Attacke der US-Regierung sprach.\footnote{
        taz, 25.04.2000, S.\,3. %taz Nr. 6126 vom 25.4.2000 Seite 3
    }
}
\ex[]{
[Ihr Fett] [weg] bekamen natürlich auch alte und neue Regierung [\ldots]\footnote{
...
\ex[]{
[Dem Zeitgeist] [Rechnung] tragen im unterfränkischen Raum die privaten, städtischen und kommunalen
    Musikschulen.\footnote{
        Fränkisches Volksblatt, zitiert nach Spiegel, 24/2002, S.\,234.
    }
}
\zl 
\eal
\ex[]{\label{ex-oel-goss-gestern}
[Öl] goß gestern das Rote-Khmer-Radio [ins Feuer].
}
\ex[]{
[Das Tüpfel] setze der Bürgermeister von Miami [aufs i], als er am Samstagmorgen von einer schändlichen 
Attacke der US-Regierung sprach.
}
\ex[]{
[Ihr Fett] bekamen natürlich auch alte und neue Regierung [weg] [\ldots].
}
\ex[]{
[Dem Zeitgeist] tragen im unterfränkischen Raum die privaten, städtischen und kommunalen
    Musikschulen [Rechnung].
}
\zl 
...
\ex[*]{
[Aufs i] setze der Bürgermeister von Miami das Tüpfel, als er am Samstagmorgen von einer schändlichen 
Attacke der US-Regierung sprach.
}
\ex[*]{
...
\ex[*]{
[Rechnung] tragen im unterfränkischen Raum die privaten, städtischen und kommunalen
Musikschulen [dem Zeitgeist].
}
\zl
Zählt man die Idiomteile zum Prädikatskomplex (siehe Kapitel~\ref{Kapitel-Verbalkomplex}),
wird das Stellungsverhalten auch mit der Generalisierung von \citet{Loetscher85a} erklärbar, dass Elemente vom
linken Rand des Prädikatskomplexes vorangestellt werden können, aber nicht aus der Mitte.

Das stellt für \slasch-basierte Ansätze ein Problem dar, denn diese gehen ja davon aus, dass in der
Analyse von (\ref{ex-Mehrfachvoranstellung-Idiome}) beide Konstituenten in \slasch sind. Man müsste also in der Lage sein,
festzulegen, dass der zweite Teil des Idioms nur dann extrahiert werden darf, wenn der erste auch
extrahiert wird. Das ist eventuell möglich, aber in jedem Fall technisch nicht trivial.
In einer Analyse mit leerem verbalen Kopf sind die Daten dagegen unproblematisch. Die Idiombestandteile
werden mit dem leeren verbalen Kopf kombiniert und stehen in der Reihenfolge, in der sie auch
stehen, wenn kein Idiombestandteil vorangestellt ist. (\mex{1}) ist das Gegenstück zu
(\ref{ex-oel-ins-feuer}): 
\ea
Das Rote-Khmer-Radio goß gestern [Öl] [ins Feuer].
\z

\noindent
Bisher haben wir keine Beispiele für satzübergreifende Voranstellung mehrerer Idiomteile gefunden,
aber man kann sie konstruieren. In (\mex{1}) ist das Idiom unter \emph{behaupten} eingebettet:
\ea 
{}[Öl] [ins Feuer] behauptete der Vorsitzende, dass gestern das Rote-Khmer-Radio gegossen habe.
\z
Für solche Beispiele müsste Wetta sein Schema in (\ref{schema-f-g-wetta}) anwenden. Mit all den
erläuterten Problemen der \slasch-basierten Ansätze.

% Das stimmt doch nicht. Wenn man Spuren hat, kann man doch die Dativsput vor der Akkusativsput
% anordnen.
% St. Mü. 02.06.2026
% Ein weiteres Problem für \slasch-basierte Ansätze ist, dass die Reihenfolge der Elemente vor dem
% finiten Verb für gewöhnlich der Reihenfolge entspricht, die diese bei unmarkierter Stellung im Mittelfeld haben würden 
% \parencites[\page412--413]{Eisenberg94a}[Abschnitt~2.10]{Mueller2005d}. Die Beispiele in (\mex{1})
% und (\mex{2}) sollen das verdeutlichen: (\mex{1}a) ist ein Beleg für mehrfache Vorfeldbesetzung und
% (\mex{1}b) die Variante mit Umstellung der Konstituenten innerhalb des Vorfelds. 
% \judgewidth{??}
% \eal
% \ex[]{\label{bsp-alle-traeume-gleichzeitig-zwei}
% {}[Alle Träume] [gleichzeitig] lassen sich nur selten verwirklichen.\footnote{
%         Broschüre der Berliner Sparkasse, 1/1999.
%         }
% }
...
% Eine Analyse ohne einen verbalen Kopf im Vorfeld kann nicht erklären, warum das nichtverbale
% Material im Vorfeld denselben Linearisierungsgesetzmäßigkeiten wie bei Nichtvoranstellung unterliegt.\footnote{
%   Siehe auch Abschnitt~\ref{sec-disc-entry} (insbesondere die Diskussion der Beispiele (\ref{bsp-partikel+anderes+material-topo-felder})
%   auf S.\,\pageref{bsp-partikel+anderes+material-topo-felder}) zu Verbpartikeln, die mit anderem Material zusammen
...
% sagen, dass Pronomina in der \slashl vor nichtpronominalen Elementen stehen müssen (siehe auch Abschnitt~\ref{sec-lex-intro-udc}
...
Im folgenden Abschnitt werden Ansätze mit variabler Verzweigung diskutiert. Auch für diesen Ansatz stellt die
scheinbar mehrfache Vorfeldbesetzung ein Problem dar.
\is{Vorfeldbesetzung!scheinbar mehrfache|)}
...
In \citew[Abschnitt~11.5.2]{Mueller99a} und \citew[Abschnitt~4]{Mueller2004b} habe ich darauf hingewiesen,
...
Auf diese Weise hat man zwar die Verarbeitungsprobleme beseitigt, die ein leerer verbaler Kopf mit sich
bringt, aber man hat auch keine Möglichkeit mehr, die scheinbar mehrfache
Vorfeldbesetzung\is{Vorfeldbesetzung!scheinbar mehrfache|(} mit Hilfe eines leeren verbalen Kopfes\is{leere Kategorie} zu erklären.

Crysmann hat in der Diskussion zu seinem Vortrag auf dem Workshop \emph{Large"=Scale Grammar Development and Grammar Engineering} 
2006 in Haifa vorgeschlagen, die Tatsache, dass die Elemente, die bei der scheinbar mehrfachen Vorfeldbesetzung
vorangestellt werden, vom selben Verb abhängen müssen (siehe Diskussion auf S.\,\pageref{ex-mult-front-same-verb}), dadurch zu erfassen, dass bei der Einführung
...
flach verzweigenden Schema gemeinsam oder aber eins nach dem anderen abgebunden werden. Probleme
\slasch-basierte Analysen wurden im vorigen Abschnitt ausführlich besprochen.
Crysmanns Vorschläge wurden nie ausgearbeitet und inzwischen hat er selbst eine Verbbewegungsanalyse
für deutsche Sätze vorgeschlagen \citep{Crysmann2022a}.
%
% Crysmann hat in der Diskussion zu seinem Vortrag auf dem Workshop \emph{Large"=Scale Grammar Development and Grammar Engineering} 
% 2006 in Haifa vorgeschlagen, die Tatsache, dass die Elemente, die bei der scheinbar mehrfachen Vorfeldbesetzung
...
%       Äußerung ungrammatisch würde bzw.\ die idiomatische Lesart verlöre. Die entsprechenden Daten
%       wurden bereits ab S.\,\pageref{page-MVF-Idiome} diskutiert.
% % \citep[Abschnitt~2.9]{Mueller2005d}.
% \end{enumerate}
...
%   Siehe \citew{Cipollone2001a,Levine2003a}, \citew[Abschnitt~3.6.1]{LH2006a}. Siehe aber auch \citew{Chaves2009a}.%
% }
...
% verbalen Kopf verbunden und Argumente werden ebenfalls unspektakulär über das Kopf"=Komplement"=Schema
...
In der \gbt und ihren Nachfolgern wird die Umstellung von Konstituenten mitunter mit Bewegung
modelliert \parencites[z.\,B.][Abschnitt~12.6]{SS88a}[1306]{Grewendorf95a}.
...
linguistischer Zeichen: In (\mex{1}a) haben die NP und das Verb die Eigenschaft, im Plural zu stehen, und
in (\mex{1}b) stehen sie Singular.
...
\label{sec-TopP-FocP-KontrP}

\mbox{}\citet[27]{Frey2004a}\is{Topik|(}\is{Fokus|(}\is{Informationsstruktur|(} nimmt eine KontrP\is{Kategorie!funktionale!Kontr} (Kontrastphrase) und 
%\citet[\page165]{Frey2000a-u} 
\citet[Abschnitt~5]{Frey2004b-u}
eine TopP\is{Kategorie!funktionale!Top} (Topikphrase) an (siehe auch \citew{Rizzi97a-u} für TopP und FocP\is{Kategorie!funktionale!Foc} (Fokusphrase) im Italienischen und 
...
%\if0
\subsubsection{Umstellung als Bewegung?}

Auf Seite~\pageref{schema-Kopf-Komplementschema-prel2} habe ich vorgeschlagen, Verben mit ihren Argumenten einfach in der Reihenfolge zu
kombinieren, wie sie im Satz auftauchen. Im Rahmen der Mainstream Generative Grammar (MGG), also
Transformationsgrammatik, Government \& Binding und Minimalismus,\footnote{
  \citet[\page 3]{CJ2005a} benutzen den Begriff, um die genannten Frameworks von anderen wie LFG,
  HPSG, Berkeley Construction Grammar und Simpler Syntax abzugrenzen, die sich zum Teil auch als
  generativ bezeichnen.
} wird oft angenommen, dass es eine
Grundreihenfolge gibt und dann alle anderen Stellungen aus dieser abgeleitet werden. Man nennt die
Grundstellung auch Transformationsbasis oder Tiefenstruktur und die dort vorhandene Abfolge
dann \emph{base order}. Aus der Tiefenstruktur können diverse andere Strukturen abgeleitet werden,
bis sich dann die so genannte Oberflächenstruktur bzw.\ surface structure oder S-structure ergibt. In diesem
Abschnitt werden einige Argumente dafür und dagegen besprochen, Oberflächenabfolgen aus
zugrundeliegenden Abfolgen abzuleiten. In früheren Auflagen gab es an
dieser Stelle nur den folgenden Abschnitt zu Skopus und Bewegung. Die Argumentation hat sich jedoch
als nicht stichhaltig herausgestellt. Ich habe den Abschnitt deshalb modifiziert und auch die weiteren Punkte aus
\citew{MuellerGenerativeGermanic} hier aufgeführt.

\subsubsubsection{Quantorenskopus}
\label{sec-Scrambling-Skopus}

%Außer\is{Skopus|(} dem Ziel, Eigenschaften von Objekten möglichst direkt zu modellieren, gibt es aber noch wichtigere
...
mehrerer umgestellter Phrasen in die Position der Spur entsprechen \citep[185]{Frey93a}.
\citet[\page 146]{Kiss2001a} und \citet[Abschnitt~2.6]{Fanselow2001a} haben darauf hingewiesen,
dass Ansätze, die Spuren annehmen, problematisch sind, denn sie sagen für Sätze, in denen es mehrere Spuren gibt, 
Lesarten voraus, die nicht wirklich vorhanden sind.
...
\ea
\label{ex-mindestens-einem-Verleger-fast-jedes-Gedicht}
...
Ich habe diese Daten also wie auch Kiss und Fanselow lange gegen bewegungsbasierte Ansätze verwendet, Korpusdaten legen jedoch nahe,
dass Skopus nicht von Baumkonfigurationen abhängt \citep{Webelhuth2022a}, weshalb Beispiele wie
(\ref{ex-mindestens-einem-Verleger-fast-jedes-Gedicht}) weder für noch gegen bewegungsbasierte
Ansätze Evidenz darstellen. Siehe auch
\citet{FanselowZimmermannPhilipp2022a-u} für Experimente und \citet{Salzmann2025a} für einen Überblick.%
%
\is{Skopus|)}
%\fi 


\subsubsubsection{Idiome}

\citet{Fanselow2012a} diskutiert Idiomdaten wie die in (\mex{1}):
\ea
Vielleicht hat er \emph{die Flinte} zu früh \emph{ins Korn geworfen}
\z
Das Idiom besteht aus den in (\mex{0}) kursiv gesetzten Teilen. Interessanterweise ist das Idiom
durch das Adjunkt \emph{zu früh} unterbrochen. \citet[\page 276]{Fanselow2012a} und
\citet{Salzmann2025a} sehen das als Evidenz für Bewegung, da sie davon ausgehen, dass nur
kontinuierliche Teile eines Baums eine idiomatische Interpretation haben können. Wenn die
Beschränkung, dass Idiome nur aus kontinuierlichen Teilbäumen bestehen können, wirklich gelten
würde, wären Beispiele mit idiomatischem Subjekt und nicht idiomatischem Objekt, wie sie Marga
\citet[\page178]{Reis82} diskutiert hat, problematisch, unabhängig davon, wie man die Umstellungen im
Mittelfeld analysieren würde:
\ea
weil \emph{die Spatzen} das \emph{vom Dach gepfiffen} haben
\z
Man kann aber Idiome einfach so analysieren, dass der Kopf die Idiomteile selegiert \citep{Sag2007a}. Dazu müssen
diese nicht unbedingt adjazent sein. Im obigen Beispiel selegiert \emph{geworfen} die PP \emph{ins
  Korn} und das Akkusativobjekt \emph{die Flinte}. Das ist unabhängig davon, ob es im Satz Adjunkte
gibt und wo diese angeordnet werden. Idiome unterscheiden sich hinsichtlich ihrer
syntaktischen Flexibilität und manche müssen nicht unbedingt aus adjazenten Teilen bestehen. Zu
Analysen verschiedener Arten von Idiomen siehe
\citew{Sailer2000a,Soehn2006a,RS2009a,SB2021a-u,Sailer2024a}.

Zusammenfassend kann man festhalten, dass Idiome keine Evidenz für eine zugrundeliegende Anordnung
sind, aus der Konstituenten umgestellt werden.
% Idioms interact in various ways with the rest of
% the grammar. For example, some idioms allow for passivization. If a lexical analysis of idioms would not be possible, this would imply that
% phenomena like passive would require movement-based analyses as well and examples like the ones in
% (\mex{1}) would have to be derived . 
% But Sag's lexical approach is
% compatible with lexical rule-based approaches to the passive described below. For more on idioms in HPSG see \citew{Sailer2024a}.



\subsubsubsection{Keine Grundstellung bei Verben mit Experiencer-Objekt}

\citet*{MaslochPoppekKiss2024a} haben Experimente durchgeführt, die zeigen, dass es für die
Argumente von Experiencer-Objekt-Verben mit Dativ-Objekt keine bevorzugte Anordnung gibt, wenn beide
Argumente belebt sind. Weder die SO- noch OS-Abfolge sollte als die Basisstellung gelten, aus der
die andere abgeleitet wäre (S.\,5, 20).\footnote{
\citet[\page 21]{MaslochPoppekKiss2024a} merken an, dass Ansätze, die von einer Grundreihenfolge
ausgehen, diese aber nicht dafür verwenden, etwas über Markiertheit von Anordnungen zu sagen,
sondern dafür nur die sichtbaren Anordnungen verwenden, ebenfalls kompatibel mit den Daten wären. 
Die Grundreihenfolge hätte dann nichts mehr mit der unmarkierten Stellung zu tun, sondern wäre nur
noch für die Zuweisung semantischer Rollen, Kasusvergabe und Bindung wichtig.%
} \citeauthor{MaslochPoppekKiss2024a} gehen von binär verzweigenden Strukturen aus, in denen jeweils
ein Argument mit einer verbalen Projektion kombiniert wird und die Argumente in der Reihenfolge
angeordnet sind, die auch sichtbar ist. Die Argumente werden in eine Linearisierungsdomäne
eingesetzt und Linearisierungsbeschränkungen werden dann auf die Elemente dieser Liste angewendet.
Siehe dazu Abschnitt~\ref{sec-Reape-Linearisierung} und \citew[\page 23]{MaslochPoppekKiss2024a}.
\largerpage

Interessanterweise setzen Analysen wie die von \citet{Kiss2001a},
\citet{Fanselow2001a}, \citet{Mueller2005c} und \citet{MaslochPoppekKiss2024a} angenommenen voraus,
dass alle Argumente in einer lokalen Domäne mit dem Verb realisiert werden, denn würde zum Beispiel
das Subjekt in einer Projektion eines anderen Kopfes (IP, TP oder AgrS) realisiert werden, wie zum Beispiel in
Abbildung~\ref{fig-Aicke-Conny-erwartet-ip} auf S.\,\pageref{fig-Aicke-Conny-erwartet-ip}, dann
müsste es auch Bewegung geben, denn Objekte können ja auch vor dem Subjekt stehen und das wäre dann
ein Bereich außerhalb der Verbalphrase. Siehe zum Beispiel \citew[\page 1304]{Grewendorf95a} für
eine Analyse mit IP und Bewegung eines Objekts.


\subsubsection{Freezing}
\is{Freezing@\emph{Freezing}|(}

In \gb nimmt man an, dass aus bewegten Phrasen nichts mehr extrahiert, \dash \zb ins Vorfeld
gestellt werden kann \citep[173]{Ross67a}[119]{WC80a-u}.
...
\eal
\label{ex-Gartenvereinsvorsitzenden}
...
Auch aus extraponierten, also ins Nachfeld verschobenen Phrasen, kann man Konstituenten
voranstellen, wie die Beispiele (\ref{bsp-um-zwei-millionen}) und (\ref{bsp-gegen-ihn}) von
S.\,\pageref{bsp-um-zwei-millionen} hier als (\mex{1}) vorweggenommen, zeigen.
\eal
\ex
{}[Um zwei Millionen Mark]$_i$ soll er versucht haben,\\{}[eine Versicherung \_$_i$ zu betrügen].\footnote{
         taz, 04.05.2001, S.\,20.
}
\ex {}[Gegen ihn]$_i$ falle es den Republikanern hingegen schwerer,\\ {}[\kern1pt [ Angriffe \_$_i$] zu lancieren].\footnote{
  taz, 08.02.2008, S.\,9.
}
\zl
Und aus ins Vorfeld gestellten Phrasen kann man extraponieren, wie die Beispiele in (\mex{1}) aus \citew[218--219]{Mueller99a}
zeigen.
\eal
\label{bsp-ex+topik}
% könnte Verbalkomplex sein
\ex {}[Welches Buch]$_i$ [\sub{S} hat Maria ihn \_$_j$ auf"|gefordert]\iw{auf"|fordern} [ \_$_i$ zu lesen]$_j$?\label{bsp-aufgefordert-zu-lesen}
\ex {}[Von diesem Sänger]$_i$ hast du Peter gebeten, \\
    {}[zu versuchen, [[ein Autogramm\iw{Autogram} \_$_i$] zu bekommen]].\footnote{
	\citet*[76]{Uszkoreit87a}\iafdata{Uszkoreit}.
}
\ex {}[Über dieses Thema]$_i$ [\sub{S} hatte Fritz Peter \_$_j$ gebeten],\\ 
    {}[[einen Vortrag \_$_i$] zu halten]$_j$.\footnote{
        \citet*[21]{HN89b}\iafdata{Hinrichs}\iafdata{Nakazawa}.
}
\ex Anruf"|beantworter dienen längst nicht mehr dazu, wichtige Nachrichten auf"|zuzeichnen, wenn
        ...
        hat\iw{Lust haben} [ \_$_i$ zu sprechen]$_j$.\footnote{
        taz, 29.05.1995, S.\,20, "`Anruf"|beantworter als Visitenkarten"'.
        }
...
\noindent
Zu guter Letzt zeigt (\mex{1}) ein Beispiel, bei dem ein Relativsatz aus einer im Mittelfeld
umgestellten Phrase extraponiert wurde.
\ea
dass die Frau [den Mantel]$_i$ dem Kind \_$_i$ geschenkt hat, den es sich schon lange gewünscht hatte 
\z
Zusammenfassend kann man wohl sagen, dass es vielleicht Freezing-Effekte gibt, dass es sich dabei
aber höchstwahrscheinlich um Verarbeitungsprobleme handelt. Es ist nicht so, dass die
Strukturen an sich unmöglich sind. Abhängig von der lexikalischen Füllung sind sie nur schwerer zu
verarbeiten als andere. Wenn Freezing strikt wäre und die Umstellung im Mittelfeld Bewegung wäre,
dürften die Sätze in (\ref{ex-Gartenvereinsvorsitzenden}) nicht grammatisch sein. Da sie aber grammatisch sind und Freezing
ohnehin nicht strikt ist, fällt auch dieses Argument für die Behandlung der mittelfeldinternen
Stellungsvarianten mittels Bewegung weg.
%
\is{Freezing@\emph{Freezing}|)}\is{Konstituentenstellung|)}
...
\item Welche Stellung von Verb und Argumenten wird als Grundstellung für das Deutsche angenommen?
\item Wie werden Verberstsätze analysiert?
\end{enumerate}
}
...
\eal
\ex dass Kurt-Martin dem Clown drei Mark gibt
...
Auf S.\,\pageref{page-verbletzt} habe ich Argumente für die Annahme aufgeführt, dass die Stellung
...
Vorfeldbesetzung \citep{Mueller2003b,Bildhauer2011a,Mueller2005d,MuellerGS} stellt wirklich harte
Evidenz gegen linearisierungsbasierte Analysen dar. \citet{MuellerGS} fasst die Ergebnisse der
Forschung zu diesem Thema zusammen.
}