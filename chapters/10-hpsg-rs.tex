...
\ex Die Stadt, [in \emph{der}] Aicke arbeitet, ist attraktiv. \label{r3}
...
\ea
Conny hatte die Schachpartie gewonnen, [was Aicke ärgerte].
\z                                              
...
sei auf \citew{Holler2003a} bzw.\ auf \citew{Bausewein90} und \citews{Mueller99b}[Abschnitt~10.4]{Mueller99a} verwiesen.
...
\eal
\ex das Thema, [über das]$_i$ Aicke Kim gebeten\iw{bitten} hat, [\sub{VP} [einen Vortrag\iw{Vortrag} \_$_i$] zu halten],
\ex Wollen wir mal da hingehen, wo$_i$\iw{wo!Relativpronomen} Jochen gesagt hat, [dass es \_$_i$ so gut schmeckt]?
\zl
...
Dieser Abschnitt ist in drei Teile geteilt: Zuerst beschäftigen wir uns
mit der Syntax der Relativsätze. Abschnitt~\ref{sec-Generalisierung-Satztypen} zeigt, wie man Generalisierungen bzgl.\ der Syntax
von Aussagesätzen und Relativsätzen erfassen kann. Im Abschnitt~\ref{Abschnitt-rs-sem-Analyse} wird
dann die Semantik der Relativsätze behandelt.
...
und dem Index des Elements in der \rell.\footnote{%
  ...
  Index als auch der \ltopw nach oben gereicht werden müssen. Zum Lexikoneintrag für das Possessivum
  siehe S.\,\pageref{le-dessen}.
}
...
Im Kapitel über die topologischen Felder habe ich bereits darauf hingewiesen, dass Verbzweitsätze,
Relativsätze und Interrogativnebensätze im Wesentlichen dieselbe Syntax haben (siehe
S.\,\pageref{Seite-Relativsätze-Interrogativnebensätze}). Es ist jeweils eine Wortgruppe
vorangestellt. Die Voranstellung kann aus einer beliebig tief eingebetteten Konstituente 
...
Interrogativnebensätzen befindet sich das finite Verb dagegen in Letztstellung. (\mex{1}) zeigt, was
alle drei Satztypen gemeinsam haben:
\ea
\label{type-filler-phrase}%
\type{filler-phrase} \impl\\
\onems{ loc$|$cat$|$nonloc \ms{  rel   & \eliste \\
                              slash & \eliste \\
                           } \\[-5\baselineskip]
   dtrs \liste{ %\begin{tabular}{@{}l@{}}
                            \ms{ loc    & \ibox{1} \\
...
\noindent
Der Typ \type{filler-phrase} ist ein Obertyp von \type{relative-clause} und von
\type{head-filler-phrase}. Siehe Abbildung~\vref{abb-sign-relcl} für einen
Überblick über die aktuelle Typhierarchie unter \type{phrase}. 
%
% \begin{figure}
% \centering
% \oneline{%
...
Die\is{Semantik|(} hier betrachteten Relativsätze (\mex{1}a) verhalten sich wie intersektive pränominale
...
Hauptrelation mit dem der modifizierten \nbar (siehe das Semantikprinzip für intersektive Modifikatoren auf S.\,\pageref{Semantikprinzip-nicht-skopaleTochter-LTOP}).
...
Eine Alternative besteht darin, ein Schema zu verwenden, das den Satz mit einer Relativphrase zu einem
...
Numerus und Genus kongruiert. Der \contw des Satzteils des Relativsatzes wird mit dem \contw des
gesamten Relativsatzes \iboxb{2} geteilt. Der \ltopw des finiten Verbs in der zweiten Tochter ist im \contw enthalten und somit mit dem \ltopw des gesamten Relativsatzes identisch. Da der \ltopw zwischen Kopftochter und Adjunkttochter bei
...
das modifizierte Nomen dasselbe Handle. Der \indw des Relativsatzes ist ebenfalls Teil von \cont und
...
identifiziert. Diese Analyse habe ich in der ERG-Grammatik für das Englische \citep{CF2000a-u,FCS2000a} gefunden und für das
...
% der Mann, dessen Kind
% das Kind, dessen Bruder
% die Frau, deren Kind
\ms{ 
  loc & \ms{ cat & \ms{ head  & \onems[det]{
                                spec|loc|cont \ms{ ltop & \ibox{1}\\
                                                   ind  & \ibox{2}}}\\
                        spr   & \eliste\\
                        comps & \eliste \\
                       } \\
...
es ist. Die Variable -- der Wert von \argzero{} -- ist \ibox{2}, auch der Index des Nomens. Die Restriktion des Quantors ist
\ibox{6} und \ibox{6} ist \qeq mit \ibox{1}, dem \ltopw des Nomens. Ansonsten bringt das
Possessivpronomen noch einen referentiellen Index mit \iboxb{5}, der auch der bzw.\ das Besitzende
...
\iboxb{2} ist der, die oder das Besessene, das Nomen, dessen Determinator das Possessivum ist.


Schema~\ref{rs-schema} enthält sowohl die bisher diskutierten syntaktischen als auch die semantischen
Beschränkungen.\footnote{%
Im Prinzip wäre es auch möglich, das \textsc{rel}-Element mit dem \contw der \nbar zu
identifizieren, da ja in \cont nur noch \ltop und \textsc{ind} enthalten
sind und der \ltopw des Relativsatzes ohnehin mit dem \ltopw des modifizierten Nomens identifiziert
wird, weil die Modifikation intersektiv ist. \ltopw des Satzes (in \ibox{2}\,) und \ltopw der
Relativphrase müssten dann aber immernoch einzeln erwähnt werden. Ich finde es klarer, die Werte einzeln aufzuführen.
}
\newpage
...
\onems{ loc \ms{ cat & \ms{ head & \onems[relativizer]{ mod \upshape \nbar\ind{1} } \\
                            ...
                            comps & \eliste } \\
                 cont  & \ibox{2}  }\\
   dtrs \liste{ \ms{ nonloc|rel & \sliste{ \ms{ ltop & \ibox{3}\\
                                                ind  & \ibox{1} } } }, %\\
                \onems{ loc  \onems{ cat [head|initial $-$]\\
...
\noindent
Die Beschränkungen, die für Strukturen vom Typ \type{relative-clause} gelten müssen einschließlich
der von Obertypen (\type{filler-phrase}) ererbten Beschränkungen sind in (\mex{1}) zu sehen:
\eas
Beschränkungen für \type{relative-clause} mit den von \type{filler-phrase} geerbten Beschränkungen:\\
\oneline{%
\onems{ loc \ms{ cat & \ms{ head & \onems[relativizer]{ mod \upshape \nbar\ind{1} \\
                                                              } \\
                            spr   & \eliste\\
         ...
         nonloc \ms{  rel   & \eliste \\
                      slash & \eliste } \\
   dtrs \liste{ \ms{ loc    & \ibox{4} \\
                     nonloc & \ms{%  que   & \eliste \\
                                     rel   & \sliste{ \ms{ ltop & \ibox{3}\\
                                                           ind  & \ibox{1} } } \\
                                     slash & \eliste } \\
                                   }, %\\
...
des Teilsatzes wird mit dem der Relativphrase identifiziert \iboxb{3}. 
...
\begin{forest}
sm edges
[\nbar{[\ltop \ibox{1}, \textsc{ind} \ibox{2}]}
  [\ibox{3} \nbar{[\ltop \ibox{1}, \textsc{ind} \ibox{2}]} [Kind]] 
  [{RS[\textsc{mod} \ibox{3}, \ltop \ibox{1}, \textsc{rel} \eliste, \textsc{slash} \eliste] }
    [{PP[\textsc{loc} \ibox{4}, \textsc{rel} \sliste{ \ibox{5} }]}
      [P [von] ]
      [{NP[\textsc{rel} \sliste{ \ibox{5} }]}
         [{Det[\textsc{rel} \sliste{ \ibox{5} \ms{ ltop & \ibox{1}\\
                                                   ind  & \ibox{2} } }] } [dessen] ]
         [N [Fahrrad] ] ] ]
    [{S[\type{fin}, \ltop \ibox{1}, \textsc{slash} \sliste{ \ibox{4} }] } 
...
\end{figure}
Der \ltopw von \emph{dessen}, also das Label von \relation{besitzen} \iboxb{1}, und der Index des
Relativpronomens \iboxb{2} werden über \rel nach oben gegeben. Im Relativsatz wird der \ltopw von
\emph{dessen} mit dem \ltopw des finiten Satzes identifiziert. Der \localw der PP entspricht der
Lücke im finiten Satz \iboxb{4}. \rel und \slasch sind auf der Ebene des Relativsatzes abgebunden,
weshalb nichts weiter nach oben gereicht wird: \rel und \slasch des Relativsatzes sind die leere Liste. Der Relativsatz modifiziert eine \nbar. Über \ibox{3} kann der
Relativsatz auf alle Eigenschaften der \nbar zugreifen und auch die Identität des des referentiellen
Indexes der modifizierten \nbar mit dem Index des Relativpronomens erzwingen. Der \ltopw des
Relativsatzes \ibox{1} wird mit dem \ltopw des modifizierten Nomens über das Semantikprinzip
identifiziert, da Relativsätze intersektive Modifikatoren sind (siehe Abschnitt~\ref{Abschnitt-Semantikprinzip}).
\largerpage
...
Abbildung~\ref{abb-Kind-von-dessen-Fahrrad-RELS} von unten 
...
in Abbildung~\ref{abb-sign-relcl} zu sehen. Nach der Besprechung der Semantik von Relativsätzen
wende ich mich nun alternativen Analysen zu.
\is{Semantik|)}
...
wäre, könnte man (\mex{0}) als Adjunkt verwenden. Und zwar als Adjunkt zu beliebigen Köpfen mit beliebigen
...
\eal
\ex[*]{
der [der Affe den Stock greift] Mann
}
\ex[*]{
dass [der Affe den Stock greift] Aicke lacht
}
\ex[*]{
auf [der Affe den Stock greift] dem Baum
}
\zl
Solcherart Modifikationen gilt es zu vermeiden. Die einzige Möglichkeit, die ich sehe, ist die
...
Kopf"=Adjunkt"=Schema (\type{simple-hd-adj-ph}). Die in früheren Auf"|lagen dieses Buches vorgestellte
...
der einer Verbalprojektion. Das erste Problem bleibt jedoch bestehen und ist Grund genug, die Analyse zu verwerfen.
...
Relativpronomen in der Relativphrase in (\mex{1}a) den Lexikoneintrag in (\mex{1}b):\footnote{
...
% which = (N\N)/(S/NP)
%         ((N\N)/(S/PP))\(PP/NP) (ich brauche links von mir eine Präposition, also etwas, das rechts von sich eine NP braucht, um eine PP zu ergeben. Dann bekomme ich about which mit der Kategorie (N\N)/(S/PP), d.h. ich brauche rechts von mir einen Satz, dem eine PP fehlt. Wenn ich diesen Satz finde, werde ich zu einem N\N, d.h. etwas, dass ein Nomen modifizieren kann und ein Nomen ergibt.
}
\eal
...
braucht, um eine vollständige Präpositionalphrase zu ergeben (PP/NP), \dash, \emph{which} selegiert die
Präposition. Nach der Kombination mit der Präposition ergibt sich \emph{about which} mit der Kategorie (N$\backslash$N)/(S/PP). Die PP braucht also rechts von sich einen Satz, dem eine PP fehlt. Nach der Kombination mit einem solchen Satz ergibt sich N$\backslash$N, die Kategorie für ein Adjunkt, das ein Nomen modifizieren kann.
 
Morrill stellt fest, dass zusätzliche Einträge für Fälle angenommen werden müssen, in
...
Englischen\il{Englisch}, Französischen\il{Französisch}, Japanischen\il{Japanisch} und Koreanischen\il{Koreanisch}. Sie besprechen auch sogenannte freie
