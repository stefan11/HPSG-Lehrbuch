...
Sätze? Zum Beispiel kann man die Sätze in (\mex{1}) beliebig verlängern:
\eal
\ex Dieser Satz geht weiter und weiter und weiter und weiter \ldots
\ex {}[Ein Satz ist ein Satz] ist ein Satz.
\zl
In (\mex{0}b) wird etwas über die Wortgruppe \emph{ein Satz ist ein Satz} ausgesagt, nämlich daß sie
...
daß sie als ganze Einheit gespeichert sind, das ist für Sätze wie die in (\mex{0}) jedoch nicht der
...
\zl
Die Sätze in (\mex{0}a,b) zeigen, daß ein Subjekt im Singular bzw.\ Plural
...
aber, daß es für (\mex{0}c,d) jeweils nur eine Lesart gibt.
...
Betrachtet man den Satz in (\mex{1}), so hat man das Gefühl, daß bestimmte Wörter zu einer
Einheit gehören.
\ea
...
in einem Satz gegen eine andere Wortfolge so austauschen,\is{Austauschbarkeit}\is{Substituierbarkeit} daß
...
\emph{der Frau das Buch zu geben} ersetzen.\enlargethispage{\baselineskip}
\eal
...
Was\is{Fragetest} sich erfragen läßt, ist eine Konstituente.
        \eal
...
bzw.\ umgestellt werden können, ist das ein Indiz dafür, daß sie eine Konstituente bilden.
        ...
        \eal
        \ex[]{
          weil keiner [diese Frau] kennt
          }
        \ex[]{
          weil diese Frau keiner kennt
          }
        \zl
...
\eal
\ex[*]{
weil Frau keiner diese kennt
}
\ex[*]{
weil Frau kennt keiner diese
}
\zl
...
Lassen\is{Koordination!-stest} sich Wortfolgen koordinieren, so ist das ein Indiz dafür, daß die koordinierten Wortfolgen
...
\z
Das Beispiel in (\mex{1}) zeigt, daß sich auch Wortgruppen mit \zui koordinieren lassen:
\ea
...
\zl
Im Gegensatz zum Akkusativ \emph{einen Mann} in (\mex{0}c,d) kann das Expletivum in (\mex{0}b) nicht
...
%% Der Pronominalisierungstest sagt, daß alles, worauf man sich mit einem Pronomen beziehen kann,
%% eine Konstituente ist. Betrachtet man den folgenden kleinen Text, so sieht man, daß
...
%% Man kann über (\mex{1}) sagen, daß \emph{viele Bücher über Mozart} eine Konstituente
...
alternative Erklärung für die Abfolgevarianten in (\mex{1}) liegt aber darin anzunehmen, daß
Adverbien an beliebiger Stelle im Satz stehen können und daß in (\mex{0}b) nur \emph{dem Mann} vor
...
für (\mex{0}a) gesagt, daß \emph{trocken} ein Adjektiv ist, das in (\mex{0}a) \emph{man}
...
Diese Wörter kann man nicht gemeinsam umstellen:\footnote{
  ...
  Das Beispiel in (\mex{1}) ist jedoch bewußt so konstruiert worden, 
  ...
  zusammenhängen, nie möglich ist. Siehe auch \citew{dKM2003a} zu Subjekten
...
der Kopf ist (\NOTE{\cite{Brame81a-u,Brame82a-u} \citealp[\page 90]{Hudson84a}; }\citealp{Hellan86a,Abney87a,Netter94,Netter98a}), als auch solche, die annehmen,
daß das Nomen der Kopf ist (\citealp[\page 49]{ps2}; \citealp{Demske2001a}).
...
Adjektiv     & Wortart, bei flektierten Formen Kasus\is{Kasus|(}, Genus\is{Genus|(} (\emph{mas}, \emph{fem}, \emph{neu}), Numerus\is{Numerus|(}\\
             & und Flexionsklasse\is{Flexion!-sklasse|(} (\emph{strong}, \emph{weak})\\\hline
\end{tabular}}
...
%% Bei Präpositionen muß man zwischen solchen unterscheiden, die nur zur rein formalen
...
%% welche Präposition in der PP vorkommt, denn das Verb läßt nur bestimmte Präpositionalobjekte
%% zu:
%% \ea[*]{
...
\suffix{er}) entsprechend markiert. Die Beispiele in (\mex{1}) zeigen, daß Adjektive im Deutschen
...
\zl
Damit man nun Fälle wie (\mex{0}d) ausschließen kann, muß sichergestellt sein, daß die Information
...
daß mehrere Personen schlafen, muß das wie in (\mex{1}) mit Hilfe von
...
\eal
\ex[]{
Er denkt an die Eisenbahn.
}
\ex[\#]{
Er denkt an der Eisenbahn.
}
\ex[]{
Er hängt an der Eisenbahn.
}
\ex[*]{
Er hängt an die Eisenbahn.
}
\zl
...
annehmen, daß diesen Ortsangaben der Status eines obligatorischen syntaktischen Arguments zukommt.\footnote{
  ...
  (Siehe \zb \citew[Kapitel~2]{Steinitz69a}, \citew[\page127]{HS73a}, \citet[\page 54]{Bierwisch88a-u-kopiert}, \citew[\page99]{Engel94}, 
...
aufgeführt.
\begin{itemize}
\item Dependenzgrammatik (DG)\is{Dependenzgrammatik (DG)}\\\citep{Tesniere59a-u,Kunze75a-u,Hudson90a-u,Weber92a,Heringer96a-u}\nocite{Tesniere80a-u}
...
\item Head-Driven Phrase Structure Grammar (HPSG)\\\citep{ps,ps2,Kiss95b,Mueller99a,Mueller2002b}
\item Konstruktionsgrammatik (CxG)\is{Konstruktionsgrammatik (CxG)}\\\citep*{FKoC88a,KF99a,Goldberg95a,FS2006a-ed}
\end{itemize}
...
So ist \emph{weil} in (\mex{1}) eine Konjunktion, \emph{das} und \emph {dem}
...
Das geschieht in den Zeilen 2--4, 6--7 und 9 der Ableitung in (\mex{1})\vpageref{bsp-anwendung-grammatik}.
...
In (\mex{0}) sind wir von einer Folge von Wörtern ausgegangen und haben gezeigt, daß wir die Regeln
...
Die Ableitung in (\mex{0}) kann auch als Baum dargestellt werden. Das zeigt Abbildung~\vref{er-das-buch-dem-mann-gibt-flat}.
\begin{figure}[htbp]
\centerline{
\begin{tabular}[t]{@{}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{}}%llll}
\multicolumn{3}{c}{\rnode{f1}{S}}\\[2ex]
\\
\rnode{f2}{NP} & \multicolumn{2}{c}{\hspace{-2ex}\rnode{f4}{NP}}     & \multicolumn{2}{c}{\hspace{-2ex}\rnode{f6}{NP}} & \rnode{f7}{V}\\[2ex]
\\
             & \rnode{f14}{D}  &  \rnode{f15}{N}    & \rnode{f16}{D}   & \rnode{f17}{N}\\
\\*[3ex]
\rnode{f8}{er} & \rnode{f9}{das} & \rnode{f12}{Buch}  & \rnode{f10}{dem} &\rnode{f13}{Mann}  & \rnode{f11}{gibt}\\
\end{tabular}%
\ncline{f1}{f2}\ncline{f1}{f6}%
\ncline{f1}{f4}\ncline{f1}{f7}%
\ncline{f7}{f11}%
...
\caption{\label{er-das-buch-dem-mann-gibt-flat}Analyse von \emph{er das Buch dem Mann gibt}}
\end{figure}
Die Symbole im Baum werden \emph{Knoten}\is{Knoten} genannt. Man sagt, daß S die NP"=Knoten
und den V"=Knoten unmittelbar dominiert\is{Dominanz}. Die anderen Knoten im Baum werden von S ebenfalls dominiert,
aber nicht unmittelbar dominiert\is{Dominanz!unmittelbare}. Will man über Beziehungen von Knoten zueinander reden,
verwendet man Verwandschaftsbezeichnungen. So ist in Abbildung~\ref{er-das-buch-dem-mann-gibt-flat}
S der Mutterknoten\is{Knoten!Mutter-} der drei NP"=Knoten und des V"=Knotens. 
Die NP"=Knoten und V sind Schwestern\is{Knoten!Schwester-}, da sie denselben Mutterknoten haben.
Hat ein Knoten zwei Töchter\is{Knoten!Tochter-} liegt eine binär\is{bin"ar} verzweigende Struktur\is{Verzweigung}\is{Verzweigung!bin"are} vor.
% stimmt nicht, das ist relativ
%hat er mehr als zwei Töchter, spricht man auch von \emph{flacher Verzweigung}. 
Gibt es genau eine Tochter, spricht man von einer unären\is{un"ar}\is{Verzweigung!un"are} Verzweigung. Zwei Konstituenten sind 
...
\ea
{}[\sub{S} [\sub{NP} er] [\sub{NP} [\sub{D} das] [\sub{N} Buch]]  [\sub{NP} [\sub{D} dem] [\sub{N} Mann]] [\sub{V} gibt]]
\z

\noindent
Hier muß darauf hingewiesen werden, daß die Grammatik in (\ref{bsp-grammatik-psg}) nicht die einzig mögliche Grammatik
für den Beispielsatz in (\ref{bsp-weil-er-das-buch-dem-mann-gibt}) ist. 
Es gibt unendlich\label{page-unendlich-viele-grammatiken} viele Grammatiken, 
die zur Analyse solcher Sätze verwendet werden können (siehe Übung \ref{ua-psg-eins}).
...
\end{tabular}
\z
Diese Grammatik lizenziert binär verzweigende Strukturen, wie die in Abbildung~\vref{er-das-buch-dem-mann-gibt-bin}.
...
\scalebox{1}{%
\begin{tabular}[t]{@{}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{\hspace{4mm}}c@{}}%llll}
\multicolumn{3}{c}{\rnode{1}{V}}\\[2ex]
\\
\rnode{2}{NP} & \multicolumn{4}{c}{\rnode{3}{V}}\\[2ex]
\\
             & \multicolumn{2}{c}{\hspace{-2ex}\rnode{4}{NP}} & \multicolumn{3}{c}{\hspace{2mm}\rnode{5}{V}}\\[2ex]
\\
             & \rnode{14}{D}  & \rnode{15}{N}     & \multicolumn{2}{c}{\hspace{-2ex}\rnode{6}{NP}} & \rnode{7}{V}\\[2ex]
\\
             &               &                  & \rnode{16}{D}   & \rnode{17}{N}\\[2ex]
\\*
...
\caption{\label{er-das-buch-dem-mann-gibt-bin}Analyse von \emph{er das Buch dem Mann gibt} mit binärer Verzweigung}
\end{figure}
...
\ex[*]{
er das Buch den Mann gibt
}
\ex[*]{
er den Buch dem Mann gibt
}
\zl
In (\mex{0}b) ist die Subjekt"=Verb"=Kongruenz verletzt, {\em ich\/} und {\em gibt\/} passen
nicht zueinander. In (\mex{0}c) sind die Kasusanforderungen des Verbs nicht erfüllt.
{\em gibt\/} verlangt ein Dativ"=Objekt. In (\mex{0}d) ist die Determinator"=Nomen"=Kongruenz
verletzt. {\em den\/} und {\em Buch\/} passen nicht zueinander, da die Genus"=Eigenschaften der
beiden Wörter verschieden sind.

Im folgenden soll überlegt werden, wie man die Grammatik verändern muß, damit die Sätze in (\mex{0})
...
und Numerus (sg, pl) übereinstimmen\is{Kongruenz} muß:
...
die nur zwei bzw.\ eins nehmen, unterscheiden können muß (\mex{1}), muß man entsprechend auch
...
Genauso können wir nicht erfassen, daß die Regeln in (\mex{0}) etwas gemeinsam haben.
...
Die Regeln in (\ref{ditrans-ps-regeln}) können wie in (\mex{1}) zusammengefaßt werden:
\ea
\begin{tabular}[t]{@{}l@{ }l@{ }l}
...
%% in denen nur der Per"=Wert oder nur der Num"=Wert identisch sein muß? Die Tatsache,
%% daß immer beide Werte gemeinsam übereinstimmen mü
...
[\sub{NP} Nomen Artikel] nie für eine Grammatik des Englischen angenommen würde und daß deshalb
...
Jemand, der Grammatiken entwickelt, müßte dann eine deutsche Grammatik mit englischen Wörtern
...
jedoch zeigen, daß es durchaus Belege für die angeblich unmöglichen Strukturen gibt (siehe auch \citew{Mueller2007c,MM2009a}).
...
\ex[*]{
Der Mann kennen diese Frau.
}
\ex[*]{
Mann der kennen diese Frau.
}
\ex[*]{
Mann der kennen Frau diese.
}
\zl
Kritiker wenden hier zu Recht ein, daß man bei (\mex{0}b--d) Abstufungen in der Akzeptabilität
feststellen kann: In (\mex{0}b) gibt es keine Kongruenz zwischen Subjekt und Verb, in (\mex{0}c)
stehen zusätzlich \emph{Mann} und \emph{der} in der falschen Reihenfolge, und in (\mex{0}d) ist \emph{Frau}
und \emph{diese} vertauscht. Genauso verletzen die Sätze in (\mex{-1}) zwar grammatische
...
\item Bestimmen Sie die Köpfe, die Adjunkte und die Argumente im Satz (\mex{1}) und in den Bestandteilen des Satzes:
  \ea
  Er hilft den kleinen Kindern in der Schule.
  \z
\end{enumerate}
...
%%       Beweisen Sie, daß das richtig ist.
%%
%%       Hilfestellung: Eine Möglichkeit, eine Behauptung zu beweisen, besteht darin, einen indirekten
%%       Beweis zu führen. Beim indirekten Beweis geht man davon aus, daß das, was man beweisen will,
%%       falsch ist, und zeigt, daß diese Annahme zu einem Widerspruch führt.
