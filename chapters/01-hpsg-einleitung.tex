...
warum es sinnvoll ist, die Erkenntnisse zu formalisieren (Abschnitt~\ref{sec-formal}).\footnote{
  Teile dieses Kapitels bilden auch die Grundlage der ersten beiden Kapitel meines Grammatiktheoriebuches
  \citep{MuellerGTBuch2}. Den Abschnitt über Interrogativnebensätze und Relativsätze und
  topologische Felder, habe ich zuerst als Bestandteil der Grammatiktheorie veröffentlicht und habe ihn nun für die
  vierte Auf"|lage dieses Buches auch hier in den Abschnitt~\ref{sec-Topologische-Felder-Zuordnung} integriert.
}
...
Sätze? 20 Wörter? 30? 40? Thomas Manns Roman "`Joseph und die Brüder"' enthält einen Satz mit 347 Wörtern. Der längste
bekannte Satz, der in einem Roman verwendet wurde, hat 1077 Wörter \citep{Hug2018a}.
Aber auch eine Grenze von 1500 Wörtern wäre willkürlich. Das kann man sich anhand der Sätze in
(\mex{1}) klarmachen. Diese Sätze kann man auf einfache Art verlängern:
\eal
\ex Dieser Satz geht weiter und weiter und weiter und weiter.
...
entsprechend um \emph{ist ein Satz} erweitern. Und auch über den Satz mit 1077 kann man folgenden
Satz schreiben, wobei die drei Punkte für den zitierten Satz stehen: 
\ea
Der Schriftsteller Hermann Broch schrieb in seinem Roman "`Der Tod des Vergils"' den Satz \ldots.
\z
Durch Erweiterungen wie die in (\mex{-1}) und (\mex{0}) können wir enorm lange und komplexe Sätze bilden.\footnote{
...
dass sie als ganze Einheit gespeichert\label{page-gespeicherte-Chunks} sind
\parencites{Bybee95a-u}{Langacker87a-u}[228--229]{PJ2005a}[64]{Goldberg2006a}[Kapitel~3]{Bybee2010a-u},\LATER{\parencites{Ullman1999a-u}{Pinker99a-u}{BaayenSdJK2002a-u}{AS2010a-u}{AMC2017a-u}{BVWDD2000a-u}{EllisMorrison1998a-u}{SBvSSF2020a-u}}
%(Ellis and Morrison, 1998; Brysbaert et al., 2000; Zevin and Seidenberg, 2002; Juhasz, 2005;
%Gibson2025a: 25: Arnon and Snider, 2010; Arnon et al., 2017; Shain et al., 2020, 2022).
 das ist für Sätze wie die in (\mex{0}) jedoch nicht der
...
Kenntnis der syntaktischen Gesetzmäßigkeiten des Deutschen weiß die Hörer*in
...
Einheit gehören.
\ea
Alle Student*innen lesen während der vorlesungsfreien Zeit Bücher.
\z
...
einem Fragepronomen auf eine Wortfolge. Der Pronominalisierungstest und somit auch der Fragetest sind Spezialfälle des Ersetzungstests.
...
Begriff der Konstituente ist jedoch allgemeiner, da er auch Fälle wie (\mex{1}a) erfasst:
\eal
...
\ea
Er hat versucht, [das Buch zu lesen] und [es dann unauf"|fällig verschwinden zu lassen].
\z
...
solche Voranstellungen, aber in diesen ist das \emph{es} immer referentiell \parencites[\page162]{Lenerz94a}[\page4]{GS97a}.
...
Teilkonstituenten wie in (\ref{ex-Agrarsubventionen}) unabhängig voneinander umgestellt werden \citep{deKuthy2002a}.
...
\footnote{
  ...
  so dass letztendlich wieder genau eine Konstituente vor dem Finitum steht \citep{Mueller2005d,MuellerGS}.
...
Diese Wörter kann man nicht gemeinsam umstellen:\footnote{
  ...
  die Diskussion der Beispiele in (\ref{bsp-mehr-vf}) auf S.\,\pageref{bsp-mehr-vf}.
...
Gleichzeitig steuert der Kopf den internen Aufbau der Phrase, \dash, der Kopf verlangt
...
ist umstritten: Es gibt sowohl Wissenschaftler*innen, die annehmen, dass der Determinator
der Kopf ist
\parencites[\page 6]{Ajdukiewicz35a-u}[231]{VH77a-u}[Abschnitt~6]{Brame81a-u}{Brame82a}[\page 90--92]{Hudson84a-u}{Hellan86a,Abney87a,Netter94,Netter98a}, als auch solche, die annehmen,
dass das Nomen der Kopf ist \parencites{vanLangendonck94a}[\page
49]{ps2}{Demske2001a}{Hudson2004a}{Bruening2009a}{MyPM2021a}{MuellerHeadless}.
% LFG ist da uneinheitlich in BATW2016a gibt es mal NP mal DP
%\LATER{LFG-Handbook}
% Dalrymple2006a:83
% In the following, we do not
% assume DP, which means that the category of a phrase
% such as the boy is NP. However, there is no general
% agreement of the status of such phrases in LFG.
Ich schließe mich den letzteren an. Argumente für den Kopfstatus des Nomens
finden sich in Abschnitt~\ref{sec-dp-analyse}, eine längere Diskussion findet sich in
\citew{MuellerHeadless} und \citew{MyPM2021a}.

Die Kombination eines Kopfes mit einer anderen Konstituente wird \emph{Projektion
des Kopfes}\is{Projektion} genannt. Die größte Projektion, die alle notwendigen Bestandteile zur Bildung
...
für menschliche Hörer*innen/""Sprecher*innen ergeben würden, so kann eine solche Erweiterung
...
\noindent
Abbildung~\ref{fig-optional-arguments-adjuncts} visualisiert noch einmal, dass sowohl Adjunkte als auch Argumente
optional sein können. Daraus, dass etwas optional ist, folgt nicht, dass es ein Adjunkt ist.
\begin{figure}
\begin{tikzpicture}
\node[anchor=east](c1) at (-1,2) {nicht optional};
...
\caption{\label{fig-optional-arguments-adjuncts}Adjunkte sind immer optional. Argumente können
  optional oder obligatorisch sein. Wenn etwas nicht optional ist, muss es
  ein Argument sein \citep[§1182]{Duden2005}.} 
\end{figure}
...
von der Bedeutung der Präposition ab. Direktionale (also eine Richtung angebende) Präpositionen 
verlangen normalerweise eine Nominalphrase im Akkusativ (\mex{1}a), lokale (also einen Ort spezifizierende)
Präpositionen nehmen einen Dativ zu sich (\mex{1}b):
\eal
...
annehmen, dass diesen Ortsangaben der Status eines obligatorischen syntaktischen Arguments zukommt.\footnote{
  ...
  \parencites[Siehe z.\,B.][Kapitel~2]{Steinitz69a}[\page127]{HS73a}[\page 54]{Bierwisch88a-u}[\page99]{Engel94}[291]{Jacobs94a}[\page119]{Kaufmann95a}[\page 21]{Abraham2005a}.
%Fanselow2003b:21
...
unterteilt. Nicht alle Köpfe müssen ein Subjekt haben (siehe Abschnitt~\ref{sec-subjekt-valenz}),
...
der Linguistik mitunter der Fall ist \citep[siehe z.\,B.][65]{Heringer96a-u}.
% \item Genitive?
% \eal
...
\item Transformationsgrammatik\is{Transformation} und deren Nachfolger
      ...
      \item Government \& Binding\indexgb\\\citep{Chomsky81a,FF87a,SS88a,Grewendorf88a,Lohnstein2014a}
      \item Minimalismus\is{Minimalistisches Programm}\\\citep{Chomsky95a-u,Grewendorf2002a,Adger2003a}
      \end{itemize}
\item Relational Grammar (RG)\\
      \citep{Perlmutter83a-ed,PR84a-ed}
...
\item Generalisierte Phrasenstrukturgrammatik (\gpsg)\\
      \citep*{GKPS85a,Uszkoreit87a}
\item Lexikalisch Funktionale Grammatik (LFG)\indexlfg\\
      \citep{Bresnan82a-ed,Bresnan2001a,BF96a-ed,Berman2003a,Dalrymple2023a-ed}
\item Head-Driven Phrase Structure Grammar (HPSG)\\\citep*{ps,ps2,Kiss95b,Sag97a,Mueller99a,Mueller2002b,HPSGHandbook}
\item Konstruktionsgrammatik (CxG)\indexcxg\\\citep*{FKoC88a,KF99a,Goldberg95a,Goldberg2006a,FS2006a-ed}
\end{itemize}
...
über Minimalismus und Dependenzgrammatik noch nicht enthalten sind \citep{MuellerGTBuch}. Auch
Diskussionskapitel zu verschiedenen theorieübergreifenden Fragen wurden in der englischen Version
hinzugefügt oder, so bereits vorhanden, ausgebaut, so dass der jeweils aktuellen Version
...
mit der man (\mex{0}) analysieren kann, ist in (\mex{1}) zu sehen:\footnote{
  ...
  Zu den anderen Verbstellungen siehe Abschnitt~\ref{sec-v1} und Kapitel~\ref{Kapitel-nla}.%
}
\ea
...
\begin{tabular}[t]{@{}r|l@{~~~}l@{~~~}l@{~~~}l@{~~~}l@{~~~}l@{\hspace{1cm}}l}
 ...
 2 & {NP}          & dem          & Affen          & den          & Stock & gibt & {NP $\to$ Aicke}  \\
...
In (\mex{0}) bin ich von einer Folge von Wörtern ausgegangen und habe gezeigt, dass man die Regeln
...
Hat ein Knoten genau zwei Töchter\is{Knoten!Tochter-}, liegt eine \emph{binär\is{binär} verzweigende Struktur}\is{Verzweigung}\is{Verzweigung!binäre} vor.
...
die zur Analyse solcher Sätze verwendet werden können (siehe Übung~\ref{ua-psg-eins} auf S.\,\pageref{ua-psg-eins}).
...
\end{tabular}
\z
Diese Grammatik lizenziert binär verzweigende Strukturen, wie die in
Abbildung~\ref{fig-Aicke-dem-Affen-den-Stock-gibt-bin}.
\largerpage
...
Zum Beispiel lassen die Grammatiken die Folge in (\ref{ex-Aicke dem Affen dem Stock gibt}) zu.
Nimmt man noch Lexikoneinträge für \emph{ich} und \emph{das} in die Grammatik auf, so werden
fälschlicherweise auch noch die Abfolgen in (\mex{1}c,d) lizenziert:
\eal
\ex[]{
Aicke dem Affen den Stock gibt
}
\ex[*]{\label{ex-Aicke dem Affen dem Stock gibt}
Aicke dem Affen dem Stock gibt
}
\ex[*]{\label{ex-ich dem Affen den Stock gibt}
...
\zl
In (\ref{ex-Aicke dem Affen dem Stock gibt}) sind die Kasusanforderungen des Verbs nicht erfüllt.
\emph{gibt} verlangt ein Akkusativ"=Objekt. In (\ref{ex-ich dem Affen den Stock gibt}) ist die Subjekt"=Verb"=Kongruenz verletzt, \emph{ich} und \emph{gibt} passen
nicht zueinander. In (\mex{0}d) ist die Determinator"=Nomen"=Kongruenz
...
insgesamt (3 * 2) + 3 = 9 Symbole für verschiedene NPen: sechs NP-Symbole für die Nominativ-NPen und
dann je ein NP-Symbol für Genitiv, Dativ und Akkusativ. Da die Verben zu den Nominalphrasen
...
würde genauso funktionieren, wenn man statt NP\_akk das Symbol Z80 oder \smiley{} verwenden würde. Somit kann die
...
der \xbar-Theorie (sprich X-bar"=Theorie; \citealt{Chomsky70a,Jackendoff77}). Diese Form abstrakter Regeln
spielt in den verschiedensten Theorien eine Rolle. Beispielhaft seien
erwähnt: Government \& Binding\indexgb \citep{Chomsky81a,FF87a,SS88a,Grewendorf88a},
Generalized Phrase Structure Grammar\indexgpsg \citep*{GKPS85a,Uszkoreit87a}
und Lexical Functional Grammar\indexlfg \citep{Bresnan82a-ed,Bresnan2001a,BF96a-ed,Dalrymple2023a-ed}.
...
\ea
\(
\begin{array}[t]{@{}l@{\hspace{4mm}}l@{\hspace{4mm}}l@{}}
\xbar\mbox{-Regel} & \mbox{mit Kategorien} & \mbox{Beispiel}\\[2mm]
\overline{\overline{\mbox{X}}} \rightarrow \overline{\overline{\mbox{Spezifikator}}}~~\xbar & \overline{\overline{\mbox{N}}} \rightarrow \overline{\overline{\mbox{DET}}}~~\overline{\mbox{N}} & \mbox{das [Bild von Paris]} \\
\xbar \rightarrow \xbar~~\overline{\overline{\mbox{Adjunkt}}}             & \overline{\mbox{N}}
                                                                           \rightarrow
                                                                           \overline{\mbox{N}}~~\overline{\overline{\mbox{REL\_SATZ}}}
                                           & \mbox{[Bild von Paris] [das alle kennen]}\\
\xbar \rightarrow \overline{\overline{\mbox{Adjunkt}}}~~\xbar             & \overline{\mbox{N}} \rightarrow \overline{\overline{\mbox{ADJ}}}~~\overline{\mbox{N}} & \mbox{schöne [Bild von Paris]}\\
\xbar \rightarrow \mbox{X}~~\overline{\overline{\mbox{Komplement}}}*               & \overline{\mbox{N}} \rightarrow \mbox{N}~~\overline{\overline{\mbox{P}}} & \mbox{Bild [von Paris]}\\
\end{array}
\)
\z
...
getrennten Beschreibungsebenen wie zum Beispiel in der Government \& Binding"=Theorie\indexgb.
(\mex{1}) zeigt Auszüge aus einer Repräsentation des Wortes \emph{Grammatik}:\footnote{Der Wert des
  \textsc{phonology}"=Merkmals ist in IPA-Umschrift angegeben. Phonologie ist nicht Gegenstand
  dieses Buches, weshalb in den folgenden Kapiteln der Einfachkeit halber die orthografische Form angegeben
  wird. Zur Phonologie in HPSG siehe zum Beispiel \citew{BK94b} und \citew{Hoehle99a-u}.
}
\ea
\ms[word]{
%phonology   & \phonliste{ Grammatik } \\[1mm]
phonology   & \phonliste{ \textipa{g\textinvscr am\textprimstress atIk} } \\[1mm]
...
Sampsons Position ist eine extreme Position, und wie so oft sind extreme Positionen als Gegenstück
...
jedoch zeigen, dass es durchaus Belege für die angeblich unmöglichen Strukturen gibt \citep[siehe auch][]{Mueller2007c,MM2009a}.
Dieser Punkt wird auch in dem obigen Zitat von Sampson angesprochen: Mitunter gibt es Äußerungskontexte, in denen
Äußerungen völlig normal erscheinen, und die jeweiligen Autor*innen sind nur nicht auf einen solchen gekommen.
...
man Sprecher*innen der jeweiligen Sprache befragen oder andere psycholinguistische Experimente
durchführen.
...
und \emph{diesem} vertauscht. Genauso verletzen die Sätze in (\mex{0}) zwar grammatische
...
Wohlgeformtheitsbedingungen kann man auch wichten %\parencites[z.\,B.][]{FKKM2001a}
\citep[z.\,B.][]{Sensekerci2024a-u}, 
so dass man erklären kann, warum bestimmte Verletzungen zu stärkeren Abweichungen führen als andere.
% \citep{Brew95a}. Ist nicht ganz das. Er sagt nur, dass der Typ am Ende eines Pfades mit einer
% bestimmten Wahrscheinlichkeit vorkommt.
% Steht auch bei Müller & Kasper, allerdings ist das nur für den Vergleich von Lexikoneinträgen. \citep[255]{FCS2000a}.
...
\item\label{ua-psg-eins}
      Auf S.\,\pageref{page-unendlich-viele-grammatiken} habe ich behauptet, dass
