...
In diesem Kapitel wird die Analyse des Verbalkomplexes vorgestellt
(Abschnitt~\ref{sec-verbalkomplex}). Anhand von Futur- und Perfektkonstruktionen wird die Argumentanziehung eingeführt. Diese spielt
auch bei der Analyse von anderen Infinitivkonstruktionen (Kapitel~\ref{Kapitel-anhebung}) und bei
der Analyse des Passivs (Kapitel~\ref{Kapitel-passiv}) und der Partikelverben (Kapitel~\ref{Kapitel-partikel}) eine wesentliche Rolle. Abschnitt~\ref{sec-pvp}
geht auf einen interessanten Bereich der deutschen Syntax ein: das Voranstellen von Phrasenteilen,
...
In verschiedenen anderen Arbeiten \citep[z.\,B.][Kapitel~3]{Uszkoreit87a} 
...
in einem Relativsatz\is{Relativsatz} auf"|treten können müsste.
\eal
...
dass niemand das Buch [[lesen können] wird]
\z
\emph{wird} kann wie in \pref{ex-struktur-lesen-koennen-wird} rechts oder wie in \pref{ex-of} links des eingebetteten Verbalkomplexes
...
der beteiligten Verben, also mit \emph{das Buch} und \emph{niemand} kombiniert.\footnote{
        Eine solche Struktur wurde schon von \citet*{Johnson86a}
...
\z
Auf S.\,\pageref{page-symmetrische-koordination} wurde dafür argumentiert,
...
so wäre die Koordination in (\mex{0}) nicht als symmetrische Koordination zu erklären.\footnote{
        ...
        Mit einem strukturierten Verbalkomplex ist (\mex{0}) jedenfalls unproblematisch. Zur
        Koordination in HPSG siehe \citew{AC2024a}.%
}
...
Das folgende Schema, das eine Abwandlung des von \citeauthor{HN94a} vorgeschlagenen ist, lizenziert die Prädikatskomplexe\is{Schema!Prädikatskomplex-}:
\begin{samepage}
...
\end{samepage}
%
Dieses Schema entspricht dem Kopf"=Komplement"=Schema, das bis zum Kapitel~\ref{Kapitel-Konstituentenreihenfolge}
benutzt wurde, bzw.\ der angepassten Version auf S.\,\pageref{schema-head-arg}.
...
immer mit dem letzten Argument begonnen.\footnote{%
  Das bedeutet nicht, dass die Stellung fest ist. Es gibt die Oberfeldumstellung, die in
  Abhängigkeit von den kombinierten Verben optional oder obligatorisch sein kann. Vergleiche
  (\ref{ex-uf}) und (\ref{ex-of}). Aber auch bei der Analyse von (\ref{ex-uf}) und (\ref{ex-of})
  wird immer das letzte Argument der \compsl mit dem regierenden Kopf kombiniert.
} Deshalb wird im Prädikatskomplexschema nur ein \emph{append} 
verwendet. Das Kopf"=Komplement"=Schema wird auf S.\,\pageref{schema-bin} noch so revidiert,
...
wird im Abschnitt~\ref{sec-subj-merkmal} revidiert.%
%% Die separate
...
\emph{Aicke} und \emph{dem Kind} von \emph{helfen}. 
\ea
dass Aicke dem Kind helfen wird
\z
...
[\ms{ head  & \ibox{1}\\
  ...
  [\iboxt{4}~\onems{ \ldots \onems{ head  \ms[verb]{ vform & bse \\
                                             }\\
...
% } 
Ohne eine solche Spezifikation wären alle drei Strukturen in (\mex{1}) möglich \parencites[\page303]{Pollard90a}{HN94b}:
\eal
...
Kopf"=Komplement"=Strukturen als \textsc{lex}$-$ spezifiziert ist (in anderen Strukturen übrigens auch), 
%(siehe Schema~\ref{schema-bin} auf S.\,\pageref{schema-bin}), 
...
Der \textsc{lex}"=Wert der Mutter in Prädikatskomplexstrukturen ist im Unterschied zu Kopf"=Komplement"=Strukturen
...
kombiniert. Die Analyse in (\mex{0}) lässt sich ausschließen, wenn man in Lexikoneinträgen für Anhebungsprädikate
...
Mit der hier angegebenen Beschränkung für angehobene Elemente wird
        ...
        kohärenter Konstruktionen siehe Abschnitt~\ref{sec-anhebung-anal}.%
%
}
\eas
\label{constr-non-complex-forming}%
list\_of\_non\_c\_forming\_synsems(\eliste).\\\\
list\_of\_non\_c\_forming\_synsems(\liste{ \onems{ loc$|$cat|comps \eliste\\ %head$|$prd $-$\\
                                                   lex $-$\\
                                                } $|$ \textrm{Rest} }) :=\\\\
\hfill        list\_of\_non\_c\_forming\_synsems(Rest).\\[2mm]
\zs
...
%            comps & \ibox{2}\\
%             }}
%    [\iboxt{4}~\onems{ \ldots \ms{ head & \ibox{3} \\
%                                comps & \ibox{2} \\
%                              }} 
%      [\iboxt{5}~\onems{ \ldots \onems{ head  \ms[verb]{ vform & ppp  \\
%                                                    }  \\
...
\caption{Analyse des Verbalkomplexes in:\ \emph{dass Aicke dem Kind geholfen haben wird}}\label{abb-kombin1}%
\end{figure}
Es übernimmt die Argumente dieses Verbs \iboxb{2}. Der resultierende Verbalkomplex hat dieselbe
...
noch durch das Kopf"=Komplement"=Schema lizenziert, da die Kombination von \emph{das Buch} und \emph{lesen} 
...
\z
Die Sätze in (\mex{-1}) können mit dem Kopf"=Komplement"=Schema analysiert werden,
da dieses eine Kombination der Elemente in der \compsl von \emph{wird} in beliebiger
Reihenfolge zulässt. Genauso gibt es eine Analyse für (\mex{0}): \emph{lesen} und \emph{niemand}
werden mit \emph{wird} über das Kopf"=Komplement"=Schema kombiniert. Eine Spur nimmt den Platz
für das Relativpronomen in einer Kopf"=Komplement"=Struktur ein, und das Relativsatzschema lizenziert
...
Es ist klar, dass einzelne Verben bzw.\ Verbalkomplexe nicht als Komplemente in
Kopf"=Komplement"=Strukturen vorkommen sollen. Ein solches Vorkommen kann man dadurch ausschließen, dass man verlangt,
dass die Argumente immer den \lexw $-$ haben. Schema~\ref{schema-bin} ist
das entsprechend angepasste Schema.\is{Schema!Kopf"=Komplement"=}
...
%                }\\
      ...
      non-head-dtrs \nliste{ [ \synsem  \ibox{2} [\textsc{lex}  $-$ ] ] }\\
}
\end{schema}\isfeat{lex}
\end{samepage}

\noindent
Die aufmerksame Leser*in wird sich fragen, was die Theorie in Bezug auf Sätze wie (\mex{1}) vorhersagt.%
\ea[*]{
dass lachen er wird
}
\z
...
mit den Anforderungen des Kopf"=Komplement"=Schemas kompatibel ist. Der Satz in (\mex{0}) wird aber dennoch
nicht von der Theorie zugelassen, da das regierende Verb, also \emph{wird}, von seinem verbalen Argument verlangt,
dass dessen \lexw + ist. Somit kommt es in Kopf"=Komplement"=Strukturen zu konfligierenden \lexwen, 
...
\zl
Sätze wie in (\mex{0}b) sind aber unproblematisch, wenn man \textsc{lex} nicht wie \citet[\page 22]{ps2}
unter \textsc{cat}, also innerhalb von \textsc{local}, sondern unter \textsc{synsem}, also außerhalb von
\textsc{local}, repräsentiert \parencites[802]{Mueller96a}[356--357]{Mueller99a}[97]{Mueller2002b}[Abschnitt~2.2]{Meurers99a}.
...
zulässig.\footnote{
        ...
        geteilt. Alles andere (\textsc{phon}, \textsc{head-dtr}, \textsc{non-head-dtrs}, \textsc{dtrs}, \textsc{synsem$|$nonlocal},
        ...
        nicht sinnvoll ist, wie \citegen[95]{Haider90a} Beispiele in (i) zeigen:
        \eal
        \ex[]{
        [Hunde füttern, die Hunger haben,] würde wohl jeder.
        }
        \ex[*]{
        daß wohl jeder [Hunde füttern, die Hunger haben,] würde
        }
        \ex[]{
        daß wohl jeder [Hunde, die Hunger haben,] füttern würde
        }
        \ex[]{
        daß wohl jeder Hunde füttern würde, die Hunger haben
        }
        \zl
...
Abbildung~\ref{abb-seiner-tochter-erzaehlen} zeigt die Analyse von (\mex{1}).\todostefan{Abbildung floated eine Seite zu weit.}
\ea
Seiner Tochter erzählen wird er das Märchen.
\z
\pagebreak
\begin{figure}
\resizebox{0.9\textwidth}{!}{\begin{sideways}%
\begin{forest}
sm edges
[V\feattab{\type{fin},\\
           \comps \eliste,\\
  ...
  [{V[\begin{tabular}[t]{@{}l@{}}
     ...
     [V\feattab{
       \type{bse},\\ 
       \comps \sliste{ \ibox{3}, \ibox{5}, \ibox{4} } } [erzählen]]]
  [V\feattab{\type{fin},\\
              \comps \eliste,\\
              \textsc{slash} \sliste{ \ibox{1} } }
     [V\feattab{\type{fin},\\
                \comps \sliste{ \ibox{6} } }
       [{V[\comps \ibox{2} $\oplus$ \sliste{ \ibox{7} } ]} [wird]]]
     [\ibox{6} V\feattab{\type{fin},\\
                         \comps \eliste,\\
                         \textsc{slash} \sliste{ \ibox{1} } }
...
\z
Siehe hierzu auch die Diskussion von (\ref{bsp-non-complex-forming}) auf S.\,\pageref{bsp-non-complex-forming}.

Die Analyse in (\mex{1}) ist durch eine allgemeine Bedingung ausgeschlossen,
die Extraktionsspuren in Kopf"|positionen verbietet.
\ea[*]{
Müssen$_i$ wird$_j$ er ihr ein Märchen [[erzählen \_$_i$] \_$_j$].
}
\z
...
Abschnitt~\ref{sec-anhebung-anal}.
...
Für die Analyse von Sätzen wie (\ref{bsp-smvfb}) auf S.\,\pageref{bsp-smvfb} schlage ich in
\textcites{Mueller2005d}[Abschnitt~3.2.1]{MuellerGS}
eine Lexikonregel vor, die zur Verbbewegungsregel in (\ref{lr-verb-movement2})
völlig parallel ist. Frühere Vorschläge von mir zur Behandlung der 
...
%% zu analysieren. Für (\ref{ex-weil-es-ihr-jemand-zu-lesen-versprochen-hat-zwei}) -- hier
...
%% Domäne, \dash in die Domäne des Kopfes \emph{versprochen}, aufgenommen (siehe Abschnitt~\ref{sec-Reape-Linearisierung}
...
%% at \emph{scheint} (\citew[Kapitel~5.1]{Kathol98b}; \citew[Abschnitt~21.1]{Mueller99a}). 
...
%% cannot be explained in Reape's framework \citep[Abschnitt~5.2]{Kathol98b}.
%% \is{clause union|)}%
...
für solche Sätze aussehen, lässt er offen. Analysiert man Sätze wie (\mex{1}) mit total
...
ein Mittelfeld\is{Feld!Mittel-}, eine rechte Satzklammer\is{Satzklammer} und ein Nachfeld\is{Feld!Nach-}.
\ea
...
erklärt werden. Siehe auch \citew[\page82]{Reis80a} und Abschnitt~\ref{sec-topo-rekursion}.
...
dass sich in (\mex{1}) zwei unabhängige Konstituenten im Vorfeld befinden.\footnote{
  Siehe auch \citew[306]{Mueller2005d}.%
}
\eal 
\ex[]{
...
\zl
In Konstruktionen mit dem sogenannten Fernpassiv (siehe Abschnitt~\ref{sec-remote-passive-phen})
...
\zl
Auch der \emph{zu}"=Infinitiv kann einzeln vorangestellt werden, wie (\mex{1}) zeigt:
%\NOTE{JB: ungrammatisch}
\ea
...
Wissenschaftler*innen wie \citet[\page281]{Haider93a}, \citet[Abschnitt~4.2.5]{deKuthy2002a},
\citet[Abschnitt~2]{dKM2001a} und \citet{Fanselow2002a} haben Restbewegungsansätze wegen
verschiedener empirischer Punkte kritisiert, die Argumentkompositionsansätze wie der hier vorgestellte nicht haben.%

\citet[281]{Haider93a} vergleicht zum Beispiel (\mex{1}a) mit (\mex{1}b):
\eal
\ex[*]{
\label{ex-dass-wer-mit-was-wen-traktiert-hat}
dass wer [mit was]$_i$ wen \_$_i$ traktiert hat
}
\ex[]{
[ \_$_i$ \_$_j$ Traktiert]$_k$ hat wer [wen]$_i$ [mit was]$_j$ \_$_k$?
}
\zl
(\mex{0}a) zeigt, dass indefinite \emph{w}-Pronomina nicht gescramblet werden dürfen. Wenn nun aber
(\mex{0}b) mit Bezug auf Scrambling erklärt werden soll, dann müssen die \emph{w}-Phrasen aus der VP
\emph{wen mit was traktiert} herausbewegt worden sein, bevor \emph{traktiert} ins Vorfeld gestellt
wurde. Da die \emph{w}"=Phrasen aber nicht umgestellt werden können, wie (\mex{0}a) zeigt, kann
(\mex{0}b) nicht abgeleitet werden.

\citet{GMueller2014a-u} greift die Kritik auf und weist darauf hin, dass die Sätze in (\mex{1}b,c)
und (\mex{1}d) keine Minimalpaare sind. 
\eal
\judgewidth{??*}
\ex[]{
dass der Karl der Maria was$_1$ gegeben hat
}
\ex[??]{
dass der Karl was$_1$ der Maria \_$_1$ gegeben hat
}
\ex[??*]{
dass was$_1$ der Karl der Maria \_$_1$ gegeben hat
}
\ex[]{
{}[\sub{VP2} \_$_1$ Gegeben] hat er ihr was$_1$ \_$_2$.
}
\zl
Kombiniert man die Voranstellungen der \emph{w}"=Elemente mit der Restbewegung bekommt man dieselben
Abweichungen wie bei Voranstellung im Mittelfeld (Scrambling):
\eal
\judgewidth{??*}
\ex[??]{
{}[\sub{VP2} \_$_1$ Gegeben] hat Karl was$_1$ der Maria \_$_2$.
}
\ex[??*]{
{}[\sub{VP2} \_$_1$ Gegeben] hat was$_1$ der Karl der Maria \_$_2$.
}
\zl
Müller erklärt das damit, dass für die Leerung der VP für die nachfolgende Voranstellung nur extrem
kurzes, so genanntes \emph{string vacuous movement} nötig ist. Die Zwischenschritte sind in
(\mex{1}) zu sehen:
\eal
\ex[]{
{} er ihr [\sub{VP2} was$_1$ gegeben] hat
}
\ex[]{
{} er ihr was$_1$ [\sub{VP2} \_$_1$ gegeben] hat
}
\ex[]{
{} hat er ihr was$_1$ [\sub{VP2} \_$_1$ gegeben]
}
\ex[]{
{}[\sub{VP2} \_$_1$ Gegeben] hat er ihr was$_1$ \_$_2$.
}
\zl
Zuerst wird das \emph{was} aus der VP bewegt und landet direkt neben ihr. In einer Position, in der
überhaupt nicht zu sehen ist, dass \emph{was} bewegt wurde. Dann wird das finite Verb vorangestellt
und dann in einem letzten Schritt die entleerte VP. Gereon Müller geht auch andere Kritikpunkte mit
dem Verweis auf extrem lokale Bewegung (``extremly local string-vacuous movement'')
durch. Betrachtet man nun aber das Beispiel von \citet[281]{Haider93a}, das oben angegeben wurde,
kann man feststellen, dass dieses Beispiel nicht string vacous erklärt werden kann.
\eal
\ex[]{
[[wen]$_i$ [mit was]$_j$ traktiert] hat
}
\ex[]{
[[mit was]$_j$ [[wen]$_i$ \_$_j$  traktiert]] hat
}
\ex[]{
[[wen]$_i$ [[mit was]$_j$ [ \_$_i$ \_$_j$  traktiert]]] hat
}
\ex[]{
[wer [[wen]$_i$ [[mit was]$_j$ [ \_$_i$ \_$_j$  traktiert]]]] hat
}
\ex[]{
[ \_$_i$ \_$_j$ Traktiert]$_k$ hat [wer [[wen]$_i$ [[mit was]$_j$ \_$_k$]]]?
}
\zl
Da Bewegung immer so erfolgt, dass ein herausgestelltes Element links oder rechts an einen
bestehenden Baum adjungiert wird, muss die Ableitung in dieser Reihenfolge erfolgen, denn würde man
zuerst \emph{wen} string vacuous bewegen, bekäme man aus (\mex{0}a) dann (\mex{1}):
\ea[]{
[wen]$_i$ [\_$_i$ [mit was]$_j$  traktiert] hat
}
\z
Die Reihenfolge in (\mex{-1}c) könnte man daraus aber nicht mehr erzeugen, weil \emph{mit was} bei Bewegung ganz links
angebunden werden muss und somit vor \emph{wen} landen würde. Das bedeutet nun aber, dass die
Bewegung in (\mex{-1}b) überhaupt nicht string vacuous ist. \emph{mit was} muss über \emph{wen}
hinwegbewegt werden, auch wenn das Ergebnis nach der Bewegung wieder so aussieht wie vorher. Da das
\emph{wen} nicht gezwungen werden kann, sich weiterzubewegen, würde man erwarten, dass (\mex{-1}b)
in Sätzen direkt verwendet werden kann. Dass dem nicht so ist, zeigt Haiders Beispiel (\ref{ex-dass-wer-mit-was-wen-traktiert-hat}).
\is{Verbalkomplex|)}
...
worden. \citeauthor{HN89a}s Arbeiten gehen auf Arbeiten im Rahmen der
...
Persischen\il{Persisch}, Französischen\il{Französisch}, Italienischen\il{Italienisch},
Spanischen\il{Spanisch}, Portugiesischen\il{Portugiesisch}, Koreanischen\il{Koreanisch} und
Deutschen bieten \citet{GS2021a}. 
}