...
Wie (\mex{1}) zeigt, muß bei einigen Nomina bei der Pluralbildung zusätzlich noch eine Stammumlautung erfolgen:
\eal
...
\noindent
Betrachtet man ein anderes Verb wie \zb \stem{lieb}, stellt man fest, daß die
...
und (\mex{0}) noch ein zusätzliches \emph{e}. Man könnte jetzt annehmen, daß \stem{red}
einfach ein Verb ist, daß in der zweiten Person Singular die Endung \suffix{est} haben
...
und wird wie folgt aufgeschrieben:\footnote{
  Zum Beispiel \citet[\page183]{Eisenberg98a} weist darauf hin, daß diese Regel nicht
...
\z
In (\mex{0}) steht das `+' für die Morphemgrenze. Die Regel besagt, daß die Morphemgrenze
...
Da man diese auch negieren kann, wie (\mex{1}) zeigt, muß sich dieser Zeitbezug auch in der
...
nahe:\is{Kongruenz|(}\NOTE{FB: list-of... muß man erklären}
...
%                                                        subj  & \eliste \\
                                                  subcat  \type{list\_of\_not\_np\_str} $\oplus$ \liste{ NP[\str]$_{2,sg}$ } $\oplus$ \etag\\
                                                } \\
                             cont  \ms[present]{
                                      soa & \ibox{4}\\
%                                       arg2 & \ms[t-overlap]{
...
lex-dtr  \onems[stem]{
           phon   \ibox{1} \\
           synsem$|$loc \ms{ cat  & \ibox{2} \ms{ head  & \textit{verb\/}\\
                                                }\\
                             cont & \ibox{4} \\
                         }\\
           }\\
...
In der Lexikonregel in (\mex{0}) wird verlangt, daß das erste Element in der \subcatl des Verbs mit
...
%% so organisieren, daß alle Flexionsformen (das gesamte Paradigma\is{Paradigma}) am selben Ort spezifiziert
...
%% %                                                     subj  & \eliste \\
%%                                                        } \\ 
%%                                        subcat & \textrm{\ibox{3} (\liste{ NP[\str]$_{\ibox{4},\ibox{5}}$ } $\oplus$ \etag)}\\
%%                                      } \\
%% %%                              cont & \ms[present]{
%% %%                                       soa & \ibox{4}\\
%% %% %                                       arg2 & \ms[t-overlap]{
...
%%                 num  & \ibox{5}\\
%%                 }\\
%%            synsem$|$loc$|$cat  \ms{ head  & \textit{verb\/}\\
%%                                         subcat & \ibox{3}\\
%%                                       }\\
%% %                             cont & \ibox{4} \\
%%            }\\
%% }\is{Lexikonregel!Verbflexion}
%% \zs
%% Durch die Typbeschränkung in (\mex{-1}) ist sichergestellt, daß die Werte der Merkmale unter \textsc{infl}
...
%% -- hier als (\mex{1}c) wiederholt -- kann mit der Regel in (\mex{0}) nicht analysiert werden. Das liegt daran, daß
...
%% \zl
%%
%% Damit die Regel allgemein verwendbar ist, muß man sie wie folgt spezifizieren:
%% \eas
%% \label{lr-verbal-inflection}
...
%%                 num  & \ibox{5}\\
%%                 }\\
%%            synsem$|$loc \ms{ cat  & \ms{ head  & \textit{verb\/}\\
%%                                         subcat & \ibox{3}\\
%%                                       }\\
%%                              cont & \ibox{4} \\
%%                            }\\
...
%% daß Formen, die normalerweise zu einem Paradigma gehören, nicht auf"|treten. Beispiele sind
...
%% spezifiziert. Dadurch daß man \emph{Leute} mit einem \numw \type{pl} ins Lexikon schreibt, kann
%% kein Singular"=Affix mit dem Nomen kombiniert werden. Nur ein Plural"=Affix wie \zb \suffix{en}
%% ist mit dem Stamm \stem{Leut} kombinierbar, so daß dann die Formen \emph{Leute} und \emph{Leuten}
...
Man sagt, daß ein Muster produktiv\is{Produktivität} ist, wenn es auch auf neu in eine Sprache
...
lex-dtr & \onems[stem]{
           ...
           synsem$|$loc \ms{ cat  & \ms{ head   & \ms[verb]{
                                         ...
                                         subcat & \sliste{ \ibox{5} NP[\str], \ibox{2} NP[\str] } $\oplus$ \ibox{3}\\
                                       } \\
                             cont & \ibox{4} \\
                          }\\
           }\\
}\is{Lexikonregel!bar-Derivation@{\em -bar}-Derivation}%
}
\zs
\vspace{-\baselineskip}\end{figure}
Eingabe der Regel ist ein transitives Verb, \dash ein Verb mit zwei Nominalphrasen mit strukturellem Kasus.
Die erste NP -- die auch das designierte Argument sein muß \iboxb{5} -- wird wie bei der Passivierung unterdrückt, 
...
cat & \ms{ head   & \ms[verb]{
           ...
           subcat & \liste{ \ibox{1} NP[\str]\ind{2}, NP[\str]\ind{3}} \\
         }\\
...
\label{le-fahrbar}
\ms{
cat & \ms{ head & \ms[adj]{ subj & \sliste{ NP[\textit{str\/}]\ind{3}} \\
                           } \\
           subcat & \eliste \\
         }\\
cont & \ms[modal-op]{
...
flektiert werden muß. In (\mex{1}b) liegt die maskuline Nominativ"=Singular"=Form vor.
\eal
...
in Abbildung~\vref{structures-pv}.\footnote{
  Bierwisch nimmt eine morphembasierte Analyse an, wir haben dagegen im vorigen
...
&\rnode{v1f}{V}\\[2ex]
\rnode{pf}{P}   &                  & \rnode{v2f}{V en}\\[2ex]
\rnode{anf}{auf} & & \rnode{v3f}{V}\\[2ex]
              & &\rnode{hoerf}{hör}\\
\end{tabular}
\ncline{v1f}{pf}\ncline{v1f}{v2f}%
\ncline{v2f}{v3f}%
...
%\nodemargin5pt
\centerline{%
a. \begin{tabular}[t]{cccccccc}
&\rnode{v1}{V}\\[2ex]
\rnode{p}{P}   &                  & \rnode{v2}{V}\\[2ex]
\rnode{an}{auf} & \rnode{v3}{V}     &                     & \rnode{en}{en}\\[2ex]
              & \rnode{hoer}{hör}\\
\end{tabular}
\ncline{v1}{p}\ncline{v1}{v2}
\ncline{v2}{v3}\ncline{v2}{en}
\ncline{p}{an}\ncline{v3}{hoer}
\hspace{2.5cm}b. \begin{tabular}[t]{cccccccc}
              &              & \rnode{v1r}{V}\\[2ex]
              & \rnode{v2r}{V} &                      & \rnode{enr}{en}\\[2ex]
\rnode{pr}{P}   &              & \rnode{v3r}{V}\\[2ex]
\rnode{anr}{auf} &              & \rnode{hoerr}{hör}\\
\end{tabular}
\ncline{v1r}{v2r}\ncline{v1r}{enr}%
\ncline{v2r}{pr}\ncline{v2r}{v3r}%
\ncline{pr}{anr}\ncline{v3r}{hoerr}%
}
\caption{Alternative Strukturen für \textit{aufhören\/}}
\label{structures-pv}
\end{figure}
%
...
In \citew{Mueller2002b} habe ich dargelegt, daß diese Beispiele nicht problematisch
...
des Verbstammes, die Partikel muß links von \prefix{ge} stehen.
...
%\nodemargin5pt
\centerline{%
a. %\resizebox{0.43\linewidth}{!}{
\begin{tabular}[t]{@{}ccccc@{}}
&\rnode{n1}{N}\\[2ex]
\rnode{p}{P}   &                  & \rnode{n2}{N}\\[2ex]
\rnode{herum}{herum} & \rnode{v3}{V}     &                     & \rnode{gee}{\gee}\\[2ex]
              & \rnode{renn}{renn}\\
\end{tabular}
\ncline{n1}{p}\ncline{n1}{n2}%
\ncline{n2}{v3}\ncline{n2}{gee}%
\ncline{p}{herum}\ncline{v3}{renn}%
%}
\hspace{0.15\linewidth}b. %\resizebox{0.43\linewidth}{!}{
\begin{tabular}[t]{@{}ccccc@{}}
              &              & \rnode{n1r}{N}\\[2ex]
              & \rnode{v2r}{V} &                      & \rnode{geer}{\gee}\\[2ex]
\rnode{pr}{P}   &              & \rnode{v3r}{V}\\[2ex]
\rnode{herumr}{herum} &              & \rnode{rennr}{renn}\\
\end{tabular}
\ncline{n1r}{v2r}\ncline{n1r}{geer}%
\ncline{v2r}{pr}\ncline{v2r}{v3r}%
\ncline{pr}{herumr}\ncline{v3r}{rennr}%
...
es so zu sein, daß man eigentlich beide Strukturen in Abbildung~\ref{structures-anfahrbar}
braucht: Man möchte, daß das Affix wie bei der \geen direkt mit dem Verbstamm
...
a. \begin{tabular}[t]{cccccccc}
&\rnode{a1}{A}\\[2ex]
\rnode{p}{P}   &                  & \rnode{a2}{A}\\[2ex]
\rnode{an}{an} & \rnode{v3}{V}     &                     & \rnode{bar}{bar}\\[2ex]
              & \rnode{fahr}{fahr}\\
\end{tabular}
\ncline{a1}{p}\ncline{a1}{a2}
\ncline{a2}{v3}\ncline{a2}{bar}
\ncline{p}{an}\ncline{v3}{fahr}
\hspace{2cm}b. \begin{tabular}[t]{cccccccc}
              &              & \rnode{a1r}{A}\\[2ex]
              & \rnode{v2r}{V} &                      & \rnode{barr}{bar}\\[2ex]
\rnode{pr}{P}   &              & \rnode{v3r}{V}\\[2ex]
\rnode{anr}{an} &              & \rnode{fahrr}{fahr}\\
\end{tabular}
\ncline{a1r}{v2r}\ncline{a1r}{barr}%
\ncline{v2r}{pr}\ncline{v2r}{v3r}%
\ncline{pr}{anr}\ncline{v3r}{fahrr}%
...
\end{figure}
Das heißt, daß man sowohl aus Skopusgründen\is{Skopus} als
...
soll gezeigt werden, daß dies nicht nötig ist und daß man aufbauend auf der
...
%                             subj  & \eliste \\
           ...
           subcat & \liste{NP[\textit{str\/}]$_{\ibox{1}~2, sg}$} \\
         }\\
cont & \ms[present]{
...
\ibox{2} Part[\begin{tabular}[t]{@{}l@{}}
              ...
              \cont \ibox{5} \relation{beginnen}\iboxb{4}]\\
              \end{tabular}}
& \rnode{prfaehrst}{V[\subcat \sliste{ \ldots{} \ibox{2} }, \cont \ibox{1} \emph{present}\iboxb{5}]}\\[3ex]
& \hspace{14ex}Flexions-LR\\[4ex]
\rnode{stringran}{los} & \rnode{prlach2}{V[\begin{tabular}[t]{@{}l@{}}
                \subcat \liste{ \ldots{} \ibox{2} Part[\textsc{mod} \ibox{3}, \cont \ibox{5}] },\\
                \cont  \ibox{5}]\\
               \end{tabular}}\\
\\[2ex]
&\hspace{8ex}PV LR\\[4ex]
&\rnode{lach2}{\ibox{3} V[\cont \ibox{4} \emph{lachen}\iboxb{6}]}\\[3ex] 
&\rnode{stringrlach2}{lach}\\
\end{tabular}
\ncline{anfaehrst}{an}\ncline{anfaehrst}{prfaehrst}%
\ncline{prfaehrst}{prlach2}%
\ncline{prlach2}{lach2}%
\ncline{lach2}{stringrlach2}%
\ncline{an}{stringran}%
}}
...
\citet{Geilfuss-Wolfgang98a}\iawrong{Geilfuss-Wolfgang} develops an OT\is{Optimality Theory (OT)} 
...
{\em ge-\/} and {\em -t\/} are attached to the stem by an inflectional lexical rule
...
cat & \onems{% head \ms[verb]{ vform & fin\\ } \\
           subcat \begin{tabular}[t]{@{}l@{}}
                  (\ibox{1} \liste{NP[\textit{str\/}]$_{\ibox{2}~2, sg}$}) $\oplus$ \ibox{3} $\oplus$ \ibox{4} $\oplus$ \\[2mm]
...
}\is{Lexikonregel!ge- -e Nominalisierung@{\em Ge- -e}-Nominalisierung}
\zs
...
muß flektiert werden, damit er in der Syntax benutzt werden kann.\footnote{
        Siehe auch \citet[\page118]{Koenig99a}\iaright{Koenig} für einen
...
erfaßt wird, daß der Numerus"=Wert\is{Numerus} in der Ausgabe der Lexikonregel ebenfalls
...
\ibox{2} Part[\begin{tabular}[t]{@{}l@{}}
              \textsc{mod} \ibox{3} [\cont \ibox{4} ]\\
              \cont \ibox{5} \relation{ziellos}\iboxb{4}]\\
              \end{tabular}}
& \rnode{prfaehrst}{N[\subcat \sliste{ \ldots{} \ibox{2} }, \cont \ibox{1} \ldots{} \relation{wiederholtesrereignis}\iboxb{5}]}\\[3ex]
& \hspace{27ex}\gee-Nominalisierungs-LR\\[4ex]
\rnode{stringran}{herum} & \rnode{prfahr2}{V[\begin{tabular}[t]{@{}l@{}}
                \subcat \sliste{ \ldots{} \ibox{2} Part[\textsc{mod} \ibox{3}, \cont \ibox{5}] },\\
                \cont  \ibox{5}]\\
               \end{tabular}}\\
\\[2ex]
&\hspace{8ex}PV LR\\[4ex]
&\rnode{fahr2}{\ibox{3} V[\cont \ibox{4} \relation{rennen}\iboxb{6}]}\\[3ex] 
&\rnode{stringrfahr2}{renn}\\
\end{tabular}
\ncline{anfaehrst}{an}\ncline{anfaehrst}{prfaehrst}%
\ncline{prfaehrst}{prfahr2}%
\ncline{prfahr2}{fahr2}%
\ncline{fahr2}{stringrfahr2}%
\ncline{an}{stringran}%
}
\caption{Analyse von \emph{Herumgerenne}}\label{fig-herumgerenne}
\end{figure}
%
Um \emph{Herumgerenne} abzuleiten, muß zuerst die Lexikonregel für produktive Partikelverbbildungen
...
\mbox{{\em renn-\/} (mit Selektion einer Partikel):}\\
%\resizebox{\linewidth}{!}{%
%\scalebox{0.9}{%
\onems{
cat \ms{ %head & verb \\
...
\mbox{{\em gerenne-\/} (mit Selektion einer Partikel):}\\
\resizebox{\linewidth}{!}{%
\onems{
cat \onems{ head \textit{noun} \\
            spr \sliste{ Det }\\
           subcat \sliste{ \usebox{\boxxcompsechs}} \\
         }\\
...
%\nodemargin5pt%
\begin{tabular}[t]{@{}c@{}}
\rnode{prfahr}{V[\subcat \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3} $\oplus$ \sliste{ Part[\subj \ibox{2}, \subcat \ibox{3}] }]}\\[3ex]
\hspace{8ex}PV LR\\[3ex]
\rnode{fahr}{V[\subcat \ibox{1} \sliste{ NP[\str] }]}\\[3ex] 
\rnode{stringrfahr}{fahr}\\
\end{tabular}
\ncline{prfahr}{fahr}%
\ncline{fahr}{stringrfahr}%
}%
...
\subcatw. Der letztendliche Wert wird durch die Partikel bestimmt, sobald die Partikel
...
%\nodemargin5pt%
\begin{tabular}[t]{@{}c@{}}
\rnode{prfahrbar}{Adj[\subj \sliste{ \ibox{4} }, \subcat \ibox{5} $\oplus$ \sliste{ Part[\subj \ibox{2}, \subcat \ibox{3}] }]}\\[3ex]
\hspace{19ex}\bard LR\\[4ex]
\rnode{prfahr2}{V[\begin{tabular}[t]{@{}l@{}}
                \subcat \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3} $\oplus$
                \sliste{ Part[\subj \ibox{2}, \subcat \ibox{3}] }]\\
                $\wedge$ \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3} = \sliste{ NP[\str], \ibox{4} NP[\str] } $\oplus$ \ibox{5}
               \end{tabular}}\\
\\[2ex]
\hspace{8ex}PV LR\\[4ex]
\rnode{fahr2}{V[\subcat \ibox{1} \sliste{ NP[\str] }]}\\[3ex] 
\rnode{stringrfahr2}{fahr}\\
\end{tabular}
\ncline{prfahrbar}{prfahr2}%
\ncline{prfahr2}{fahr2}%
\ncline{fahr2}{stringrfahr2}%
}%
\caption{Anwendung der Lexikonregel für die \bard auf \stem{fahr} mit selegierter Partikel}\label{fig-p-fahrbar-val}
\end{figure}
Der \subcatw des Eingabezeichens muß eine Liste sein, die mit zwei Nominalphrasen mit strukturellem Kasus
beginnt (\sliste{ NP[\str], \ibox{4} NP[\str] } $\oplus$ \ibox{5}\,).
Da der \subcatw des Eingabezeichens für die \bard in Abbildung~\ref{fig-p-fahrbar-val} die
Verknüpfung der \subcatl des Basisverbs mit dem \subjw und dem \subcatw der selegierten Partikel ist, können nur Partikeln, die eine
NP[\str] in ihrer \subjl oder in ihrer \subcatl haben, mit dem Ergebnis der \bard kombiniert werden.

Abbildung~\vref{fig-anfahrbar-val} zeigt die Kombination von {\em an}$_5$ und \emph{fahrbar}.\footnote{
...
\begin{tabular}[t]{@{}cc@{}}
\multicolumn{2}{@{}c@{}}{
\rnode{anfahrbar}{Adj[\subj \sliste{ \ibox{4} }, \subcat \ibox{5} \eliste]}}\\
\\[1ex]
\hspace{7ex}CL & H\\
\\[2ex]
\rnode{an}{
\ibox{6} Part[\begin{tabular}[t]{@{}l@{}}
              \subj \ibox{2} \sliste{ NP[\str] },\\
              \subcat \ibox{3} \eliste ]\\
               \end{tabular}}
& \rnode{prfahrbar}{Adj[\subj \sliste{ \ibox{4} }, \subcat \ibox{5} $\oplus$ \sliste{ \ibox{6} }]}\\[3ex]
& \hspace{19ex}\bard LR\\[4ex]
\rnode{stringran}{an} & \rnode{prfahr2}{V[\begin{tabular}[t]{@{}l@{}}
                \subcat \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3} $\oplus$
                \sliste{ Part[\subj \ibox{2}, \subcat \ibox{3}] }]\\
                $\wedge$ \ibox{1} $\oplus$ \ibox{2} $\oplus$ \ibox{3} = \sliste{ NP[\str], \ibox{4} NP[\str] } $\oplus$ \ibox{5}\\
               \end{tabular}}\\
...
\end{figure}
Die Partikel hat eine NP[\str] in ihrer \subjl \iboxb{2}. Der \subcatw der Partikel \iboxb{3} ist die leere Liste.
...
\begin{tabular}[t]{@{}cc@{}}
\multicolumn{2}{c}{
\rnode{anfahrbar}{Adj[\cont \ibox{1}]}}\\
\\[1ex]
\hspace{7ex}CL & H\\
\\[2ex]
\rnode{an}{
\ibox{2} Part[\begin{tabular}[t]{@{}l@{}}
              \textsc{mod} \ibox{3} [\cont \ibox{4} ]\\
              \cont \ibox{5} \relation{gerichtetrauf}(\,\ibox{4}, \ibox{6}\,)]\\
              \end{tabular}}
& \rnode{prfahrbar}{Adj[\subcat \sliste{ \ibox{2} }, \cont \ibox{1} \emph{modal-op}(\,\ibox{5}\,)]}\\[3ex]
& \hspace{19ex}\bard LR\\[4ex]
\rnode{stringran}{an} & \rnode{prfahr2}{V[\begin{tabular}[t]{@{}l@{}}
                \subcat \sliste{ \ldots{} \ibox{2} Part[\textsc{mod} \ibox{3}, \cont \ibox{5}] },\\
                \cont  \ibox{5}]\\
...
\\[2ex]
&\hspace{8ex}PV LR\\[4ex]
&\rnode{fahr2}{\ibox{3} V[\cont \ibox{4} \emph{fahren}\iboxb{7}]}\\[3ex] 
&\rnode{stringrfahr2}{fahr}\\
\end{tabular}
\ncline{anfahrbar}{an}\ncline{anfahrbar}{prfahrbar}%
\ncline{prfahrbar}{prfahr2}%
...
\caption{Kombination von {\em an}$_5$ und {\em fahrbar} (Semantik)}\label{fig-anfahrbar-sem}
\end{figure}
Die Partikelverblexikonregel führt eine Partikel in die \subcatl ein,
die die Eingaberepräsentation über \textsc{mod} selegiert \iboxb{3}. In der Ausgabe der
Lexikonregel wird der \contw \iboxb{5} mit dem \contw der Partikel in der \subcatl identifiziert.
...
des Stammes \stem{fahr} identifiziert, da das in der \subcatl des abgeleiteten
...
%
Erst nach der Kombination von \emph{an} und {\em fahrbar} ist klar, wie der Wert von \iboxt{5}
...
\onems{
cat \ms{ %head & verb \\
           subcat & \textrm{(\,\ibox{1} \liste{NP[\str]\ind{2}})} $\oplus$ \ibox{3} $\oplus$ \ibox{4} $\oplus$\\[3mm]
...
\liste{ \onems{ l \onems{ c \onems{ h \onems[particle]{ mod$|$l \onems{
                                                                                            c \ms{ head & verb \\
                                                                                                       subcat & \ibox{1'} \liste{ NP[\str]\ind{2'}} \\
                                                                                                     }\\
                                                                                            cont \ms[fahren]{ agens & \ibox{2'} \\
                                                                                                            } \\
                                                                                            }\\
...
\onems{
c \ms{  head  & verb \\
           subcat & $\ibox{1'} \oplus \ibox{3'} \oplus \ibox{4'}~\oplus$  \\
...
besagt, daß die Valenzliste des Partikelverbs (\,\ibox{1'} $\oplus$ \ibox{3'} $\oplus$ \ibox{4'}\,) 
in zwei Listen auf"|teilbar sein muß: eine Liste mit zwei NPen mit strukturellem Kasus und eine
Restliste \iboxb{3}. Die Restliste ergibt zusammen mit der Partikel die \subcatl des Mutterknotens.
...
Wenn die Struktur in (\mex{0}) mit der Partikel {\em an$_5$\/} in (\ref{le-an5}) auf Seite~\pageref{le-an5}
...
Da die \subcatl der Partikel leer ist, wird \iboxt{4'} durch die leere Liste instantiiert. 
...
ich kurz noch die Beispiele in (\mex{1}) diskutieren, die zeigen, daß Elemente, die von Partikelverben
...
\word{Herumgerede} mit \prefix{Pseudo} kombiniert. Man muß deshalb zulassen,
...
%% einteilen. Man kann dann sagen, daß Männer 
...
synsem$|$loc  \ms{ cat  & \ms{ head   & \ms[adj]{ 
                                        ...
                                        subcat & \ibox{3} $\oplus$ \sliste{ V[\subcat \liste{ NP[\str], \ibox{2} NP[\str] } $\oplus$ \ibox{3}]:\ibox{4}} \\
                                      } \\
                            cont & \ms[modal-op]{
                                    soa & \ibox{4}\\
                                   }\\
                         }\\
}}\is{Morphem!-{\em bar}-Derivation}%
\zs
...
von \stem{lös} und \suffix{bar} einen Stamm ergibt, da \emph{lösbar} noch flektiert werden muß, bevor es in
...
daß nur \type{synsem}"=Objekte selegiert werden, und die Typ"=Information \type{stem} bzw.\ \type{word} außerhalb
...
%% werden explizit beschrieben. Bei Riehemann wird für die Tochter verlangt, daß sie vom Typ \type{trans-verb}
%% ist.
...
%%            phon   \ibox{1} \\
%%            synsem$|$loc \ms{ cat  & \ms{ head   & \textit{verb}\\
%%                                          subcat & \sliste{ NP[\str], \ibox{2} NP[\str] } $\oplus$ \ibox{3}\\
%%                                        } \\
%%                              cont & \ibox{4} \\
%%                           }\\
%%            }\\
...
daß es Sprachen wie Portugiesisch\il{Portugiesisch} gibt, in denen mehrere Argumente eines Verbs
...
der semantische Beitrag muß also vom Flexionsaffix kommen.\is{Morphologie|)}

%% Man kann doch einfach sagen, daß Stämme oder Wörter vorn kein Plus haben dürfen. Das ist doch
...
%% dadurch ausgeschlossen, daß Wörter nie mit einem \phonliste{ + } anfangen dürfen (S.\,79).
...
%% }
%% \zl
%% Man könnte jetzt einwenden, daß man (\mex{0}b) durch eine weiter Beschränkung ausschließt,
%% die besagt, daß Präfixe nicht mit etwas kombiniert werden können, das mit einem `+' beginnt.
%% Diese Beschränkung läßt sich aber nicht ausdrücken, wenn man die Tatsache, daß es sich bei einem
%% Element um ein Präfix handelt, nicht explizit kenntlich gemacht hat. Im Lexikoneintrag für das Präfix selbst
%% könnte man natürlich verlangen, daß der Stamm mit dem das Präfix kombiniert wird, keinen \phonw
