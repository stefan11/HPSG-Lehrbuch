%% -*- coding:utf-8 -*-

\chapter{Lösungen zu den Übungsaufgaben}

\section{Einleitung}


\begin{enumerate}
% Auf S.\,\pageref{page-unendlich-viele-grammatiken} habe ich behauptet, dass
%      es unendlich viele Grammatiken gibt, die (\ref{bsp-weil-er-das-buch-dem-mann-gibt})
%      analysieren können.
%      Überlegen Sie sich, wieso diese Behauptung richtig ist.
\item Man kann zu jeder Grammatik zusätzliche Symbole und Regeln annehmen, die unnötige Struktur
  erzeugen oder einfach nie angewendet werden, weil es keine Wörter oder Phrasen gibt, die in der
  rechten Regelseite verwendet werden können. Wenn wir unserer Grammatik zum Beispiel die folgende
  Regel hinzufügen, haben wir eine komplexere Grammatik, die allerdings denselben Sprachausschnitt
  analysieren kann.
\ea
Trallala $\to$ Trulla Trollolo
\z
Siehe hierzu auch Abschnitt~\ref{sec-kriterien}.
% \item Stellen Sie Überlegungen dazu an, wie man ermitteln kann, welche der unendlich vielen Grammatiken
%       die beste ist bzw.\ welche der Grammatiken die besten sind.
\item Allgemein geht man davon aus, dass die Grammatik mit den wenigsten Regeln die beste ist. Somit
  kann man Grammatiken, die unnötige Regeln wie (\mex{0}) enthalten, ablehnen.

Allerdings muss man im Auge behalten, was das Ziel der Grammatiktheorie ist. Setzt man sich zum Ziel,
unser menschliches Sprachvermögen zu beschreiben, so kann eine Grammatik mit mehr Regeln die bessere
sein. Das liegt daran, dass psycholinguistische Forschungen gezeigt haben, dass hochfrequente
Einheiten einfach als ganze in unseren Köpfen abgespeichert sind und nicht jedes mal neu aus
Einzelteilen aufgebaut werden, obwohl wir natürlich prinzipiell in der Lage dazu wären. Zu Quellen
siehe S.\,\pageref{page-gespeicherte-Chunks}.

Damit die letzten beiden Sätze ausgeschlossen sind, muss die Grammatik Kasus- und Genusinformation
enthalten. Die folgende Grammatik leistet das Gewünschte:
\eal
\ex s $\to$ np(nom), v(nom\_dat), np(dat)
\ex s $\to$ np(nom), v(nom\_dat\_akk), np(dat), np(akk)
\ex s $\to$ np(nom), v(nom\_pp\_auf), pp(auf,akk)
%
\ex pp(Pform,Kas) $\to$ p(Pform,Kas), np(Kas)
\ex np(Kas) $\to$ d(Kas,Gen), n(Kas,Gen)
%
\ex v(nom\_dat) $\to$ hilft
\ex v(nom\_dat\_akk) $\to$ gibt
\ex v(nom\_pp\_auf) $\to$ wartet
%
\ex np(nom) $\to$ Conny
\ex np(nom) $\to$ er
\ex np(dat) $\to$ ihr
%
\ex d(nom,mas) $\to$ der
\ex d(dat,neu) $\to$ dem
\ex d(dat,mas) $\to$ dem
\ex d(akk,neu) $\to$ das
\ex d(akk,neu) $\to$ ein
%
\ex n(nom,mas) $\to$ Delphin
\ex n(dat,neu) $\to$ Kind
\ex n(akk,neu) $\to$ Buch
\ex n(akk,neu) $\to$ Wunder
%
\ex p(auf,akk) $\to$ auf
\zl



\end{enumerate}


\section{Der Formalismus}
\label{Abschnitt-Formalismus-Lösungen}\label{Seite-Lösungen-Listen}

\begin{enumerate}
\setcounter{enumi}{1}
\item Listen\is{Liste} kann man mittels rekursiver Strukturen beschreiben, die jeweils aus einem Listenanfang
  und einem Rest bestehen. Der Rest kann entweder eine nicht leere Liste (\type{ne\_list}) oder die
  leere Liste (\type{e\_list}) sein. Die Liste \relliste{ \type{a}, \type{b}, \type{c} } kann man wie folgt darstellen:
\ea
\scalebox{\scalefactor}{%
\ms[ne\_list]{
first & a\\
rest  & \ms[ne\_list]{
        first & b\\
        rest  & \ms[ne\_list]{
                first & c\\
                rest & e\_list\\
                }\\
        }\\
}}
\z
\item\label{page-difflists}
 Erweitert man die Datenstruktur in (\mex{0}) um zwei weitere Merkmale, kann man auf
  \emph{append} verzichten. Das Stichwort heißt \emph{Differenzliste}\is{Liste!Differenz-}. Eine Differenzliste besteht
  aus einer Liste und einem Zeiger auf das letzte Element der Liste. 
\ea
\scalebox{\scalefactor}{%
\ms[diff-list]{
list & \ms[ne\_list]{
       first & a\\
       rest  & \ms[ne\_list]{
               first & b\\
               rest  & \ibox{1} list\\
               }\\
       } \\
last & \ibox{1}\\
}}
\z

Im Gegensatz zur
  Listenrepräsentation in (\mex{-1}) ist jedoch der \textsc{rest}"=Wert des Listenendes nicht \type{e\_list}
  sondern einfach \type{list}. Es ist damit möglich, eine Liste zu verlängern, indem man an die
  Stelle, an der sie endet, eine andere Liste anfügt. Die Verkettung von (\mex{0}) und (\mex{1}a) ist (\mex{1}b).
\ea
\begin{tabular}{@{}l@{~}l@{\hspace{1cm}}l@{~}l}
a. & 
\scalebox{\scalefactor}{%
\ms[diff-list]{
list & \ms[ne\_list]{
       first & c\\
       rest  & \ibox{2} list\\
       } \\
last & \ibox{2}\\
}} & b. & 
\scalebox{\scalefactor}{%
\ms[diff-list]{
list & \ms[ne\_list]{
       first & a\\
       rest  & \ms[ne\_list]{
               first & b\\
               rest  & \ms[ne\_list]{
                       first & c\\
                       rest  & \ibox{2} list\\
                       }\\
               }\\
       } \\
last & \ibox{2}\\
}}
\end{tabular}
\z
Für die Verkettung der Listen muss lediglich der \textsc{list}"=Wert der zweiten Liste mit dem \textsc{last}"=Wert der ersten Liste identifiziert werden. Der \textsc{last}"=Wert der Ergebnisliste
entspricht dem \textsc{last}"=Wert der zweiten Liste (im Beispiel \ibox{2}\,).

Information zur Differenzlistenkodierung lässt sich \zb mit einer Suchanfrage mit den Stichwörtern \emph{list} \emph{append} und
\emph{feature structure} finden. Bei dieser Suchanfrage bekommt man Grammatikentwicklungsseiten, die
Differenzlisten erklären.
\end{enumerate}



\section{Valenz und Grammatikregeln}

      \eal
      \ex er: \eliste
      \ex ihre (in \emph{ihre Ankündigung}): \eliste
      \ex schnarcht: \sliste{ NP[\type{nom}] } 
      \ex denkt: \sliste{ NP[\type{nom}], (PP[\type{an},\type{akk}]) } $\vee$ \sliste{
        NP[\type{nom}], (CP[\type{dass}]) }
% wenn das semantisch zwei verschiedene denken sind, dann sollten beide Objekte optional sein.
      \ex graut: \sliste{ (NP[\type{es}]), NP[\type{dat}], (PP[\type{vor}, \type{dat}]) }
      \zl


\section{Dominanzstrukturen und Prinzipien}

\begin{enumerate}
\item 
% Zeichnen Sie einen Syntaxbaum für (\mex{1}):
% \ea
% dass der Mann das spannende Buch liest
% \z
% Markieren Sie die Kanten im Baum mit Ad für Adjunkt, Ar für Argument und
% H für Kopf.

Die Struktur von (\mex{1}) ist in Abbildung~\ref{fig-dass-der-mann} zu sehen. 
\ea
dass das Kind das spannende Buch liest
\z
Dabei steht \nbar für
ein Nomen, das einen Determinator verlangt (siehe S.\,\pageref{ex-abkuerzung-nbar}), V steht für ein lexikalisches
Verb, \vbar für eine ungesättigte und VP für eine gesättigte Verbalprojektion.
\begin{figure}
\scalebox{\scalefactor}{%
\begin{forest}
sm edges
[CP
  [C [dass]]
  [VP 
    [NP 
      [Det [das]]
      [\nbar [Kind]]]
    [\vbar
      [NP
        [Det [das]]
        [\nbar 
          [Adj [spannende]]
          [\nbar [Buch]]]]
      [V [liest]]]]] 
\end{forest}}
\caption{\label{fig-dass-der-mann}Syntaxstruktur von \emph{dass das Kind das spannende Buch liest}}
\end{figure}
\todostefan{Label H Arg Adj fehlen noch}
\item 
\largerpage[2]
\eanoraggedright
\scalebox{.85}{%
\onems[head-complement-phrase~]{
      phon  \phonliste{ das, Kind, schläft }\\
      head  \ibox{1}\\
      spr   \ibox{2} \eliste\\
      comps \ibox{3} \eliste \\
      head-dtr \onems[word]{ phon \phonliste{ schläft }\\
                                     head \ibox{1} \ms[verb]{vform & fin \\
                                                            }\\
                                     spr \ibox{2} \eliste\\
                                     comps \ibox{3}  $\oplus$ \sliste{ \ibox{4} } \\
                       } \\
      non-head-dtrs \liste{ \ibox{4} \onems[head-specifier-phrase~]{ 
                                        phon \phonliste{ das, Kind }\\
                                        head \ibox{5} \ms[noun]{ cas & \ibox{6} nom\\
                                                      } \\
                                        spr   \eliste\\
                                        comps   \eliste \\
                                        head-dtr \onems[word]{ 
                                                  phon \phonliste{ Kind }\\
                                                  head \ibox{5} \\
                                                  spr  \sliste{ \ibox{7} } \\
                                                  comps \eliste }\\
                                        non-head-dtrs \liste{ \ibox{7} \onems[word]{ 
                                                                phon \phonliste{ das }\\
                                                                head \ms[det]{
                                                                      cas & \ibox{6} \\ 
                                                                      }\\
                                                                spr \eliste\\
                                                                comps \eliste } }\\
                                     }
                       }\\
}}
\z
\end{enumerate}


\section{Semantik}

\begin{enumerate}

\item 
%% Wie kann man den semantischen Beitrag von \emph{lacht} repräsentieren? Geben Sie den
%%   Lexikoneintrag für \emph{lacht} an und berücksichtigen Sie das Linking.

\eas
%\label{le-lachen}%
\emph{lacht}:\\
\scalebox{.85}{%
\ms{
cat & \ms{ %head   & verb \\
           arg-st & \nliste{ NP[\str]\ind{1} } }\\[2mm]
cont & \ms{ ltop & \ibox{2}\\
            ind  & \ibox{3} event\\ 
            rels & \liste{ \ms[lachen]{ 
                   lbl  & \ibox{2} \\
                   arg0 & \ibox{3} \\
                   arg1 & \ibox{1} } }\\
            hcons & \eliste }}}
\zs
\largerpage[2]
\item

\ea
\scalebox{.85}{%
\onems[head-complement-phrase~]{
      phon  \phonliste{ er, lacht }\\
      cat \ms{ head  & \ibox{1}\\
               spr   & \ibox{2} \eliste\\
               comps & \ibox{3} \eliste \\
             }\\
      cont \ms{ ltop & \ibox{4}\\
                ind  & \ibox{5}\\
                rels & \liste{ \ms[lachen]{ 
                               lbl  & \ibox{4}\\
                               arg0 & \ibox{5}\\
                               arg1 & \ibox{6} } }\\
                hcons & \eliste } \\
      head-dtr \ms[word]{ phon & \phonliste{ lacht }\\
                          cat & \ms{ head & \ibox{1} \ms[verb]{vform & fin }\\
                                     spr   & \ibox{2} \eliste\\
                                     comps & \ibox{3}  $\oplus$ \sliste{ \ibox{7} } \\
                                   }\\
                          cont & \ms{ ltop & \ibox{4}\\
                                      ind  & \ibox{5} event\\
                                      rels & \liste{ \ms[lachen]{ 
                                                     lbl  & \ibox{4}\\
                                                     arg0 & \ibox{5}\\
                                                     arg1 & \ibox{6} } }\\
                                      hcons & \eliste } } \\
      non-head-dtrs \liste{ \ibox{7} \ms[word~]{ 
                                        phon & \phonliste{ er }\\
                                        cat & \ms{ head  & \ms[noun]{ cas & nom } \\
                                                   spr   & \eliste\\
                                                   comps & \eliste \\
                                                 }\\
                                        cont & \ms{ ind & \ibox{6} \ms{ per & 3\\
                                                                      num & sg\\
                                                                      gen & mas\\
                                                                    }\\
                                                    rels & \eliste\\
                                                    hcons & \eliste } } }
}}
\z

\item

\eas
\scalebox{\scalefactor}{%
\ms{
phon & \phonliste{ großem }\\
cat & \ms{ head & \ms[adj]{
                  mod & \ms{ cat & \ms{ head & noun\\
                                        spr  & \nliste{ Det }\\
                                        comps & \eliste }\\
                             cont & \ms{ %ltop & \ibox{1}\\
                                         ind  & \ibox{1}\\
                                       }}}\\
           spr & \eliste\\
           comps & \eliste\\
         }\\
cont & \ms{ ltop & \ibox{2}\\
            ind  & \ibox{3} property\\
            rels & \liste{ \ms[groß]{
                           lbl  & \ibox{2}\\ 
                           arg0 & \ibox{3}\\
                           arg1 & \ibox{1} \ms{ per & 3 \\
                                   num & sg \\
                                   gen & mas $\vee$ neu \\
                               }\\
                           } }\\
            hcons & \eliste\\
          }\\
}
}
\zs

\end{enumerate}



\section{Das Lexikon}

\begin{enumerate}
\item
\eas 
\scalebox{\scalefactor}{%
\ms{
cat & \ms{ head   & \ms[adj]{
                    mod & none\\
                    }\\
           arg-st & \sliste{ NP[\nom]\ind{1} } $\oplus$ \etag\\
         }\\
} $\mapsto$ \ms{
cat|head   & \ms[adj]{
              mod & \nbar\ind{1} } }}
\zs
Per Konvention werden alle in der Ausgabe nicht erwähnten Werte von der Eingabe zur Ausgabe
übertragen. Das heißt, dass der \argstw und der \contw unverändert übernommen werden. Wichtig ist
bei der Lexikonregel der \modw in der Ausgabe und das Linking des modifizierten Nomens an das erste
Element der \argstl. Da die \argstl und der \contw übernommen werden, ist auch in der Ausgabe der
Lexikonregel das modifizierte Nomen an das \argone gelinkt.

Der Kasus der NP im Eingabezeichen muss spezifiziert werden, da die Lexikonregel sonst auch auf
subjektlose Adjektive anwendbar wäre:
\eal
\ex[]{
Dem Mann ist schwindelig/schlecht.
}
\ex[*]{
der schwindelige/schlechte Mann
}
\zl
\emph{der schlechte Mann} ist zwar grammatisch, jedoch mit einer anderen Bedeutung als in (\mex{0}a).

Kasus, Numerus und Genus sind im Ausgabezeichen hier nicht spezifiziert. Welche Werte im
Ausgabezeichen dann stehen müssen, hängt von der gewählten Endung ab.

\end{enumerate}

\section{Ein topologisches Modell des deutschen Satzes}

\begin{enumerate}
\item \eal
\ex \field{Aicke}{VF} \field{isst}{LS}.
\ex \field{Der Delfin}{VF} \field{hilft}{LS} \field{dem Kind}{MF}, \field{\field{den}{VF} \field{jeder}{MF} \field{kennt}{RS}}{NF}.
\ex \field{Der Delfin}{VF} \field{hilft}{LS} \field{dem Kind, \field{das}{VF} \field{jeder}{MF} \field{kennt}{RS}}{MF}.
%\ex \field{Die Studenten}{VF} \field{behaupten}{LS}, \field{\field{nur wegen der Hitze}{MF} \field{einzuschlafen}{RS}}{MF}.
\ex \field{Die Kinder}{VF} \field{haben}{LS} \field{behauptet}{RS}, \field{\field{nur wegen der Hitze}{MF} \field{einzuschlafen}{RS}}{NF}.
\ex \field{\field{Dass}{LS} \field{Aicke nicht}{MF} \field{kommt}{RS}}{VF}, \field{ärgert}{LS} \field{Conny}{MF}.
\ex \field{\field{Einen Roman}{MF} \field{lesen}{RS}, \field{\field{der}{VF} \field{sie}{MF} \field{langweilt}{RS}}{NF}}{VF}, \field{würde}{LS} \field{sie nie}{MF}.
\zl
\medskip

Zu (\mex{0}c): Theoretisch könnte hier auch eine Extraposition des Relativsatzes ins Nachfeld
vorliegen. Da \emph{dem Kind, das jeder kennt} aber eine Konstituente ist, geht man davon aus, dass
keine Umstellung des Relativsatzes stattgefunden hat, sondern dass die einfachere Struktur mit
\emph{dem Kind, das jeder kennt} als kompletter NP im Mittelfeld vorliegt.

\end{enumerate}

\section{Konstituentenreihenfolge}

%% \begin{enumerate}
%% \item Skizzieren Sie die Analysebäume für folgende Sätze und erläutern Sie,
%%       wie die Unterschiede zwischen den Sätzen von der in diesem Kapitel
%%       vorgestellten Analyse erfaßt werden:
%% \eal
%% \ex dass Kurt-Martin drei Mark dem Clown gibt
%% \ex dass dem Clown Kurt-Martin drei Mark gibt 
%% \ex Gibt Kurt-Martin dem Clown drei Mark?
%% \zl

\begin{figure}[H]
\scalebox{\scalefactor}{%
\begin{forest}
sm edges
[{C[\comps \eliste ] }
   [{C[\comps \sliste{ \ibox{1} }]} [dass] ]
      [{\ibox{1} V[\comps \eliste ] }
        [{\ibox{2} NP[\type{nom}]} [Kurt-Martin] ]
          [{V[\comps \sliste{ \ibox{2} } ] } 
            [{\ibox{3} NP[\type{dat}]} [dem Clown, roof] ]
            [{V[\comps \sliste{ \ibox{2}, \ibox{3} } ] }
               [{\ibox{4} NP[\type{acc}]} [drei Mark, roof] ]
                      [{V[\comps \sliste{ \ibox{2}, \ibox{3}, \ibox{4} } ] } [gibt] ] ] ] ]
]
\end{forest}}
\end{figure}
\todostefan{add labels to lines: H Cl and so on}


\begin{figure}[H]
\scalebox{\scalefactor}{%
\begin{forest}
sm edges
[{C[\comps \eliste ] }
  [{C[\comps \sliste{ \ibox{1} } ] } [dass] ]
  [{\ibox{1} V[\comps \eliste ] }
    [{\ibox{3} NP[\type{dat}]} [dem Clown, roof] ]
    [{V[\comps \sliste{ \ibox{3} } ] } 
       [{\ibox{2} NP[\type{nom}]} [Kurt-Martin] ]
       [{V[\comps \sliste{ \ibox{2}, \ibox{3} } ] }
          [{\ibox{4} NP[\type{acc}]} [drei Mark,roof] ]
          [{V[\comps \sliste{ \ibox{2}, \ibox{3}, \ibox{4} } ] } [gibt] ] ] ] ] ]
\end{forest}}
\end{figure}

\begin{figure}[H]
%\oneline{%
\scalebox{\scalefactor}{%
\begin{forest}
sm edges
[{V[\comps \eliste ] }
       [{V[\comps \sliste{ \ibox{1} } ] }, l sep+=\baselineskip 
         [{V\ibox{5}[\comps \sliste{ \ibox{2}, \ibox{3}, \ibox{4} } ] }, tier=np, edge
         label={node[midway,right]{V1-LR}}
            [gibt] ] ]
       [{\ibox{1} V\feattab{\dsl \ibox{5},\\
                             \comps \eliste} }
          [{\ibox{2} NP[\type{nom}]}, tier=np [Kurt-Martin] ]
          [{V\feattab{\dsl \ibox{5},\\
                       \comps \sliste{ \ibox{2} } } } 
            [{\ibox{3} NP[\type{dat}]} [dem Clown,roof] ]
            [{V\feattab{\dsl \ibox{5},\\
                         \comps \sliste{ \ibox{2}, \ibox{3} } } }
              [{\ibox{4} NP[\type{acc}]} [drei Mark,roof] ]
              [{V\ibox{5}\feattab{\dsl \ibox{5},\\
                           \comps \sliste{ \ibox{2}, \ibox{3}, \ibox{4} } } } [\trace{}] ] ] ] ] ]
\end{forest}}
\end{figure}


\section{Nichtlokale Abhängigkeiten}

\begin{figure}[H]
%\oneline{%
\scalebox{\scalefactor}{%
\begin{forest}
sm edges
[{V\feattab{\comps \eliste,\\
                  \slasch \eliste } }, s sep+=2em
       [{NP\ibox{1}[\type{acc}]} [diese Wohnung,roof] ]
       [V\feattab{ \comps \eliste,\\
                   \slasch \sliste{ \ibox{1} } }  
         [{V[\comps \sliste{ \ibox{2} } ] }, l sep+=\baselineskip
           [{V\ibox{3}[\comps \sliste{ \ibox{4}, \ibox{5} } ] }, tier=np, edge label={node[midway,right]{V1-LR}} [mietet] ] ]
         [{\ibox{2} V\feattab{ \dsl \ibox{3},\\
                                \comps \eliste,\\
                                \slasch \sliste{ \ibox{1} } } } 
           [\ibox{5} NP\ibox{1}\feattab{ \slasch \sliste{ \ibox{1} } }, tier=np [\trace] ]
           [V\feattab{\dsl \ibox{3},\\
                      \comps \sliste{ \ibox{4}, \ibox{5} },\\
                              \slasch \eliste }
              [{\ibox{4} NP[\type{nom}]} [Kim] ]
              [V\ibox{3}\feattab{\dsl \ibox{3},\\
                                 \comps \sliste{ \ibox{4}, \ibox{5} },\\
                                 \slasch \eliste }  [\trace] ] ] ] ] ]
\end{forest}
}
\end{figure}



\section{Relativsätze}

\begin{enumerate}
\item (\mex{1}) gibt einige Beispiele für Relativwörter:
\eal
\ex Relativpronomina: der, die, das, denen, \ldots, deren, dessen
\ex Relativadverb: wo, woher, \ldots, wie 
\zl


%% \item Skizzieren Sie die Analyse für die folgende Phrase:
%% \ea
%% die Blume, die alle lieben
%% \z
%% Gehen Sie auf die \slashwe und die \relwe ein.
\item \begin{figure}[H]
\scalebox{\scalefactor}{%
\begin{forest}
sm edges
[NP
  [\ibox{1} Det [die] ]
  [{\nbar[\spr \sliste{ \ibox{1} }] } 
    [{\ibox{2} \nbar[\spr \sliste{ \ibox{1} }]\ind{3} } [Blume] ] 
    [RC\feattab{\textsc{mod} \ibox{2},\\
                \rel \eliste,\\
                \slasch \eliste }
      [NP~\ibox{4}\feattab{\type{acc},\\
                           \textsc{rel} \nliste{ [\textsc{ind} \ibox{3}] }} [die] ]
      [V\feattab{ \comps \eliste,\\
                  \slasch \sliste{ \ibox{4} }}
         [\ibox{5} NP\feattab{\HPSGloc \ibox{4},\\
                              \slasch \sliste{ \ibox{4} }} [\trace{}] ]
         [V\feattab{ \comps \sliste{ \ibox{5} },\\
                     \slasch \eliste }
           [{\ibox{6}~NP[\type{nom}]} [alle] ]
           [V\feattab{ \comps \sliste{ \ibox{6}, \ibox{5} },\\
                       \slasch \eliste }  [lieben] ] ] ] ] ] ]
\end{forest}}
\end{figure}
\end{enumerate}

% keine Fragen zu Lokalität
\stepcounter{section}

\section{Kongruenz}

\begin{enumerate}
\item Das Nomen \emph{Mädchen} muss syntaktisch mit dem Determinator im Genus kongruieren. Dieser
  muss Neutrum sein. Der semantische Index von \emph{Mädchen} kann dagegen Neutrum oder Femininum sein.
\end{enumerate}

\section{Kasus}

\eal
\ex Der Vogel (str) singt.  
\ex Mich (lex) friert.    
\ex Kim (str) zerstört das Auto (str).
\ex Das (str) dauert ein ganzes Jahr (lex = semantischer Kasus).
\ex Sie (str) hat nur einen Tag (str) dafür angegeben.
\ex Conny (str) denkt an den morgigen Tag (lex).
\zl

\section{Der Verbalkomplex}

\begin{figure}[H]
\oneline{
%\scalebox{\scalefactor}{%
\begin{forest}
sm edges
[{\ms[cat]{ head   & \ibox{1} \\
           comps & \eliste\\
            }}
 [{\ibox{2} NP[\type{nom}]} [Aicke]]
 [{\ms[cat]{ head   & \ibox{1} \\
             comps & \ibox{3}\\
            }}
   [\iboxt{5}~\onems{ loc \ms{ head & \ibox{4} \\
                               comps & \ibox{3} \\
                                 }}
     [\iboxt{6}~\onems{ loc \onems{ head  \ms[verb]{ vform & bse  \\
                                                }  \\
                                comps~\ibox{3} \nliste{ \ibox{2} NP[\nom] } \\
                              }\\
                } [singen]]
     [{\onems[cat]{ head~\ibox{4} \ms[verb]{ vform & bse \\
                            } \\
            comps ~ \ibox{3} $\oplus$ \sliste{ \ibox{6} } \\
          }} [können]]]
    [{\ms[cat]{ head & \ibox{1} \ms[verb]{ vform & fin \\
                                                    } \\
                      comps & \ibox{3} $\oplus$ \sliste{ \ibox{5} } \\
                    }} [muss]]]]
\end{forest}}
\end{figure}


\section{Kohärenz, Inkohärenz, Anhebung und Kontrolle}

\begin{enumerate}
\item
      \eal
      \ex scheinen (Anhebung)
      \ex anfangen (Anhebung, Phasenverb)
      \ex versprechen (zwei Varianten: Kontrolle und Anhebung)
      \ex drohen (zwei Varianten: Kontrolle und Anhebung)
      \ex verbinden (weder noch)
      \ex zwingen (Kontrolle)
      \ex hören (Anhebung)
      \zl

\item
\eanoraggedright
\oneline{%
%\scalebox{\scalefactor}{%
\onems{
\ldots \ms{ cat & \ms{% head   & verb \\ 
                     arg-st & \ibox{1} $\oplus$ \sliste{ NP[\type{ldat}]\ind{2} } $\oplus$ \ibox{3}
                     $\oplus$ \sliste{ V[\type{inf}, \textsc{lex}+, \textsc{subj}~\ibox{1},
                       \textsc{comps}~\ibox{3}, \ltop \ibox{4}]} }\\
              cont & \ms{ ltop & \ibox{5}\\
                          ind  & \ibox{6} event } }\\
\rels \liste{ \ms[scheinen]{
                lbl  & \ibox{5}\\
                arg0 & \ibox{6}\\
                arg1 & \ibox{2}\\
                arg2 & \ibox{7} } }\\
hcons \liste{ \ms[qeq]{
               harg & \ibox{7}\\
               larg & \ibox{4} }  }
}
}\iw{scheinen|uu}
\z
\emph{scheinen} muss parallel zu \emph{glauben} behandelt werden, denn Quantoren können zwischen
\emph{scheinen} und dem eingebetteten Verb skopen \citep[202--203]{Kiss95a}:
\ea
Ein Einhorn scheint sich zu nähern.
\z
(\mex{0}) hat zwei Lesarten: In der einen gibt es ein Einhorn, das sich zu nähern scheint, und in
der anderen scheint sich irgend etwas zu nähern, aber es muss kein Einhorn geben. Durch die
\qeq-Relation zwischen \ibox{7} und dem Label des eingebetteten Verbs \iboxb{4} wird das korrekt
erfasst. Zur Semantik von \emph{glauben} siehe Abschnitt~\ref{Abschnitt glauben und Einhörner}.
\end{enumerate}

\section{Passiv}

\begin{enumerate}
\item Weil \emph{zwei Wochen} kein Akkusativobjekt sondern ein Akkusativ der Zeit ist. Die NP hat
  semantischen Kasus, der sich nicht in Abhängigkeit von der syntaktischen Umgebung ändert.

\end{enumerate}


\section{Partikelverben}

\begin{enumerate}
\item \emph{vor} verhält sich ähnlich wie \emph{los}, kann aber mit zweistelligen Verben kombiniert
  werden. Das Verb, das im Partikelverb \emph{vorkochen} vorkommt, wird durch die Lexikonregel in
  (\ref{lr-pv}) auf S.\,\pageref{lr-pv} lizenziert. Es wird keine zusätzliche Lexikonregel benötigt.

\eas
\label{le-vor}%
\emph{vor}:\\
\scalebox{\scalefactor}{%
\ms{
cat & \ms{ head & \ms[particle]{ mod  & \upshape V 
                               } \\
           arg-st & \eliste \\
         }
}}
\zs\iw{vor|uu}

Das \emph{vor} beschränkt anders als \emph{los} die Valenz des Verbs, mit dem es zusammengeht,
nicht. Somit kann es mit einstelligen (\emph{vorarbeiten}), zweistelligen (\emph{vorkochen}) und
auch sogar mit dreistelligen Verben (\emph{vorschenken}) kombiniert werden:
\ea
vater würde ihn mir zu xmas vorschenken\footnote{
  \url{http://www.elitepvpers.com/forum/crossfire-trading/2138728-b-lt-colonel-gg-lm-mg3-gold-etc-6.html}, 20.02.2013.
}
\z

\eas
\label{le-kochen}%
\stem{koch}:\\
\scalebox{\scalefactor}{%
\ms{ head   & verb \\
     arg-st & \liste{NP[\str], NP[\str]} \\[2mm]
}}
\iw{kochen|uu}
\zs

\end{enumerate}


\section{Morphologie}



\begin{enumerate}

\item


\eas
\label{lr-ung-nom}%
Lexikonregel für \emph{ung}"=Nominalisierung:\\
\scalebox{\scalefactor}{%
\ms[ung-derived-noun-stem]{
phon & \ibox{1} $\oplus$ \phonliste{ ung }\\
\ldots &  \ms{ cat  & \ms{ head   & noun\\
                           arg-st & \upshape \nliste{ Det } $\oplus$ \texttt{realize-nomilization-args}(\ibox{2}) }\\
%                          comps & \eliste\\
               cont & \ms{ ltop & \ibox{3}\\
                           ind  & \ibox{4} \ms{ per & 3\\
                                                gen & fem } } }\\   
rels & \liste{
       \ms[\ldots]{
       ltop & \ibox{3}\\
       arg0 & \ibox{4}\\
       arg1 & \ibox{5} } } $\oplus$ \ibox{6}\\   
lex-dtr & \onems[stem]{
           phon   \ibox{1} \\
           synsem$|$loc \onems{ cat \ms{ head & \type{verb}\\
                                         arg-st & \ibox{2} }\\
                                cont|ltop  \ibox{5} \\
                        }\\
           rels \ibox{6}
           }
}}\is{Lexikonregel!ung Nominalisierung@{\em ung}-Nominalisierung}
\zs
Zur Semantik der \emph{ung}-Nominalisierung siehe \citew{ER2000a-u}.

\item Die \suffix{ung}-Nominalisierungsregel wird auf den Stamm des Partikelverbs angewendet. Dieser
  enthält bereits die Partikel in der \argstl. Es muss also nicht erst aus dem Simplex"=Verb
  \emph{treten} \noword{Tretung} gebildet werden, um dann \emph{Abtretung} bilden zu können.

\end{enumerate}
