...
wir nur drei sehr abstrakte Dominanzschemata für Kopf"=Argument"= und Kopf"=Adjunkt"=Strukturen. Dafür sind jetzt aber
...
welchen Typ der Wert haben muss. Siehe hierzu auch Abschnitt~\ref{sec-modelle-theorien} und \citew[98]{Richter2024a}.

Bei Nomen ohne Argument und Verben ist die \hconsl für
gewöhnlich leer (siehe jedoch den Lexikoneintrag für \emph{glauben} auf S.\,\pageref{le-glauben}).
\ea
...
\ea
Alle Nomina, die nur einen Determinator verlangen (\type{det-noun-root}):\\
\ms{
  cat &  \ms{ spr    & \nliste{ [] }\\
              arg-st & \nliste{ Det } }\\
  cont & \ms{ ind|per & 3 }}
...
Argumentrealisierungsprinzip\is{Prinzip!Argumentrealisierungs-} (siehe (\ref{Prinzip-Argumentrealisierung-einfach}) auf
...
%%    }
%% \z
In den Beschränkungen für die oben genannten Typen ist Information isoliert, die für alle Nomina oder bestimmte Teilklassen
...
cat  & \ms{ head & verb \\
            spr    & \eliste\\
            arg-st & \nliste{ NP[\type{nom}]\ind{1}, NP[\type{dat}]\ind{2}  } \\
          } \\
cont &  \ms{ 
             ltop & \ibox{3}\\
             ind  & \ibox{4} event\\
...
\noindent
Die Information kann man wie folgt aufteilen: Alle Verben teilen die Information
in (\mex{1}).
\ea
...
\z
Das $\oplus$ \etag bedeutet, dass man die einstellige Liste mit etwas verknüpfen kann. Wofür \etag
...
Siehe auch Abschnitt~\ref{sec-Linking} zum Linking\is{Linking}. 
...
Abschnitt~\ref{Abschnitt-Valenzprinzip} bzw.\
Abschnitt~\ref{Abschnitt-Semantikprinzip} besprochen (siehe Abbildung~\ref{fig-type-sign-Valenzprinzip}
auf S.\,\pageref{fig-type-sign-Valenzprinzip} und
Abbildung~\ref{abb-headed-phrase-head-non-adjunct-structure} auf S.\,\pageref{abb-headed-phrase-head-non-adjunct-structure}). Hier sollen jetzt nur die Untertypen von
...
Man verwendet stattdessen Lexikonregeln, die zwei Wörter, zwei Stämme oder einen Stamm und ein
...
\ea
\label{pass-lr-mlr}
Lexikonregel für persönliches Passiv in Anlehnung an \citet[276]{Kiss92}:\\
\onems[stem]{
  cat~ \ms{ head   & verb  \\ 
            arg-st & \liste{ NP[\type{nom}], NP[\type{acc}]$_{\ibox{1}}$ } $\oplus$ \ibox{2} \\
          } } $\mapsto$ \\[1mm]
\mbox{}\hfill\onems[word]{
...
zu erfassen. Ja, es ist sogar möglich, Gemeinsamkeiten zwischen einfachen Wörtern, Lexikonregeln und
Phrasen zu repräsentieren, denn alle linguistischen Objekte sind Untertypen des Typs \type{sign}.
...
Diese Funktion mag der Leser*in obskur erscheinen, wir werden aber im Abschnitt~\ref{sec-morph-flex-anal}
...
Natürlich kann man Lexikonregeln auch zur Modellierung der derivationellen Morphologie benutzen \citep{Riehemann93a,Riehemann97a}.
...
bereits eingeführt. 
\itdopt{Das muss eigentlich hinter die Linearisierung.}
...
\label{Abschnitt-Konstruktionsgrammatik-phrasal}
...
Kay, Tom Wasow) in den 2000ern entwickelte, heißt Sign"=Based Construction Grammar
(SBCG,\is{Construction Grammar (CxG)!Sign-Based (SBCG)}
...
\citep{SDB2006a-u,SteelsFluid-ed,vanTrijp2013a} viele
...
formalisieren müsste.\footnote{%
  Die Vorschläge in \citew{KF99a} und \citew{Kay2002a,Kay2005a} im Rahmen der Berkeley
  Construction Grammar (BCG)\is{Construction Grammar (CxG)!Berkeley} sind teilweise in sich inkonsistent bzw.\ miteinander inkompatibel.
...
% Die Relationen werden da aber nicht erwähnt.
  Zur Diskussion der formalen Grundlagen von Konstruktionsgrammatik insbesondere der BCG siehe auch \citew[\page
...
Goldberg benutzt eine Variante von Vererbung, die von \citet*[263]{FPW85a} vorgeschlagen wurde,
aber im ersten Buch von \citet{ps} nicht mehr verwendet wurde \citep[309]{GoldbergShirtz2025a}.%
}

\subsubsection{Phrasale vs.\ lexikalische Argumentstruktur-Konstruktionen}
...
Objekten operieren.\footnote{
  ...
  Passiv und Impersonalkonstruktion annehmen. So kann eine Generalisierung nicht erfasst werden \citep{MuellerUnifying,MuellerLFGTAG,MuellerTAGCxG}.
} Solcherart komplexe Transformationen wurden in den 70er und 80er Jahren
in den auf Chomskys Arbeiten basierenden Grammatikmodellen\indexgb
eliminiert. Die genauen Gründe hierfür sind gut in \citew[Abschnitt~3.1]{Klenk2003a} zusammengefasst.
...
\parencites{Wechsler97a,WN2001a,Verspoor97a}[Kapitel~6]{Mueller2002b,MWArgSt,MuellerLFGphrasal}.\footnote{
  Siehe auch \citew[\page 45]{Wunderlich92a-u} und
...
\citet{Kay2005a}, noch im Rahmen der Berkeley Construction Grammar\is{Construction Grammar (CxG)!Berkeley} aber schon mit einer MRS"=Semantik, schlägt Argumentstruktur"=Konstruktionen vor,
die zu den HPSG"=Lexikonregeln äquivalent sind. Zur Diskussion phrasaler Ansätze siehe
\textcites{Mueller2006d,Mueller2006c,Mueller2007d,MuellerUnifying,MuellerLFGphrasal,MuellerLexikon,MuellerGT-Eng5,MuellerLFGTAG,MuellerTAGCxG}
und \textcites{MWArgSt,MWArgStReply}.
%
...
\subsubsection{Erzwingung (Coercion) und Lexikonregeln}
\label{sec-Erzwingung}

Ich möchte hier nicht die gesamte Diskussion aus den im vorigen Abschnitt diskutierten Quellen
wiederholen, aber auf eine Sache möchte ich dennoch hinweisen: Die phrasalen und lexikalischen
Analysen liegen bei der Frage der Einsetzung von Material in Strukturen gar nicht so weit
auseinander, wie immer behauptet wird. Die Grundannahme phrasaler Ansätze ist, dass Verben (oder
allgemeiner Köpfe) in Strukturen eingesetzt werden können, in die sie eigentlich nicht passen. So
kann zum Beispiel ein ditransitives Verb mit Subjekt und zwei Objekten benutzt werden
(\mex{1}a). Genauso kann aber das zweistellige Verb \emph{bake} mit Subjekt und zwei Objekten
benutzt werden (\mex{1}b) und das Beispiel in (\mex{1}c) zeigt, dass sogar einstellige Verben mit
Subjekt und zwei Objekten benutzt werden können.
\eal
\ex She gave her a book.
\ex I baked her a cake.
\ex She smiled herself an upgrade.\footnote{
  Douglas Adams, Hitchhiker’s Guide to the Galaxy, Harmony Books. Zitiert nach \citew[220]{Goldberg2003b}.
}
\zl
\citet[Kapitel~6]{Goldberg95a} erklärt das so, dass die Verben in eine dreistellige Konstruktion hineingewürgt werden,
obwohl sie von ihrer Stelligkeit nicht passen würden. In der Dativ-Konstruktion stehen dann weitere
Argument-Slots zur Verfügung und die Dativkonstruktion bringt auch die entsprechende Bedeutung mit.

In den vorangegangenen Kapiteln wurde eine Theorie entwickelt, in der die phrasalen Schemata meist keine
Bedeutung beisteuern (über \textsc{c-cont} ist das aber möglich, siehe Abschnitt~\ref{sec-Semantik-C-CONT}). Die
Bedeutung ist für gewöhnlich in den Lexikoneinträgen enthalten. Der wichtige Punkt dieses Abschnitts ist:
Während phrasale Konstruktionsgrammatikansätze Elemente, die nicht passen, in Konfigurationen
einsetzen, benutzen lexikalische Ansätze Lexikonregeln, um Elemente passend zu machen. Sie sind
sozusagen explizite Beschreibungen dessen, was in phrasalen Ansätzen einfach Zauberei ist. Die
Abbildung~\ref{abb-phrasal-vs-lexikalisch} soll das verdeutlichen.
...
[S
  ...
  [VP x z y%, green
    [V%,green 
      [V x y, circle edge, edge={<-},tier=words 
        [baked]]]
    [NP$_z$, calign with current %,green 
      [her,tier=words]]
    [NP$_y$%,green 
...
\end{forest}
\hfill\mbox{}
\caption{Erzwingung: Einstöpselansatz vs.\ Lexikonregel aus \citew[22]{MuellerLexikon}}\label{abb-phrasal-vs-lexikalisch}
\end{figure}

In der linken Abbildung wird ein zweistelliges Verb in eine Konfiguration eingesetzt, in der dann
Subjekt und Objekte realisiert werden. In der Abbildung auf der rechten Seite wird ein zweistelliges
Verb zu einem dreistelligen in Beziehung gesetzt. Dieses wird in der Lexikonregel auch um die
entsprechenden semantischen Bestandteile erweitert. Das dreistellige Verb kann dann wie ein normales
dreistelliges Verb in den entsprechenden Strukturen des Englischen benutzt werden. Eine
entsprechende Lexikonregel wurde bereits von \citet[Abschnitt~5]{BC99a} vorgeschlagen.
Ich habe schon im
Abschnitt~\ref{sec-CxG-sprachübergreifende-Generalisierungen} darauf hingewiesen, dass die lexikalischen Ansätze den Vorteil haben, dass sie
sprachübergreifend funktionieren. Ich habe eine entsprechende lexikalische Analyse ausgearbeitet,
die sowohl für das Englische als auch für das Deutsche funktioniert \citep{MuellerLFGphrasal}.
...
Genau das wurde aber von \citet{KF99a} und \citet[264]{vanTrijp2025a} im Rahmen der Konstruktionsgrammatik und 
...
Folgenden sollen die jeweiligen Vorschläge diskutiert werden.\footnote{
  Entsprechende Vorschläge wurden auch in TAG\indextag \parencites{Candito96a}[\page 188]{CK2003a-u} und Simpler Syntax\is{Simpler Syntax} \citep[Abschnitt~6.3]{CJ2005a} gemacht. Diese können hier nicht diskutiert werden. Die interessierte Leser*in sei jedoch auf \citew{MuellerUnifying} und \citew{MWArgSt} verwiesen.
}
...
Durch ihre spezielle Vorstellung von Mengen ist es möglich, die Anzahl von Elementen in Mengen
...
\{ NP[\nom], NP[\acc] \} $\wedge$ \{ NP[\nom] \} = \{ NP[\nom], NP[\acc] \}
\z
Nach dieser Auf"|fassung führt die Unifikation einer Menge mit einer Menge, die ein kompatibles
...
\ea
Die Unifikation\is{Unifikation} zweier Beschreibungen AVM$_1$ und AVM$_2$ (AVM$_1$ $\wedge$ AVM$_2$) ist diejenige Beschreibung AVM$_3$, die sowohl von
AVM$_1$ als auch von AVM$_2$ subsumiert\is{Subsumtion} wird und selbst nicht von einer anderen Struktur subsumiert wird,
für die das auch gilt.
\z
Dabei subsumiert eine Beschreibung AVM$_1$ die Beschreibung AVM$_3$, wenn AVM$_3$ alle Merkmal"=Wert"=Paare und
alle Strukturteilungen aus AVM$_1$ enthält. AVM$_3$ kann darüber hinaus noch weitere Merkmal"=Wert"=Paare
oder Strukturteilungen enthalten. AVM$_3$ ist die minimale Struktur, die alle Information aus
AVM$_1$ und AVM$_2$ enthält (siehe Abschnitt~\ref{sec-unifikation}). Die Konsequenz ist, dass wenn die Unifikation von Valenzmengen
...
\end{tabular}
\z
Das heißt, eine Merkmalbeschreibung mit einer Valenzliste, die nur eine NP[\nom] enthält, ist allgemeiner
als eine Merkmalbeschreibung mit einer Valenzliste, die eine NP[\nom] und eine NP[\acc] enthält (\mex{0}b).
...
\label{ex-Subject-Construction-Michaelis}%
\ms{
syn & \ms{ cat & v\\
       }\\
...
\eal
\ex Judit schlägt den Weltmeister.
\ex Der Weltmeister wird (von Judit) geschlagen.
\zl
...
verbinden, so würde die Auf"|fassung von Mengenunifikation, die Kay und Fillmore vertreten, dazu führen,
...
ein phonologisch nicht realisiertes Expletivpronomen\is{Pronomen!Expletiv-}, das nur in Valenzlisten sein leeres Dasein
...
\eal
\ex[*]{
Der Delphin schläft den Ball.
}
\ex[*]{
Der Delphin denkt an den Fisch den Ball.
}
\zl
...
von Sprecher*innen nach dem Erwerb des neuen Verbs auch ohne Weiteres passiviert werden können.\is{Menge|)}%
\is{Passiv|)}%
...
formulieren, die mit dieser Konstruktion kompatibel sein sollen.\footnote{
  Hierzu ist eine Repräsentation der Relationen als getypte Merkmalbeschreibung notwendig, so
  wie sie in Abschnitt~\ref{sec-Relationen-und-Merkmalbeschreibungen} eingeführt wurde. Mit einer solchen
...
siehe Abschnitt~\ref{sec-anhebung}). Wie man an der Subjekt"=Verb"=Kongruenz in (\mex{1})
...
Subjekt anzunehmen oder explizit zu spezifizieren, welche Verben von der Subjekt"=Konstruktion in (\ref{ex-Subject-Construction-Michaelis})
...
Interaktionen mit anderen Konstruktionen siehe auch \citew{Mueller2006d,Mueller2006c}.
...
Die Verwendung von Mengen\is{Menge} nach Kay und Fillmore sollte man möglichst schnell wieder vergessen,
...
nicht gibt.\footnote{
        ...
        werden auch nicht von allen Sprecher*innen des Süddeutschen akzeptiert. 
        Zu einer Kritik an Felix' Ansatz siehe auch 
        \citet*[\page230]{Oppenrieder91a}.
}
Es reicht also aus, Listen zu verwenden.

Listen kann man bei Verwendung normaler Unifikation nicht verkürzen oder verlängern.\footnote{%
Siehe \citet[104]{Copestake2002a} und auch S.\,\pageref{page-difflists} des vorliegenden Buches zur Kodierung von Differenzlisten\is{Liste!Differenz-} mittels
Merkmal"=Wert"=Paaren. Differenzlisten kann man verlängern. Der Trick besteht darin, dass
Differenzlisten kein festgelegtes Ende haben und man nur einen Zeiger auf die Stelle verwendet, an
der die Liste zu Ende sein soll. Bei der Verknüpfung von Listen kann man einen neuen Zeiger auf ein
neues Ende setzen. Für die hier diskutierten Ansätze hilft dieser Trick aber auch nicht, denn, wenn
man sich in einem Vererbungsansatz einmal festgelegt hat, kann man den Zeiger auf das Listenende
nicht überschreiben. Zur Verwendung von Defaults siehe \citew{MuellerDefaults}.
}
Den Effekt
...
angepasstes Kopf"=Komplement"=Schema, das Bezug auf den \textsc{realized}"=Wert von Argumenten nimmt
(siehe Abschnitt~\ref{kasus-anhang} für eine Ausarbeitung dieses Vorschlags).
...
\is{Passiv|(}%
Das Problem mit zusätzlichen Argumenten lässt sich lösen, wenn man mehrere Repräsentationsmöglichkeiten für
...
lexikalischer Elemente verwendet. Wenn man also im Lexikoneintrag für ein Verb
...
mit den zusätzlichen Typen in Abbildung~\ref{bsp-Partitionen-ausgerechnet}.
\begin{figure}
\begin{forest}
  type hierarchy,
  for level=2{
...
\ea
\ms[lesen]{
\multicolumn{2}{@{}l@{}}{\textsc{stem-phon} \phonliste{ les }}\\
cat       & \ms{
...
cat       & \ms{
            comps & \ibox{1}\\
            arg-st & \ibox{1} \sliste{ NP\ind{2}, NP\ind{3} } \\
            }\\
cont      & \ms[lesen-rel]{
            agens & \ibox{2}\\
            thema & \ibox{3}\\
...
formuliert werden. Das ist möglich: Man könnte auf die Kasusprinzipien,
die im Kapitel~\ref{Kapitel-Kasus} erläutert werden, zurückgreifen. Allerdings könnte Kasus dann
nicht mehr auf der \argstl zugewiesen werden, denn diese wäre ja bei Aktiv und Passiv gleich.
...
ein Wort dreimal von \type{causative} erbt, bekommt man nichts anderes, als wenn das Wort
...
Irische\il{Irisch} \citep{Noonan94a} und das Türkische\il{Türkisch} \parencites{Ozkaragoez86a}[Abschnitt~2.3.3]{Knecht85a-u}, in denen Kombinationen aus Passivierung und
Impersonalkonstruktion möglich sind \citep{Blevins2003a}. (\mex{1}) zeigt
die türkischen Beispiele von \citet[\page 77]{Ozkaragoez86a} mit der von ihm verwendeten Glossierung:
%% \eal
%% \ex
...
\zl
\suffix{In}, \suffix{n} und \suffix{Il} sind Allomorphe des Passiv- bzw.\ Impersonalmorphems.
Laut \citet{Blevins2003a} werden die Daten durch eine Analyse am besten erfasst, die davon ausgeht, dass ein passiviertes transitives
Verb in der Impersonalkonstruktion verwendet werden kann, so dass das Objekt des transitiven Verbs
zuerst zum Subjekt wird und dann in der Impersonalkonstruktion unterdrückt wird. Solche
Interaktionen aus Passivierung und Impersonalkonstruktion lassen sich ebenfalls nicht über Vererbung
erfassen, denn die Impersonalkonstruktion braucht ein passiviertes Verb als "`Eingabe"'. Es gibt
natürlich immer die Möglichkeit, Strukturen, die sich aus Interaktionen ergeben würden, direkt
aufzuschreiben und das entsprechende Wissen in Vererbungshierarchien auszudrücken. Dabei wird aber
nicht die Interaktion zwischen Konstruktionen erfasst.
...
Resultativkonstruktionen sowohl im Passiv (\mex{1}b) als auch in Medialkonstruktionen\is{Medialkonstruktion} (\mex{2})
...
Lexikonregel in (\mex{2}) analysiert wurden.\footnote{\label{fn-Extrapositionslexikonregel}%
  ...
  Argumentrealisierungsprinzip\is{Prinzip!Argumentrealisierungs-} (siehe S.\,\pageref{Prinzip-Argumentrealisierung-einfach})
  annimmt. Es würde dann ausreichen, in der Lexikonregel nur die \argstl zu erwähnen. Das \subjm nutzt Koenig zur
  Repräsentation des Subjekts. Im restlichen Buch folge ich \citet*[Abschnitt~4.3]{SWB2003a} und
...
werden. Siehe auch S.\,\pageref{rel-shuffle}. Für unser konkretes Beispiel in
...
der dem folgenden ähnelt:\footnote{
  ...
  \textsc{arg-list} stehen, ist nicht festgelegt. Ohne weitere Spezifikationen ist über die \compsl
  ...
  \textsc{arg-list}, müsste also -- wenn man von einer einfachen Zuordnung ausgeht -- auch das letzte Element der \compsl sein. %
}
\ea
...
die Analyse beliebig vieler ungrammatischer Abfolgen mit \emph{likely} benutzt werden kann,
denn die \compsl von (\mex{0}) ist wegen der Punkte am Anfang und am Ende nicht geschlossen.
...
Valenzlisten berechnet. Somit hätte die \compsl dann zwar eine definierte Anzahl von Elementen, ungrammatische
Wortfolgen könnten aber dennoch analysiert werden.

Auf S.~75 gibt Koenig folgenden Typ für die Extraposition von Indefinita im Französischen\il{Französisch} an:\footnote{
...
Dative\is{Kasus!Dativ} auch mit Resultativkonstruktionen vorkommen
(siehe \citealt{MuellerLFGphrasal} für eine ausgearbeitete Analyse). Freie
...
\eal
\ex Conny bemalt den Tisch.
\ex Conny bemalt ihm den Tisch.
\ex Er bekommt den Tisch bemalt.
\zl
...
\eal
\ex Conny fischt ihm den Teich leer.
...
\eal
\ex Conny fischt den Teich leer.
\ex Dass Conny fischt, verursacht, dass der Teich leer wird.
\ex cause(fischen(Conny), become(leer(Teich)))
\zl
...
Auf S.~97 gibt er Beschreibungen für \emph{cat} `Katze' und \emph{cats} `Katzen' an, die den folgenden ähneln:\footnote{
...
führen.\footnote{
  ...
  auf S.~72 angibt, hat ebenfalls eine komplexe Struktur (siehe auch (\ref{le-vendre})
  im vorliegenden Buch). Ich verzichte in den folgenden Abbildungen auf diese Unterscheidung.%
}
\begin{figure}
\begin{forest}
type hierarchy
...
Auf S.~72 gibt Koenig Einträge für das französische \emph{vendre} und Passiv- bzw.\
...
Zum Beispiel wird der Nominativ als Default"=Kasus\is{Kasus!Default-} bezeichnet \parencites[\page
  78]{Sternefeld95a}[\page3]{Jacobs91a}% 
% Ist in der LB-Version von 1992 anders.
[\page 265, \page 417]{Abraham2005a}, %für das Deutsche Svenonius2005:19 für das Isländische
andere Autor*innen bezeichnen den Akkusativ als Default"=Kasus \parencites[\page969]{Zwicky86}[\page688]{Hoeksema91c} und \citet[\page 733]{Wunderlich93a} nennt alle strukturellen Kasus
(\dash die meisten Nominative und Akkusative und je nach Sichtweise auch einige Dative, siehe Abschnitt~\ref{sec-struk-lex-kas}) Default"=Kasus.
...
in constraint"=basierten Grammatiken aber nicht ungefährlich: Zum Beispiel kann Kasus"=Information
...
Information wird ja identifiziert. Würde man so etwas sagen wie: "`Wenn der Kasus nirgends
...
Es kann durchaus sinnvoll sein, von Default"=Werten zu sprechen, doch mitunter lassen Autor*innen im Dunkeln,
...
Die entsprechende Hierarchie mit Eigenschaften zeigt Abbildung~\vref{pinguin}.\todostefan{Quelle}
\begin{figure}
\begin{forest}
type hierarchy
[Lebewesen
...
Stattdessen repräsentiert man am Typ \type{Vogel} nur die tatsächlichen Eigenschaften,
...
des Generalisierten Kopfmerkmalsprinzips\is{Prinzip!Generalisiertes Kopfmerkmals-}
aus:

\ea
Generalisiertes Kopfmerkmalsprinzip \citep[\page33]{GSag2000a-u}:\istype{headed"=phrase}\\[2pt]
...
Linguist*innen unintuitiv finden. 
% Auch in den Metagrammatikansätzen in TAG.
Lässt man solcherart Klassifikationen zu, stellt sich sofort die Frage,
...
% MR S. 40, aber eigentlich das ganze Buch.
...
das Wort \emph{Lesbarkeit} in einer Vererbungshierarchie wie in Abbildung~\vref{abb-lesbarkeit} repräsentieren.\footnote{
  ...
  Ausdrücke in Merkmalbeschreibungen) möglich. 
...
Außerdem hat man mit einem so ausgelegten Override Principle das Problem, dass eine automatische
...
vollständige Version der Lexikonregel für die \bard siehe (\ref{lr-bar-adj}) auf S.\,\pageref{lr-bar-adj}).
...
\zs
% Es muss in der Mitte zwischen Nom und Acc was stehen können und danach auch.
...
auf S.\,\pageref{Seite-Alle-Sätze-aufschreiben}
diskutierten Aufschreiben aller möglichen Sätze. Sie ist somit wenig erhellend und spiegelt die
...
\ex Das war eine sehr seltene Gabe für Einhörner und das letzte Einhorn mit dieser Fähigkeit war
...
Im Abschnitt~\ref{sec-index} auf S.\,\pageref{abkuerzungen-sem} haben wir Abkürzungen eingeführt,
...
Sie in den Dateien \texttt{le\_macros.pl} und \texttt{syntax.pl}, die Sie über das Menü \texttt{File|Open File} öffnen können.
Die genaue Syntax der Typbeschränkungen ist im Trale"= bzw.\ ALE"=Manual beschrieben.\footnote{
\url{http://www.ale.cs.toronto.edu/docs/}. 06.03.2026.%
}
\end{enumerate}
}
...
ersten Aufsatz geht es um die Struktur des Lexikons, um all die Dinge, die auch in diesem Kapitel
besprochen wurden: Vererbung, Lexikonregeln, Defaults. Im zweiten Aufsatz geht es um die Verbindung
