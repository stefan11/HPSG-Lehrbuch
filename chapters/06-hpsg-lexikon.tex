...
Das HPSG-Lexikon enthält Stämme wie \zb \stem{helf} und \stem{Kind} aber auch Wörter wie \emph{sie}
oder \emph{auf}. Aus den Stämmen leiten morphologische Regeln flektierte Fformen wie \emph{helfen},
\emph{hilft} und \emph{geholfen} bzw.\ \emph{Kind}, \emph{Kindes} und \emph{Kinder}
ab. Morphologische Regeln werden in Kapitel~\ref{Kapitel-Morphologie} behandelt.

Im folgenden soll gezeigt werden, wie man einen Lexikoneintrag für ein Nomen wie \emph{Kind} repräsentieren
kann. Der Lexikoneintrag für das Nomen \emph{Kind} ist in (\mex{1}) angegeben:
\ea
\ms{
 phon & \phonliste{ \highlight{Kind} } \\[2mm]
 cat  & \ms{ head & noun \\
             spr  & \nliste{ Det } \\
             comps & \eliste{ }\\
             arg-st & \nliste{ Det } \\
...
% kommt aus der Morphologie            num & sg \\
           ...
           rels & \liste{ \ms[\highlight{kind}]{ lbl & \ibox{1}\\
                                                 arg0 & \ibox{2} \\ }} \\
           hcons & \eliste\\
           } \\
\ldots & \ldots\\
...
\z
\type{noun} steht dabei für die Wortart.
...
ihrer Restriktionen. Das genaue Prädikat (\relation{buch} oder \relation{haus}) hängt vom einzelnen
...
\z
Alle zweistelligen Verben mit \argone und \argtwo haben zusätzlich
zu (\mex{-1}) die folgenden Eigenschaften:
\ea
\label{bsp-linking-agens-experiencer}
\ms{ cat & \ms{ arg-st & \sliste{ [ cont$|$ind \ibox{1} ], [ cont$|$ind \ibox{2} ] }\\ 
              } \\
     cont & \ms{
            rels & \ms[arg12-rel]{
                   arg1 & \ibox{1} \\
...
entsprechenden Typen integrieren müsste. \type{root} steht dabei für Wurzel.
%
\begin{figure}
\begin{sideways}
\scalebox{.5}{%
