...
\eal
\ex[*]{
daß Peter das Buch schläft
}
\ex[*]{
daß Peter erwartet
}
\ex[*]{
daß Peter über den Mann erwartet 
}
\zl
...
daß ein bestimmter Kopf für bestimmte Argumente subkategorisiert\is{Subkategorisierung} ist. Diese Redeweise kommt
...
\emph{X verlangt Y} oder \emph{X selegiert Y}\is{Selektion} heißt. Man sagt auch, daß X Y \emph{regiert}.\is{Rektion}
% Rektion schließt in der Dependenzgrammatik auch Adjunkte ein. Das ist also vielleicht etwas
% irreführend, aber irgendwo muß Rektion noch erwähnt werden.
...
\eal
\ex daß Peter schläft
\ex\label{bsp-weil-peter-maria-erwartet}
daß Peter Maria erwartet
\zl
...
Abbildung~\vref{fig-Peter-schlaeft} zeigt den durch das Regelschema lizenzierten Baum.
\begin{figure}[htbp]
\centerline{\begin{pspicture}(2.6,0)(7.4,4.3)
%\psgrid
\rput[B](3,0){\rnode{Peter}{Peter}}
\rput[B](6,0){\rnode{schlaeft}{schläft}}
%
...
%
\end{pspicture}}
\caption{\label{fig-Peter-schlaeft}Analyse für \emph{Peter schläft}}
\end{figure}
...
Die Information darüber, daß ein Verb mit einer NP kombiniert wird, wird nicht in
...
angewendet werden, so daß man dann eine vollständig gesättigte Phrase bekommt. Das zeigt
Abbildung~\vref{fig-Peter-Maria-erwartet}.

%\psset{xunit=1cm,yunit=5.4mm}
\begin{figure}[htbp]
\centerline{\begin{pspicture}(0.6,0)(8,6.2)
%\psgrid
\rput[B](1,0){\rnode{Peter}{Peter}}
\rput[B](3,0){\rnode{Maria}{Maria}}
\rput[B](6,0){\rnode{erwartet}{erwartet}}
%
\rput[B](6,2){\rnode{v}{V[\subcat \sliste{ \ibox{1}, \ibox{2} }]}}
\rput[B](3,2){\rnode{np1}{\ibox{2} NP}}
%
\rput[B](4.5,4){\rnode{vs}{V[\subcat \sliste{ \ibox{1} }]}}
\rput[B](1,4){\rnode{np2}{\ibox{1} NP}}
%
\rput[B](3,6){\rnode{vp}{V[\subcat \eliste ]}}
%
\psset{angleA=-90,angleB=90,arm=0pt}
%
\ncdiag{v}{erwartet}
...
%
\end{pspicture}}
\caption{\label{fig-Peter-Maria-erwartet}Analyse für \emph{Peter Maria erwartet}}
\end{figure}
...
und es wird behauptet, daß eine Voranstellung des Verbs mit einem Dativ\is{Kasus!Dativ} nicht möglich
...
SU $>$ IO $>$ DO annehmen. Die Beispiele in (\mex{1}) zeigen jedoch, daß mit demselben Verb Dativ +
...
in Abbildung~\vref{fig-Peter-Maria-erwartet-ip} angenommen (siehe \zb \citew[\page 49]{Grewendorf88a}).
%
\begin{figure}[htbp]
\psset{xunit=1cm,yunit=5.4mm}
...
%\psgrid
\rput[B](5,1){\rnode{jeder}{Peter}}
\rput[B](7,1){\rnode{ihnmf}{Maria}}
\rput[B](9,1){\rnode{kennt}{erwartet}}
\rput[B](11,1){\rnode{t2}{ \_ }}
...
skizzierten Ansatz folgen, geht man davon aus, daß das Subjekt\is{Subjekt} einen Sonderstatus hat und
...
davon aus, daß Subjekte nicht subkategorisiert sind (siehe \zb \citew[\page26--28]{Chomsky93a}, \citew[\page33]{Hoekstra87a}).
...
\citet[\page349--350]{Bresnan82c} diskutiert folgende Beispiele, die zeigen, daß es durchaus
...
\zl
\citet[\page29]{Marantz84a} merkt an, daß Beispiele wie (\ref{cat-tounge}) für die Diskussion seiner
...
Die Behauptung, daß Subjekte nie Bestandteile von idiomatischen Wendungen sind, die ein frei belegbares
...
\zl
\citet[\page208--209]{Marantz97a} formuliert eine Variante seiner Behauptung, die besagt, daß ein Agens\is{semantische Rolle!Agens} 
...
%daß also bei diesen Idiomen keine Inkorporierung der Idiombestandteile
%in das Verb vorliegen kann.
%% Hier meint er wohl im Idiom schon selbst.
%% Bsp: Das ist an den Haaren herbeigezogen.
%%
%% Die Passivierungen in (\ref{vom-teufel-geritten}) zeigen außerdem, daß Marantz' Behauptung \citeyearpar[\page208]{Marantz97a},
%% daß Vorgangspassive nicht mit Idiomen verträglich sind, sondern nur Zustandspassive, falsch ist.%

\citet[\page435]{Sternefeld85a} merkt an, daß die Nominative in den von Marga Reis
...
%% daß die Anzahl der Argumente nicht größer als die Anzahl der durch ein Prädikat 
...
motiviert, daß man sagt, daß sich das \emph{es} in (\mex{0}) genauso verhält wie eine
normale Nominalphrase in bestimmten Infinitivkonstruktionen.
Man sagt, daß in (\mex{1}) das Subjekt von \emph{fährt} mit dem Subjekt von \emph{anzustrengen}
referenzidentisch ist, sich also auf dasselbe Individuum bezieht (in (\mex{1}) auf \emph{Peter}).
\ea
Peter fährt schnell, ohne sich anzustrengen.
\z
Man kann aber Fälle wie (\mex{-1}) auch erfassen, ohne annehmen
zu müssen, daß \emph{es} eine semantische Rolle bekommt: Man muß für (\mex{-1}) und (\mex{0})
lediglich verlangen, daß der semantische Index\is{Index} der beiden
...
% Hoekstra87a:72
Die Behauptung, daß alle Sätze ein Subjekt haben müssen, die Bestandteil des
...
Man könnte für Fälle wie (\mex{0}) behaupten, daß es ein Subjekt gibt, daß
...
%% von Verben erfassen, so würde man vorhersagen, daß man alle Verben mit einem expletiven
%% Subjekt verwenden kann. So müßte nach Haider (\mex{1}) auch bedeuten können,
%% daß sich gestern ein Anrufensereignis mit mir als Angerufenem ereignete.
%% \ea
%% Gestern hat es mich angerufen.
%% \z
%% Das ist jedoch nicht der Fall. Es handelt sich also um Eigenheiten der Verben
%% in (\mex{-1}) und dies muß sich in der Argumentstruktur dieser Verben widerspiegeln.
%% Das Subjekt muß demzufolge Bestandteil der Argumentstruktur von Verben sein.
...
%% infinite Verbphrasen wie \zb \emph{Maria zu lieben} in (\mex{1})
%% taucht allerdings kein Subjekt an der Oberfläche auf.
%% \ea
%% Karl behauptet, Maria zu lieben.
%% \z
...
%% Verben liegt darin, daß sowohl Verbphrasen wie die oben auf"|geführte als
%% auch finite Sätze Maximalprojektionen sind und somit ihr ähnliches
%% syntaktisches Verhalten erklärt werden kann.
%%
%% \citet*{Borsley87}\iaright{Borsley} plädierte schon 1987 dafür, 
%% das Subjekt nicht in die Sub\-cat"=Liste auf"|zunehmen.
%% \citet*{Kiss92}\iaright{Kiss} dagegen schlug vor, nur das Subjekt 
