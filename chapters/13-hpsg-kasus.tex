...
Kasus eingeführt. Abschnitt~\ref{sec-sem-kasus} beschäftigt sich mit dem\todostefan{Bruno: den? sonst passt "bestimmten Kasusformen" nicht so gut
} sogenannten
...
Es gibt Prädikate, deren Argumente einen Kasus haben, der von der syntaktischen Umgebung,
...
Die Zuweisung struktureller Kasus wird durch das folgende Prinzip geregelt \parencites{Prze99}{Meurers99b}[Abschnitt~10.4.1.4]{Meurers2000b}{MdK2001a}[Abschnitt~7.2]{MuellerGermanic}:\footnote{
  Eine korrekte Formalisierung wurde von Detmar Meurers
  vorgeschlagen \parencites{Meurers99b}{MdK2001a}[Abschnitt~10.4.1.4]{Meurers2000b}.
  ...
  Satzsubjekte wurden weder von Meurers noch von \prz berücksichtigt.
%
}
...
S.\,\pageref{page-unmarkierte-Reihenfolge}), \dash, das Subjekt steht an erster Stelle. Wenn es ein
...
a. & \emph{schläft}:     & \argst \sliste{ NP[\type{str}]$_j$ }\\[2mm]
b. & \emph{unterstützt}: & \argst \sliste{ NP[\type{str}]$_j$, NP[\type{str}]$_k$ }\\[2mm]
c. & \emph{hilft}:       & \argst \sliste{ NP[\type{str}]$_j$, NP[\type{ldat}]$_k$ }\\[2mm]
d. & \emph{schenkt}:     & \argst \sliste{ NP[\type{str}]$_j$, NP[\type{ldat}]$_k$, NP[\type{str}]$_l$ }\\
\end{tabular}
\z
...
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
a. & \emph{geschlafen wird}:  & \argst \sliste{ }\\[2mm]
b. & \emph{unterstützt wird}: & \argst \sliste{ NP[\type{str}]$_k$ }\\[2mm]
c. & \emph{geholfen wird}:    & \argst \sliste{ NP[\type{ldat}]$_k$ }\\[2mm]
d. & \emph{geschenkt wird}:   & \argst \sliste{ NP[\type{ldat}]$_k$, NP[\type{str}]$_l$ }\\
\end{tabular}
\z
...
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
a. & \emph{geholfen bekommt}:    & \argst \sliste{ NP[\type{str}]$_k$ }\\[2mm]
b. & \emph{geschenkt bekommt}:   & \argst \sliste{ NP[\type{str}]$_k$, NP[\type{str}]$_l$ }\\
\end{tabular}
\z
...
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
a. & \emph{schlafen lässt}:     & \argst \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$ }\\[2mm]
b. & \emph{unterstützen lässt}: & \argst \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{str}]$_k$ }\\[2mm]
c. & \emph{helfen lässt}:       & \argst \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{ldat}]$_k$ }\\[2mm]
d. & \emph{schenken lässt}:     & \argst \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{ldat}]$_k$, NP[\type{str}]$_l$ }\\
\end{tabular}
\z
...
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
a. & \emph{kleiner werden}:     & \argst \sliste{ NP[\str]$_j$ }\\[2mm]
...
} Elemente, die mit ihrem Kopf über das Kopf"=Argument"=Schema
\todostefan{Hat Adam P COMPS?}
...
vorausgesetzt, dieses enthält eine weitere NP mit strukturellem Kasus oder wie in (\ref{ex-Dass
  Sandy sich so für diese Sache einsetzt, begeistert mich}) auf S.\,\pageref{ex-Dass Sandy sich so für diese Sache einsetzt, begeistert mich} ein Satzargument, dass
dem Subjekt entspricht.
...
\parencites{Meurers99b}[Abschnitt~10.4.1.4]{Meurers2000b}. \prz verwendet also statt des
...
ausgedrückten Subjekte) und es kann Kasus zugewiesen werden
