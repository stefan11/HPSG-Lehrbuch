...
\chapter{Kasus}
\label{Kapitel-kasus}\label{Kapitel-Kasus}
...
Ein ditransitives Verb wie \word{geben} hat dann einen \compsw \sliste{ NP[\str], NP[\str], NP[\ldat] },
...
Für das Dativpassiv muss man ein bißchen zaubern: Bei der Kombination von \emph{geholfen} und
...
b. & \emph{geschenkt bekommt}:   & \comps \sliste{ NP[\type{str}]$_l$, NP[\type{str}]$_k$ }\\
\end{tabular}
\z
...
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
a. & \emph{schlafen läßt}:     & \comps \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$ }\\[2mm]
b. & \emph{unterstützen läßt}: & \comps \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{str}]$_k$ }\\[2mm]
c. & \emph{helfen läßt}:       & \comps \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{ldat}]$_k$ }\\[2mm]
d. & \emph{schenken läßt}:     & \comps \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{str}]$_k$, NP[\type{ldat}]$_l$ }\\
\end{tabular}
\z
...
[case
  [structural
    [structural\_nga 
      [snom\strut, name=snom, tier=max]
      [sgen\strut, name=sgen, tier=max]
      [sdat\strut, name=sdat, tier=max]
      [sacc\strut, name=sacc, tier=max]]
...
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l@{}}
a. & \emph{schlafen}:     & \comps \sliste{ NP[\type{str\_nga}] }\\
b. & \emph{unterstützen}: & \comps \sliste{ NP[\type{str\_nga}], NP[\type{str\_nga}] }\\
c. & \emph{helfen}:       & \comps \sliste{ NP[\type{str\_nga}], NP[\type{str\_nd}] }\\
d. & \emph{schenken}:     & \comps \sliste{ NP[\type{str\_nga}], NP[\type{str\_nga}], NP[\type{str\_nd}] }\\
\end{tabular}
\z
...
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l@{}}
a. & \emph{schlafen}:     & \comps \sliste{ NP[$snom \vee sacc \vee sgen$] }\\
b. & \emph{unterstützen}: & \comps \sliste{ NP[$snom \vee sacc \vee sgen$], NP[$snom \vee sacc \vee sgen$] }\\
c. & \emph{helfen}:       & \comps \sliste{ NP[$snom \vee sacc \vee sgen$], NP[$snom \vee sdat$] }\\
d. & \emph{schenken}:     & \comps $\langle$ \begin{tabular}[t]{@{}l@{}}
...
\ea
\emph{geholfen}: \comps \sliste{ NP[\type{str\_nd}] }
\z
...
erste Element der \compsl des Partizips strukturellen Kasus hat. Dieses Element mit strukturellem
...
\compsl wie in (\mex{0}) müßte man dagegen in der Lexikonregel für die Adjektivderivation
...
dass wenn es an der ersten Stelle der \compsl der Kopf"|tochter eine NP mit strukturellem
Kasus gibt, diese NP Nominativ bekommt:\footnote{
  Im 99er Buch gehe ich davon aus, dass die Argumente vom Ende der \compsl vor denen am Anfang der
  ...
  Verbprojektionen immer an erster Stelle der \compsl. Es kann nicht passieren, dass das Subjekt
  gesättigt wird und dann eine Objektsnominalphrase mit strukturellem Kasus an erster Stelle der \compsl steht.%
}
...
\onems[head"=argument"=structure]{ 
     ...
     h-dtr$|$synsem$|$loc$|$cat$|$comps \sliste{ NP[\type{str}] } $\oplus$ \ibox{1} \\
    } ~\impl \\ \\
\mbox{}\hspace{3cm}\ms{ h-dtr$|$synsem$|$loc$|$cat$|$comps & \sliste{ NP[\type{snom}] } $\oplus$ \ibox{1}  \\
   }
\zs
...
\ea
\emph{auszukosten erlaubt wurde}:   \comps \sliste{ NP[\type{ldat}], NP[\type{str}] }
\z
...
Yip, Maling und Jackendoff ist, dass das Prinzip~\ref{case-p} wegen der Anhebungseinschränkung
...
HPSG. Im Kapitel werden Kasussynchretismen behandelt. Dabei geht es um den Zusammenfall von Formen
