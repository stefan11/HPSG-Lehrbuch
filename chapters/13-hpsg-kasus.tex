...
Kasus eingeführt. Abschnitt~\ref{sec-Satzargumente} beschäftigt sich mit Satzargumenten und geht der Frage nach, ob
diese als Subjekte oder Objekte einzuordnen sind und wie sie mit der Kasusvergabe
interagieren. Abschnitt~\ref{sec-sem-kasus} behandelt den sogenannten
semantischen Kasus, \dash Kasusformen, die durch die Verwendung eines Adjunkts
...
Es gibt Prädikate, deren Argumente einen Kasus haben, der von der syntaktischen Umgebung abhängt,
in der das Prädikat realisiert wird. Man bezeichnet solche Argumente
...
Beispiele für strukturellen Kasus sind:\footnote{
        ...
        \emph{sehen} sowie \emph{lassen}. Siehe auch Abschnitt~\ref{sec-aci}.
}
\eal
...
% Eisenberg2004a:134 Seine Stellung unter den Passivdiathesen zeigt noch einmal, daß der Dativ als
% struktureller Kasus fungieren kann, daß er aber in der Grammtikalisierungshierarchie nach dem Nom
% und Akk rangiert.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%----------------------------------------------------------------------------------------------------
oder ob alle Dative lexikalischen (oder zumindest nicht-strukturellen) Kasus bekommen \parencites[\page 80]{Haider85b}%
[\page 9]{Haider86}[\page 207, 217, 228]{HM94a}%
[270--272]{Mueller99a}[Abschnitt~1.2.1]{Mueller2001a}[\page 289]{Mueller2003e}%
...
%the dative.
[67]{VS98a}[236]{Abraham95a-u}%
[\page 187]{McIntyre2006a}% 
[Abschnitt~3]{Woolford2006a}.% p. 112 Icelandic, dativ auch inherent case S. 126 ist Zusammenfassung
% lexical case = idiosynkratisch, inherent case = regulät, aber nicht strukturell
...
\parencites[Abschnitt~4.1]{Haider86}[\page 228]{HM94a}[\page 298]{Mueller99a}.
...
zählt, ist, dass man die Dativobjekte zweistelliger Verben nicht von Akkusativobjekten
...
unterscheiden, was nicht adäquat ist.\footnote{
  ...
  Abschnitt~\ref{sec-alternativen-struc-dativ}. Eine andere Möglichkeit wäre die Verwendung
  grammatischer Funktionen für direkte und indirekte Objekte. Zu \textsc{erg} siehe Abschnitt~\ref{Abschnitt-Kathol-Pollard-ERG-Merkmal}.%
}
...
\citet[\page 187]{Stechow90a} und Autor*innen, die die Unterscheidung zwischen
...
Es ist richtig, dass das Dativpassiv mit zweistelligen Verben selten ist \citep[175]{HW95a},
...
Seltenheit mit der niedrigen Frequenz zweistelliger unakkusativischer\is{Verb!unakkusativisches}\footnote{ 
  ...
  siehe Abschnitt~\ref{sec-unakkusativitaet}. Die unergativischen Verben erlauben kein Passiv
  oder wenn, dann nur unter sehr eingeschränkten Bedingungen.%
}
Verben mit Dativobjekt.
\textcites[\page 134]{Wegener85a}[\page 75]{Wegener90} 
...
\citet[175]{HW95a} stellen fest, dass es im Korpus\is{Korpus} des Instituts für Deutsche Sprache in Mannheim keine
Belege für das Dativpassiv mit zweistelligen Verben gibt, stufen dieses Muster aber als grammatisch
möglich ein. Eine Sichtweise, die auch von \citeauthor{Wegener85a}, \citeauthor{Fanselow87a} und \citeauthor{Eisenberg94a} geteilt wird (siehe (\ref{ex-er-kriegte-geholfen})).
...
%%   Merkwürdigerweise ist (i) für einige Sprecher*innen akzeptabel:
%% \ea
...
weshalb es sinnvoll ist, diese Akkusative zu den lexikalischen Akkusativen zu zählen. Auch
Maßakkusative wie bei \emph{dauern}, \emph{wiegen}, \emph{messen} und \emph{kosten} werden zu den
lexikalischen Kasus gezählt. (\mex{1}) illustriert das am Beispiel von \emph{wiegen}.
\eal
\ex[]{
Der Sack wiegt einen Zentner.
}
\ex[*]{
Ein Zentner wird (von dem Sack) gewogen.
}
\zl
Wie (\mex{0}b) zeigt, ist kein Passiv möglich. \emph{wiegen} ist also kein transitives Verb
\citep[§531]{Duden2005-Authors}.
%\footnote{Zu Problemen in der theoretischen Diskussion, die sich aus
%  terminologischen Schwierigkeiten ergeben, siehe \citew[42]{MuellerGT-Eng3}.
%}

\is{Präposition|(}%
\citet[82]{Haider85b} argumentiert dafür, dass auch die Nominalphrasen
...
\eal
\ex Conny war sich dessen sicher.\iw{sicher}
\ex Alex ist ihm treu.\iw{treu}
\zl
...
\subsection{Satzargumente}
\label{sec-Satzargumente}

In der HPSG ist die Frage, ob \emph{dass}-Sätze wie der in (\mex{1}a) Objekte sind, eigentlich nicht
interessant, weil grammatische Funktionen im Gegensatz zu Theorien wie LFG
\citep{Bresnan82a-ed,BATW2016a,Dalrymple2023a-ed} keine Primitiva der Theorie sind. In LFG werden Argumentsätze
unterschiedlich eingestuft. Je nach Autor*in haben sie die Kategorie \textsc{comp} oder
\textsc{subj} bzw.\ \textsc{obj}
\citep*{DL2000a-u,Berman2003b-u,Berman2007a-u,AMM2005a-u,Forst2006a-u}. Von Forst ist auch das 
Beispiel in (\mex{1}b):
\eal
\ex Wir akzeptieren, dass die Erde rund ist.
\ex Dass die Erde rund ist, wurde nicht allgemein akzeptiert.
\zl
Der \emph{dass}-Satz ist laut Forst ein Objekt im Aktiv und ein Subjekt im Passiv. Innerhalb der
HPSG ist es aber egal, ob man den \emph{dass}-Satz Subjekt oder Objekt nennt, denn er ist im Aktiv
nur ein nicht besonders gekennzeichnetes Element der \compsl. Für infinite Formen des Passivs würde
es einen Unterschied machen, ob der \emph{dass}-Satz ein Subjekt ist oder nicht (siehe Abschnitt~\ref{sec-subj-merkmal}),
aber auch das wäre nur für die Analyse von Kontrollkonstruktionen wichtig und die funktionieren nur
mit Verben, die ein nominales Subjekt brauchen. Das kann man sich an den folgenden Beispielen klar machen:
\eal
\ex Dass die Erde rund ist, stimmt.
\ex Dass die Erde rund ist, scheint zu stimmen.
\ex Conny liest einen Roman.
\zl
(\mex{0}a) zeigt einen Satz mit einem \emph{dass}-Satz als Argument. (\mex{0}b) zeigt, dass das
Satz-Argument auch realisiert werden kann, wenn \emph{stimmen} als \emph{zu}-Infinitiv verwendet
wird. Bei der Konstruktion mit \emph{scheinen} handelt es sich um eine so genannte
Anhebungskonstruktion (siehe Abschnitt~\ref{sec-anhebung}). In (\mex{1}) liegen dagegen so genannte
Kontrollkonstruktionen vor:
\eal
\ex[]{
Conny behauptet, einen Roman zu lesen.
}
\ex[*]{
Conny behauptet, zu stimmen, dass die Erde rund ist.
}
\ex[*]{
Conny behauptet zu stimmen.
}
\zl
Wie (\mex{0}a) zeigt, werden die Subjekte in Kontrollkonstruktionen nicht ausgedrückt. Wenn also das
Satz-Argument nicht ausgedrückt würde und entsprechende Verben in Kontrollkonstruktionen verwendet
werden könnten, dann wäre das ein Beleg dafür, dass das Satzargument ein Subjekt ist. Leider sind
beide Sätze in (\mex{0}b,c) ungrammatisch. Das bedeutet, dass der Test für Satzargumente nicht
anwendbar ist und das Satz-Argument also sowohl als Subjekt als auch als Objekt eingeordnet werden
könnte. Auch Subjekt"=Verb"=Kongruenz kann nicht als Test für den Status als Subjekt oder Nicht-Subjekt
herangezogen werden, denn subjektlose Verben müssen immer in der dritten Person Singular stehen und
die Form des Verbs wäre auch dritte Person Singular, wenn das Satz-Argument ein Subjekt wäre.

Es gibt jedoch einen Bereich in der Grammatik des Deutschen, wo man sehen kann, dass bestimmte
\emph{dass}-Sätze Subjekte sein müssen. Das liegt daran, dass sie bei Passivierungen über
\emph{von}- bzw.\ \emph{durch}"=Korrelatkonstruktionen ausgedrückt werden können: 
\eal
\ex
\label{ex-Dass Sandy sich so für diese Sache einsetzt, begeistert mich}%
Dass Sandy sich so für diese Sache einsetzt, begeistert mich.
\ex Ich bin davon begeistert, dass Sandy sich so für die Sache einsetzt.
\zl
\eal
\ex Dass [\ldots], radikalisiert diesen Gedanken.
\ex Dieser Gedanke wird erstens dadurch radikalisiert, dass er [\ldots]\footnote{
...
\noindent
Das bedeutet, dass die \emph{dass}-Sätze in (\mex{-1}a) und (\mex{0}a) Subjekte sind. Das Objekt ist
\emph{mich} bzw.\ \emph{diesen Gedanken}. Die Objekte stehen in den Aktivsätzen jeweils im Akkusativ
und in den Passivsätzen im Nominativ. Die entsprechenden Nominalphrasen haben also strukturellen Kasus.

In der HPSG-Literatur wurde die Vergabe von strukturellem Kasus bisher immer so geregelt, dass der
ersten NP mit strukturellem Kasus in verbalen Umgebungen Nominativ zugewiesen wurde und allen
anderen NPen mit strukturellem Kasus Akkusativ. Die Satzsubjekte kommen einem bei dieser eleganten
Art der Kasuszuweisung in die Quere. Eine Möglichkeit, beim alten Kasusprinzip zu bleiben, bestünde
darin, zu behaupten, dass die Sätze eigentlich NPen sind. Man würde dann entweder ein leeres Nomen
annehmen, dass einen Satz oder eine VP verlangt und dann eine NP ergibt, oder man würde eine unäre
Projektion annehmen, die aus dem Satz/der VP eine NP macht. Die NP wäre dann so etwas wie die NP in
(\mex{1}b) nur dass der nominale Teil \emph{die Tatsache} unsichtbar wäre:
\eal
\ex Sandy setzt sich sehr für diese Sache ein. [Das/diese Tatsache] begeistert mich.
\ex {}[Die Tatsache, dass Sandy sich sehr für diese Sache einsetzt,] begeistert mich.
\zl
Man müsste für Verben, die ein Satz/VP-Argument verlangen, dann jeweils nur Einträge mit Selektion
von NPen annehmen. Eine solche Analyse wäre attraktiv, aber sie würde voraussetzen, dass die übergeordneten Verben
immer sowohl NP als auch Satz/VP erlauben würden. Das ist aber nicht bei allen Verben der
Fall:\footnote{
Siehe auch \citet[235]{Mueller99a} für parallele Beispiele mit Infinitiven mit \emph{zu}.
}
\eal
\ex[]{\label{ex-Satz-mit-Verb+dass-Satz}
Die Entführer haben gedroht, dass sie die Geiseln ermorden würden.
}
\ex[]{
Die Entführer haben mit der Ermordung der Geiseln gedroht.
}
\ex[]{
Die Entführer haben damit gedroht, dass sie die Geiseln ermorden würden.
}
\ex[*]{
Die Entführer haben der/die Ermordung der Geiseln gedroht.
}
\zl
Wie (\mex{0}a) zeigt, erlaubt \emph{drohen} ein Satzsubjekt, als Objekt muss jedoch eine
\emph{mit}-PP verwendet werden. Eine NP als Objekt ist ungrammatisch. Das bedeutet, dass es mit
einer leeren NP oder einer entsprechenden Projektion nicht getan wäre. Man müsste eine leere PP
annehmen oder zulassen, dass Sätze/VPen auch zu PPen projiziert werden können. 

Die Duden-Grammatik listet die folgenden grammatischen Funktionen, die Sätze prinzipiell einnehmen können
\citep[§1650]{Duden2005-Authors}: Subjekt (\mex{1}), Akkusativobjekt (\mex{2}), Genitivobjekt
(\mex{3}), Präpositionalobjekt (\mex{3}).
\eal
\ex {}[Dass Anna fehlte], fiel ihren Kolleginnen sofort auf.
\ex {}[Das/dieser Sachverhalt] fiel ihren Kolleginnen sofort auf.
\ex {}[Euch zu helfen] ist mein größter Wunsch.
\ex {}[Das] ist mein größter Wunsch.
\zl
\eal
\ex Ich bemerkte, [dass Otto fehlte].
\ex Ich bemerkte [das/diesen Sachverhalt].
\zl
\eal
\ex Wir waren uns bewusst, [dass es hier nachts sehr kühl werden kann].
\ex Wir waren uns [dessen/dieses Sachverhalts] bewusst.
\zl
\eal
\ex Ich freue mich, [dass ihr beide auch mitkommt].
\ex Ich freue mich darüber.
\zl
\largerpage
Also bräuchte man Regeln oder lexikalische Elemente, die NPen im Nominativ, Akkusativ, Genitiv und
PPen projizieren. Wenn man nun aber eine Regel hat, die aus einem \emph{dass}"=Satz eine
\emph{mit}"=PP macht, wie man das für (\ref{ex-Satz-mit-Verb+dass-Satz}) braucht, dann würde man
erwarten, dass \emph{dass}"=Sätze auch an der Stelle des PP-Arguments anderer Sätze auftreten
können. Wie (\mex{1}b) zeigt, ist dem aber nicht so:
\eal
\ex[]{
Sie rechnet damit, dass niemand kommt.
}
\ex[*]{
Sie rechnet, dass niemand kommt.
}
\zl 
Wenn es für alle Sätze eine unäre Projektion gäbe, die sie zur PP macht, dann müsste (\mex{0}b) in
Analogie zu (\mex{0}a) möglich sein. Wegen solcher Probleme scheint es angebracht zu sein, keine
leeren Elemente oder unäre Projektionen zur Umkategorisierung anzunehmen, sondern jeweils die
Lexikoneinträge für die entsprechenden Valenzmuster anzusetzen. Das heißt, wenn ein Verb mit einem
Satzargument vorkommt, gibt es dafür einen Lexikoneintrag und wenn es mit einem nominalen Argument
oder einer PP realisiert werden kann, gibt es einen weiteren Eintrag. Leider kann man aufgrund
derselben Daten die Lexikoneinträge auch nicht über eine Lexikonregel ableiten.
...
auf"|treten. \citet[80--82]{Haider85b} gibt folgende 
...
vor (\mex{1}). Genitiv kommt vor bei Lokalangaben oder Zeitangaben (\mex{2}).
\eal
...
%% man erwarten, dass in AcI"=Konstruktionen (siehe Abschnitt~\ref{sec-aci}) beide Nominalphrasen im Akkusativ stehen.
...
da dieses in dieser Form der Infinitivkonstruktion nie realisiert wird. \emph{Ein- nach d- ander-} bezieht sich
...
deren Hilfe oben gezeigt wurde, dass Dative im Deutschen keine Subjekte sind (siehe hierzu auch S.\,\pageref{page-dativsubjekte}).
...
Die Zuweisung struktureller Kasus wird durch das folgende Prinzip geregelt:\footnote{
  Ähnliche Prinzipien wurden von Detmar Meurers und Adam Przepiórkowski
  vorgeschlagen \parencites[238]{Prze99}[80,93--94]{Prze99b}[206--207]{Meurers99b}[51--53]{MdK2001a}[Abschnitt~10.4.1.4]{Meurers2000b}.
  Die Formalisierung, die ich verwende, basiert auf der von Meurers. Sie ist recht komplex und wird
  deshalb in einem Anhang diskutiert (siehe Abschnitt~\ref{kasus-anhang}). Zu \prz{}s Ansatz siehe Abschnitt~\ref{kasus-adamp}.

  Satzsubjekte und das Fernpassiv mit Objektkontrollverben wurden weder von Meurers noch von \prz berücksichtigt.
%
}
...
\end{itemize}
\end{prinzip-break}
Mit "`kasusrelevanten Elementen"'\label{page-kasusrelevante-XP} sind Satzargumente und Nominalphrasen mit strukturellem Kasus
gemeint. Die Formulierung ist komplex aber leider notwendig. Würde man sich -- wie ich das in
früheren Auf"|lagen dieses Buches getan habe, nur auf Nominalgruppen
mit strukturellem Kasus beziehen, so wäre die NP in (\ref{ex-Dass Sandy sich so für diese Sache
  einsetzt, begeistert mich}) -- hier als (\mex{1}) wiederholt -- die erste NP mit strukturellem Kasus
und bekäme Nominativ zugewiesen. 
\ea
\label{ex-Dass Sandy sich so für diese Sache einsetzt, begeistert mich-zwei}%
Dass Sandy sich so für diese Sache einsetzt, begeistert mich.
\z
...
%% \citealp{Meurers99b}; \citealp[Abschnitt~10.4.1.4]{Meurers2000b}; \citealp{MdK2001a}).
%% Die Unterschiede zu den von diesen Autor*innen vorgeschlagenen Prinzipien werden im
%% Abschnitt~\ref{sec-kasus-alternativen} diskutiert.

Das Wirken des Kasusprinzips soll an den Beispielen in (\mex{1}) erklärt werden. Die \argstlen der
jeweiligen Verben sind entsprechend der unmarkierten Reihenfolge der Argumente geordnet (siehe
S.\,\pageref{page-unmarkierte-Reihenfolge}), \dash, das Subjekt steht an erster Stelle. Wenn es
außerdem nur ein
...
\begin{tabular}[t]{@{}l@{~}l@{~}l}
   &                     & \argst\\[1mm]
a. & \emph{schläft}:     & \sliste{ NP[\type{str}]$_j$ }\\[1mm]
b. & \emph{unterstützt}: & \sliste{ NP[\type{str}]$_j$, NP[\type{str}]$_k$ }\\[1mm]
c. & \emph{hilft}:       & \sliste{ NP[\type{str}]$_j$, NP[\type{ldat}]$_k$ }\\[1mm]
d. & \emph{schenkt}:     & \sliste{ NP[\type{str}]$_j$, NP[\type{ldat}]$_k$, NP[\type{str}]$_l$ }\\[1mm]
e. & \emph{radikalisiert}:  & \sliste{ CP$_j$, NP[\type{str}]$_k$ }
\end{tabular}
\z
Die \argstl von \emph{schläft} enthält genau ein Element: eine NP mit strukturellem Kasus.
Da diese NP das erste Element der \argstl ist, bekommt sie Nominativ. Genauso bekommt die erste NP
in der \argstl von \emph{unterstützt} Nominativ, die zweite bekommt allerdings Akkusativ. Bei
...
bekommt Akkusativ. Bei \emph{radikalisiert} ist die CP kasusrelevant. Da sie keine NP mit strukturellem
Kasus ist, bekommt sie keinen Nominativ. Aber das zweite Element in der \argstl ist eine NP mit
strukturellem Kasus. Und obwohl es die erste NP mit strukturellem Kasus in der \argstl ist, bekommt
diese NP trotzdem Akkusativ, weil die vorangehende CP ein kasusrelevantes Element ist. Die NP ist
somit das zweite kasusrelevante Element.

Bei Passivierung der Verben in (\mex{0}) ergeben sich die folgenden \argst"=Listen für das Hilfsverb
\emph{wird}:\footnote{%
  Zu den Details siehe Abschnitt~\ref{sec-agentive-passive-da}.
}
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
   &                          & \argst\\[1mm]
a. & \emph{geschlafen wird}:  & \sliste{ V }\\[1mm]
b. & \emph{unterstützt wird}: & \sliste{ NP[\type{str}]$_k$, V }\\[1mm]
c. & \emph{geholfen wird}:    & \sliste{ NP[\type{ldat}]$_k$, V }\\[1mm]
d. & \emph{geschenkt wird}:   & \sliste{ NP[\type{ldat}]$_k$, NP[\type{str}]$_l$, V }\\[1mm]
e. & \emph{radikalisiert wird}:  & \sliste{ NP[\type{str}]$_k$, V }
\end{tabular}
\z
...
NP die erste mit strukturellem Kasus ist, bekommt sie Nominativ. \emph{radikalisiert} verhält sich wie
\emph{unterstützt}: Die Subjekt-CP wird unterdrückt, die NP mit strukturellem Kasus steht an erster
Stelle und bekommt Nominativ.
...
Dativ beim eingebetteten Verb zu einem strukturellen Kasus bei der Kombination mit dem Passiv"=Hilfsverb. (\mex{1}) zeigt die \argstlen von \emph{bekommt} mit entsprechenden eingebetteten Verben:
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
   &                          & \argst\\[1mm]
a. & \emph{geholfen bekommt}:    & \sliste{ NP[\type{str}]$_k$, V }\\[1mm]
b. & \emph{geschenkt bekommt}:   & \sliste{ NP[\type{str}]$_k$, NP[\type{str}]$_l$, V }
\end{tabular}
\z
Wie diese Umwandlung genau passiert, wird im Abschnitt~\ref{sec:dat-pass} ausführlicher erklärt. Hier ist
nur wichtig, wie die Kasusvergabe funktioniert: Dadurch dass das Dativargument bei Kombination mit
dem Passivhilfsverb strukturellen Kasus hat und an erster Stelle in der Argumentstrukturliste von
...
Entsprechende \argstlen sind in (\mex{1}) zu sehen:
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
   &                            & \argst\\[1mm]
a. & \emph{schlafen lässt}:     & \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, V }\\[1mm]
b. & \emph{unterstützen lässt}: & \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{str}]$_k$, V }\\[1mm]
c. & \emph{helfen lässt}:       & \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{ldat}]$_k$, V }\\[1mm]
d. & \emph{schenken lässt}:     & \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, NP[\type{ldat}]$_k$, NP[\type{str}]$_l$, V }
\end{tabular}
\z
...
verbunden, und es entsteht eine \argstl, die die Argumente des Adjektivs enthält (\mex{1}a). Zu
den Details siehe \citew[226]{MuellerPredication}. Wird
ein solcher Komplex noch unter ein AcI"=Verb wie \emph{sehen} eingebettet, erhält man die Liste in (\mex{1}b):
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
   &                            & \argst\\[1mm]
a. & \emph{kleiner werden}:     & \argst \sliste{ NP[\str]$_j$, Adj }\\[1mm]
b. & \emph{kleiner werden sah}: & \argst \sliste{ NP[\str]$_i$, NP[\type{str}]$_j$, V }
\end{tabular}
\z
...
\zl
In (\mex{0}) liegen ganz gewöhnliche Argumente vor, in (\mex{-1}) dagegen Adjunkte. Adjunkte
...
den Dativ als lexikalischen Kasus behandeln. Er entwickelt eine Typhierarchie,
in die er zwei spezielle Typen \type{scase1} und \type{scase2} aufnimmt \citep[96]{Gunkel2003b}. Dabei steht \type{scase1}
...
[case
  [structural
    [structural\_nga 
      ...
      [sdat, name=sdat, tier=max, no edge]
...
\end{tabular}
\z
Gunkels Kasusprinzip (Abschnitt~2.3.12) sagt dann, dass in verbalen Umgebungen das erste Argument Nominativ bekommt
...
auf S.\,\pageref{lr-adjective-formation-da-approach} ist für den im vorigen Abschnitt vorgestellten
...
Verben befindet sich das logische Subjekt in der \textsc{ia}"=Liste, bei unergativen Verben ist es das Akkusativobjekt \citep[\page 97]{Gunkel2003b}.
...
schlagen Kasusprinzipien vor, die Kasus innerhalb von Kopf"=Argument"=Strukturen, \dash in bestimmten
...
Kasus gibt, diese NP Nominativ bekommt:\footnote{
  ...
  S.\,\pageref{schema-bin-prel} mit einem \emph{append} und nicht dem Schema auf
  S.\,\pageref{schema-Kopf-Komplementschema-prel2} mit zwei Vorkommen von \emph{append}. Deshalb steht das Subjekt auch bei
...
\onems[head"=argument"=structure]{ 
     synsem|loc$|$cat$|$head \ms[verb]{ vform & fin \\ } \\[3mm] 
...
entwickelten Grammatik führen. Zum Beispiel bekommt man unechte Mehrdeutigkeiten\is{Mehrdeutigkeit!unechte},
...
Schlafen will Kim.
\z
Bei einer Analyse für (\mex{0}), wie sie im Abschnitt~\ref{sec-pvp} vorgestellt wird, ergeben
sich dann zwei Strukturen: eine, in der \emph{schlafen} direkt Füller für \emph{will Kim}
ist, und eine, in der \emph{schlafen} vorher projiziert wird (zu diesem und anderen Problemen siehe
\citew[Abschnitt~15.8]{Mueller99a}).

Gunkel schlägt eine andere Behandlung der Voranstellung von Teilphrasen vor, die
zwar nicht dieses Problem, aber dafür andere, schwerwiegendere hat. Gunkels Analyse
für die Voranstellung von Phrasen wird in Abschnitt~\ref{sec-pvp-vdrei} genauer diskutiert.
...
\citet{Prze99} schlägt vor, Kasus in Bezug auf die \argstl zuzuweisen. 
% \argst\isfeat{arg-st} ist eine
% Liste, die sämtliche Argumente eines Kopfes enthält. Die Verwendung einer
% Liste mit allen Argumenten wurde von \citet[Kapitel~9]{ps2} eingeführt. \citeauthor{ps2} schlagen vor, Fernabhängigkeiten\is{Extraktion} über Lexikonregeln einzuführen.
...
%
\citeauthor{Prze99} schlägt vor, die Repräsentation von Argumenten in Valenzlisten
...
oder angehoben wurde.\footnote{%
  Man beachte, dass dieses Merkmal im Prinzip dieselbe Funktion hat, wie Meurers' Merkmal
  \textsc{raised} \citep{Meurers99b}. Meurers benutzt zusätzlich ein Merkmal \textsc{realized}, was zu
  Verwirrung beim Vergleich der Analysen führen kann.
} Elemente, die mit ihrem Kopf über entsprechende Schemata verbunden werden, werden als
\textsc{realized}+ gekennzeichnet. Dasselbe gilt für Argumente, die extrahiert oder als
Klitikon\is{Klitisierung} morphologisch realisiert werden.
...
\citet[\page 238]{Prze99} gibt folgendes Kasusprinzip an:\footnote{%
  Die Angabe des Typs \type{cat} ist eigentlich nicht nötig, da er sich aus der Anwesenheit von
  \head und \argst ergibt.
}
\eal
\label{case-prz}
\ex \ms[cat]{
    ...
    \mbox{}\hfill\ms{ arg-st & \nliste{ [\textsc{argument}~ NP[\type{snom}] ] } $\oplus$ \ibox{2} }
\ex\label{case-prz-acc}
\ms[cat]{
    head & verb\\
    arg-st & \ibox{1} \type{ne\_list} $\oplus$ \liste{ \ms{ argument & \upshape NP[\str]\\
                          realized & $+$\\
                        } } $\oplus$ \ibox{2}\\ } \impl \\
    \mbox{}\hfill\ms{ arg-st & \ibox{1} $\oplus$ \nliste{ [\textsc{argument}~ NP[\type{sacc}] ] } $\oplus$ \ibox{2} }
\zl
...
in (\mex{1}) werden im Abschnitt~\ref{sec-remote-passive-phen}
...
\z
Die Implikation in (\ref{case-prz-acc}) würden der NP[\str] Akkusativ zuweisen.
...
Nominativ bekommt, \dash, entweder steht vor dieser NP nichts oder NPen mit lexikalischem
Kasus. Analog muss die zweite Implikation 
so geändert werden, dass beliebiges Material vor der NP[\str] stehen kann,
vorausgesetzt, dieses enthält eine weitere NP mit strukturellem Kasus oder wie in (\ref{ex-Dass
  Sandy sich so für diese Sache einsetzt, begeistert mich}) auf S.\,\pageref{ex-Dass Sandy sich so
  für diese Sache einsetzt, begeistert mich} ein Satzargument, das dem Subjekt entspricht.
...
besetzt wird.
\eal
\label{bsp-der-den-aufsatz-gelesen}%
...
siehe auch (\ref{bsp-subjekt-im-vf-passiv}) auf S.\,\pageref{bsp-subjekt-im-vf-passiv}.
...
\emph{der/den Aufsatz gelesen} ist. Sie ist nur bei \emph{gelesen} selbst repräsentiert. Das Problem
mit der Kasuszuweisung ist nicht existent, wenn man für die zwei Partizipien in
(\ref{bsp-der-den-aufsatz-gelesen}) verschiedene Lexikonelemente annimmt. Für das Passivpartizip
wäre die \argstl einelementig und für das Partizip Perfekt zweielementig. Im Rahmen der HPSG folgen
Autor*innen jedoch meist Haiders Ansatz und nehmen für Partizipien und andere infinite Verbformen
nur jeweils ein Lexikonelement an. Will man bei Haiders Ansatz bleiben, so könnte man das Problem zu
lösen versuchen, indem man die Argumentstruktur 
...
Ansatz ist für die Kasusvergabe entscheidend, wo ein Element realisiert wird. Das Problem mit
(\mex{0}) besteht natürlich auch dann weiter, wenn man zwei verschiedene Lexikonelement für
(\ref{bsp-der-den-aufsatz-gelesen}) annimmt.

\citet[93]{Prze99b} weist selbst auf ein weiteres Problem seines Ansatzes hin: In
Infinitivkonstruktionen werden Subjekte nicht realisiert, aber Höhles Tests mit kongruierenden
Adverbialen haben gezeigt, dass diese Subjekte im Nominativ stehen (siehe
Abschnitt~\ref{sec-kasus-nicht-realisierter-subj}).\footnote{%
Man könnte natürlich behaupten, dass die nicht realisierten Subjekte als \textsc{realized} `+'
markiert werden, denn die Subjekte müssen durch den übergeordneten Kopf als pronominal ausgewiesen
werden. Im Zuge dessen könnten sie auch als realisiert markiert werden. Das entspricht der
Realisierung durch ein PRO in Theorien wie Government \& Binding\indexgb \citep[Abschnitt~2.4.1]{Chomsky81a}.
} Das bedeutet, dass Kasusvergabe nicht davon
abhängen kann, ob ein Element in der Argumentstrukturliste lokal realisiert wurde oder nicht.
Die relevante Unterscheidung ist vielmehr, ob ein Element angehoben wurde oder nicht
\parencites{Meurers99b}[Abschnitt~10.4.1.4]{Meurers2000b}. \citeauthor{Prze99b} verwendet also statt des
\textsc{realized}-Merkmals das Merkmal \textsc{raised}\isfeat{raised}. Dieses hat den Wert `+', wenn ein
Argument angehoben wird, und den Wert $-$, wenn das nicht der Fall ist. Mit dieser Analyse kann
Kasus zugewiesen werden, auch wenn ein Argument nicht realisiert wird (im Fall der nicht
ausgedrückten Subjekte).
...
\eal
\ex[*]{
Der Delphin, den ihn kennt, schwimmt dort.
}
\ex[*]{
Conny kommt herüber, um der Delphin zu füttern.
}
\zl
...
wurde: Die Argumentstruktur wird auch bei \citet[80]{Prze99b} nicht projiziert. Deshalb ist
...
Ansätze,\is{Kasus!Null-|(} die davon ausgehen, dass der Nominativ nur vom finiten Verb\footnote{
  ...
  wird. Aus den Texten wird dann nicht ganz klar, ob die Autor*innen annehmen, dass INFL bzw.\ T nur für
...
haben \parencites[\page 161]{Grewendorf88a}[\page 42]{Frey93a}.
...
\ex Der Vogel singt.  
\ex Mich friert.    
\ex Kim zerstört das Auto.
\ex Das dauert ein ganzes Jahr.
\ex Sie hat nur einen Tag dafür angegeben.
\ex Conny denkt an den morgigen Tag.
\zl
...
\furtherreading{
Kasus werden von Wissenschaftler*innen aus dem angloamerikanischen Raum oft vernachlässigt, da das
...
Das von \citeauthor{Meurers99a} und \citeauthor{Prze99b} ausgearbeitete und hier weiterentwickelte
Kasusprinzip ähnelt sehr stark dem von \citet*{YMJ87} vorgeschlagenen und kann 
damit auch die Kasussysteme verschiedener Sprachen erklären, die von den genannten Autor*innen besprochen
wurden, insbesondere auch das komplizierte Kasussystem des Isländischen\il{Isländisch}. Siehe hierzu
auch \citew[Kapitel~7]{MuellerGermanic}. Ein wesentlicher Unterschied zwischen den
HPSG-Formalisierungen und dem Kasusprinzip von \citeauthor*{YMJ87} ist, dass die HPSG-Prinzipien wegen der
Anhebungseinschränkung monoton\is{Monotonie} sind, \dash, Kasus, die einmal zugewiesen wurden, werden
nicht von einem übergeordneten Prädikat überschrieben. 

\citet{Przepiorkowski:2021a} gibt einen Überblick über die Behandlung von Kasus im Rahmen der
HPSG. In seinem Beitrag werden Kasussynchretismen behandelt. Dabei geht es um den Zusammenfall von Formen
wie zum Beispiel bei der Nominalgruppe \emph{die Frau}, die sowohl Nominativ oder Akkusativ sein kann. In
