...
In den bisherigen Kapitel wurde erklärt, wie Verbletzt- und Verberstsätze analysiert werden können. In diesem Kapitel werden 
die Mechanismen eingeführt, die zu einer adäquaten Behandlung der Vorfeldbesetzung benötigt
...
\largerpage
Im\is{Extraktion|(}\is{nichtlokale Abhängigkeit|(}\is{Konstituentenstellung|(} Deutschen stehen die Konstituenten in
Aussagesätzen nicht wie im Englischen in Subjekt-Verb-Objekt-Reihenfolge. Das finite Verb steht
initial und die Position vor dem finiten Verb kann mit einer beliebigen Konstituente (Adjunkt, Subjekt oder Komplement) besetzt sein \parencites[Abschnitt~2.4]{Erdmann1886a}[\page69, \page 77]{Paul1919a}, weshalb das Deutsche zu den Verbzweitsprachen\is{Verbzweitsprache} gezählt wird.\footnote{%
  Die Kategorisierung des Deutschen als Verbzweitsprache scheint im Widerspruch zur Aussage zu stehen, 
  dass das Deutsche eine Verbletztsprache\is{Verbletztsprache} (siehe S.\,\pageref{page-verbletzt}) ist. Dem ist
  ...
  typologisch wie andere SOV"=Sprachen verhält (Japanisch\il{Japanisch}, Koreanisch\il{Koreanisch}, Persisch\il{Persisch}, siehe
  \citealt{Haider2020a}). Das Verb kann jedoch auch nach vorn gestellt werden. Im Gegensatz zum
  Englischen\il{Englisch} kann im Deutschen nicht nur das Subjekt vor dem finiten Verb stehen, sondern auch
  ...
  V2"=Sprachen kennt. Das Dänische\il{Dänisch} ist eine SVO"=Sprache aber auch gleichzeitig eine V2"=Sprache
  \citep{Vikner95a-u,MOeDanish,MuellerGermanic}. Das heißt, die Eigenschaften SOV/SVO/\ldots{} und V2 sind
  unabhängig voneinander.%  
}%\todostefan{Sebastian findet, dass alles nach Im Gegensatz weg soll.}
...
der Verberstsätze aufzufassen: Das Verb würde dann mit dem Kopf"=Komplement"=Schema mit seinen
...
%% In einer Analyse mit flachen Strukturen (Abschnitt~\ref{sec-flache-strukturen}), 
%% mit variabler Verzweigungsrichtung oder in einer Linearisierungsgrammatik, wie sie in Abschnitt~\ref{sec-Linearisierung} besprochen wurde,
...
%% und auch \citet[Abschnitt~6.3]{Kathol95a} entwickelt eine Linearisierungsanalyse
...
Die Stelle, an der die Vorfeldkonstituente stehen würde, wird meist durch `\_' gekennzeichnet. Man nennt
diesen Platzhalter auch Spur\is{Spur}, Lücke oder \emph{Trace} bzw.\ \emph{Gap}.
Vergleiche auch Abschnitt~\ref{sec-v1}. Das zur Lücke gehörige Element im Vorfeld wird auch Füller\is{Füller} genannt.

Abhängigkeiten, die über Phrasengrenzen hinweggehen, nennt man
%\NOTE{JB: schwer verständlich, Def-Abgrenzung}
...
In (\mex{1}a) ist ein Relativsatz extraponiert worden, der \emph{Kind} im \mf modifiziert.
In (\mex{1}b) ist ein Infinitivkomplement ins Nachfeld gestellt worden. 
\eal
\label{bsp-extrap}
\ex Der Delphin hat [dem Kind \_$_i$] den Ball gegeben, [das gewunken hat]$_i$.
\ex Der Delphin hat \_$_i$ versucht, [einem Kind den Ball zu geben]$_i$.
\zl
Wie bei den Sätzen
...
ist. Zu solchen und anderen Extrapositionsdaten siehe auch \citew[Abschnitt~13.1]{Mueller99a}
...
\citew[Abschnitt~13.2]{Mueller99a}. Spezielle Analysen für die Relativsatzextraposition findet man in
...
Sie werden in diesem Buch nicht besprochen. Im Folgenden wenden wir uns der Vorfeldbesetzung
zu.
%\itdopt{Feldhaus97a:29 eine mit wievielen Kugeln Eis gefüllte Tüte: Ich möchte wissen über eine
%  mit wievielen Kugeln Eis gefüllte Tüte ihr gesprochen habt}


\section{Vorfeldbesetzung}
\label{sec-Vorfeldbesetzung}

Im letzten Kapitel haben Sie bereits Spuren kennengelernt. Die Abhängigkeit zwischen vorangestelltem
...
Die Datenstruktur in (\ref{geom-loc}) auf S.\,\pageref{geom-loc} wird also zu (\mex{1}) erweitert:
\ea
\label{geom-nonloc}
\ms{
...
\z
%\todostefan{\type{cont} müsste \type{hook}\istype{hook} sein. Das überall im Buch. Auch bei Lokalität.}
%\largerpage
...
Relativpronomina und von bestimmten \ltopwen. \citet[305]{CFPS2005a} benutzen ein Merkmal \textsc{hook},
deren Wert vom Typ \type{hook} ist. Unter \textsc{hook}\isfeat{hook} werden genau der Index und der \ltopw
angegeben. Im Abschnitt~\ref{Abschnitt MRS und Fernabhängigkeiten} erkläre ich, warum ich als \contw nur Objekte vom Typ \type{hook}
verwende und die \relslen auf der obersten Ebene ansiedle.
Auf \textsc{rel} wird im Kapitel~\ref{Kapitel-rs} über
...
%\settowidth{\iboxlength}{\ibox{1}~}%
Spur für das Akkusativkomplement von \emph{kennen} (vorläufig):\\[1mm]
\ms[word]{
 phon & \phonliste{} \\[1mm]
loc   & \hspace{-2.1ex}\ibox{1} \ms{ cat \ms{ head & \ms[noun]{
                                                     cas & acc } \\
                                              spr   & \eliste\\
                                              comps & \eliste } }\\[9mm]
...
Die Spur in (\mex{0}) hat die Eigenschaften eines Akkusativobjekts. Dass das Akkusativobjekt
...
\begin{prinzip-break}[Prinzip der nichtlokalen Merkmale]\label{Prinzip-NONLOC}
...
Das Kopf"=Füller"=Schema (Schema~\ref{hf-schemaa}) lizenziert den obersten Knoten in
Abbildung~\ref{abb-das-buch-kennt-er-variable-verzweigung}.
%\begin{figure}
\begin{schema}[Kopf"=Füller"=Schema (vorläufig)]
\label{hf-schemaa}\is{Schema!Kopf"=Füller"=}%
\type{head-filler-phrase}\istype{head"=filler"=phrase} \impl\\
\onems{ 
nonloc$|$slash  \eliste\\
head-dtr \ibox{1}\\
dtrs \liste{ \onems{ loc \ibox{2}\\
                                           nonloc$|$slash \eliste \\
                                 }, \ibox{1} \onems{ loc$|$cat \onems{ head \ms[verb]{vform & fin\\
                                                                 initial & $+$\\
                                                                }\\
%                                                  spr   \eliste\\   Kann nicht anders sein.
                                                  comps \eliste\\
                                               }\\
                             nonloc$|$slash   \sliste{ \ibox{2} }\\
                        }}\\
}
\end{schema}
%\vspace{-\baselineskip}
...
In der Struktur werden keine Argumente gesättigt. Der Typ \type{head-filler-phrase} ist Untertyp von
\type{head-non-complement-phrase} (siehe Abbildung~\ref{abb-sign-hf-struc}).
\begin{figure}
\oneline{
...
Der semantische Beitrag (\ltop- und \textsc{index}-Wert) kommt vom Verb (der Kopf"|tochter). Das
folgt aus dem Semantikprinzip (siehe Abschnitt~\ref{Abschnitt-Semantikprinzip}), denn der
...
für diesen Typ sieht wie folgt aus:\footnote{%
  Alternativ könnte man auch eine relationale Beschränkung \texttt{collect-slash}\isrel{collect-slash} verwenden, die
  analog zu \texttt{collect-rels} und \texttt{collect-hcons} (siehe
  S.\,\pageref{constraint-collect-rels-collect-hcons}) die \slasch-Elemente aller Töchter
  aufsammelt. Dieses Aufsammeln könnte man mit dem Abbinden über ein Merkmal \textsc{to-bind|slash}
  kombinieren, wie das \citet[163--164]{ps2} für die Behandlung von \emph{tough}-Movement
  vorgeschlagen haben. Da es dieses Phänomen im Deutschen so nicht gibt \citep[275]{Demske94a},
%\parencites[Abschnitt~6.2]{Thiersch78a}[52]{Hoehle78a}, 
belasse ich es hier vorerst bei der
  einfacheren Version.
}
\eas
\type{head"=non"=filler"=phrase}\istype{head"=non"=filler"=phrase} \impl\\
\onems{ 
nonloc$|$slash \ibox{1} $\oplus$ \ibox{2}\\[1mm]
dtrs \sliste{ [nonloc$|$slash \ibox{1}], [nonloc$|$slash \ibox{2}] }
}
\zs
...
wiederholt sind, stellt (\mex{0}) die Formalisierung des Prinzips der nichtlokalen Merkmale
dar.\footnote{%
  Auf S.\,\pageref{type-filler-phrase} wird der Typ \type{filler-phrase} definiert, der sowohl Obertyp des Typs
  \type{head-filler-phrase} als auch des Typs \type{relative-clause} ist. Die Beschränkungen in
  (\mex{1}) sind dann Beschränkungen des allgemeineren Typs \type{filler-phrase}.
}
\eas
\type{head-filler-phrase}\istype{head"=filler"=phrase} \impl\\
\onems{ 
nonloc$|$slash            \eliste\\[1mm]
dtrs \sliste{ [loc \ibox{1}], [nonloc$|$slash \sliste{ \ibox{1} }] }\\
}
\zs
\largerpage
...
mit der Verberstanalyse aus Abschnitt~\ref{sec-v1} kombiniert werden kann.
...
\end{itemize}
Diese Analyse kann Sprachen erklären, in denen bestimmte Elemente abhängig davon flektieren,
ob sie Bestandteil einer Konstituente sind, durch die eine Fernabhängigkeit hindurchgeht. \citet*{BMS2001a} nennen 
Irisch\il{Irisch}, Chamorro\il{Chamorro}, Palauisch\il{Palauisch}, Isländisch\il{Isländisch},
...
Fernabhängigkeiten eine Rolle spielen, zugreifen. So können \zB bestimmte Komplementierer im
Irischen verlangen, dass sich in dem Satz, den sie als Argument nehmen, eine Lücke befindet.
...
Repräsentation für die Bedeutung des gesamten Satzes doppelt enthalten wären. Das würde auch dazu führen,
...
was \citet*[\page 305, 307]{CFPS2005a} als Wert von \textsc{hook} annehmen. Entlang der Kopf"|pfade
wird also unter \cont der Index projiziert; das \ltop\hyp Label kann von einem Adjunkt kommen,
...
[\type{word}\\
 phon & \phonliste{}\\
 loc & \ibox{1}\\
 \punk{nonloc$|$slash}{\sliste{ \ibox{1} }} \\ 
 rels  & \eliste\\
 hcons & \eliste ]}
\zs
Neu ist hier, dass die Spur weder Relationen noch Skopusbeschränkungen
beiträgt.\footnote{%
...
Relationen im \contw der Argumente für die Berechnung des semantischen Beitrags von Phrasen
unberücksichtigt bleiben. Zu spurenlosen Ansätzen siehe Abschnitt~\ref{Abschnitt-Argumentrealisierungsprinzip-BMS}.}
Das Gleiche gilt für die Verbspur aus Abschnitt~\ref{Abschnitt-V1} von S.\,\pageref{le-verbspur}.
...
stehen, weshalb man auch von Vorfeldellipse\is{Vorfeldellipse} spricht (\citealt{Fries88b}; siehe
auch S.\,\pageref{ex-Vorfeldellipse}).
...
für einen detaillierteren Überblick siehe \citew[Abschnitt~4]{Zifonun97a}.
...
prinzipiell beide Möglichkeiten benötigt, wäre (\mex{-1}a) mehrdeutig. Um dieses Problem kann man
...
% aus Abschnitt~\ref{Abschnitt-V1} in der Repräsentation von \rels und \hcons und in der zusätzlichen Information
...
fungiert als Füller in einer Kopf"=Füller"=Struktur.
\is{Satztyp|)} 
...
analysieren können (siehe Abschnitt~\ref{sec-mf}), wären ohne eine weitere Einschränkung
...
%\todostefan{use parencites here and find solution for GMueller}
G.\,\citealp[\page 189]{GMueller2004a}, \citealp{Frey2004a}),
% Weiß nicht mehr wo.
%\NOTE{Koster zitieren: \citealp{Koster78a-u}} 
...
%% \z
%% Die Beschränkung drückt Folgendes aus: Wenn die Nicht"=Kopf"|tochter in einer Kopf"=Komplement"=Struktur
...
%% von \emph{gelesen} bzw.\ \emph{in der Kneipe gelesen} mit \emph{hat} nicht über das Kopf"=Komplement"=Schema,
...
%% ins Vorfeld zu (siehe Abschnitt~\ref{sec-pvp}). Die Implikation in (\ref{implikation-extraktion-hoechstes-element})
%% sagt nur etwas über Kopf"=Komplement"=Strukturen aus, betrifft also das Prädikatskomplexschema nicht.
%% \is{Verbalkomplex|)}%
...
benutzt (siehe \citealt{Reape90a,Reape92a,Reape94a} und Abschnitt~\ref{sec-Reape-Linearisierung}).
...
\zl\todostefan{Infinitive mit zu gehen nicht}
...
analysieren (siehe S.\,\pageref{Seite-Relativsätze-Interrogativnebensätze} und Abschnitt~\ref{sec-Generalisierung-Satztypen}). Eine extrahierte Phrase
...
\draw[out=336,in=140] (subord.south) to (s-decl.north);
...
\citew[Abschnitt~5.4.1]{Kathol95a} schließt jedoch phonologisch leere Domänenelemente explizit aus.
...
ist die sogenannte Vorfeldellipse\is{Ellipse!Vorfeld-}, die auch \emph{Topic Drop} genannt
wird. \citet[Abschnitt~1.5]{Huang84},\NOTE{FB: Scheint ein Widerspruch zu früherer Anmerkung zu sein, dass das
  Vorfeld nichts mit pragmatischen Funktionen zu tun hat.}
\citet{Fries88b} und \citet[574--576, Abschnitt~4.1.1.1]{Hoffmann97a} diskutieren diese Konstruktion im Detail.
...
mehrfacher Vorfeldbesetzung\is{Vorfeldbesetzung!scheinbar mehrfache}, wie sie schon in den
Abschnitten~\ref{sec-konst-test-probleme-voranstellung} und~\ref{sec-Wetta}
...
\z
Das ist aber problematisch, da Kathol\label{page-Kathol-1-2-3-4} Linearisierungsbeschränkungen annimmt, die dafür sorgen,
...
auch im Abschnitt~\ref{sec-v1} für die Analyse der Verbstellung verwendet wurde. Wenn man keine
...
in die Grammatik aufgenommen werden müssen. Aus diesem Grund ist der in Abschnitt~\ref{sec-v1}
...
\ea
Argument Structure Extension \citep*[\page 12]{BMS2001a}\\[2pt]
\type{verb} \impl \ms{
arg-st & \ibox{1}\\
...
\eas
Argumentrealisierungsprinzip\is{Prinzip!Argumentrealisierungs-} \citep*[\page 12]{BMS2001a}\\[2pt]
\type{word} \impl \ms{
subj  & \ibox{1} \\
...
Elemente ist \textsc{deps}. Davon wird die Subjekt-Liste \iboxb{1} abgetrennt und von der restlichen Liste \iboxb{2}
werden die Elemente abgezogen, die Gaps sind, und das Ergebnis der Subtraktion wird mit der
...
Folgt man diesem Ansatz für das Deutsche, bekommt man für die finiten Formen eines ditransitiven Verbs wie \emph{geben} 
...
\todostefan{Diskussion der Extraktion von Adjunkten und Koordination hinzufügen.}
\largerpage
...
S.\,\pageref{schema-Kopf-Komplementschema-prel2}). Für das Verb \emph{geben}, wie es in (\mex{1}a) 
...
extrahiert wurde, müsste bereits im Lexikoneintrag eines Verbs enthalten sein.
...
LFG\is{Lexical Functional Grammar\indexlfgstart
...
gemacht wird. Diese Analysen wurden bereits im Abschnitt~\ref{sec-Scrambling-Skopus} kritisiert. Will
...
Grammatik angelangt, die von Chomsky und den Vertreter*innen der GPSG als nicht adäquat eingestuft wurde.
...
Abschnitt~\ref{sec-sem-kasus} zu anderen Beispielen für Adjunkt"=Nominalphrasen.
\eal
...
prädikative Nominalphrasen.\indexlfgend
\end{comment}
\is{Extraktion|)}\is{nichtlokale Abhängigkeit|)}\is{Konstituentenstellung|)}
...
366]{ps2}[Abschnitt~13.2]{Mueller99a}, es hat sich aber gezeigt, dass die Extraposition zum Teil mit 
