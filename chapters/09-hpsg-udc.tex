...
\end{figure}
Dadurch wird der \compsw der Kopf"|tochter automatisch mit dem der Mutter identifiziert (siehe Abschnitt~\ref{sec-valp} zum Valenzprinzip\is{Prinzip!Valenz-}).
...
\type{word} \impl \ms{
comps & \ibox{1} $\ominus$ list\textrm{(}`gap-ss'\textrm{)}\\
deps   & \ibox{1}\\
} 
\z
...
benutzt wird, müßte man also eine \compsl wie in (\mex{1}b) annehmen:
\eal
\ex weil dem Mann eine Frau ein Buch gibt
...
Objekte vom Typ \type{synsem} sind (siehe Kapitel~\ref{Kapitel-lokalitaet} zur Lokalität der Selektion)
