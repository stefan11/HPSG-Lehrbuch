...
\citealp[\page69, \page 77]{Paul1919a}), weshalb das Deutsche zu den Verbzweitsprachen\is{Verbzweitsprache} gezählt wird.\footnote{%
...
\eas
\label{lr-verb-movement4}%
Regel für Verb in Erststellung (mit Satztypinformation):\\
\type{verb-initial-rule} \impl\\
\ms{
loc & \ms{ cat & \ms{ head & \ms[verb]{ vform & fin\\
                                      initial & $+$\\
...
c-cont & \ms{ ind  & \ibox{2}\\ 
              ltop & \ibox{4}\\
%              key  & 
              rels & \liste{ \ms{ lbl  & \ibox{4} \\
                                  arg1 & \ibox{5}}}\\[3mm]
              hcons & \liste{ \ms[qeq]{
                              harg & \ibox{5}\\
                              ltop & \ibox{3} }}}\\
...
\zs
Über den konstruktionellen Beitrag (\ccont) wird die Satztyp"=Relation in die \relsl der Mutter
eingefügt. Der genaue Typ der Relation ist jedoch unterspezifiziert und in (\mex{0}) nicht
angegeben. Allerdings ist festgelegt, dass das Argument der Satztyp"=Relation \iboxb{5} den \ltopw
der Projektion der Verbspur im Skopus hat (\ibox{5} \qeq \ibox{3}). Das Label der Satztyp"=Relation
\iboxb{4} ist auch der \ltopw in \ccont und somit der \ltopw der Mutter.

Welcher Satztyp vorliegt wird in Abhängigkeit vom
...
\hfill\ms{
rels & \nliste{ \type{conditional\_or\_imperative\_or\_interrogative} }\\
}
\zs
...
\hfill\ms{
rels & \nliste{ \type{assertion\_or\_imperative\_or\_interrogative} }\\
}
\zs
...
die Vorfeldellipse abgebunden \citep{Mueller2004e,VMY2024a} oder ein leeres Element im Vorfeld fungiert als Füller in einer 
