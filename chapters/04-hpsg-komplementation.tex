...
\ea
\begin{tabular}[t]{@{}l@{~}lll}
a. & N[\spr \ibox{1}] & $\to$ & Det~~~N[\spr \ibox{1}\,\eliste{} $\oplus$ \nliste{ Det } ]\\
b. & V[\comps \ibox{1}] & $\to$ & V[\comps \ibox{1}\,\eliste{} $\oplus$ \nliste{ NP } ]~~~NP\\
\end{tabular}
\z
	...
	[V{[\type{fin}, \comps \sliste{ \ibox{1} }]}, name=fin2
		...
		[V{[\type{fin}, \comps \sliste{ \ibox{1}, \ibox{2} }]}, name=fin3
			...
			[V{[\type{fin}, \comps \sliste{ \ibox{1}, \ibox{2}, \ibox{3} }]}, name=fin4
				[gibt]]]]]	
tikz={\draw[<->] ($(fin1.south west)+(\offsetup,2pt)$) to ($(fin2.north west)+(\offset,-1pt)$);
      \draw[<->] ($(fin2.south west)+(\offsetup,2pt)$) to ($(fin3.north west)+(\offset,-1pt)$);
      \draw[<->] ($(fin3.south west)+(\offsetup-4pt,2pt)$) to ($(fin4.north west)+(\offset,-1pt)$);}
\end{forest}
}
...
% \begin{forest}
% sm edges, for tree={l+=\baselineskip}
% [V{\subnode[inner sep=0pt]{fin1}{[}\type{fin}, \comps \eliste]}
% 	[\ibox{1} NP{[\type{nom}]}
% 		[Aicke]]
% 	[V{\subnode[inner sep=0pt]{fin2}{[}\type{fin}, \comps \sliste{ \ibox{1} }]}
% 		[\ibox{2} NP{[\textit{dat}]}
% 			[dem Eichhörnchen,roof]]
% 		[V{[\type{fin}, \comps \sliste{ \ibox{1}, \ibox{2} }]}, name=fin3
% 			[\ibox{3} NP{[\textit{acc}]}
...
Die Wortart und die Verbform sollen immer gemeinsam projiziert werden. Man könnte das zwar erreichen, indem man jeweils einzelne Strukturteilungen
...
Kasusmerkmal\isfeat{case}\label{page-ref-case-feat}. (\mex{1}a) und (\mex{1}b) zeigen entsprechende
...
angegeben wurde, kann jetzt wie in (\mex{1}) präzisiert werden.
\begin{figure}
\ea
\emph{gibt}:\\
\ms[word]{ 
     phon   & \phonliste{ gibt }\\[1mm]
     head   & \ms[verb]{ vform & fin} \\[2mm]
%     spr    & \eliste\\
%     comps  & 
...
Abschnitt~\ref{sec-subj-merkmal} über nicht-finite Verben revidiert werden.
...
\end{prinzip-break}
Bisher wurden erst drei Dominanzschemata behandelt, aber in den kommenden Kapiteln werden
noch weitere Schemata dazukommen, \zb Schemata für die Abbindung von Fernabhängigkeiten und für Verbalkomplexe. Das Kopfmerkmalsprinzip ist ein allgemeines Prinzip,
...
%    [non-headed-phrase]
    [headed-phrase
      ...
      [head-complement-phrase,calign with current] % calign puts this type in the middle so that the
                                % line is straight
      [head-adjunct-phrase]]]]
\end{forest}}
...
In der Abbildung gibt es als Untertyp von \type{phrase} nur den Typ \type{headed-phrase}. \type{headed-phrase} hat die
Untertypen \type{head-specifier-phrase}, \type{head"=complement"=phrase} und
\type{head-adjunct-phrase}, Typen, die ich bereits besprochen habe bzw.\ im
Abschnitt~\ref{sec-Kopf-Adjunkt-Strukturen} vorstellen werde. Auf andere
Typen werden ich in den kommenden Kapiteln eingehen. Gewöhnlich haben Strukturen einen Kopf, aber
die Analyse von Relativsätzen, die in Kapitel~\ref{Kapitel-rs} besprochen wird, ist kopflos. Eine
Verbalprojektion wird mit einer Relativphrase verknüpft. Das Ergebnis ist ein Relativsatz. Die Kopf"|information wird
nicht von der Verbalprojektion zum Relativsatz übertragen. Relativsätze bilden einen Untertyp des
Typs \type{phrase}, sind aber keine Untertypen von \type{headed"=phrase}. Ein anderes Beispiel für Strukturen ohne Kopf sind NPN-Konstruktionen
\citep{Matsuyama2004a,Jackendoff2008a}. (\mex{1}) zeigt ein deutsches Beispiel:
\ea
Zeile für Zeile\footnote{%
  \emph{Zwölf Städte}. Einstürzende Neubauten. Fünf auf der nach oben offenen Richterskala, 1987.
}
\z
In solchen Konstruktionen wird ein Nomen mit beliebig vielen Kombinationen aus Präposition und
demselben Nomen verknüpft. Zur Analyse siehe \citew{Bargmann2015a} bzw.\ den Übersichtsartikel \citew{MuellerCxG}.
...
einem Element als Spezifikator kombinieren. Zum Beispiel mit einer Präposition, einem einzelnen Determinator oder einer NP. Der
...
Mit diesen zusätzlichen Beschränkungen sind fast alle Probleme gelöst, aber ein Detail verlangt noch Aufmerksamkeit: Die
...
Bedeutungsunterschied gibt. Solche Mehrdeutigkeiten werden unechte Mehrdeutigkeiten\is{Mehrdeutigkeit!unechte}
...
zusätzlich ist der \sprw als leere Liste spezifiziert. Diese Beschreibung entspricht dem Satzsymbol S\is{Startsymbol} in der Grammatik auf
S.\,\pageref{bsp-grammatik-psg}, wobei (\mex{0}) noch Information über die Verbform enthält.



Mit drei Dominanzschemata, Vererbung und Typhierarchien und verschiedenen Prinzipien sind bereits einige der wesentlichen Konzepte der HPSG"=Theorie eingeführt. Im folgenden
...
%\largerpage
      ...
      Überlegen Sie, wieso die Merkmale die Werte haben, die sie haben!\pagebreak
