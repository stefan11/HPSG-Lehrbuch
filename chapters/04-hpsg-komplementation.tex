...
und Verb"=NP"=Kombinationen zwei mögliche Instantiierungen für die in (\ref) angegebenen Schemata:
\ea
\begin{tabular}[t]{@{}l@{~}lll}
a. & N[\spr \ibox{1}] & $\to$ & Det N[\spr \ibox{1} $\oplus$ \sliste{ \textrm{Det} } ]\\
b. & V[\comps \ibox{1}] & $\to$ & V[\comps \ibox{1} $\oplus$ \sliste{ NP } ]~~ NP\\
\end{tabular}
\z
  ...
  [phrase
    ...
    [headed-phrase
      ...
      [head-non-complement-phrase,name=non-complement
        [\ldots\strut, no edge, name=some-type]
        [head-specifier-phrase,name=specifier]]]]]
\draw (non-specifier.south) to (some-type.north);
\draw (non-complement.south) to (some-type.north);
\end{forest}}
