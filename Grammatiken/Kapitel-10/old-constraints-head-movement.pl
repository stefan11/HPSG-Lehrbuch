...
% V1-Koordination und wir können verlangen, dass die DSL-Information des rechten Konjunkts und damit beider Konjunkte none ist.

(coord_phrase,
 loc:cat:head:dsl:local,
 dtrs:[phrase:plus, dtrs:[loc:cat:head:coord,
                          (word,
                           phon:ne_list)]]) *> loc:cat:head:dsl:none.
...
(coord_phrase,
 ...
 dtrs:[(word,
        phon:ne_list), dtrs:[loc:cat:head:coord,
                             phrase:plus]]) *> loc:cat:head:dsl:none.

*/

% Da Koordination von Projektionen von Verbspuren ohnehin nihct geht, kann man die auch gleich ausschließen.
% Kennt Aicke den Roman und Conny den Film?
% geht nicht, weil die Spuren und damit die Valenzlisten identisch sein müssten.
...
% Wenn wir schon was in DSL haben, dann bitte auch Zyklen.
% Geht nicht für die Koordination in der linken Satzklammer.
%(word,
% loc:cat:head:(verb,
%               dsl:local)) *> loc:(Local,
%                                   cat:head:dsl:Local).
